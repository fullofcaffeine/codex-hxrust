import codexhx.runtime.tui.resume.live.ResumePickerReloadScrollPreservationRenderGate;

class ResumePickerReloadScrollPreservationRenderHarness {
	static function main():Void {
		final report = ResumePickerReloadScrollPreservationRenderGate.run();
		...
		assertContains(report.finalSnapshot, "loader status=scroll_clamped_after_shrink detail=selected=thread-anchor;index=2;scrollTop=0;previousScrollTop=2;maxScrollTop=0");
		...
		assertContains(report.summary(), "preserved:query=scroll;rows=6;selected=4;thread=thread-anchor;scrollTop=2;viewRows=3;moreAbove=true;moreBelow=false;footer=scroll preserved;loader=scroll_preserved_by_window");
		assertContains(report.summary(), "clamped:query=scroll;rows=3;selected=2;thread=thread-anchor;scrollTop=0;viewRows=3;moreAbove=false;moreBelow=false;footer=scroll clamped;loader=scroll_clamped_after_shrink");
