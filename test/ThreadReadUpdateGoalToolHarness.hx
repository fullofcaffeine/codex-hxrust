...
import codexhx.runtime.app.threadread.ThreadReadUpdateGoalToolPolicy;
import codexhx.runtime.app.threadread.ThreadReadUpdateGoalToolReport;
...
class ThreadReadUpdateGoalToolHarness {
	static function main():Void {
		final root = fixtureRoot();
		final cases = arrayField(root, "cases");
		final report = ThreadReadUpdateGoalToolPolicy.buildCases(requests(cases));
		...
		while (i < cases.length) {
			...
			assertEquals(stringField(request, "threadId", ""), outcome.threadId);
			assertEquals(stringField(request, "turnId", ""), outcome.turnId);
			assertEquals(stringField(request, "callId", ""), outcome.callId);
			assertEquals(boolText(boolField(expect, "ok", false)), boolText(outcome.ok));
			assertEquals(stringField(expect, "code", ""), outcome.code);
			assertEquals(stringField(expect, "requestedStatus", ""), outcome.requestedStatus);
			...
			assertEquals(stringField(expect, "accountingOutcomeKind", ""), outcome.accountingOutcomeKind);
			assertEquals(boolText(boolField(expect, "metricsReadAttempted", false)), boolText(outcome.metricsReadAttempted));
			assertEquals(stringField(expect, "metricsReadOutcomeKind", ""), outcome.metricsReadOutcomeKind);
			assertEquals(stringField(expect, "previousStatus", ""), outcome.previousStatus);
			assertEquals(boolText(boolField(expect, "updateAttempted", false)), boolText(outcome.updateAttempted));
			assertEquals(stringField(expect, "updateOutcomeKind", ""), outcome.updateOutcomeKind);
			assertEquals(boolText(boolField(expect, "terminalMetricRecorded", false)), boolText(outcome.terminalMetricRecorded));
			assertEquals(boolText(boolField(expect, "analyticsStatusChanged", false)), boolText(outcome.analyticsStatusChanged));
			assertEquals(boolText(boolField(expect, "currentTurnCleared", false)), boolText(outcome.currentTurnCleared));
			assertEquals(stringField(expect, "clearedTurnId", ""), outcome.clearedTurnId);
	...
	static function requests(values:Array<Value>):Array<ThreadReadUpdateGoalToolRequest> {
	...
	static function assertReport(root:Value, report:ThreadReadUpdateGoalToolReport):Void {
	...
	static function fixtureRoot():Value {
		return expectParse(CodexJson.parse(File.getContent("fixtures/hxrust/thread-read-update-goal-tool.v1.json")));
	}
