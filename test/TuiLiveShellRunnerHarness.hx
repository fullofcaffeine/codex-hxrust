...
class TuiLiveShellRunnerHarness {
	static function main():Void {
		...
		testReadinessSourcePrefixThenLineReadRejectionRoutesThroughRunner();
		...
		testReadinessSourceStaleInterruptedRejectionRoutesThroughRunner();
		testReadinessStaleInterruptedCompletionRejectionRoutesThroughRunner();
		testReadinessSourceStaleInterruptedCompletionRejectionRoutesThroughRunner();
	...
	static function testAppServerSessionReadinessSourceFeedsSubmittedTurnReadinessThroughRunner():Void {
		final shell = ChatWidgetShellState.initial("pending");
		final activeThread = thread("00000000-0000-0000-0000-000000110001");
		final activeSession = session("00000000-0000-0000-0000-000000119999");
		final promptEnvelope = new TuiPromptSubmitEnvelope(RequestId.fromInteger(9), activeSession, activeThread, "session");
		final turnId = TuiPromptTurnStartResponse.fromEnvelope(promptEnvelope).turnId;
		final lateLines = [
			new TuiPromptAgentMessageDeltaNotification(activeThread, turnId, item("item-runner-session-source-9"),
				"runner app-server session readiness delta").messageJson()
				+ "\n",
			turnCompletedLine(activeThread, turnId)
		];
		final appServerTransport = new PersistentTuiAppServerJsonRpcLineConnectedTransport(TuiAppServerJsonRpcLineEndpoint.Stdio(stdioPersistentPlan([])),
		...
		assertStringEquals("app_server_session", sessionReadinessSource.sourceKindText(), "runner app-server session readiness source kind");
		assertIntEquals(2, readinessSource.remaining(), "runner app-server session readiness source initial remaining");
		final requestValue = request(shell, backend, [],
			TuiLiveShellRunPolicy.bounded(40, 3)).withJsonRpcPromptTransport(promptTransport).withReadinessSource(readinessSource);
		final outcome = TuiLiveShellRunner.run(requestValue);

		assertIntEquals(1, outcome.submittedPrompts(), "runner app-server session readiness source submitted prompts");
		assertIntEquals(1, outcome.acceptedPrompts(), "runner app-server session readiness source accepted prompts");
		assertIntEquals(2, outcome.appServerReadinessEvents(), "runner app-server session readiness source event count");
		assertIntEquals(2, outcome.appServerReadinessDrained(), "runner app-server session readiness source drained count");
		assertIntEquals(0, outcome.appServerReadinessNoPending(), "runner app-server session readiness source no-pending count");
		assertIntEquals(1, outcome.appServerReadinessNoDataCount(), "runner app-server session readiness source no-data count");
		assertStringEquals("turn-9", outcome.latestNoDataReadinessActiveTurnIdText(), "runner app-server session readiness source no-data active retained");
		assertStringEquals(TuiPromptSubmittedTurnLateJsonlDrainStatus.Completed.text(), outcome.latestReadinessLateJsonlDrainStatusText(),
			"runner app-server session readiness source final drain status");
		assertStringEquals("completed", outcome.latestReadinessLateJsonlDrainCode(), "runner app-server session readiness source final drain code");
		assertStringEquals(TuiAppServerJsonRpcTransportStatus.Accepted.text(), outcome.latestReadinessLateJsonlLineStatusText(),
			"runner app-server session readiness source line status");
		assertStringEquals("accepted", outcome.latestReadinessLateJsonlLineCode(), "runner app-server session readiness source line code");
		assertIntEquals(2, outcome.latestReadinessLateJsonlAppliedNotificationCount(), "runner app-server session readiness source applied notification count");
		assertIntEquals(1, outcome.latestReadinessLateJsonlAssistantDeltaCount(), "runner app-server session readiness source assistant delta count");
		assertIntEquals(1, outcome.latestReadinessLateJsonlCompletionCount(), "runner app-server session readiness source completion count");
		assertStringEquals(activeThread.toString(), outcome.latestReadinessLateJsonlThreadIdText(),
			"runner app-server session readiness source applied thread evidence");
		assertStringEquals("turn-9", outcome.latestReadinessLateJsonlTurnIdText(), "runner app-server session readiness source applied turn evidence");
		assertStringEquals("runner app-server session readiness delta", outcome.latestReadinessLateJsonlDeltaText(),
			"runner app-server session readiness source applied delta evidence");
		assertStringEquals("turn-9", outcome.lastStartedTurnIdText(), "runner app-server session readiness source last started");
		assertStringEquals("turn-9", outcome.lastCompletedTurnIdText(), "runner app-server session readiness source last completed");
		assertStringEquals("", outcome.activeTurnIdText(), "runner app-server session readiness source active cleared");
		assertIntEquals(1, outcome.completedTurns(), "runner app-server session readiness source completed exactly once");
		assertStringEquals("user> session", shell.transcriptAt(1).renderText(), "runner app-server session readiness source user row");
		assertStringEquals("assistant> runner app-server session readiness delta", shell.transcriptAt(2).renderText(),
			"runner app-server session readiness source assistant row");
		assertStringEquals("assistant> runner app-server session readiness delta", outcome.finalFrameLineAt(4),
			"runner app-server session readiness source final frame");
		assertIntEquals(0, readinessSource.remaining(), "runner app-server session readiness source consumed by runner");
		assertIntEquals(0, sessionReadinessSource.remaining(), "runner app-server session readiness session source consumed by runner");
	}
	...
	static function testAppServerSessionReadinessSourceLineReadRejectionRoutesThroughRunner():Void {
		final shell = ChatWidgetShellState.initial("pending");
		final appServerTransport = new PersistentTuiAppServerJsonRpcLineConnectedTransport(TuiAppServerJsonRpcLineEndpoint.Stdio(stdioPersistentPlan([])),
			new DryRunTuiAppServerJsonRpcLineConnector(new DryRunTuiAppServerJsonRpcLineNativeOpener(),
				new RunnerRejectedLateJsonlLineTransportAttacher("runner_session_late_jsonl_read_failed")));
		final promptTransport = new JsonRpcTuiPromptTransport(appServerTransport, TuiPromptTurnAcceptanceMode.Submitted);
		final backend = new HeadlessTerminalBackend([
			TerminalEvent.Key(TerminalKey.Character("s")),
			TerminalEvent.Key(TerminalKey.Character("e")),
		...
		final readinessSource = TuiLiveShellReadinessSource.fromAppServerSession(sessionReadinessSource);
		assertStringEquals("app_server_session", sessionReadinessSource.sourceKindText(), "runner app-server session line-read source kind");
		assertIntEquals(1, readinessSource.remaining(), "runner app-server session line-read readiness source initial remaining");
		final requestValue = request(shell, backend, [],
			TuiLiveShellRunPolicy.bounded(68, 2)).withJsonRpcPromptTransport(promptTransport).withReadinessSource(readinessSource);
		final outcome = TuiLiveShellRunner.run(requestValue);

		assertIntEquals(1, outcome.submittedPrompts(), "runner app-server session line-read readiness submitted prompts");
		assertIntEquals(1, outcome.acceptedPrompts(), "runner app-server session line-read readiness accepted prompts");
		assertIntEquals(1, outcome.appServerReadinessEvents(), "runner app-server session line-read readiness event count");
		assertIntEquals(1, outcome.appServerReadinessDrained(), "runner app-server session line-read readiness drained count");
		assertIntEquals(0, outcome.appServerReadinessNoPending(), "runner app-server session line-read readiness no-pending count");
		assertStringEquals(TuiAppServerReadinessInteractionStatus.Drained.text(), outcome.latestReadinessStatusText(),
			"runner app-server session line-read readiness status");
		assertStringEquals(TuiPromptSubmittedTurnLateJsonlDrainStatus.LineReadRejected.text(), outcome.latestReadinessLateJsonlDrainStatusText(),
			"runner app-server session line-read readiness late jsonl drain status");
		assertStringEquals("runner_session_late_jsonl_read_failed", outcome.latestReadinessLateJsonlDrainCode(),
			"runner app-server session line-read readiness late jsonl drain code");
		assertStringEquals(TuiAppServerJsonRpcTransportStatus.Disconnected.text(), outcome.latestReadinessLateJsonlLineStatusText(),
			"runner app-server session line-read readiness late jsonl line status");
		assertStringEquals("runner_session_late_jsonl_read_failed", outcome.latestReadinessLateJsonlLineCode(),
			"runner app-server session line-read readiness late jsonl line code");
		assertIntEquals(0, outcome.latestReadinessLateJsonlAppliedNotificationCount(),
			"runner app-server session line-read readiness applied notification count");
		assertIntEquals(0, outcome.latestReadinessLateJsonlAssistantDeltaCount(), "runner app-server session line-read readiness assistant delta count");
		assertIntEquals(0, outcome.latestReadinessLateJsonlCompletionCount(), "runner app-server session line-read readiness completion count evidence");
		assertStringEquals("turn-13", outcome.latestReadinessActiveTurnIdText(),
			"runner app-server session line-read readiness active retained after rejection");
		assertStringEquals("turn-13", outcome.lastStartedTurnIdText(), "runner app-server session line-read readiness last started");
		assertStringEquals("", outcome.lastCompletedTurnIdText(), "runner app-server session line-read readiness no completion");
		assertStringEquals("turn-13", outcome.activeTurnIdText(), "runner app-server session line-read readiness active retained");
		assertIntEquals(0, outcome.completedTurns(), "runner app-server session line-read readiness completed count");
		assertIntEquals(2, shell.transcriptCount(), "runner app-server session line-read readiness transcript count");
		assertStringEquals("user> sessionfail", shell.transcriptAt(1).renderText(), "runner app-server session line-read readiness user row");
		assertTrue(outcome.promptTransportLineCloseRecorded(), "runner app-server session line-read readiness close recorded");
		assertIntEquals(1, outcome.promptTransportOutboundLineCount(), "runner app-server session line-read readiness outbound lines");
		assertIntEquals(2, outcome.promptTransportInboundLineCount(), "runner app-server session line-read readiness no late inbound lines");
		assertIntEquals(0, readinessSource.remaining(), "runner app-server session line-read readiness source consumed");
		assertIntEquals(0, sessionReadinessSource.remaining(), "runner app-server session line-read session source consumed");
	}
	...
	static function testAppServerSessionReadinessSourceSchemaRejectionRoutesThroughRunner():Void {
		final shell = ChatWidgetShellState.initial("pending");
		final lateLines = ["{\"jsonrpc\":\"2.0\",\"method\":\"turn/completed\",\"params\":{}}\n"];
		final appServerTransport = new PersistentTuiAppServerJsonRpcLineConnectedTransport(TuiAppServerJsonRpcLineEndpoint.Stdio(stdioPersistentPlan([])),
			new DryRunTuiAppServerJsonRpcLineConnector(new DryRunTuiAppServerJsonRpcLineNativeOpener(),
		...
		final readinessSource = TuiLiveShellReadinessSource.fromAppServerSession(sessionReadinessSource);
		assertStringEquals("app_server_session", sessionReadinessSource.sourceKindText(), "runner app-server session schema source kind");
		assertIntEquals(2, readinessSource.remaining(), "runner app-server session schema readiness source initial remaining");
		final requestValue = request(shell, backend, [],
			TuiLiveShellRunPolicy.bounded(82, 3)).withJsonRpcPromptTransport(promptTransport).withReadinessSource(readinessSource);
		final outcome = TuiLiveShellRunner.run(requestValue);

		assertIntEquals(1, outcome.submittedPrompts(), "runner app-server session schema readiness submitted prompts");
		assertIntEquals(1, outcome.acceptedPrompts(), "runner app-server session schema readiness accepted prompts");
		assertIntEquals(2, outcome.appServerReadinessEvents(), "runner app-server session schema readiness event count");
		assertIntEquals(2, outcome.appServerReadinessDrained(), "runner app-server session schema readiness drained count");
		assertIntEquals(0, outcome.appServerReadinessNoPending(), "runner app-server session schema readiness no-pending count");
		assertIntEquals(1, outcome.appServerReadinessNoDataCount(), "runner app-server session schema readiness no-data count");
		assertStringEquals("turn-15", outcome.latestNoDataReadinessActiveTurnIdText(), "runner app-server session schema readiness no-data active retained");
		assertStringEquals(TuiAppServerReadinessInteractionStatus.Drained.text(), outcome.latestReadinessStatusText(),
			"runner app-server session schema readiness status");
		assertStringEquals(TuiPromptSubmittedTurnLateJsonlDrainStatus.BatchRejected.text(), outcome.latestReadinessLateJsonlDrainStatusText(),
			"runner app-server session schema readiness late jsonl drain status");
		assertStringEquals("missing_field", outcome.latestReadinessLateJsonlDrainCode(), "runner app-server session schema readiness late jsonl drain code");
		assertStringEquals(TuiAppServerJsonRpcTransportStatus.Accepted.text(), outcome.latestReadinessLateJsonlLineStatusText(),
			"runner app-server session schema readiness late jsonl line status");
		assertStringEquals("accepted", outcome.latestReadinessLateJsonlLineCode(), "runner app-server session schema readiness late jsonl line code");
		assertIntEquals(0, outcome.latestReadinessLateJsonlAppliedNotificationCount(), "runner app-server session schema readiness applied notification count");
		assertIntEquals(0, outcome.latestReadinessLateJsonlAssistantDeltaCount(), "runner app-server session schema readiness assistant delta count");
		assertIntEquals(0, outcome.latestReadinessLateJsonlCompletionCount(), "runner app-server session schema readiness completion count evidence");
		assertStringEquals("turn-15", outcome.latestReadinessActiveTurnIdText(), "runner app-server session schema readiness active retained after rejection");
		assertStringEquals("turn-15", outcome.lastStartedTurnIdText(), "runner app-server session schema readiness last started");
		assertStringEquals("", outcome.lastCompletedTurnIdText(), "runner app-server session schema readiness no completion");
		assertStringEquals("turn-15", outcome.activeTurnIdText(), "runner app-server session schema readiness active retained");
		assertIntEquals(0, outcome.completedTurns(), "runner app-server session schema readiness completed count");
		assertIntEquals(2, shell.transcriptCount(), "runner app-server session schema readiness transcript count");
		assertStringEquals("user> sessionschema", shell.transcriptAt(1).renderText(), "runner app-server session schema readiness user row");
		assertTrue(outcome.promptTransportLineCloseRecorded(), "runner app-server session schema readiness close recorded");
		assertIntEquals(1, outcome.promptTransportOutboundLineCount(), "runner app-server session schema readiness outbound lines");
		assertIntEquals(3, outcome.promptTransportInboundLineCount(), "runner app-server session schema readiness inbound lines");
		assertIntEquals(0, readinessSource.remaining(), "runner app-server session schema readiness source consumed");
		assertIntEquals(0, sessionReadinessSource.remaining(), "runner app-server session schema session source consumed");
	}
	...
	static function testAppServerSessionReadinessSourcePrefixThenSchemaRejectionRoutesThroughRunner():Void {
		final shell = ChatWidgetShellState.initial("pending");
		final activeThread = thread("00000000-0000-0000-0000-000000110001");
		final turnId = turn("turn-21");
		final lateLines = [
			new TuiPromptAgentMessageDeltaNotification(activeThread, turnId, item("item-runner-session-prefix-schema-21"),
				"runner session prefix schema delta").messageJson()
				+ "\n",
			"{\"jsonrpc\":\"2.0\",\"method\":\"turn/completed\",\"params\":{}}\n"
		];
		final appServerTransport = new PersistentTuiAppServerJsonRpcLineConnectedTransport(TuiAppServerJsonRpcLineEndpoint.Stdio(stdioPersistentPlan([])),
		...
		final readinessSource = TuiLiveShellReadinessSource.fromAppServerSession(sessionReadinessSource);
		assertStringEquals("app_server_session", sessionReadinessSource.sourceKindText(), "runner app-server session prefix-schema source kind");
		assertIntEquals(2, readinessSource.remaining(), "runner app-server session prefix-schema readiness source initial remaining");
		final requestValue = request(shell, backend, [],
			TuiLiveShellRunPolicy.bounded(108, 3)).withJsonRpcPromptTransport(promptTransport).withReadinessSource(readinessSource);
		final outcome = TuiLiveShellRunner.run(requestValue);

		assertIntEquals(1, outcome.submittedPrompts(), "runner app-server session prefix-schema readiness submitted prompts");
		assertIntEquals(1, outcome.acceptedPrompts(), "runner app-server session prefix-schema readiness accepted prompts");
		assertIntEquals(2, outcome.appServerReadinessEvents(), "runner app-server session prefix-schema readiness event count");
		assertIntEquals(2, outcome.appServerReadinessDrained(), "runner app-server session prefix-schema readiness drained count");
		assertIntEquals(0, outcome.appServerReadinessNoPending(), "runner app-server session prefix-schema readiness no-pending count");
		assertIntEquals(1, outcome.appServerReadinessNoDataCount(), "runner app-server session prefix-schema readiness no-data count");
		assertStringEquals("turn-21", outcome.latestNoDataReadinessActiveTurnIdText(),
			"runner app-server session prefix-schema readiness no-data active retained");
		assertStringEquals(TuiAppServerReadinessInteractionStatus.Drained.text(), outcome.latestReadinessStatusText(),
			"runner app-server session prefix-schema readiness status");
		assertStringEquals(TuiPromptSubmittedTurnLateJsonlDrainStatus.BatchRejected.text(), outcome.latestReadinessLateJsonlDrainStatusText(),
			"runner app-server session prefix-schema readiness late jsonl drain status");
		assertStringEquals("missing_field", outcome.latestReadinessLateJsonlDrainCode(),
			"runner app-server session prefix-schema readiness late jsonl drain code");
		assertStringEquals(TuiAppServerJsonRpcTransportStatus.Accepted.text(), outcome.latestReadinessLateJsonlLineStatusText(),
			"runner app-server session prefix-schema readiness late jsonl line status");
		assertStringEquals("accepted", outcome.latestReadinessLateJsonlLineCode(), "runner app-server session prefix-schema readiness late jsonl line code");
		assertIntEquals(1, outcome.latestReadinessLateJsonlAppliedNotificationCount(),
			"runner app-server session prefix-schema readiness applied notification count");
		assertIntEquals(1, outcome.latestReadinessLateJsonlAssistantDeltaCount(), "runner app-server session prefix-schema readiness assistant delta count");
		assertIntEquals(0, outcome.latestReadinessLateJsonlCompletionCount(), "runner app-server session prefix-schema readiness completion count evidence");
		assertStringEquals(activeThread.toString(), outcome.latestReadinessLateJsonlThreadIdText(),
			"runner app-server session prefix-schema readiness applied thread evidence");
		assertStringEquals("turn-21", outcome.latestReadinessLateJsonlTurnIdText(), "runner app-server session prefix-schema readiness applied turn evidence");
		assertStringEquals("runner session prefix schema delta", outcome.latestReadinessLateJsonlDeltaText(),
			"runner app-server session prefix-schema readiness applied delta evidence");
		assertStringEquals("turn-21", outcome.latestReadinessActiveTurnIdText(),
			"runner app-server session prefix-schema readiness active retained after rejection");
		assertStringEquals("turn-21", outcome.lastStartedTurnIdText(), "runner app-server session prefix-schema readiness last started");
		assertStringEquals("", outcome.lastCompletedTurnIdText(), "runner app-server session prefix-schema readiness no completion");
		assertStringEquals("turn-21", outcome.activeTurnIdText(), "runner app-server session prefix-schema readiness active retained");
		assertIntEquals(0, outcome.completedTurns(), "runner app-server session prefix-schema readiness completed count");
		assertIntEquals(3, shell.transcriptCount(), "runner app-server session prefix-schema readiness transcript count");
		assertStringEquals("user> sessionprefixschema", shell.transcriptAt(1).renderText(), "runner app-server session prefix-schema readiness user row");
		assertStringEquals("assistant> runner session prefix schema delta", shell.transcriptAt(2).renderText(),
			"runner app-server session prefix-schema readiness assistant row");
		assertStringEquals("assistant> runner session prefix schema delta", outcome.finalFrameLineAt(4),
			"runner app-server session prefix-schema readiness final frame");
		assertTrue(outcome.promptTransportLineCloseRecorded(), "runner app-server session prefix-schema readiness close recorded");
		assertIntEquals(1, outcome.promptTransportOutboundLineCount(), "runner app-server session prefix-schema readiness outbound lines");
		assertIntEquals(4, outcome.promptTransportInboundLineCount(), "runner app-server session prefix-schema readiness inbound lines");
		assertIntEquals(0, readinessSource.remaining(), "runner app-server session prefix-schema readiness source consumed");
		assertIntEquals(0, sessionReadinessSource.remaining(), "runner app-server session prefix-schema session source consumed");
	}
	...
	static function testAppServerSessionReadinessSourcePrefixThenDecodeRejectionRoutesThroughRunner():Void {
		final shell = ChatWidgetShellState.initial("pending");
		final activeThread = thread("00000000-0000-0000-0000-000000110001");
		final turnId = turn("turn-21");
		final lateLines = [
			new TuiPromptAgentMessageDeltaNotification(activeThread, turnId, item("item-runner-session-prefix-decode-21"),
				"runner session prefix decode delta").messageJson()
				+ "\n",
			"{\"jsonrpc\":\"2.0\",\"method\":\"unknown/event\",\"params\":{}}\n"
		];
		final appServerTransport = new PersistentTuiAppServerJsonRpcLineConnectedTransport(TuiAppServerJsonRpcLineEndpoint.Stdio(stdioPersistentPlan([])),
		...
		final readinessSource = TuiLiveShellReadinessSource.fromAppServerSession(sessionReadinessSource);
		assertStringEquals("app_server_session", sessionReadinessSource.sourceKindText(), "runner app-server session prefix-decode source kind");
		assertIntEquals(2, readinessSource.remaining(), "runner app-server session prefix-decode readiness source initial remaining");
		final requestValue = request(shell, backend, [],
			TuiLiveShellRunPolicy.bounded(108, 3)).withJsonRpcPromptTransport(promptTransport).withReadinessSource(readinessSource);
		final outcome = TuiLiveShellRunner.run(requestValue);

		assertIntEquals(1, outcome.submittedPrompts(), "runner app-server session prefix-decode readiness submitted prompts");
		assertIntEquals(1, outcome.acceptedPrompts(), "runner app-server session prefix-decode readiness accepted prompts");
		assertIntEquals(2, outcome.appServerReadinessEvents(), "runner app-server session prefix-decode readiness event count");
		assertIntEquals(2, outcome.appServerReadinessDrained(), "runner app-server session prefix-decode readiness drained count");
		assertIntEquals(0, outcome.appServerReadinessNoPending(), "runner app-server session prefix-decode readiness no-pending count");
		assertIntEquals(1, outcome.appServerReadinessNoDataCount(), "runner app-server session prefix-decode readiness no-data count");
		assertStringEquals("turn-21", outcome.latestNoDataReadinessActiveTurnIdText(),
			"runner app-server session prefix-decode readiness no-data active retained");
		assertStringEquals(TuiAppServerReadinessInteractionStatus.Drained.text(), outcome.latestReadinessStatusText(),
			"runner app-server session prefix-decode readiness status");
		assertStringEquals(TuiPromptSubmittedTurnLateJsonlDrainStatus.BatchRejected.text(), outcome.latestReadinessLateJsonlDrainStatusText(),
			"runner app-server session prefix-decode readiness late jsonl drain status");
		assertStringEquals("unknown_inbound_method", outcome.latestReadinessLateJsonlDrainCode(),
			"runner app-server session prefix-decode readiness late jsonl drain code");
		assertStringEquals(TuiAppServerJsonRpcTransportStatus.Accepted.text(), outcome.latestReadinessLateJsonlLineStatusText(),
			"runner app-server session prefix-decode readiness late jsonl line status");
		assertStringEquals("accepted", outcome.latestReadinessLateJsonlLineCode(), "runner app-server session prefix-decode readiness late jsonl line code");
		assertIntEquals(1, outcome.latestReadinessLateJsonlAppliedNotificationCount(),
			"runner app-server session prefix-decode readiness applied notification count");
		assertIntEquals(1, outcome.latestReadinessLateJsonlAssistantDeltaCount(), "runner app-server session prefix-decode readiness assistant delta count");
		assertIntEquals(0, outcome.latestReadinessLateJsonlCompletionCount(), "runner app-server session prefix-decode readiness completion count evidence");
		assertStringEquals(activeThread.toString(), outcome.latestReadinessLateJsonlThreadIdText(),
			"runner app-server session prefix-decode readiness applied thread evidence");
		assertStringEquals("turn-21", outcome.latestReadinessLateJsonlTurnIdText(), "runner app-server session prefix-decode readiness applied turn evidence");
		assertStringEquals("runner session prefix decode delta", outcome.latestReadinessLateJsonlDeltaText(),
			"runner app-server session prefix-decode readiness applied delta evidence");
		assertStringEquals("turn-21", outcome.latestReadinessActiveTurnIdText(),
			"runner app-server session prefix-decode readiness active retained after rejection");
		assertStringEquals("turn-21", outcome.lastStartedTurnIdText(), "runner app-server session prefix-decode readiness last started");
		assertStringEquals("", outcome.lastCompletedTurnIdText(), "runner app-server session prefix-decode readiness no completion");
		assertStringEquals("turn-21", outcome.activeTurnIdText(), "runner app-server session prefix-decode readiness active retained");
		assertIntEquals(0, outcome.completedTurns(), "runner app-server session prefix-decode readiness completed count");
		assertIntEquals(3, shell.transcriptCount(), "runner app-server session prefix-decode readiness transcript count");
		assertStringEquals("user> sessionprefixdecode", shell.transcriptAt(1).renderText(), "runner app-server session prefix-decode readiness user row");
		assertStringEquals("assistant> runner session prefix decode delta", shell.transcriptAt(2).renderText(),
			"runner app-server session prefix-decode readiness assistant row");
		assertStringEquals("assistant> runner session prefix decode delta", outcome.finalFrameLineAt(4),
			"runner app-server session prefix-decode readiness final frame");
		assertTrue(outcome.promptTransportLineCloseRecorded(), "runner app-server session prefix-decode readiness close recorded");
		assertIntEquals(1, outcome.promptTransportOutboundLineCount(), "runner app-server session prefix-decode readiness outbound lines");
		assertIntEquals(4, outcome.promptTransportInboundLineCount(), "runner app-server session prefix-decode readiness inbound lines");
		assertIntEquals(0, readinessSource.remaining(), "runner app-server session prefix-decode readiness source consumed");
		assertIntEquals(0, sessionReadinessSource.remaining(), "runner app-server session prefix-decode session source consumed");
	}
	...
	static function testReadinessSourcePrefixThenMalformedJsonRejectionRoutesThroughRunner():Void {
		final shell = ChatWidgetShellState.initial("pending");
		final activeThread = thread("00000000-0000-0000-0000-000000110001");
		final turnId = turn("turn-21");
		final lateLines = [
			new TuiPromptAgentMessageDeltaNotification(activeThread, turnId, item("item-runner-source-prefix-malformed-21"),
				"runner source prefix malformed delta").messageJson()
				+ "\n",
			"{\"jsonrpc\":\"2.0\",\"method\":\"turn/completed\",\"params\":\n"
		];
		final appServerTransport = new PersistentTuiAppServerJsonRpcLineConnectedTransport(TuiAppServerJsonRpcLineEndpoint.Stdio(stdioPersistentPlan([])),
		...
		final requestValue = request(shell, backend, [],
			TuiLiveShellRunPolicy.bounded(104, 3)).withJsonRpcPromptTransport(promptTransport).withReadinessSource(TuiLiveShellReadinessSource.queued([
				TuiAppServerReadinessEvent.SubmittedTurnLateJsonlReady(1, 3),
				TuiAppServerReadinessEvent.SubmittedTurnLateJsonlReady(1, 3)
			]));
		final outcome = TuiLiveShellRunner.run(requestValue);

		assertIntEquals(1, outcome.submittedPrompts(), "runner source prefix-malformed readiness submitted prompts");
		assertIntEquals(1, outcome.acceptedPrompts(), "runner source prefix-malformed readiness accepted prompts");
		assertIntEquals(2, outcome.appServerReadinessEvents(), "runner source prefix-malformed readiness event count");
		assertIntEquals(2, outcome.appServerReadinessDrained(), "runner source prefix-malformed readiness drained count");
		assertIntEquals(0, outcome.appServerReadinessNoPending(), "runner source prefix-malformed readiness no-pending count");
		assertIntEquals(1, outcome.appServerReadinessNoDataCount(), "runner source prefix-malformed readiness no-data count");
		assertStringEquals("turn-21", outcome.latestNoDataReadinessActiveTurnIdText(), "runner source prefix-malformed readiness no-data active retained");
		assertStringEquals(TuiAppServerReadinessInteractionStatus.Drained.text(), outcome.latestReadinessStatusText(),
			"runner source prefix-malformed readiness status");
		assertStringEquals(TuiPromptSubmittedTurnLateJsonlDrainStatus.BatchRejected.text(), outcome.latestReadinessLateJsonlDrainStatusText(),
			"runner source prefix-malformed readiness late jsonl drain status");
		assertStringEquals("invalid_json_line", outcome.latestReadinessLateJsonlDrainCode(), "runner source prefix-malformed readiness late jsonl drain code");
		assertStringEquals(TuiAppServerJsonRpcTransportStatus.Accepted.text(), outcome.latestReadinessLateJsonlLineStatusText(),
			"runner source prefix-malformed readiness late jsonl line status");
		assertStringEquals("accepted", outcome.latestReadinessLateJsonlLineCode(), "runner source prefix-malformed readiness late jsonl line code");
		assertIntEquals(1, outcome.latestReadinessLateJsonlAppliedNotificationCount(), "runner source prefix-malformed readiness applied notification count");
		assertIntEquals(1, outcome.latestReadinessLateJsonlAssistantDeltaCount(), "runner source prefix-malformed readiness assistant delta count");
		assertIntEquals(0, outcome.latestReadinessLateJsonlCompletionCount(), "runner source prefix-malformed readiness completion count evidence");
		assertStringEquals(activeThread.toString(), outcome.latestReadinessLateJsonlThreadIdText(),
			"runner source prefix-malformed readiness applied thread evidence");
		assertStringEquals("turn-21", outcome.latestReadinessLateJsonlTurnIdText(), "runner source prefix-malformed readiness applied turn evidence");
		assertStringEquals("runner source prefix malformed delta", outcome.latestReadinessLateJsonlDeltaText(),
			"runner source prefix-malformed readiness applied delta evidence");
		assertStringEquals("turn-21", outcome.latestReadinessActiveTurnIdText(), "runner source prefix-malformed readiness active retained after rejection");
		assertStringEquals("turn-21", outcome.lastStartedTurnIdText(), "runner source prefix-malformed readiness last started");
		assertStringEquals("", outcome.lastCompletedTurnIdText(), "runner source prefix-malformed readiness no completion");
		assertStringEquals("turn-21", outcome.activeTurnIdText(), "runner source prefix-malformed readiness active retained");
		assertIntEquals(0, outcome.completedTurns(), "runner source prefix-malformed readiness completed count");
		assertIntEquals(3, shell.transcriptCount(), "runner source prefix-malformed readiness transcript count");
		assertStringEquals("user> sourceprefixmalform", shell.transcriptAt(1).renderText(), "runner source prefix-malformed readiness user row");
		assertStringEquals("assistant> runner source prefix malformed delta", shell.transcriptAt(2).renderText(),
			"runner source prefix-malformed readiness assistant row");
		assertStringEquals("assistant> runner source prefix malformed delta", outcome.finalFrameLineAt(4),
			"runner source prefix-malformed readiness final frame");
		assertTrue(outcome.promptTransportLineCloseRecorded(), "runner source prefix-malformed readiness close recorded");
		assertIntEquals(1, outcome.promptTransportOutboundLineCount(), "runner source prefix-malformed readiness outbound lines");
		assertIntEquals(4, outcome.promptTransportInboundLineCount(), "runner source prefix-malformed readiness inbound lines");
	}
	...
	static function testReadinessSourcePrefixThenLineReadRejectionRoutesThroughRunner():Void {
		final shell = ChatWidgetShellState.initial("pending");
		final activeThread = thread("00000000-0000-0000-0000-000000110001");
		final turnId = turn("turn-18");
		final lateLines = [
			new TuiPromptAgentMessageDeltaNotification(activeThread, turnId, item("item-runner-source-prefix-line-read-18"),
				"runner source prefix line-read delta").messageJson()
				+ "\n"];
		final appServerTransport = new PersistentTuiAppServerJsonRpcLineConnectedTransport(TuiAppServerJsonRpcLineEndpoint.Stdio(stdioPersistentPlan([])),
			new DryRunTuiAppServerJsonRpcLineConnector(new DryRunTuiAppServerJsonRpcLineNativeOpener(),
				new RunnerNoDataThenRejectedLateJsonlLineTransportAttacher(lateLines, "runner_late_jsonl_read_failed")));
		...
		assertIntEquals(1, outcome.submittedPrompts(), "runner source prefix-line-read readiness submitted prompts");
		assertIntEquals(1, outcome.acceptedPrompts(), "runner source prefix-line-read readiness accepted prompts");
		assertIntEquals(2, outcome.appServerReadinessEvents(), "runner source prefix-line-read readiness event count");
		assertIntEquals(2, outcome.appServerReadinessDrained(), "runner source prefix-line-read readiness drained count");
		assertIntEquals(0, outcome.appServerReadinessNoPending(), "runner source prefix-line-read readiness no-pending count");
		assertIntEquals(1, outcome.appServerReadinessNoDataCount(), "runner source prefix-line-read readiness no-data count");
		assertStringEquals("turn-18", outcome.latestNoDataReadinessActiveTurnIdText(), "runner source prefix-line-read readiness no-data active retained");
		assertStringEquals(TuiAppServerReadinessInteractionStatus.Drained.text(), outcome.latestReadinessStatusText(),
			"runner source prefix-line-read readiness status");
		assertStringEquals(TuiPromptSubmittedTurnLateJsonlDrainStatus.LineReadRejected.text(), outcome.latestReadinessLateJsonlDrainStatusText(),
			"runner source prefix-line-read readiness late jsonl drain status");
		assertStringEquals("runner_late_jsonl_read_failed", outcome.latestReadinessLateJsonlDrainCode(),
			"runner source prefix-line-read readiness late jsonl drain code");
		assertStringEquals(TuiAppServerJsonRpcTransportStatus.Disconnected.text(), outcome.latestReadinessLateJsonlLineStatusText(),
			"runner source prefix-line-read readiness late jsonl line status");
		assertStringEquals("runner_late_jsonl_read_failed", outcome.latestReadinessLateJsonlLineCode(),
			"runner source prefix-line-read readiness late jsonl line code");
		assertIntEquals(1, outcome.latestReadinessLateJsonlAppliedNotificationCount(), "runner source prefix-line-read readiness applied notification count");
		assertIntEquals(1, outcome.latestReadinessLateJsonlAssistantDeltaCount(), "runner source prefix-line-read readiness assistant delta count");
		assertIntEquals(0, outcome.latestReadinessLateJsonlCompletionCount(), "runner source prefix-line-read readiness completion count evidence");
		assertStringEquals(activeThread.toString(), outcome.latestReadinessLateJsonlThreadIdText(),
			"runner source prefix-line-read readiness applied thread evidence");
		assertStringEquals("turn-18", outcome.latestReadinessLateJsonlTurnIdText(), "runner source prefix-line-read readiness applied turn evidence");
		assertStringEquals("runner source prefix line-read delta", outcome.latestReadinessLateJsonlDeltaText(),
			"runner source prefix-line-read readiness applied delta evidence");
		assertStringEquals("turn-18", outcome.latestReadinessActiveTurnIdText(), "runner source prefix-line-read readiness active retained after rejection");
		assertStringEquals("turn-18", outcome.lastStartedTurnIdText(), "runner source prefix-line-read readiness last started");
		assertStringEquals("", outcome.lastCompletedTurnIdText(), "runner source prefix-line-read readiness no completion");
		assertStringEquals("turn-18", outcome.activeTurnIdText(), "runner source prefix-line-read readiness active retained");
		assertIntEquals(0, outcome.completedTurns(), "runner source prefix-line-read readiness completed count");
		assertIntEquals(3, shell.transcriptCount(), "runner source prefix-line-read readiness transcript count");
		assertStringEquals("user> sourceprefixfail", shell.transcriptAt(1).renderText(), "runner source prefix-line-read readiness user row");
		assertStringEquals("assistant> runner source prefix line-read delta", shell.transcriptAt(2).renderText(),
			"runner source prefix-line-read readiness assistant row");
		assertStringEquals("assistant> runner source prefix line-read delta", outcome.finalFrameLineAt(4),
			"runner source prefix-line-read readiness final frame");
		assertTrue(outcome.promptTransportLineCloseRecorded(), "runner source prefix-line-read readiness close recorded");
		assertIntEquals(1, outcome.promptTransportOutboundLineCount(), "runner source prefix-line-read readiness outbound lines");
		assertIntEquals(3, outcome.promptTransportInboundLineCount(), "runner source prefix-line-read readiness inbound lines");
	}
	...
	static function testReadinessSourcePrefixThenStaleInterruptedRejectionRoutesThroughRunner():Void {
		final shell = ChatWidgetShellState.initial("pending");
		final activeThread = thread("00000000-0000-0000-0000-000000110001");
		final turnId = turn("turn-19");
		final lateLines = [
			new TuiPromptAgentMessageDeltaNotification(activeThread, turnId, item("item-runner-source-prefix-stale-19"),
				"runner source prefix stale delta").messageJson()
				+ "\n",
			new TuiPromptAgentMessageDeltaNotification(activeThread, turnId, item("item-runner-source-prefix-stale-late-19"),
				"runner source stale after prefix").messageJson()
				+ "\n"];
		final appServerTransport = new PersistentTuiAppServerJsonRpcLineConnectedTransport(TuiAppServerJsonRpcLineEndpoint.Stdio(stdioPersistentPlan([])),
			new DryRunTuiAppServerJsonRpcLineConnector(new DryRunTuiAppServerJsonRpcLineNativeOpener(),
				new RunnerNoDataLateJsonlLineTransportAttacher(lateLines)));
		...
		assertIntEquals(1, outcome.submittedPrompts(), "runner source prefix-stale readiness submitted prompts");
		assertIntEquals(1, outcome.acceptedPrompts(), "runner source prefix-stale readiness accepted prompts");
		assertIntEquals(3, outcome.appServerReadinessEvents(), "runner source prefix-stale readiness event count");
		assertIntEquals(3, outcome.appServerReadinessDrained(), "runner source prefix-stale readiness drained count");
		assertIntEquals(0, outcome.appServerReadinessNoPending(), "runner source prefix-stale readiness no-pending count");
		assertIntEquals(1, outcome.appServerReadinessNoDataCount(), "runner source prefix-stale readiness no-data count");
		assertStringEquals("turn-19", outcome.latestNoDataReadinessActiveTurnIdText(), "runner source prefix-stale readiness no-data active retained");
		assertStringEquals(TuiAppServerReadinessInteractionStatus.Drained.text(), outcome.latestReadinessStatusText(),
			"runner source prefix-stale readiness status");
		assertStringEquals(TuiPromptSubmittedTurnLateJsonlDrainStatus.BatchRejected.text(), outcome.latestReadinessLateJsonlDrainStatusText(),
			"runner source prefix-stale readiness late jsonl drain status");
		assertStringEquals(TuiPromptSubmittedTurnStreamDeliveryStatus.StaleInterruptedTurn.text(), outcome.latestReadinessLateJsonlDrainCode(),
			"runner source prefix-stale readiness late jsonl drain code");
		assertStringEquals(TuiAppServerJsonRpcTransportStatus.Accepted.text(), outcome.latestReadinessLateJsonlLineStatusText(),
			"runner source prefix-stale readiness late jsonl line status");
		assertStringEquals("accepted", outcome.latestReadinessLateJsonlLineCode(), "runner source prefix-stale readiness late jsonl line code");
		assertIntEquals(0, outcome.latestReadinessLateJsonlAppliedNotificationCount(),
			"runner source prefix-stale readiness rejected notification applied count");
		assertIntEquals(0, outcome.latestReadinessLateJsonlAssistantDeltaCount(), "runner source prefix-stale readiness rejected assistant delta count");
		assertIntEquals(0, outcome.latestReadinessLateJsonlCompletionCount(), "runner source prefix-stale readiness completion count evidence");
		assertStringEquals(activeThread.toString(), outcome.latestReadinessLateJsonlThreadIdText(),
			"runner source prefix-stale readiness rejected thread evidence");
		assertStringEquals("turn-19", outcome.latestReadinessLateJsonlTurnIdText(), "runner source prefix-stale readiness rejected turn evidence");
		assertStringEquals("runner source stale after prefix", outcome.latestReadinessLateJsonlDeltaText(),
			"runner source prefix-stale readiness rejected delta evidence");
		assertStringEquals("", outcome.latestReadinessActiveTurnIdText(), "runner source prefix-stale readiness active remains cleared after rejection");
		assertStringEquals("turn-19", outcome.lastStartedTurnIdText(), "runner source prefix-stale readiness last started");
		assertStringEquals("", outcome.lastCompletedTurnIdText(), "runner source prefix-stale readiness no completion");
		assertStringEquals("turn-19", outcome.lastInterruptedTurnIdText(), "runner source prefix-stale readiness last interrupted");
		assertStringEquals("", outcome.activeTurnIdText(), "runner source prefix-stale readiness active cleared by interrupt");
		assertIntEquals(0, outcome.completedTurns(), "runner source prefix-stale readiness completed count");
		assertIntEquals(1, outcome.interruptedTurns(), "runner source prefix-stale readiness interrupted count");
		assertStringEquals("accepted", outcome.lastInterruptCode(), "runner source prefix-stale readiness interrupt code");
		assertIntEquals(3, shell.transcriptCount(), "runner source prefix-stale readiness transcript count");
		assertStringEquals("user> sourceprefixstale", shell.transcriptAt(1).renderText(), "runner source prefix-stale readiness user row");
		assertStringEquals("assistant> runner source prefix stale delta", shell.transcriptAt(2).renderText(),
			"runner source prefix-stale readiness assistant prefix row");
		assertStringEquals("assistant> runner source prefix stale delta", outcome.finalFrameLineAt(4),
			"runner source prefix-stale readiness final frame preserves prefix");
		assertTrue(outcome.promptTransportLineCloseRecorded(), "runner source prefix-stale readiness close recorded");
		assertIntEquals(2, outcome.promptTransportOutboundLineCount(), "runner source prefix-stale readiness outbound lines");
		assertIntEquals(5, outcome.promptTransportInboundLineCount(), "runner source prefix-stale readiness inbound lines");
	}
	...
	static function testReadinessSourceStaleInterruptedRejectionRoutesThroughRunner():Void {
		final shell = ChatWidgetShellState.initial("pending");
		final activeThread = thread("00000000-0000-0000-0000-000000110001");
		final turnId = turn("turn-13");
		final lateLines = [
			new TuiPromptAgentMessageDeltaNotification(activeThread, turnId, item("item-runner-source-stale-13"), "runner source stale delta").messageJson()
				+ "\n"];
		final appServerTransport = new PersistentTuiAppServerJsonRpcLineConnectedTransport(TuiAppServerJsonRpcLineEndpoint.Stdio(stdioPersistentPlan([])),
			new DryRunTuiAppServerJsonRpcLineConnector(new DryRunTuiAppServerJsonRpcLineNativeOpener(),
				new RunnerNoDataLateJsonlLineTransportAttacher(lateLines)));
		...
		final requestValue = request(shell, backend, [],
			TuiLiveShellRunPolicy.bounded(48, 3)).withJsonRpcPromptTransport(promptTransport).withReadinessSource(TuiLiveShellReadinessSource.queued([
				TuiAppServerReadinessEvent.SubmittedTurnLateJsonlReady(1, 2),
				TuiAppServerReadinessEvent.SubmittedTurnLateJsonlReady(1, 2)
			]));
		final outcome = TuiLiveShellRunner.run(requestValue);

		assertIntEquals(1, outcome.submittedPrompts(), "runner source stale readiness submitted prompts");
		assertIntEquals(1, outcome.acceptedPrompts(), "runner source stale readiness accepted prompts");
		assertIntEquals(2, outcome.appServerReadinessEvents(), "runner source stale readiness event count");
		assertIntEquals(2, outcome.appServerReadinessDrained(), "runner source stale readiness drained count");
		assertIntEquals(0, outcome.appServerReadinessNoPending(), "runner source stale readiness no-pending count");
		assertIntEquals(1, outcome.appServerReadinessNoDataCount(), "runner source stale readiness no-data count");
		assertStringEquals("turn-13", outcome.latestNoDataReadinessActiveTurnIdText(), "runner source stale readiness no-data active retained");
		assertStringEquals(TuiAppServerReadinessInteractionStatus.Drained.text(), outcome.latestReadinessStatusText(), "runner source stale readiness status");
		assertStringEquals(TuiPromptSubmittedTurnLateJsonlDrainStatus.BatchRejected.text(), outcome.latestReadinessLateJsonlDrainStatusText(),
			"runner source stale readiness late jsonl drain status");
		assertStringEquals(TuiPromptSubmittedTurnStreamDeliveryStatus.StaleInterruptedTurn.text(), outcome.latestReadinessLateJsonlDrainCode(),
			"runner source stale readiness late jsonl drain code");
		assertStringEquals(TuiAppServerJsonRpcTransportStatus.Accepted.text(), outcome.latestReadinessLateJsonlLineStatusText(),
			"runner source stale readiness late jsonl line status");
		assertStringEquals("accepted", outcome.latestReadinessLateJsonlLineCode(), "runner source stale readiness late jsonl line code");
		assertIntEquals(0, outcome.latestReadinessLateJsonlAppliedNotificationCount(), "runner source stale readiness rejected notification applied count");
		assertIntEquals(0, outcome.latestReadinessLateJsonlAssistantDeltaCount(), "runner source stale readiness rejected assistant delta count");
		assertIntEquals(0, outcome.latestReadinessLateJsonlCompletionCount(), "runner source stale readiness completion count evidence");
		assertStringEquals(activeThread.toString(), outcome.latestReadinessLateJsonlThreadIdText(), "runner source stale readiness rejected thread evidence");
		assertStringEquals("turn-13", outcome.latestReadinessLateJsonlTurnIdText(), "runner source stale readiness rejected turn evidence");
		assertStringEquals("runner source stale delta", outcome.latestReadinessLateJsonlDeltaText(), "runner source stale readiness rejected delta evidence");
		assertStringEquals("", outcome.latestReadinessActiveTurnIdText(), "runner source stale readiness active remains cleared after rejection");
		assertStringEquals("turn-13", outcome.lastStartedTurnIdText(), "runner source stale readiness last started");
		assertStringEquals("", outcome.lastCompletedTurnIdText(), "runner source stale readiness no completion");
		assertStringEquals("turn-13", outcome.lastInterruptedTurnIdText(), "runner source stale readiness last interrupted");
		assertStringEquals("", outcome.activeTurnIdText(), "runner source stale readiness active cleared by interrupt");
		assertIntEquals(0, outcome.completedTurns(), "runner source stale readiness completed count");
		assertIntEquals(1, outcome.interruptedTurns(), "runner source stale readiness interrupted count");
		assertStringEquals("accepted", outcome.lastInterruptCode(), "runner source stale readiness interrupt code");
		assertIntEquals(2, shell.transcriptCount(), "runner source stale readiness transcript count");
		assertStringEquals("user> sourcestale", shell.transcriptAt(1).renderText(), "runner source stale readiness user row");
		assertTrue(outcome.promptTransportLineCloseRecorded(), "runner source stale readiness close recorded");
		assertIntEquals(2, outcome.promptTransportOutboundLineCount(), "runner source stale readiness outbound lines");
		assertIntEquals(4, outcome.promptTransportInboundLineCount(), "runner source stale readiness inbound lines");
	}
	...
	static function testReadinessSourceStaleInterruptedCompletionRejectionRoutesThroughRunner():Void {
		final shell = ChatWidgetShellState.initial("pending");
		final activeThread = thread("00000000-0000-0000-0000-000000110001");
		final turnId = turn("turn-12");
		final lateLines = [turnCompletedLine(activeThread, turnId)];
		final appServerTransport = new PersistentTuiAppServerJsonRpcLineConnectedTransport(TuiAppServerJsonRpcLineEndpoint.Stdio(stdioPersistentPlan([])),
			new DryRunTuiAppServerJsonRpcLineConnector(new DryRunTuiAppServerJsonRpcLineNativeOpener(),
				new RunnerNoDataLateJsonlLineTransportAttacher(lateLines)));
		...
		assertIntEquals(1, outcome.submittedPrompts(), "runner source stale completion readiness submitted prompts");
		assertIntEquals(1, outcome.acceptedPrompts(), "runner source stale completion readiness accepted prompts");
		assertIntEquals(2, outcome.appServerReadinessEvents(), "runner source stale completion readiness event count");
		assertIntEquals(2, outcome.appServerReadinessDrained(), "runner source stale completion readiness drained count");
		assertIntEquals(0, outcome.appServerReadinessNoPending(), "runner source stale completion readiness no-pending count");
		assertIntEquals(1, outcome.appServerReadinessNoDataCount(), "runner source stale completion readiness no-data count");
		assertStringEquals("turn-12", outcome.latestNoDataReadinessActiveTurnIdText(), "runner source stale completion readiness no-data active retained");
		assertStringEquals(TuiAppServerReadinessInteractionStatus.Drained.text(), outcome.latestReadinessStatusText(),
			"runner source stale completion readiness status");
		assertStringEquals(TuiPromptSubmittedTurnLateJsonlDrainStatus.BatchRejected.text(), outcome.latestReadinessLateJsonlDrainStatusText(),
			"runner source stale completion readiness late jsonl drain status");
		assertStringEquals(TuiPromptSubmittedTurnCompletionStatus.StaleInterruptedTurn.text(), outcome.latestReadinessLateJsonlDrainCode(),
			"runner source stale completion readiness late jsonl drain code");
		assertStringEquals(TuiAppServerJsonRpcTransportStatus.Accepted.text(), outcome.latestReadinessLateJsonlLineStatusText(),
			"runner source stale completion readiness late jsonl line status");
		assertStringEquals("accepted", outcome.latestReadinessLateJsonlLineCode(), "runner source stale completion readiness late jsonl line code");
		assertIntEquals(0, outcome.latestReadinessLateJsonlAppliedNotificationCount(),
			"runner source stale completion readiness rejected completion applied count");
		assertIntEquals(0, outcome.latestReadinessLateJsonlAssistantDeltaCount(), "runner source stale completion readiness rejected assistant delta count");
		assertIntEquals(0, outcome.latestReadinessLateJsonlCompletionCount(), "runner source stale completion readiness completion count evidence");
		assertStringEquals(activeThread.toString(), outcome.latestReadinessLateJsonlThreadIdText(),
			"runner source stale completion readiness rejected thread evidence");
		assertStringEquals("turn-12", outcome.latestReadinessLateJsonlTurnIdText(), "runner source stale completion readiness rejected turn evidence");
		assertStringEquals("", outcome.latestReadinessLateJsonlDeltaText(), "runner source stale completion readiness no rejected delta evidence");
		assertStringEquals("", outcome.latestReadinessActiveTurnIdText(), "runner source stale completion readiness active remains cleared after rejection");
		assertStringEquals("turn-12", outcome.lastStartedTurnIdText(), "runner source stale completion readiness last started");
		assertStringEquals("", outcome.lastCompletedTurnIdText(), "runner source stale completion readiness no completion");
		assertStringEquals("turn-12", outcome.lastInterruptedTurnIdText(), "runner source stale completion readiness last interrupted");
		assertStringEquals("", outcome.activeTurnIdText(), "runner source stale completion readiness active cleared by interrupt");
		assertIntEquals(0, outcome.completedTurns(), "runner source stale completion readiness completed count");
		assertIntEquals(1, outcome.interruptedTurns(), "runner source stale completion readiness interrupted count");
		assertStringEquals("accepted", outcome.lastInterruptCode(), "runner source stale completion readiness interrupt code");
		assertIntEquals(2, shell.transcriptCount(), "runner source stale completion readiness transcript count");
		assertStringEquals("user> sourcedone", shell.transcriptAt(1).renderText(), "runner source stale completion readiness user row");
		assertTrue(outcome.promptTransportLineCloseRecorded(), "runner source stale completion readiness close recorded");
		assertIntEquals(2, outcome.promptTransportOutboundLineCount(), "runner source stale completion readiness outbound lines");
		assertIntEquals(4, outcome.promptTransportInboundLineCount(), "runner source stale completion readiness inbound lines");
	}
