...
class ResumePickerLiveAppServerBoundaryRenderHarness {
	static function main():Void {
		...
		assertContains(snapshots[0], "loader status=live_boundary_backpressure detail=request=live-blocked-reload-page;cursor=cursor-live-2;query=kernel;sort=created_at;filter=all;pending=1;skipped=0");
		...
		assertContains(snapshots[2], "loader status=live_boundary_error_mapped detail=request=live-missing-page;sourceCode=missing_page_fixture;preservedThread=thread-boundary-a;rows=2");
		assertContains(report.finalSnapshot, "> Boundary recovered row | thread-boundary-recovered | turns=8 | 2026-06-19T21:10:00Z | cwd=/workspace/codex-hxrust");
		...
		assertContains(report.summary(), "id=live-blocked-reload-page;cursor=cursor-live-2;query=kernel;pageSize=2;sort=created_at;filter=all;cwd=/workspace/codex-hxrust;showAll=true;includeNonInteractive=true");
		...
		assertContains(report.summary(), "mapped-error:query=kernel;sort=created_at;filter=all;rows=2;selected=0;thread=thread-boundary-a;errorShown=true;failure=app_server_thread_list_failed");
