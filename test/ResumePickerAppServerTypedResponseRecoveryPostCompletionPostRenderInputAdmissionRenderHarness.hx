import codexhx.runtime.tui.resume.live.ResumePickerAppServerTypedResponseRecoveryPostCompletionPostRenderInputAdmissionRenderGate;

class ResumePickerAppServerTypedResponseRecoveryPostCompletionPostRenderInputAdmissionRenderHarness {
	static function main():Void {
		final report = ResumePickerAppServerTypedResponseRecoveryPostCompletionPostRenderInputAdmissionRenderGate.run();
		...
		assertContains(report.summary(), "replayCount=2;snapshotOrderPreserved=true;selectedMarkersPreserved=true");
		...
		assertContains(report.summary(),
			"noLeftoverScheduledRenderRequest=true;sourceSchedulerRequestCount=1;consumedScheduledRequestCount=1;sourceRenderCount=1");
		...
		assertContains(report.summary(), "noModelCall=true;noFilesystemMutation=true;reason=post_completion_post_render_input_admitted_after_replay");
		assertContains(report.summary(), "handoffKind=completed_recovered_selection;completionReady=true");
		assertContains(report.summary(), "reason=post_completion_post_render_replay_completed_for_next_slice");
		assertNotContains(report.summary(), "post_completion_post_render_input_rejected_before_replay");
