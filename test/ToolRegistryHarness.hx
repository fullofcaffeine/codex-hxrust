...
class ToolRegistryHarness {
	...
	static function fixtureRegistry():ToolRegistry {
		...
		expectLookup(registry.registerLocalFunction("get_goal", "Read the current thread goal.",
			object(["type", "additionalProperties"], [Value.JString("object"), Value.JBool(false)])));
		expectLookup(registry.registerMcpTool("music-studio", "get-strudel-guide", "Return Strudel music help.", object(["type", "properties"], [
			Value.JString("object"),
			object(["query"], [object(["type"], [Value.JString("string")])])
		]), "connector-music", "Music Studio", true));
		expectLookup(registry.registerDynamicTool("agent", "summarize", "Summarize selected thread context.", object(["type"], [Value.JString("object")]),
			true));
		return registry;
	}
