import codexhx.runtime.tui.resume.live.ResumePickerAppServerTypedResponseRecoveryPostCompletionPostRenderReplayAwareRenderedStateEighthCycleInputAdmissionRenderGate;

class ResumePickerAppServerTypedResponseRecoveryPostCompletionPostRenderReplayAwareRenderedStateEighthCycleInputAdmissionRenderHarness {
	static function main():Void {
		final report = ResumePickerAppServerTypedResponseRecoveryPostCompletionPostRenderReplayAwareRenderedStateEighthCycleInputAdmissionRenderGate.run();
		...
		assertUnder(17000, summary.length, "summary length");
		assertContains(summary, "admissionKind=input_admitted;inputAdmitted=true;sourceReadinessDecisionCount=2");
		assertContains(summary, "sourceRenderStateCount=2;sourceFrameRequests=2;sourceKeyboardRenderCount=2;replayCount=2;sourceReplayCount=2");
		assertContains(summary, "sourceHandoffReplayCount=2;sourceHandoffReadinessDecisionCount=2;sourceHandoffRenderStateCount=2");
		...
		assertContains(summary, "finalFooter=footer keyboard move_up selected=0 selectedThread=thread-surface-a");
		assertContains(summary, "finalSelectionPreserved=true;finalFooterPreserved=true;completionReady=true;nextSliceReady=true");
		...
		assertContains(summary, "renderedSnapshotPreserved=true;sourceRenderedSnapshotPreserved=true");
		...
		assertContains(summary, "kind=input_admitted;intent=confirm_recovered_selection;admitted=true;sequence=1;finalThread=thread-surface-a");
		assertContains(summary,
			"noModelCall=true;noFilesystemMutation=true;reason=post_completion_post_render_replay_aware_rendered_state_eighth_cycle_input_admitted_after_replay");
		assertContains(summary, "handoffKind=completed_recovered_selection;completionReady=true");
		assertContains(summary, "reason=post_completion_post_render_replay_aware_rendered_state_eighth_cycle_replay_completed_for_next_slice");
		assertNotContains(summary, "post_completion_post_render_replay_aware_rendered_state_eighth_cycle_input_rejected_before_replay");
