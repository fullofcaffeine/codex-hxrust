...
import codexhx.runtime.app.threadread.RolloutSummaryItem;
import codexhx.runtime.app.threadread.ThreadReadTurnProjection;
import codexhx.runtime.app.threadread.ThreadReadTurnProjectionReport;
import haxe.json.Value;
import sys.io.File;

class ThreadReadTurnProjectionHarness {
	static function main():Void {
		final root = fixtureRoot();
		final cases = arrayField(root, "cases");
		final report = ThreadReadTurnProjection.projectCases(projectCases(cases));
	...
	static function projectCases(values:Array<Value>):Array<Array<RolloutSummaryItem>> {
		final out:Array<Array<RolloutSummaryItem>> = [];
		for (value in values) {
			final caseObject = objectValue(value);
			final items:Array<RolloutSummaryItem> = [];
			for (itemValue in arrayField(caseObject, "items")) {
				items.push(rolloutItem(objectValue(itemValue)));
			}
			out.push(items);
		}
		return out;
	...
	static function fixtureRoot():Value {
		return expectParse(CodexJson.parse(File.getContent("fixtures/hxrust/thread-read-turn-projection.v1.json")));
	}
