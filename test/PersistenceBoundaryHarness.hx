...
class PersistenceBoundaryHarness {
    ...
    static function runCase(caseValue:Value):Void {
        ...
        if (summaryNeedle.length > 0) assertContains(outcome.summary, summaryNeedle);
    }
    ...
    static function requestFromCase(caseValue:Value):PersistenceBoundaryRequest {
        ...
        return new PersistenceBoundaryRequest(
            cast stringField(caseValue, "backend", ""),
            ...
            boolField(native, "stateDb", false),
            boolField(native, "logDb", false),
            boolField(native, "reconcileRollout", false),
