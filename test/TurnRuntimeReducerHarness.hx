...
class TurnRuntimeReducerHarness {
    ...
    static function parseAction(value:Value):TurnRuntimeAction {
        ...
        return switch actionType {
            ...
            case "task_completed":
                TurnRuntimeAction.taskCompleted(
                    stringField(value, "lastAgentMessage", ""),
                    boolField(value, "fromReplay", false),
    ...
    static function maybeBool(object:Value, field:String, scenario:String, actual:Bool):Void {
        ...
        switch found.value {
            case JBool(expected):
                if (expected != actual) throw scenario + "." + field + " expected " + Std.string(expected) + " but got " + Std.string(actual);
            case _:
        ...
        return switch object {
            case JObject(keys, values):
                ...
                while (i < keys.length && i < values.length) {
                    if (keys[i] == name) return new FieldLookup(true, values[i]);
                    i = i + 1;
                }
