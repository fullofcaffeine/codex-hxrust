...
class ResumePickerReloadTranscriptOverlayInvalidationRenderHarness {
	static function main():Void {
		...
		assertContains(snapshots[1], "loader status=transcript_overlay_preserved_for_selected_thread detail=selected=thread-a;index=1;pending=thread-a;cells=3");
		...
		assertContains(report.finalSnapshot, "loader status=transcript_overlay_invalidated_after_selection_change detail=missing=thread-a;selected=thread-b;pendingCleared=true;overlayClosed=true;cells=0");
		...
		assertContains(report.summary(), "loaded-overlay:query=overlay;rows=3;selected=0;thread=thread-a;pending=thread-a;transcript=loaded;overlayOpen=true;overlayCloseCount=0;cells=3");
		assertContains(report.summary(), "preserved:query=overlay;rows=3;selected=1;thread=thread-a;pending=thread-a;transcript=loaded;overlayOpen=true;overlayCloseCount=0;cells=3");
		assertContains(report.summary(), "invalidated:query=overlay;rows=2;selected=0;thread=thread-b;pending=<empty>;transcript=<empty>;overlayOpen=false;overlayCloseCount=1;cells=0");
