import codexhx.runtime.tui.resume.live.ResumePickerAppServerTypedResponseRecoveryPostCompletionPostRenderReplayAwareRenderedStateEighthCycleKeyboardRenderStateRenderGate;
...
class ResumePickerAppServerTypedResponseRecoveryPostCompletionPostRenderReplayAwareRenderedStateEighthCycleKeyboardRenderStateRenderHarness {
	static function main():Void {
		final report = ResumePickerAppServerTypedResponseRecoveryPostCompletionPostRenderReplayAwareRenderedStateEighthCycleKeyboardRenderStateRenderGate.run();
		...
		assertContains(summary, "readinessDecisionCount=2;readinessAdmittedCount=2;renderStateCount=2;frameRequests=2;renderCount=2");
		assertContains(summary, "selectedMarkerMoved=true;recoveredSelectionRestored=true;noLeftoverScheduledRenderRequest=true");
		assertContains(summary, "sourceSchedulerRequestCount=1;consumedScheduledRequestCount=1;sourceRenderCount=1");
		...
		assertContains(summary, "selectedMarkersPreserved=true;footerSummariesPreserved=true;sourceSelectedMarkerMoved=true");
		assertContains(summary, "sourceRecoveredSelectionRestored=true;sourcePreExecutionSchedulerRequestCount=1");
		...
		assertContains(summary, "kind=navigation_render_state;intent=move_down;sequence=1;selectedIndex=1;selectedThread=thread-surface-b");
		assertContains(summary, "kind=navigation_render_state;intent=move_up;sequence=2;selectedIndex=0;selectedThread=thread-surface-a");
		assertContains(summary, "selectedMarker=> Recovered navigation target | thread-surface-b");
		assertContains(summary, "selectedMarker=> Recovered surface thread | thread-surface-a");
		assertContains(summary, "reason=recovery_keyboard_render_state_projected");
		assertContains(summary, "kind=post_render_keyboard_ready;sourceHandoffKind=rendered_idle_list_ready");
		assertContains(summary, "reason=post_completion_post_render_replay_aware_rendered_state_eighth_cycle_keyboard_ready");
		assertContains(summary, "reason=post_completion_post_render_replay_aware_rendered_state_eighth_cycle_keyboard_render_state_decision_ready");
		assertContains(summary, "noModelCall=true;noFilesystemMutation=true");
		assertNotContains(summary, "post_completion_post_render_replay_aware_rendered_state_eighth_cycle_keyboard_render_state_decision_rejected");
