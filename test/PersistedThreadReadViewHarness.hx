...
import codexhx.runtime.app.persistence.PersistedThreadReadReport;
import codexhx.runtime.app.persistence.PersistedThreadReadRequest;
import codexhx.runtime.app.persistence.PersistedThreadReadViewBuilder;
import codexhx.runtime.app.persistence.ThreadPersistenceMetadata;
import haxe.json.Value;
import sys.io.File;

class PersistedThreadReadViewHarness {
	static function main():Void {
		final root = fixtureRoot();
		final adapterCommands = arrayField(root, "adapterCommands");
		final adapterReport = StateSqliteBridge.runInMemory(commandsFromFixture(adapterCommands));
		assertAdapterReport(root, adapterReport);

		final readRequests = arrayField(root, "readRequests");
		final readReport = PersistedThreadReadViewBuilder.fromAdapterReport(adapterReport, readRequestsFromFixture(readRequests));
		assertReadReport(root, readReport);
		assertEquals(Std.string(readRequests.length), Std.string(readReport.outcomes.length));

		var i = 0;
		while (i < readRequests.length) {
			final expect = objectField(objectValue(readRequests[i]), "expect");
			final outcome = readReport.outcomes[i];
	...
	static function assertAdapterReport(root:Value, report:StateSqliteAdapterReport):Void {
		final expect = objectField(root, "expectAdapterReport");
	...
	static function assertReadReport(root:Value, report:PersistedThreadReadReport):Void {
		final expect = objectField(root, "expectReadReport");
		assertEquals(Std.string(intField(expect, "operationCount", 0)), Std.string(report.outcomes.length));
		assertEquals(Std.string(intField(expect, "okCount", 0)), Std.string(report.okCount()));
		assertEquals(Std.string(intField(expect, "failureCount", 0)), Std.string(report.failureCount()));
	...
	static function fixtureRoot():Value {
		return expectParse(CodexJson.parse(File.getContent("fixtures/hxrust/persisted-thread-read-view.v1.json")));
	}
