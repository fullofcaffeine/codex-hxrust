import codexhx.runtime.tui.resume.live.ResumePickerAppServerTypedResponseRecoveryPostCompletionPostRenderReplayAwareScheduledRenderExecutionRenderGate;

class ResumePickerAppServerTypedResponseRecoveryPostCompletionPostRenderReplayAwareScheduledRenderExecutionRenderHarness {
	static function main():Void {
		final report = ResumePickerAppServerTypedResponseRecoveryPostCompletionPostRenderReplayAwareScheduledRenderExecutionRenderGate.run();
		...
		assertContains(report.summary(), "sourceRenderStateCount=2;sourceFrameRequests=2;sourceKeyboardRenderCount=2;finalThread=thread-surface-a");
		assertContains(report.summary(), "finalFooter=footer keyboard move_up selected=0 selectedThread=thread-surface-a");
		assertContains(report.summary(), "inputAdmitted=true;replayCount=2;sourceReplayCount=2;snapshotOrderPreserved=true");
		...
		assertContains(report.summary(), "reason=post_completion_post_render_replay_aware_local_render_request_executed");
		assertContains(report.summary(), "scheduleKind=local_render_scheduled;scheduleRequested=true;scheduled=true");
		assertContains(report.summary(), "schedulerSummary=typed_response_recovery_post_completion_post_render_replay_aware_local_render:thread-surface-a");
		assertNotContains(report.summary(), "post_completion_post_render_replay_aware_local_render_request_execution_rejected");
