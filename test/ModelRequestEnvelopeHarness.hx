...
import codexhx.runtime.model.request.ModelRequestEnvelopeReport;
import codexhx.runtime.model.request.ModelRequestEnvelopeRequest;
import codexhx.runtime.model.request.ModelRequestPromptEnvelope;
import codexhx.runtime.model.request.ModelRequestRouteIntent;
import haxe.json.Value;
import sys.io.File;

class ModelRequestEnvelopeHarness {
	static function main():Void {
		final root = fixtureRoot();
		final cases = arrayField(root, "cases");
		final report = ModelRequestEnvelopePolicy.buildCases(requests(cases));
		...
		while (i < cases.length) {
			...
			assertEquals(stringField(expect, "planCode", ""), outcome.planCode);
			...
			assertEquals(stringField(expect, "routeIntent", ""), outcome.routeIntent);
	...
	public static function requests(values:Array<Value>):Array<ModelRequestEnvelopeRequest> {
		...
		for (value in values) {
			...
			out.push(new ModelRequestEnvelopeRequest(stringField(request, "requestId", ""), planRequest(objectField(request, "planRequest")),
				promptEnvelope(objectField(request, "prompt")), routeIntent(stringField(request, "routeIntent", "")),
				boolField(request, "liveNetworkAllowed", false), boolField(request, "hasCredentialMaterial", false), stringField(request, "secretProbe", "")));
		}
		return out;
	}

	static function planRequest(request:Value):TurnModelPlanRequest {
		return new TurnModelPlanRequest(stringField(request, "requestId", ""), catalogRequest(objectField(request, "catalogRequest")),
	...
	static function promptEnvelope(value:Value):ModelRequestPromptEnvelope {
		return new ModelRequestPromptEnvelope(intField(value, "inputItemCount", 0), intField(value, "toolSpecCount", 0),
			boolField(value, "parallelToolCalls", false), boolField(value, "supportsReasoningSummaries", false), stringField(value, "reasoningEffort", ""),
			stringField(value, "reasoningSummary", "none"), boolField(value, "supportsVerbosity", false), stringField(value, "requestedVerbosity", ""),
			boolField(value, "outputSchemaPresent", false), boolField(value, "outputSchemaStrict", false), stringField(value, "requestedServiceTier", ""),
			stringField(value, "windowId", ""), boolField(value, "turnMetadataHeaderPresent", false), boolField(value, "betaFeaturesHeaderPresent", false),
			boolField(value, "enableRequestCompression", false));
	}
	...
	static function routeIntent(value:String):ModelRequestRouteIntent {
		return switch value {
			case "plan_only": ModelRequestRouteIntent.PlanOnly;
			case "responses_http": ModelRequestRouteIntent.ResponsesHttp;
			case "responses_websocket": ModelRequestRouteIntent.ResponsesWebsocket;
			case _: throw "invalid route intent: " + value;
		}
	}

	static function assertReport(root:Value, report:ModelRequestEnvelopeReport):Void {
		...
		assertEquals(Std.string(intField(expect, "liveRouteDeniedCount", 0)), Std.string(report.liveRouteDeniedCount()));
		assertEquals(Std.string(intField(expect, "planDeniedCount", 0)), Std.string(report.planDeniedCount()));
		assertContains(report.summary(), stringField(expect, "summaryContains", ""));
	}

	static function fixtureRoot():Value {
		return expectParse(CodexJson.parse(File.getContent("fixtures/hxrust/model-request-envelope.v1.json")));
	}
