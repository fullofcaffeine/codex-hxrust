...
class RuntimeBootstrapHarness {
	...
	static function bootstrapCase(value:Value):codexhx.runtime.app.bootstrap.BootstrapValidationOutcome {
		return CodexBootstrapSession.bootstrap(modeField(value, "mode"), stringField(value, "requestId", "initialize"), initializeParams(value),
			initializeResponse(objectField(value, "response")), startupMetadata(objectField(value, "startup")));
	}

	static function initializeParams(value:Value):BootstrapInitializeParams {
		return new BootstrapInitializeParams(clientInfo(objectField(value, "clientInfo")), true, capabilities(objectField(value, "capabilities")));
	}
	...
	static function capabilities(value:Value):BootstrapCapabilities {
		return new BootstrapCapabilities(boolField(value, "experimentalApi", false), boolField(value, "requestAttestation", false),
	...
	static function initializeResponse(value:Value):BootstrapInitializeResponse {
		return new BootstrapInitializeResponse(stringField(value, "userAgent", ""), stringField(value, "codexHome", ""),
			stringField(value, "platformFamily", ""), stringField(value, "platformOs", ""));
	}
	...
	static function startupMetadata(value:Value):BootstrapStartupMetadata {
		...
		for (warning in arrayField(value, "configWarnings")) {
			...
			warnings.push(new BootstrapConfigWarning(stringField(warningObject, "summary", ""), stringField(warningObject, "details", ""),
				stringField(warningObject, "path", "")));
		}
		return new BootstrapStartupMetadata(nullableStringField(account, "authMode"), nullableStringField(account, "planType"),
			nullableStringField(account, "email"), stringField(model, "id", ""), stringField(model, "provider", ""), warnings);
	}
