...
class RuntimeTransportHarness {
	...
	static function runAction(transport:FixtureLiveTransport, action:Value):Void {
		...
		switch actionType {
			case "sendRequest":
				assertOutcome(transport.sendRequest(stringField(action, "requestId", ""), stringField(action, "method", ""),
					JsonValueCodec.encode(valueField(action, "params"))),
					stringField(action, "expectCode", ""));
			case "completeResponse":
				assertOutcome(transport.completeResponse(stringField(action, "requestId", ""), stringField(action, "method", ""),
					JsonValueCodec.encode(valueField(action, "result"))),
					stringField(action, "expectCode", ""));
			case "failResponse":
				assertOutcome(transport.failResponse(stringField(action, "requestId", ""), stringField(action, "method", ""),
					JsonValueCodec.encode(valueField(action, "error"))),
					stringField(action, "expectCode", ""));
			case "cancelRequest":
				assertOutcome(transport.cancelRequest(stringField(action, "requestId", ""), stringField(action, "method", ""),
					stringField(action, "reason", "")),
					stringField(action, "expectCode", ""));
			case "notification":
				assertOutcome(transport.receiveNotification(stringField(action, "method", ""), JsonValueCodec.encode(valueField(action, "params"))),
					stringField(action, "expectCode", ""));
			case "disconnect":
