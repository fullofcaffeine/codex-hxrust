...
import codexhx.runtime.app.threadread.ThreadReadIdleGoalProgressAccountingPolicy;
import codexhx.runtime.app.threadread.ThreadReadIdleGoalProgressAccountingReport;
import codexhx.runtime.app.threadread.ThreadReadIdleGoalProgressAccountingRequest;
import haxe.json.Value;
import sys.io.File;

class ThreadReadIdleGoalProgressAccountingHarness {
	static function main():Void {
		final root = fixtureRoot();
		final cases = arrayField(root, "cases");
		final report = ThreadReadIdleGoalProgressAccountingPolicy.buildCases(requests(cases));
		...
		while (i < cases.length) {
			...
			assertEquals(boolText(boolField(expect, "idleProgressSnapshotAvailable", false)), boolText(outcome.idleProgressSnapshotAvailable));
			...
			assertEquals(boolText(boolField(expect, "idleBaselineReset", false)), boolText(outcome.idleBaselineReset));
			...
			assertEquals(stringField(expect, "emittedTurnId", ""), outcome.emittedTurnId);
	...
	static function requests(values:Array<Value>):Array<ThreadReadIdleGoalProgressAccountingRequest> {
	...
	static function assertReport(root:Value, report:ThreadReadIdleGoalProgressAccountingReport):Void {
		...
		assertEquals(Std.string(intField(expect, "idleBaselineResetCount", 0)), Std.string(report.idleBaselineResetCount()));
		assertContains(report.summary(), stringField(expect, "summaryContains", ""));
	}

	static function fixtureRoot():Value {
		return expectParse(CodexJson.parse(File.getContent("fixtures/hxrust/thread-read-idle-goal-progress-accounting.v1.json")));
	}
