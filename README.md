...
```text
[########################################] 100% (707/707 non-epic Beads closed)
```
...
The current queue is intentionally pivoted toward a minimal live TUI shell. `TUI-LIVE-0` through `TUI-LIVE-62` move the shell from a terminal backend contract through generated terminal restore, typed input, redraw scheduling, fake app-server attach, JSON-RPC prompt envelopes, connector-backed stdio/process transports, persistent prompt sessions, runner-owned shutdown, and active-turn tracking. `TUI-LIVE-63` / `codex-hxrust-s0fu` adds the typed `turn/interrupt` request path for active turns. No non-epic Beads are currently ready after this slice; the next step is to file/select the next raw upstream live-TUI or app-server runtime milestone rather than returning to trace-only smoke expansion.

Latest slice: `TUI-LIVE-63` / `codex-hxrust-s0fu` adds a typed `turn/interrupt` request path for the live-shell prompt boundary. The facade targets the current active turn, rejects idle interrupts, clears active turn state as interrupted without incrementing completed turns, and the runner routes Ctrl-C to interrupt when a turn is active while preserving idle Ctrl-C exit behavior. This still does not own async cancellation, sockets, model calls, persistence, or tools; it proves the typed app-server request/effect surface needed before real cancellation.
