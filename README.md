...
```text
[#######################################.] 99% (712/713 non-epic Beads closed)
```
...
The current queue is intentionally pivoted toward a minimal live TUI shell. `TUI-LIVE-0` through `TUI-LIVE-64` move the shell from a terminal backend contract through generated terminal restore, typed input, redraw scheduling, fake app-server attach, JSON-RPC prompt envelopes, connector-backed stdio/process transports, persistent prompt sessions, runner-owned shutdown, active-turn tracking, a typed active-turn interrupt request path, and persistent `turn/interrupt` JSONL transport. `TUI-LIVE-65` / `codex-hxrust-o8lu` adds opt-in submitted/running turn acceptance so a persistent turn can remain active for Ctrl-C interruption. `TUI-LIVE-66` / `codex-hxrust-rsz5` adds typed delayed submitted-turn completion delivery through the app-server event pump. `TUI-LIVE-67` / `codex-hxrust-a0g9` adds turn-scoped late assistant stream delivery for submitted turns. `TUI-LIVE-68` / `codex-hxrust-q0dv` connects that path to deterministic late JSONL notification handoff. The ready next step is `TUI-LIVE-69` / `codex-hxrust-vrdo`, which should route delayed `turn/completed` JSONL notifications into the submitted-turn completion path rather than returning to trace-only smoke expansion.

Latest slice: `TUI-LIVE-68` / `codex-hxrust-q0dv` decodes late notification-only JSONL lines, projects `item/agentMessage/delta` into `AssistantTurnDelta(threadId, turnId, delta)`, and delegates to the submitted-turn stream delivery checks. The shell can render a late assistant row from raw JSONL while `activeTurn` remains populated, then complete later through the TUI-LIVE-66 completion path; unknown-method JSONL, wrong-thread, wrong-turn, no-active, completed-turn, and stale-interrupted delivery attempts are typed rejections with no transcript mutation. This still does not own async cancellation, sockets, provider streaming, model calls, persistence, or tools.
