...
class AppProtocol {
	static final REQUEST_METHODS:Array<String> = [
		"thread/start",
		"thread/resume",
		"thread/fork",
		"thread/archive",
	...
	static final SERVER_REQUEST_METHODS:Array<String> = [
	...
	static final NOTIFICATION_METHODS:Array<String> = [
		"thread/started",
	...
	public static function schemaFingerprintJson():String {
		return "{\"basis\":" + quote(FINGERPRINT_BASIS) + ",\"fingerprint\":" + quote(FINGERPRINT)
			+ ",\"schema\":\"codex-hxrust.app-protocol-fingerprint.v1\"}";
	}
	...
	public static function parseFixtureItem(item:Value):AppProtocolParseOutcome {
		return switch item {
			case JObject(keys, values):
				...
				if (!kind.ok)
					return kind.toOutcome();

				final method = optionalString(keys, values, "method", "");
				final message = requiredValue(keys, values, "message", "$.message");
				if (!message.ok)
					return message.toOutcome();

				final validation = validateMessage(kind.value, method, message.value);
				if (!validation.ok)
					return validation;

				AppProtocolParseOutcome.success(new AppProtocolMessage(id.value, kind.value, method, JsonValueCodec.encode(message.value),
					validation.message.summary, FINGERPRINT));
			case _:
	...
	static function validateMessage(kind:String, fixtureMethod:String, message:Value):AppProtocolParseOutcome {
		...
		if (!object.ok)
			return object.toOutcome();

		if (kind == "serverRequest")
			return validateServerRequest(fixtureMethod, object);
		if (kind == "serverResponse")
			return validateServerResponse(fixtureMethod, object);

		final jsonrpc = requiredString(object.keys, object.values, "jsonrpc", "$.message.jsonrpc");
		if (!jsonrpc.ok)
			return jsonrpc.toOutcome();
		if (jsonrpc.value != "2.0")
			return fail("invalid_jsonrpc_version", "$.message.jsonrpc", "expected JSON-RPC 2.0");
	...
	static function validateServerRequest(fixtureMethod:String, object:ProtocolObjectField):AppProtocolParseOutcome {
		if (!contains(SERVER_REQUEST_METHODS, fixtureMethod))
			return fail("unsupported_method", "$.method", "unsupported server request method");
		...
		if (!method.ok)
			return method.toOutcome();
		if (method.value != fixtureMethod)
			return fail("method_mismatch", "$.message.method", "message method differs from fixture method");

		final params = requiredObjectField(object.keys, object.values, "params", "$.message.params");
		if (!params.ok)
			return params.toOutcome();
	...
	static function validateServerResponse(fixtureMethod:String, object:ProtocolObjectField):AppProtocolParseOutcome {
		if (!contains(SERVER_REQUEST_METHODS, fixtureMethod))
			return fail("unsupported_method", "$.method", "unsupported server response method");
		final id = requiredId(object.keys, object.values, "$.message.id");
		if (!id.ok)
			return id.toOutcome();
	...
	static function validateRequest(fixtureMethod:String, object:ProtocolObjectField):AppProtocolParseOutcome {
		if (!contains(REQUEST_METHODS, fixtureMethod))
			return fail("unsupported_method", "$.method", "unsupported request method");
		final id = requiredId(object.keys, object.values, "$.message.id");
		if (!id.ok)
			return id.toOutcome();
	...
	static function validateResponse(fixtureMethod:String, object:ProtocolObjectField):AppProtocolParseOutcome {
		if (!contains(REQUEST_METHODS, fixtureMethod))
			return fail("unsupported_method", "$.method", "unsupported response method");
		final id = requiredId(object.keys, object.values, "$.message.id");
		if (!id.ok)
			return id.toOutcome();
		...
		return switch fixtureMethod {
			...
			case "thread/name/set" | "thread/settings/update" | "thread/memoryMode/set" | "memory/reset" | "thread/compact/start" | "thread/shellCommand" |
				"thread/approveGuardianDeniedAction" | "thread/backgroundTerminals/clean" | "thread/inject_items":
			...
			case "getConversationSummary":
				validateV1GetConversationSummaryResponse(result);
			case "gitDiffToRemote":
				validateV1GitDiffToRemoteResponse(result);
			case "getAuthStatus":
				validateV1GetAuthStatusResponse(result);
			case "fuzzyFileSearch":
				validateLegacyFuzzyFileSearchResponse(result);
	...
	static function validateNotification(fixtureMethod:String, object:ProtocolObjectField):AppProtocolParseOutcome {
		if (!contains(NOTIFICATION_METHODS, fixtureMethod))
			return fail("unsupported_method", "$.method", "unsupported notification method");
		final method = requiredString(object.keys, object.values, "method", "$.message.method");
		if (!method.ok)
			return method.toOutcome();
		if (method.value != fixtureMethod)
	...
	static function optionalEmptyParams(object:ProtocolObjectField):ProtocolObjectField {
		...
		if (paramsIndex < 0)
			return ProtocolObjectField.success([], []);
	...
	static function validateEmptyObject(object:ProtocolObjectField, path:String, label:String):AppProtocolParseOutcome {
		if (object.keys.length != 0)
			return fail("unexpected_field", path + "." + object.keys[0], "expected empty JSON object");
	...
	static function validateParams(method:String, params:ProtocolObjectField):AppProtocolParseOutcome {
		return switch method {
			...
			case "thread/increment_elicitation" | "thread/decrement_elicitation" | "thread/goal/get" | "thread/goal/clear" | "thread/compact/start" |
				"thread/backgroundTerminals/clean":
				validateThreadIdParams(params, method);
			...
			case "getConversationSummary":
				validateV1GetConversationSummaryParams(params);
			case "gitDiffToRemote":
				validateV1GitDiffToRemoteParams(params);
			case "getAuthStatus":
				validateV1GetAuthStatusParams(params);
			case "fuzzyFileSearch":
				validateLegacyFuzzyFileSearchParams(params);
	...
	static function validateMcpServerToolCallParams(params:ProtocolObjectField):AppProtocolParseOutcome {
		...
		if (arguments.ok && !validJsonValue(arguments.value))
			return fail("invalid_json_value", "$.message.params.arguments", "expected JSON value");
		final meta = optionalValue(params.keys, params.values, "_meta");
		if (meta.ok && !validJsonValue(meta.value))
			return fail("invalid_json_value", "$.message.params._meta", "expected JSON value");
	...
	static function validateEnvironmentAddParams(params:ProtocolObjectField):AppProtocolParseOutcome {
		...
		if (!environmentId.ok)
			return environmentId.toOutcome();
		if (environmentId.value.length == 0)
			return fail("empty_string", "$.message.params.environmentId", "expected non-empty JSON string");
		final execServerUrl = requiredString(params.keys, params.values, "execServerUrl", "$.message.params.execServerUrl");
		if (!execServerUrl.ok)
			return execServerUrl.toOutcome();
		if (execServerUrl.value.length == 0)
			return fail("empty_string", "$.message.params.execServerUrl", "expected non-empty JSON string");
	...
	static function validateCollaborationModeMask(mask:ProtocolObjectField, path:String):AppProtocolParseOutcome {
		...
		if (name.value.length == 0)
			return fail("empty_string", path + ".name", "expected non-empty JSON string");
	...
	static function validatePluginMarketplaceResponse(result:ProtocolObjectField, featured:Bool, label:String):AppProtocolParseOutcome {
		...
		if (featured) {
			...
			if (!featuredPluginIds.ok)
				return featuredPluginIds.toOutcome();
	...
	static function validateSkillsConfigWriteResponse(result:ProtocolObjectField):AppProtocolParseOutcome {
		...
		if (!effectiveEnabled.ok)
			return effectiveEnabled.toOutcome();
	...
	static function validatePluginShareUpdateTargetsResponse(result:ProtocolObjectField):AppProtocolParseOutcome {
		...
		if (!principalObjects.ok)
			return principalObjects;
	...
	static function validatePluginShareCheckoutResponse(result:ProtocolObjectField):AppProtocolParseOutcome {
		...
		if (!remoteVersion.ok)
			return remoteVersion.toOutcome();
	...
	static function validateMcpServerOauthLoginResponse(result:ProtocolObjectField):AppProtocolParseOutcome {
		...
		if (!authorizationUrl.ok)
			return authorizationUrl.toOutcome();
	...
	static function validateMcpServerToolCallResponse(result:ProtocolObjectField):AppProtocolParseOutcome {
		...
		if (!content.ok)
			return content.toOutcome();
		final structured = optionalValue(result.keys, result.values, "structuredContent");
		if (structured.ok && !validJsonValue(structured.value))
			return fail("invalid_json_value", "$.message.result.structuredContent", "expected JSON value");
		final meta = optionalValue(result.keys, result.values, "_meta");
		if (meta.ok && !validJsonValue(meta.value))
			return fail("invalid_json_value", "$.message.result._meta", "expected JSON value");
	...
	static function validateServerResponsePayload(method:String, response:ProtocolObjectField):AppProtocolParseOutcome {
		return switch method {
			...
			case "item/fileChange/requestApproval":
				...
				if (!decision.ok)
					return decision.toOutcome();
				if (!validFileChangeApprovalDecision(decision.value)) fail("invalid_file_change_approval_decision", "$.message.response.decision",
					"unsupported file change approval decision") else success("server-response:item/fileChange/requestApproval");
			case "item/permissions/requestApproval":
	...
	static function validateMcpServerElicitationRequestParams(params:ProtocolObjectField):AppProtocolParseOutcome {
		...
		if (mode.value == "form") {
			...
			if (!requestedSchema.ok)
				return requestedSchema.toOutcome();
	...
	static function validateChatgptAuthTokensRefreshParams(params:ProtocolObjectField):AppProtocolParseOutcome {
		...
		if (reason.value != "unauthorized")
			return fail("invalid_chatgpt_auth_tokens_refresh_reason", "$.message.params.reason", "unsupported auth token refresh reason");
		final previousAccountId = validateOptionalNullableString(params, "previousAccountId", "$.message.params.previousAccountId");
		if (!previousAccountId.ok)
	...
	static function validatePermissionsRequestApprovalResponse(response:ProtocolObjectField):AppProtocolParseOutcome {
		...
		if (!scope.ok)
			return scope.toOutcome();
		if (scope.value != "turn" && scope.value != "session")
			return fail("invalid_permission_grant_scope", "$.message.response.scope", "unsupported permission grant scope");
		final strictAutoReview = validateOptionalBool(response, "strictAutoReview", "$.message.response.strictAutoReview");
		if (!strictAutoReview.ok)
	...
	static function validateMcpServerElicitationRequestResponse(response:ProtocolObjectField):AppProtocolParseOutcome {
		...
		if (!action.ok)
			return action.toOutcome();
		if (action.value != "accept" && action.value != "decline" && action.value != "cancel")
			return fail("invalid_mcp_elicitation_action", "$.message.response.action", "unsupported MCP elicitation action");
	...
	static function validateOptionalCommandExecutionDecisionArray(object:ProtocolObjectField, name:String, path:String):AppProtocolParseOutcome {
		...
		if (i < 0)
			return success("command-exec-decisions:missing");
	...
	static function validateCommandExecutionApprovalDecision(value:Value, path:String):AppProtocolParseOutcome {
		return switch value {
			case JString(decision):
				if (validCommandExecutionApprovalDecision(decision)) success("command-exec-decision:" + decision) else
			...
			case JObject(keys, values):
				if (hasField(keys, "acceptWithExecpolicyAmendment")) {
					...
					if (!amendment.ok)
						return amendment.toOutcome();
					final execpolicy = requiredObjectField(amendment.keys, amendment.values, "execpolicy_amendment",
						path + ".acceptWithExecpolicyAmendment.execpolicy_amendment");
					if (!execpolicy.ok)
						return execpolicy.toOutcome();
					...
					if (!amendment.ok)
						return amendment.toOutcome();
					final policy = requiredObjectField(amendment.keys, amendment.values, "network_policy_amendment",
						path + ".applyNetworkPolicyAmendment.network_policy_amendment");
					if (!policy.ok)
						return policy.toOutcome();
	...
	static function validateRequestPermissionProfile(profile:ProtocolObjectField, path:String, requireNullableKeys:Bool, label:String):AppProtocolParseOutcome {
		final network = if (requireNullableKeys) validateRequiredNullableObject(profile, "network",
			path + ".network") else validateOptionalNullableObject(profile, "network", path + ".network");
		if (!network.ok)
			return network;
		final fileSystem = if (requireNullableKeys) validateRequiredNullableObject(profile, "fileSystem",
			path + ".fileSystem") else validateOptionalNullableObject(profile, "fileSystem", path + ".fileSystem");
		if (!fileSystem.ok)
			return fileSystem;
		return success(label);
	}
	...
	static function validateToolRequestUserInputOptions(question:ProtocolObjectField, name:String, path:String):AppProtocolParseOutcome {
		...
		if (i < 0)
			return fail("missing_field", path, "required field is missing");
	...
	static function validateTurn(turn:ProtocolObjectField, path:String):AppProtocolParseOutcome {
		...
		if (!status.ok)
			return status.toOutcome();
		if (!validTurnStatus(status.value))
			return fail("invalid_turn_status", path + ".status", "unsupported turn status");
	...
	static function validateThreadOpenParams(params:ProtocolObjectField, method:String):AppProtocolParseOutcome {
		...
		final approvalsReviewer = validateOptionalNullableStringEnum(params, "approvalsReviewer", "$.message.params.approvalsReviewer",
			["user", "auto_review", "guardian_subagent"], "invalid_approvals_reviewer");
		if (!approvalsReviewer.ok)
			return approvalsReviewer;
		final sandbox = validateOptionalNullableStringEnum(params, "sandbox", "$.message.params.sandbox",
			["read-only", "workspace-write", "danger-full-access"], "invalid_sandbox_mode");
		if (!sandbox.ok)
			return sandbox;
		final personality = validateOptionalNullableStringEnum(params, "personality", "$.message.params.personality", ["none", "friendly", "pragmatic"],
			"invalid_personality");
		if (!personality.ok)
			return personality;
	...
	static function validateThreadOpenResponse(result:ProtocolObjectField, path:String, label:String):AppProtocolParseOutcome {
		...
		if (!approval.ok)
			return approval;
		final approvalsReviewer = validateRequiredStringEnum(result.keys, result.values, "approvalsReviewer", path + ".approvalsReviewer",
			["user", "auto_review", "guardian_subagent"], "invalid_approvals_reviewer");
		if (!approvalsReviewer.ok)
			return approvalsReviewer;
	...
	static function validateThreadUnsubscribeResponse(result:ProtocolObjectField):AppProtocolParseOutcome {
		...
		if (!status.ok)
			return status.toOutcome();
		if (!contains(["notLoaded", "notSubscribed", "unsubscribed"], status.value))
			return fail("invalid_thread_unsubscribe_status", "$.message.result.status", "unsupported unsubscribe status");
	...
	static function validateThreadListParams(params:ProtocolObjectField):AppProtocolParseOutcome {
		...
		if (!searchTerm.ok)
			return searchTerm;
		final sortDirection = validateOptionalNullableStringEnum(params, "sortDirection", "$.message.params.sortDirection", ["asc", "desc"],
			"invalid_sort_direction");
		if (!sortDirection.ok)
			return sortDirection;
		final sortKey = validateOptionalNullableStringEnum(params, "sortKey", "$.message.params.sortKey", ["created_at", "updated_at"],
			"invalid_thread_sort_key");
		if (!sortKey.ok)
			return sortKey;
		final useStateDbOnly = validateOptionalBool(params, "useStateDbOnly", "$.message.params.useStateDbOnly");
		if (!useStateDbOnly.ok)
			return useStateDbOnly;
	...
	static function validateThreadSearchParams(params:ProtocolObjectField):AppProtocolParseOutcome {
		...
		if (!searchTerm.ok)
			return searchTerm.toOutcome();
		if (StringTools.trim(searchTerm.value).length == 0)
			return fail("empty_string", "$.message.params.searchTerm", "expected non-empty JSON string");
		final cursor = validateOptionalNullableString(params, "cursor", "$.message.params.cursor");
		if (!cursor.ok)
			return cursor;
		final limit = validateOptionalNullableUInt(params, "limit", "$.message.params.limit");
	...
	static function validateV1GetConversationSummaryParams(params:ProtocolObjectField):AppProtocolParseOutcome {
		final hasRolloutPath = fieldIndex(params.keys, "rolloutPath") >= 0;
		final hasConversationId = fieldIndex(params.keys, "conversationId") >= 0;
		if (hasRolloutPath == hasConversationId)
			return fail("invalid_v1_conversation_summary_params", "$.message.params", "expected exactly one legacy conversation summary selector");
		if (hasRolloutPath) {
			final rolloutPath = requiredString(params.keys, params.values, "rolloutPath", "$.message.params.rolloutPath");
			if (!rolloutPath.ok)
				return rolloutPath.toOutcome();
			return success("params:getConversationSummary:rolloutPath");
		}
		final conversationId = requiredString(params.keys, params.values, "conversationId", "$.message.params.conversationId");
		if (!conversationId.ok)
			return conversationId.toOutcome();
		return success("params:getConversationSummary:conversationId");
	}

	static function validateV1GetConversationSummaryResponse(result:ProtocolObjectField):AppProtocolParseOutcome {
		final summary = requiredObjectField(result.keys, result.values, "summary", "$.message.result.summary");
		if (!summary.ok)
			return summary.toOutcome();
		final summaryResult = validateV1ConversationSummary(summary, "$.message.result.summary");
		if (!summaryResult.ok)
			return summaryResult;
		return success("response:getConversationSummary");
	}

	static function validateV1ConversationSummary(summary:ProtocolObjectField, path:String):AppProtocolParseOutcome {
		for (field in [
			"conversationId",
			"path",
			"preview",
			...
			final value = requiredString(summary.keys, summary.values, field, path + "." + field);
			if (!value.ok)
				return value.toOutcome();
		}
		final timestamp = validateOptionalNullableString(summary, "timestamp", path + ".timestamp");
		if (!timestamp.ok)
			return timestamp;
		final updatedAt = validateOptionalNullableString(summary, "updatedAt", path + ".updatedAt");
		if (!updatedAt.ok)
			return updatedAt;
		final gitInfo = validateV1ConversationGitInfo(summary, "gitInfo", path + ".gitInfo");
		if (!gitInfo.ok)
			return gitInfo;
		return success("v1-conversation-summary");
	}

	static function validateV1ConversationGitInfo(object:ProtocolObjectField, name:String, path:String):AppProtocolParseOutcome {
		final i = fieldIndex(object.keys, name);
		if (i < 0)
			return success("v1-git-info:missing");
		return switch object.values[i] {
			case JNull:
				success("v1-git-info:null");
			case JObject(keys, values):
				final gitInfo = ProtocolObjectField.success(keys, values);
				for (field in ["sha", "branch", "origin_url"]) {
					final value = validateOptionalNullableString(gitInfo, field, path + "." + field);
					if (!value.ok)
						return value;
				}
				success("v1-git-info");
	...
	static function validateV1GitDiffToRemoteParams(params:ProtocolObjectField):AppProtocolParseOutcome {
		final cwd = requiredString(params.keys, params.values, "cwd", "$.message.params.cwd");
		if (!cwd.ok)
			return cwd.toOutcome();
		return success("params:gitDiffToRemote");
	}

	static function validateV1GitDiffToRemoteResponse(result:ProtocolObjectField):AppProtocolParseOutcome {
		final sha = requiredString(result.keys, result.values, "sha", "$.message.result.sha");
		if (!sha.ok)
			return sha.toOutcome();
		final diff = requiredString(result.keys, result.values, "diff", "$.message.result.diff");
		if (!diff.ok)
			return diff.toOutcome();
		return success("response:gitDiffToRemote");
	}

	static function validateV1GetAuthStatusParams(params:ProtocolObjectField):AppProtocolParseOutcome {
		final includeToken = validateOptionalBool(params, "includeToken", "$.message.params.includeToken");
		if (!includeToken.ok)
			return includeToken;
		final refreshToken = validateOptionalBool(params, "refreshToken", "$.message.params.refreshToken");
		if (!refreshToken.ok)
			return refreshToken;
		return success("params:getAuthStatus");
	}

	static function validateV1GetAuthStatusResponse(result:ProtocolObjectField):AppProtocolParseOutcome {
		final authMethod = validateOptionalNullableStringEnum(result, "authMethod", "$.message.result.authMethod",
			["apikey", "chatgpt", "chatgptAuthTokens", "agentIdentity", "personalAccessToken"], "invalid_account_auth_mode");
		if (!authMethod.ok)
			return authMethod;
		final authToken = validateOptionalNullableString(result, "authToken", "$.message.result.authToken");
		if (!authToken.ok)
			return authToken;
		final requiresOpenaiAuth = validateOptionalNullableBool(result, "requiresOpenaiAuth", "$.message.result.requiresOpenaiAuth");
		if (!requiresOpenaiAuth.ok)
			return requiresOpenaiAuth;
		return success("response:getAuthStatus");
	}

	static function validateLegacyFuzzyFileSearchParams(params:ProtocolObjectField):AppProtocolParseOutcome {
		final query = requiredString(params.keys, params.values, "query", "$.message.params.query");
		if (!query.ok)
			return query.toOutcome();
		final roots = requiredArray(params.keys, params.values, "roots", "$.message.params.roots");
		if (!roots.ok)
			return roots.toOutcome();
		final rootResult = validateStringArrayEntries(roots.values, "$.message.params.roots");
		if (!rootResult.ok)
			return rootResult;
		final cancellationToken = validateOptionalNullableString(params, "cancellationToken", "$.message.params.cancellationToken");
		if (!cancellationToken.ok)
			return cancellationToken;
		return success("params:fuzzyFileSearch");
	}

	static function validateLegacyFuzzyFileSearchResponse(result:ProtocolObjectField):AppProtocolParseOutcome {
		final files = requiredArray(result.keys, result.values, "files", "$.message.result.files");
		if (!files.ok)
			return files.toOutcome();
		var i = 0;
		while (i < files.values.length) {
			final filePath = "$.message.result.files[" + Std.string(i) + "]";
			final file = requireObject(files.values[i], filePath);
			if (!file.ok)
				return file.toOutcome();
			final fileResult = validateFuzzyFileSearchResult(file, filePath);
			if (!fileResult.ok)
				return fileResult;
			i = i + 1;
		}
		return success("response:fuzzyFileSearch");
	}
	...
	static function validateThreadRealtimeStartParams(params:ProtocolObjectField):AppProtocolParseOutcome {
		...
		if (!outputModality.ok)
			return outputModality.toOutcome();
		if (!validRealtimeOutputModality(outputModality.value))
			return fail("invalid_realtime_output_modality", "$.message.params.outputModality", "unsupported realtime output modality");
		...
		if (!transport.ok)
			return transport;
		final version = validateOptionalNullableStringEnum(params, "version", "$.message.params.version", ["v1", "v2"],
			"invalid_realtime_conversation_version");
		if (!version.ok)
			return version;
	...
	static function validateOptionalThreadRealtimeStartTransport(object:ProtocolObjectField, name:String, path:String):AppProtocolParseOutcome {
		final i = fieldIndex(object.keys, name);
		if (i < 0)
			return success("thread-realtime-transport:missing");
		return switch object.values[i] {
			case JNull:
				success("thread-realtime-transport:null");
			case JObject(keys, values):
				final transportType = requiredString(keys, values, "type", path + ".type");
				if (!transportType.ok) transportType.toOutcome() else if (!validRealtimeTransportType(transportType.value)) fail("invalid_realtime_transport",
					path + ".type", "unsupported realtime transport") else if (transportType.value == "webrtc") {
					final sdp = requiredString(keys, values, "sdp", path + ".sdp");
					if (!sdp.ok)
						sdp.toOutcome()
					else
						success("thread-realtime-transport:webrtc");
	...
	static function validateThreadRealtimeListVoicesResponse(result:ProtocolObjectField):AppProtocolParseOutcome {
		...
		if (!defaultV1.ok)
			return defaultV1.toOutcome();
		if (!validRealtimeVoice(defaultV1.value))
			return fail("invalid_realtime_voice", "$.message.result.voices.defaultV1", "unsupported realtime voice");
		final defaultV2 = requiredString(voices.keys, voices.values, "defaultV2", "$.message.result.voices.defaultV2");
		if (!defaultV2.ok)
			return defaultV2.toOutcome();
		if (!validRealtimeVoice(defaultV2.value))
			return fail("invalid_realtime_voice", "$.message.result.voices.defaultV2", "unsupported realtime voice");
	...
	static function validateRealtimeVoiceArray(values:Array<Value>, path:String):AppProtocolParseOutcome {
		...
		while (i < values.length) {
			switch values[i] {
				case JString(value):
					if (!validRealtimeVoice(value))
						return fail("invalid_realtime_voice", path + "[" + Std.string(i) + "]", "unsupported realtime voice");
				case _:
	...
	static function validateOptionalNullableRealtimeVoice(object:ProtocolObjectField, name:String, path:String):AppProtocolParseOutcome {
		final i = fieldIndex(object.keys, name);
		if (i < 0)
			return success("nullable-realtime-voice:missing");
		return switch object.values[i] {
			case JNull:
	...
	static function validateRemoteControlStatusResponse(result:ProtocolObjectField, path:String, label:String):AppProtocolParseOutcome {
		...
		if (!installationId.ok)
			return installationId.toOutcome();
		...
		if (!status.ok)
			return status.toOutcome();
		if (!validRemoteControlStatus(status.value))
			return fail("invalid_remote_control_status", path + ".status", "unsupported remote control status");
	...
	static function validateRemoteControlPairingStatusParams(params:ProtocolObjectField):AppProtocolParseOutcome {
		...
		if (hasPairingCode && hasManualPairingCode)
			return fail("remote_control_pairing_code_conflict", "$.message.params", "expected exactly one pairing code field");
		if (!hasPairingCode && !hasManualPairingCode)
			return fail("missing_remote_control_pairing_code", "$.message.params", "expected pairingCode or manualPairingCode");
	...
	static function validateRemoteControlClientsListParams(params:ProtocolObjectField):AppProtocolParseOutcome {
		...
		if (!limit.ok)
			return limit;
		final order = validateOptionalNullableStringEnum(params, "order", "$.message.params.order", ["asc", "desc"],
			"invalid_remote_control_clients_list_order");
		if (!order.ok)
			return order;
	...
	static function validateRemoteControlClientsListResponse(result:ProtocolObjectField):AppProtocolParseOutcome {
		...
		while (i < data.values.length) {
			switch data.values[i] {
				case JObject(keys, values):
					final clientResult = validateRemoteControlClient(ProtocolObjectField.success(keys, values), "$.message.result.data["
						+ Std.string(i)
						+ "]");
					if (!clientResult.ok)
						return clientResult;
	...
	static function validateThreadTurnsListParams(params:ProtocolObjectField):AppProtocolParseOutcome {
		...
		if (!sortDirection.ok)
			return sortDirection;
		final itemsView = validateOptionalNullableStringEnum(params, "itemsView", "$.message.params.itemsView", ["notLoaded", "summary", "full"],
			"invalid_turn_items_view");
		if (!itemsView.ok)
			return itemsView;
	...
	static function validateThreadGoalSetParams(params:ProtocolObjectField):AppProtocolParseOutcome {
		...
		if (!objective.ok)
			return objective;
		final status = validateOptionalNullableStringEnum(params, "status", "$.message.params.status",
			["active", "paused", "blocked", "usageLimited", "budgetLimited", "complete"], "invalid_thread_goal_status");
		if (!status.ok)
			return status;
	...
	static function validateThreadGoalGetResponse(result:ProtocolObjectField):AppProtocolParseOutcome {
		...
		if (i < 0)
			return success("response:thread/goal/get");
	...
	static function validateThreadGoal(goal:ProtocolObjectField, path:String):AppProtocolParseOutcome {
		...
		if (!status.ok)
			return status.toOutcome();
		if (!validThreadGoalStatus(status.value))
			return fail("invalid_thread_goal_status", path + ".status", "unsupported thread goal status");
	...
	static function validateOptionalThreadMetadataGitInfo(object:ProtocolObjectField, name:String, path:String):AppProtocolParseOutcome {
		final i = fieldIndex(object.keys, name);
		if (i < 0)
			return success("thread-metadata-git-info:missing");
		return switch object.values[i] {
			case JNull:
	...
	static function validateThreadSettingsUpdateParams(params:ProtocolObjectField):AppProtocolParseOutcome {
		...
		if (!effort.ok)
			return effort;
		final summary = validateOptionalNullableStringEnum(params, "summary", "$.message.params.summary", ["auto", "concise", "detailed", "none"],
	...
	static function validateThreadMemoryModeSetParams(params:ProtocolObjectField):AppProtocolParseOutcome {
		...
		if (!threadId.ok)
			return threadId.toOutcome();
		final mode = validateRequiredStringEnum(params.keys, params.values, "mode", "$.message.params.mode", ["enabled", "disabled"],
			"invalid_thread_memory_mode");
		if (!mode.ok)
			return mode;
	...
	static function validateThreadRollbackParams(params:ProtocolObjectField):AppProtocolParseOutcome {
		...
		if (!numTurns.ok)
			return numTurns.toOutcome();
		if (numTurns.value == 0)
			return fail("expected_positive_uint", "$.message.params.numTurns", "expected unsigned JSON integer greater than zero");
	...
	static function validateThreadSettings(settings:ProtocolObjectField, path:String):AppProtocolParseOutcome {
		...
		if (!approvalPolicy.ok)
			return approvalPolicy;
		final approvalsReviewer = validateRequiredStringEnum(settings.keys, settings.values, "approvalsReviewer", path + ".approvalsReviewer",
			["user", "auto_review", "guardian_subagent"], "invalid_approvals_reviewer");
		if (!approvalsReviewer.ok)
			return approvalsReviewer;
		...
		if (!effort.ok)
			return effort;
		final summary = validateOptionalNullableStringEnum(settings, "summary", path + ".summary", ["auto", "concise", "detailed", "none"],
			"invalid_reasoning_summary");
		if (!summary.ok)
			return summary;
		final collaborationMode = requiredObjectField(settings.keys, settings.values, "collaborationMode", path + ".collaborationMode");
		if (!collaborationMode.ok)
			return collaborationMode.toOutcome();
		...
		final personality = validateOptionalNullableStringEnum(settings, "personality", path + ".personality", ["none", "friendly", "pragmatic"],
			"invalid_personality");
		if (!personality.ok)
			return personality;
	...
	static function validateOptionalNullableActivePermissionProfile(object:ProtocolObjectField, name:String, path:String):AppProtocolParseOutcome {
		final i = fieldIndex(object.keys, name);
		if (i < 0)
			return success("active-permission-profile:missing");
		return switch object.values[i] {
			case JNull:
	...
	static function validateOptionalNullableCollaborationMode(object:ProtocolObjectField, name:String, path:String):AppProtocolParseOutcome {
		final i = fieldIndex(object.keys, name);
		if (i < 0)
			return success("collaboration-mode:missing");
		return switch object.values[i] {
			case JNull:
	...
	static function validateReviewStartResponse(result:ProtocolObjectField):AppProtocolParseOutcome {
		...
		if (!reviewThreadId.ok)
			return reviewThreadId.toOutcome();
	...
	static function validateTurnPlanSteps(entries:Array<Value>, path:String):AppProtocolParseOutcome {
		...
		while (i < entries.length) {
			...
			if (!status.ok)
				return status.toOutcome();
			if (!validPlanStepStatus(status.value))
				return fail("invalid_plan_step_status", stepPath + ".status", "unsupported plan step status");
			i = i + 1;
		}
	...
	static function validateMcpServerStatusUpdatedNotification(params:ProtocolObjectField):AppProtocolParseOutcome {
		...
		if (!status.ok)
			return status.toOutcome();
		if (!validMcpServerStartupStatus(status.value))
			return fail("invalid_mcp_server_startup_status", "$.message.params.status", "unsupported MCP server startup status");
	...
	static function validateAccountUpdatedNotification(params:ProtocolObjectField):AppProtocolParseOutcome {
		...
		if (authModeIndex >= 0) {
			switch params.values[authModeIndex] {
				case JNull:
				case JString(value):
					if (!validAccountAuthMode(value))
						return fail("invalid_account_auth_mode", "$.message.params.authMode", "unsupported account auth mode");
				case _:
		...
		if (planTypeIndex >= 0) {
			switch params.values[planTypeIndex] {
				case JNull:
				case JString(value):
					if (!validAccountPlanType(value))
						return fail("invalid_account_plan_type", "$.message.params.planType", "unsupported account plan type");
				case _:
	...
	static function validateCancelLoginAccountResponse(result:ProtocolObjectField):AppProtocolParseOutcome {
		...
		if (!status.ok)
			return status.toOutcome();
		if (!validCancelLoginAccountStatus(status.value))
			return fail("invalid_cancel_login_account_status", "$.message.result.status", "unsupported account login cancel status");
	...
	static function validateGetAccountResponse(result:ProtocolObjectField):AppProtocolParseOutcome {
		...
		if (!requiresOpenaiAuth.ok)
			return requiresOpenaiAuth.toOutcome();
	...
	static function validateOptionalNullableAccount(object:ProtocolObjectField, name:String, path:String):AppProtocolParseOutcome {
		final i = fieldIndex(object.keys, name);
		if (i < 0)
			return success("account:missing");
		return switch object.values[i] {
			case JNull:
	...
	static function validateAccountObject(account:ProtocolObjectField, path:String):AppProtocolParseOutcome {
		...
		return switch typeField.value {
			...
			case "chatgpt":
				...
				if (!email.ok)
					return email.toOutcome();
				final planType = requiredString(account.keys, account.values, "planType", path + ".planType");
				if (!planType.ok)
					return planType.toOutcome();
				if (!validAccountPlanType(planType.value)) fail("invalid_account_plan_type", path + ".planType",
					"unsupported account plan type") else success("account:chatgpt");
			case _:
	...
	static function validateOptionalRateLimitSnapshotMap(object:ProtocolObjectField, name:String, path:String):AppProtocolParseOutcome {
		final i = fieldIndex(object.keys, name);
		if (i < 0)
			return success("rate-limits:missing-map");
		return switch object.values[i] {
			case JNull:
	...
	static function validateOptionalNullableAccountTokenUsageBuckets(object:ProtocolObjectField, name:String, path:String):AppProtocolParseOutcome {
		final i = fieldIndex(object.keys, name);
		if (i < 0)
			return success("account-token-usage:missing-buckets");
		return switch object.values[i] {
			case JNull:
	...
	static function validateSendAddCreditsNudgeEmailParams(params:ProtocolObjectField):AppProtocolParseOutcome {
		...
		if (!creditType.ok)
			return creditType.toOutcome();
		if (!validAddCreditsNudgeCreditType(creditType.value))
			return fail("invalid_add_credits_nudge_credit_type", "$.message.params.creditType", "unsupported add-credits nudge credit type");
	...
	static function validateSendAddCreditsNudgeEmailResponse(result:ProtocolObjectField):AppProtocolParseOutcome {
		...
		if (!status.ok)
			return status.toOutcome();
		if (!validAddCreditsNudgeEmailStatus(status.value))
			return fail("invalid_add_credits_nudge_email_status", "$.message.result.status", "unsupported add-credits nudge email status");
	...
	static function validateFeedbackUploadParams(params:ProtocolObjectField):AppProtocolParseOutcome {
		...
		if (!classification.ok)
			return classification.toOutcome();
	...
	static function validateCommandExecParams(params:ProtocolObjectField):AppProtocolParseOutcome {
		...
		if (!outputBytesCap.ok)
			return outputBytesCap;
		final disableOutputCap = optionalBoolValue(params, "disableOutputCap", "$.message.params.disableOutputCap");
		if (!disableOutputCap.ok)
			return disableOutputCap.toOutcome();
		...
		if (!disableTimeout.ok)
			return disableTimeout.toOutcome();
		...
		if (!size.ok)
			return size;
		if (hasNonNullField(params, "size") && !tty.value)
			return fail("terminal_size_without_tty", "$.message.params.size", "size is only valid when tty is true");

		final sandboxPolicy = validateOptionalCommandExecSandboxPolicy(params, "sandboxPolicy", "$.message.params.sandboxPolicy");
		if (!sandboxPolicy.ok)
			return sandboxPolicy;
		final permissionProfile = validateOptionalNullableString(params, "permissionProfile", "$.message.params.permissionProfile");
		if (!permissionProfile.ok)
	...
	static function validateProcessSpawnParams(params:ProtocolObjectField):AppProtocolParseOutcome {
		...
		if (!processHandle.ok)
			return processHandle.toOutcome();
		final cwd = requiredString(params.keys, params.values, "cwd", "$.message.params.cwd");
		if (!cwd.ok)
			return cwd.toOutcome();
	...
	static function validateConfigBatchWriteParams(params:ProtocolObjectField):AppProtocolParseOutcome {
		...
		if (!reloadUserConfig.ok)
			return reloadUserConfig;
	...
	static function validateConfigEdit(edit:ProtocolObjectField, path:String):AppProtocolParseOutcome {
		...
		if (!keyPath.ok)
			return keyPath.toOutcome();
		final mergeStrategy = requiredString(edit.keys, edit.values, "mergeStrategy", path + ".mergeStrategy");
		if (!mergeStrategy.ok)
			return mergeStrategy.toOutcome();
		if (!validConfigMergeStrategy(mergeStrategy.value))
			return fail("invalid_config_merge_strategy", path + ".mergeStrategy", "unsupported config merge strategy");
	...
	static function validateConfigWriteResponse(result:ProtocolObjectField):AppProtocolParseOutcome {
		...
		if (!status.ok)
			return status.toOutcome();
		if (!validConfigWriteStatus(status.value))
			return fail("invalid_config_write_status", "$.message.result.status", "unsupported config write status");
		final version = requiredString(result.keys, result.values, "version", "$.message.result.version");
		if (!version.ok)
			return version.toOutcome();
	...
	static function validateOptionalOverriddenMetadata(object:ProtocolObjectField, name:String, path:String):AppProtocolParseOutcome {
		final i = fieldIndex(object.keys, name);
		if (i < 0)
			return success("overridden-metadata:missing");
		return switch object.values[i] {
			case JNull:
			...
			case JObject(keys, values):
				...
				if (!overridingLayer.ok)
					return overridingLayer.toOutcome();
				...
				if (!effectiveValue.ok)
					return effectiveValue.toOutcome();
	...
	static function validateConfigRequirementsReadResponse(result:ProtocolObjectField):AppProtocolParseOutcome {
		...
		if (i < 0)
			return success("response:configRequirements/read");
		return switch result.values[i] {
			case JNull:
	...
	static function validateConfigRequirements(requirements:ProtocolObjectField, path:String):AppProtocolParseOutcome {
		...
		if (!approvalPolicies.ok)
			return approvalPolicies;
		final sandboxModes = validateOptionalNullableStringEnumArray(requirements, "allowedSandboxModes", path + ".allowedSandboxModes",
			["read-only", "workspace-write", "danger-full-access"], "invalid_sandbox_mode");
		if (!sandboxModes.ok)
			return sandboxModes;
		final windowsSandboxModes = validateOptionalNullableStringEnumArray(requirements, "allowedWindowsSandboxImplementations",
			path + ".allowedWindowsSandboxImplementations", ["elevated", "unelevated"], "invalid_windows_sandbox_setup_mode");
		if (!windowsSandboxModes.ok)
			return windowsSandboxModes;
		final webSearchModes = validateOptionalNullableStringEnumArray(requirements, "allowedWebSearchModes", path + ".allowedWebSearchModes",
			["disabled", "cached", "live"], "invalid_web_search_mode");
		if (!webSearchModes.ok)
			return webSearchModes;
		...
		if (!computerUse.ok)
			return computerUse;
		final enforceResidency = validateOptionalNullableStringEnum(requirements, "enforceResidency", path + ".enforceResidency", ["us"],
			"invalid_residency_requirement");
		if (!enforceResidency.ok)
			return enforceResidency;
	...
	static function validateOptionalNullableAskForApprovalArray(object:ProtocolObjectField, name:String, path:String):AppProtocolParseOutcome {
		final i = fieldIndex(object.keys, name);
		if (i < 0)
			return success("approval-array:missing");
		return switch object.values[i] {
			case JNull:
	...
	static function validateOptionalNullableStringEnumArray(object:ProtocolObjectField, name:String, path:String, values:Array<String>,
			errorCode:String):AppProtocolParseOutcome {
		final i = fieldIndex(object.keys, name);
		if (i < 0)
			return success("enum-array:missing");
		return switch object.values[i] {
			case JNull:
	...
	static function validateOptionalNullableBoolMap(object:ProtocolObjectField, name:String, path:String):AppProtocolParseOutcome {
		final i = fieldIndex(object.keys, name);
		if (i < 0)
			return success("bool-map:missing");
		return switch object.values[i] {
			case JNull:
	...
	static function validateOptionalComputerUseRequirements(object:ProtocolObjectField, name:String, path:String):AppProtocolParseOutcome {
		final i = fieldIndex(object.keys, name);
		if (i < 0)
			return success("computer-use:missing");
		return switch object.values[i] {
			case JNull:
				success("computer-use:null");
			case JObject(keys, values):
				final allowLocked = validateOptionalNullableBool(ProtocolObjectField.success(keys, values), "allowLockedComputerUse",
					path + ".allowLockedComputerUse");
	...
	static function validateConfigReadConfig(config:ProtocolObjectField, path:String):AppProtocolParseOutcome {
		...
		final autoCompactScope = validateOptionalNullableStringEnum(config, "model_auto_compact_token_limit_scope",
			path + ".model_auto_compact_token_limit_scope", ["total", "body_after_prefix"], "invalid_auto_compact_token_limit_scope");
		if (!autoCompactScope.ok)
			return autoCompactScope;
		final approvalPolicy = validateOptionalAskForApproval(config, "approval_policy", path + ".approval_policy");
		if (!approvalPolicy.ok)
			return approvalPolicy;
		final approvalsReviewer = validateOptionalNullableStringEnum(config, "approvals_reviewer", path + ".approvals_reviewer",
			["user", "auto_review", "guardian_subagent"], "invalid_approvals_reviewer");
		if (!approvalsReviewer.ok)
			return approvalsReviewer;
		final sandboxMode = validateOptionalNullableStringEnum(config, "sandbox_mode", path + ".sandbox_mode",
			["read-only", "workspace-write", "danger-full-access"], "invalid_sandbox_mode");
		if (!sandboxMode.ok)
			return sandboxMode;
		...
		if (!forcedWorkspaceId.ok)
			return forcedWorkspaceId;
		final forcedLoginMethod = validateOptionalNullableStringEnum(config, "forced_login_method", path + ".forced_login_method", ["chatgpt", "api"],
			"invalid_forced_login_method");
		if (!forcedLoginMethod.ok)
			return forcedLoginMethod;
		final webSearch = validateOptionalNullableStringEnum(config, "web_search", path + ".web_search", ["disabled", "cached", "live"],
		...
		if (!reasoningEffort.ok)
			return reasoningEffort;
		final reasoningSummary = validateOptionalNullableStringEnum(config, "model_reasoning_summary", path + ".model_reasoning_summary",
			["auto", "concise", "detailed", "none"], "invalid_reasoning_summary");
		if (!reasoningSummary.ok)
			return reasoningSummary;
		final verbosity = validateOptionalNullableStringEnum(config, "model_verbosity", path + ".model_verbosity", ["low", "medium", "high"],
			"invalid_model_verbosity");
		if (!verbosity.ok)
			return verbosity;
	...
	static function validateOptionalAskForApproval(object:ProtocolObjectField, name:String, path:String):AppProtocolParseOutcome {
		final i = fieldIndex(object.keys, name);
		if (i < 0)
			return success("ask-for-approval:missing");
		return switch object.values[i] {
			case JNull:
				success("ask-for-approval:null");
			case JString(value):
				if (contains(["untrusted", "on-failure", "on-request", "never"],
					value)) success("ask-for-approval") else fail("invalid_approval_policy", path, "unsupported approval policy");
			case JObject(keys, values):
				final granular = requiredObjectField(keys, values, "granular", path + ".granular");
				if (!granular.ok)
					return granular.toOutcome();
	...
	static function validateRequiredStringEnum(keys:Array<String>, values:Array<Value>, name:String, path:String, options:Array<String>,
			errorCode:String):AppProtocolParseOutcome {
		final field = requiredString(keys, values, name, path);
		if (!field.ok)
			return field.toOutcome();
		if (!contains(options, field.value))
			return fail(errorCode, path, "unsupported enum value");
	...
	static function validateOptionalSandboxWorkspaceWrite(object:ProtocolObjectField, name:String, path:String):AppProtocolParseOutcome {
		final i = fieldIndex(object.keys, name);
		if (i < 0)
			return success("sandbox-workspace-write:missing");
		return switch object.values[i] {
			case JNull:
	...
	static function validateOptionalForcedChatgptWorkspaceId(object:ProtocolObjectField, name:String, path:String):AppProtocolParseOutcome {
		final i = fieldIndex(object.keys, name);
		if (i < 0)
			return success("forced-chatgpt-workspace-id:missing");
		return switch object.values[i] {
	...
	static function validateOptionalToolsV2(object:ProtocolObjectField, name:String, path:String):AppProtocolParseOutcome {
		final i = fieldIndex(object.keys, name);
		if (i < 0)
			return success("tools-v2:missing");
		return switch object.values[i] {
			case JNull:
	...
	static function validateOptionalWebSearchToolConfig(object:ProtocolObjectField, name:String, path:String):AppProtocolParseOutcome {
		final i = fieldIndex(object.keys, name);
		if (i < 0)
			return success("web-search-tool:missing");
		return switch object.values[i] {
			case JNull:
			...
			case JObject(keys, values):
				...
				final contextSize = validateOptionalNullableStringEnum(config, "context_size", path + ".context_size", ["low", "medium", "high"],
					"invalid_web_search_context_size");
				if (!contextSize.ok)
					return contextSize;
	...
	static function validateOptionalWebSearchLocation(object:ProtocolObjectField, name:String, path:String):AppProtocolParseOutcome {
		final i = fieldIndex(object.keys, name);
		if (i < 0)
			return success("web-search-location:missing");
		return switch object.values[i] {
			case JNull:
	...
	static function validateOptionalAnalyticsConfig(object:ProtocolObjectField, name:String, path:String):AppProtocolParseOutcome {
		final i = fieldIndex(object.keys, name);
		if (i < 0)
			return success("analytics:missing");
		return switch object.values[i] {
			case JNull:
					...
					switch values[entryIndex] {
						...
						case JObject(appKeys, appValues):
							...
							if (keys[entryIndex] == "_default") {
								...
								if (!enabled.ok)
									return enabled;
								final approvalMode = validateOptionalNullableStringEnum(app, "approval_mode", entryPath + ".approval_mode",
									["auto", "prompt", "approve"], "invalid_app_tool_approval");
								if (!approvalMode.ok)
									return approvalMode;
							}
						case _:
	...
	static function validateOptionalNullableConfigLayers(object:ProtocolObjectField, name:String, path:String):AppProtocolParseOutcome {
		final i = fieldIndex(object.keys, name);
		if (i < 0)
			return success("config-layers:missing");
		return switch object.values[i] {
			case JNull:
	...
	static function validateExternalAgentConfigImportParams(params:ProtocolObjectField):AppProtocolParseOutcome {
		...
		if (!migrationItems.ok)
			return migrationItems.toOutcome();
	...
	static function validateExternalAgentConfigMigrationItem(item:ProtocolObjectField, path:String):AppProtocolParseOutcome {
		...
		if (!description.ok)
			return description.toOutcome();
		final itemType = requiredString(item.keys, item.values, "itemType", path + ".itemType");
		if (!itemType.ok)
			return itemType.toOutcome();
		if (!validExternalAgentConfigMigrationItemType(itemType.value))
			return fail("invalid_external_agent_config_item_type", path + ".itemType", "unsupported migration item type");
	...
	static function validateOptionalMigrationDetails(object:ProtocolObjectField, name:String, path:String):AppProtocolParseOutcome {
		final i = fieldIndex(object.keys, name);
		if (i < 0)
			return success("migration-details:missing");
		return switch object.values[i] {
			case JNull:
	...
	static function validateOptionalNamedMigrationArray(object:ProtocolObjectField, name:String, path:String):AppProtocolParseOutcome {
		final i = fieldIndex(object.keys, name);
		if (i < 0)
			return success("named-migration-array:missing");
		return switch object.values[i] {
			case JArray(entries):
				var entryIndex = 0;
				while (entryIndex < entries.length) {
	...
	static function validateOptionalPluginsMigrationArray(object:ProtocolObjectField, name:String, path:String):AppProtocolParseOutcome {
		final i = fieldIndex(object.keys, name);
		if (i < 0)
			return success("plugins-migration-array:missing");
		return switch object.values[i] {
			case JArray(entries):
				var entryIndex = 0;
				while (entryIndex < entries.length) {
	...
	static function validateOptionalSessionMigrationArray(object:ProtocolObjectField, name:String, path:String):AppProtocolParseOutcome {
		final i = fieldIndex(object.keys, name);
		if (i < 0)
			return success("session-migration-array:missing");
		return switch object.values[i] {
			case JArray(entries):
				var entryIndex = 0;
				while (entryIndex < entries.length) {
	...
	static function validateRequiredNonEmptyStringArray(values:Array<Value>, path:String):AppProtocolParseOutcome {
		if (values.length == 0)
			return fail("empty_array", path, "expected non-empty string array");
		var i = 0;
		while (i < values.length) {
			switch values[i] {
				case JString(_):
	...
	static function validateOptionalCommandExecTerminalSize(object:ProtocolObjectField, name:String, path:String):AppProtocolParseOutcome {
		final i = fieldIndex(object.keys, name);
		if (i < 0)
			return success("command-exec-size:missing");
		return switch object.values[i] {
			case JNull:
	...
	static function validateCommandExecTerminalSizeObject(size:ProtocolObjectField, path:String):AppProtocolParseOutcome {
		...
		if (!rows.ok)
			return rows.toOutcome();
		final cols = requiredUInt(size.keys, size.values, "cols", path + ".cols");
		if (!cols.ok)
			return cols.toOutcome();
		if (rows.value > 65535)
			return fail("expected_uint16", path + ".rows", "expected unsigned 16-bit JSON integer");
		if (cols.value > 65535)
			return fail("expected_uint16", path + ".cols", "expected unsigned 16-bit JSON integer");
	...
	static function validateOptionalCommandExecSandboxPolicy(object:ProtocolObjectField, name:String, path:String):AppProtocolParseOutcome {
		final i = fieldIndex(object.keys, name);
		if (i < 0)
			return success("sandbox-policy:missing");
		return switch object.values[i] {
			case JNull:
			...
			case JObject(keys, values):
				...
				switch policyType.value {
					...
					case "workspaceWrite":
						...
						if (!excludeSlashTmp.ok)
							return excludeSlashTmp;
						final excludeTmpdirEnvVar = validateOptionalBool(ProtocolObjectField.success(keys, values), "excludeTmpdirEnvVar",
							path + ".excludeTmpdirEnvVar");
						if (!excludeTmpdirEnvVar.ok)
							return excludeTmpdirEnvVar;
						final writableRoots = validateOptionalStringArray(ProtocolObjectField.success(keys, values), "writableRoots", path + ".writableRoots",
							false);
	...
	static function validateOptionalExternalSandboxNetworkAccess(keys:Array<String>, values:Array<Value>, path:String):AppProtocolParseOutcome {
		...
		if (i < 0)
			return success("external-sandbox-network:missing");
		return switch values[i] {
			case JString(value):
				if (value == "restricted" || value == "enabled") success("external-sandbox-network") else fail("invalid_network_access", path,
					"unsupported external sandbox network access");
			case _:
	...
	static function validateRemoteControlStatusChangedNotification(params:ProtocolObjectField):AppProtocolParseOutcome {
		...
		if (!status.ok)
			return status.toOutcome();
		if (!validRemoteControlStatus(status.value))
			return fail("invalid_remote_control_status", "$.message.params.status", "unsupported remote control status");
	...
	static function validateModelReroutedNotification(params:ProtocolObjectField):AppProtocolParseOutcome {
		...
		if (!fromModel.ok)
			return fromModel.toOutcome();
		final toModel = requiredString(params.keys, params.values, "toModel", "$.message.params.toModel");
		if (!toModel.ok)
			return toModel.toOutcome();
		final reason = requiredString(params.keys, params.values, "reason", "$.message.params.reason");
		if (!reason.ok)
			return reason.toOutcome();
		if (!validModelRerouteReason(reason.value))
			return fail("invalid_model_reroute_reason", "$.message.params.reason", "unsupported model reroute reason");
	...
	static function validateModelVerificationNotification(params:ProtocolObjectField):AppProtocolParseOutcome {
		...
		if (!verifications.ok)
			return verifications.toOutcome();

		var i = 0;
		while (i < verifications.values.length) {
			switch verifications.values[i] {
				case JString(value):
					if (!validModelVerification(value))
						return fail("invalid_model_verification", "$.message.params.verifications[" + Std.string(i) + "]", "unsupported model verification");
				case _:
	...
	static function validateOptionalNullableTextRange(object:ProtocolObjectField, name:String, path:String):AppProtocolParseOutcome {
		final i = fieldIndex(object.keys, name);
		if (i < 0)
			return success("text-range:missing");
		return switch object.values[i] {
			case JNull:
	...
	static function validateTextPosition(position:ProtocolObjectField, path:String):AppProtocolParseOutcome {
		...
		if (line.value < 0)
			return fail("expected_uint", path + ".line", "expected unsigned JSON integer");
		final column = requiredInteger(position.keys, position.values, "column", path + ".column");
		if (!column.ok)
			return column.toOutcome();
		if (column.value < 0)
			return fail("expected_uint", path + ".column", "expected unsigned JSON integer");
	...
	static function validateFuzzyFileSearchResult(file:ProtocolObjectField, path:String):AppProtocolParseOutcome {
		...
		if (!fileName.ok)
			return fileName.toOutcome();
		final matchType = requiredString(file.keys, file.values, "match_type", path + ".match_type");
		if (!matchType.ok)
			return matchType.toOutcome();
		if (!validFuzzyFileSearchMatchType(matchType.value))
			return fail("invalid_fuzzy_file_search_match_type", path + ".match_type", "unsupported fuzzy file search match type");
		...
		if (!root.ok)
			return root.toOutcome();
		final score = requiredInteger(file.keys, file.values, "score", path + ".score");
		if (!score.ok)
			return score.toOutcome();
		if (score.value < 0)
			return fail("expected_uint", path + ".score", "expected unsigned JSON integer");
	...
	static function validateOptionalNullableUIntArray(object:ProtocolObjectField, name:String, path:String):AppProtocolParseOutcome {
		final i = fieldIndex(object.keys, name);
		if (i < 0)
			return success("uint-array:missing");
		return switch object.values[i] {
			case JNull:
	...
	static function validateThreadRealtimeStartedNotification(params:ProtocolObjectField):AppProtocolParseOutcome {
		...
		if (!version.ok)
			return version.toOutcome();
		if (!validRealtimeConversationVersion(version.value))
			return fail("invalid_realtime_conversation_version", "$.message.params.version", "unsupported realtime conversation version");
		final realtimeSessionId = validateOptionalNullableString(params, "realtimeSessionId", "$.message.params.realtimeSessionId");
		if (!realtimeSessionId.ok)
			return realtimeSessionId;
	...
	static function validateWindowsSandboxSetupStartParams(params:ProtocolObjectField):AppProtocolParseOutcome {
		...
		if (!mode.ok)
			return mode.toOutcome();
		if (!validWindowsSandboxSetupMode(mode.value))
			return fail("invalid_windows_sandbox_setup_mode", "$.message.params.mode", "unsupported Windows sandbox setup mode");
		final cwd = validateOptionalNullableString(params, "cwd", "$.message.params.cwd");
		if (!cwd.ok)
			return cwd;
	...
	static function validateWindowsSandboxReadinessResponse(result:ProtocolObjectField):AppProtocolParseOutcome {
		...
		if (!status.ok)
			return status.toOutcome();
		if (!validWindowsSandboxReadiness(status.value))
			return fail("invalid_windows_sandbox_readiness", "$.message.result.status", "unsupported Windows sandbox readiness status");
	...
	static function validateOptionalAppBranding(app:ProtocolObjectField, name:String, path:String):AppProtocolParseOutcome {
		...
		if (i < 0)
			return success("app-list:missing-branding");
		return switch app.values[i] {
			...
			case JObject(keys, values):
				...
				if (!isDiscoverable.ok)
					return isDiscoverable.toOutcome();
	...
	static function validateOptionalAppMetadata(app:ProtocolObjectField, name:String, path:String):AppProtocolParseOutcome {
		final i = fieldIndex(app.keys, name);
		if (i < 0)
			return success("app-list:missing-metadata");
		return switch app.values[i] {
			case JNull:
	...
	static function validateOptionalRateLimitWindow(snapshot:ProtocolObjectField, name:String, path:String):AppProtocolParseOutcome {
		...
		if (i < 0)
			return success("rate-limits:missing-window");
	...
	static function validateOptionalCreditsSnapshot(snapshot:ProtocolObjectField, name:String, path:String):AppProtocolParseOutcome {
		final i = fieldIndex(snapshot.keys, name);
		if (i < 0)
			return success("rate-limits:missing-credits");
		return switch snapshot.values[i] {
			case JNull:
	...
	static function validateOptionalSpendControlLimitSnapshot(snapshot:ProtocolObjectField, name:String, path:String):AppProtocolParseOutcome {
		final i = fieldIndex(snapshot.keys, name);
		if (i < 0)
			return success("rate-limits:missing-spend-control");
		return switch snapshot.values[i] {
			case JNull:
	...
	static function validateOptionalAccountPlanType(object:ProtocolObjectField, name:String, path:String):AppProtocolParseOutcome {
		final i = fieldIndex(object.keys, name);
		if (i < 0)
			return success("account:missing-plan-type");
		return switch object.values[i] {
			case JNull:
	...
	static function validateOptionalRateLimitReachedType(object:ProtocolObjectField, name:String, path:String):AppProtocolParseOutcome {
		final i = fieldIndex(object.keys, name);
		if (i < 0)
			return success("rate-limits:missing-reached-type");
		return switch object.values[i] {
			case JNull:
				success("rate-limits:null-reached-type");
			case JString(value):
				if (!validRateLimitReachedType(value)) fail("invalid_rate_limit_reached_type", path,
					"unsupported rate limit reached type") else success("rate-limits:reached-type");
			case _:
				fail("expected_nullable_string", path, "expected JSON string or null");
		}
	}
	...
	static function validateCommandExecOutputDeltaNotification(params:ProtocolObjectField):AppProtocolParseOutcome {
		...
		if (!stream.ok)
			return stream.toOutcome();
		if (!validCommandExecOutputStream(stream.value))
			return fail("invalid_command_exec_stream", "$.message.params.stream", "unsupported command exec output stream");
		final deltaBase64 = requiredString(params.keys, params.values, "deltaBase64", "$.message.params.deltaBase64");
		if (!deltaBase64.ok)
			return deltaBase64.toOutcome();
		final capReached = requiredBool(params.keys, params.values, "capReached", "$.message.params.capReached");
		if (!capReached.ok)
			return capReached.toOutcome();
	...
	static function validateProcessOutputDeltaNotification(params:ProtocolObjectField):AppProtocolParseOutcome {
		...
		if (!stream.ok)
			return stream.toOutcome();
		if (!validProcessOutputStream(stream.value))
			return fail("invalid_process_output_stream", "$.message.params.stream", "unsupported process output stream");
		final deltaBase64 = requiredString(params.keys, params.values, "deltaBase64", "$.message.params.deltaBase64");
		if (!deltaBase64.ok)
			return deltaBase64.toOutcome();
		final capReached = requiredBool(params.keys, params.values, "capReached", "$.message.params.capReached");
	...
	static function validateProcessExitedNotification(params:ProtocolObjectField):AppProtocolParseOutcome {
		...
		if (!stdoutCapReached.ok)
			return stdoutCapReached.toOutcome();
		...
		if (!stderrCapReached.ok)
			return stderrCapReached.toOutcome();
	...
	static function validateRawResponseItem(item:ProtocolObjectField, path:String):AppProtocolParseOutcome {
		...
		if (!itemType.ok)
			return itemType.toOutcome();
		if (itemType.value != "message")
			return fail("unsupported_raw_response_item", path + ".type", "only raw message response items are supported in this subset");
	...
	static function validateUserInputArray(entries:Array<Value>, path:String):AppProtocolParseOutcome {
		...
		while (i < entries.length) {
			...
			if (!inputType.ok)
				return inputType.toOutcome();
			if (inputType.value != "text")
				return fail("unsupported_user_input", entryPath + ".type", "only text user input is supported in this subset");
	...
	static function validRealtimeVoice(value:String):Bool {
		return value == "alloy" || value == "arbor" || value == "ash" || value == "ballad" || value == "breeze" || value == "cedar" || value == "coral"
			|| value == "cove" || value == "echo" || value == "ember" || value == "juniper" || value == "maple" || value == "marin" || value == "sage"
			|| value == "shimmer" || value == "sol" || value == "spruce" || value == "vale" || value == "verse";
	}
	...
	static function validExternalAgentConfigMigrationItemType(value:String):Bool {
		return value == "AGENTS_MD" || value == "CONFIG" || value == "SKILLS" || value == "PLUGINS" || value == "MCP_SERVER_CONFIG" || value == "SUBAGENTS"
			|| value == "HOOKS" || value == "COMMANDS" || value == "SESSIONS";
	}
	...
	static function requiredString(keys:Array<String>, values:Array<Value>, name:String, path:String):ProtocolStringField {
		...
		if (i < 0)
			return ProtocolStringField.failure("missing_field", path, "required field is missing");
		return switch values[i] {
	...
	static function requiredNonEmptyString(keys:Array<String>, values:Array<Value>, name:String, path:String):ProtocolStringField {
		...
		if (value.value.length == 0)
			return ProtocolStringField.failure("empty_string", path, "expected non-empty JSON string");
		return value;
	}
	...
	static function requiredNumber(keys:Array<String>, values:Array<Value>, name:String, path:String):ProtocolNumberField {
		final i = fieldIndex(keys, name);
		if (i < 0)
			return ProtocolNumberField.failure("missing_field", path, "required field is missing");
		return switch values[i] {
	...
	static function requiredInteger(keys:Array<String>, values:Array<Value>, name:String, path:String):ProtocolNumberField {
		...
		if (!number.ok)
			return number;
		if (number.value % 1 != 0)
			return ProtocolNumberField.failure("expected_integer", path, "expected JSON integer");
		return number;
	}
	...
	static function requiredUInt(keys:Array<String>, values:Array<Value>, name:String, path:String):ProtocolNumberField {
		...
		if (!number.ok)
			return number;
		if (number.value < 0)
			return ProtocolNumberField.failure("expected_uint", path, "expected unsigned JSON integer");
		return number;
	}
	...
	static function requiredBool(keys:Array<String>, values:Array<Value>, name:String, path:String):ProtocolBoolField {
		final i = fieldIndex(keys, name);
		if (i < 0)
			return ProtocolBoolField.failure("missing_field", path, "required field is missing");
		return switch values[i] {
	...
	static function validateOptionalNullableString(object:ProtocolObjectField, name:String, path:String):AppProtocolParseOutcome {
		final i = fieldIndex(object.keys, name);
		if (i < 0)
			return success("nullable-string:missing");
		return switch object.values[i] {
			case JString(_) | JNull:
	...
	static function validateOptionalNullableNonEmptyString(object:ProtocolObjectField, name:String, path:String):AppProtocolParseOutcome {
		final i = fieldIndex(object.keys, name);
		if (i < 0)
			return success("nullable-non-empty-string:missing");
		return switch object.values[i] {
			case JNull:
	...
	static function validateOptionalNullableStringEnum(object:ProtocolObjectField, name:String, path:String, values:Array<String>,
			errorCode:String):AppProtocolParseOutcome {
		final i = fieldIndex(object.keys, name);
		if (i < 0)
			return success("nullable-string-enum:missing");
		return switch object.values[i] {
			case JNull:
	...
	static function validateOptionalNullableNumber(object:ProtocolObjectField, name:String, path:String):AppProtocolParseOutcome {
		final i = fieldIndex(object.keys, name);
		if (i < 0)
			return success("nullable-number:missing");
		return switch object.values[i] {
	...
	static function validateOptionalNullableInteger(object:ProtocolObjectField, name:String, path:String):AppProtocolParseOutcome {
		final i = fieldIndex(object.keys, name);
		if (i < 0)
			return success("nullable-integer:missing");
		return switch object.values[i] {
			case JNull:
	...
	static function validateOptionalNullableUInt(object:ProtocolObjectField, name:String, path:String):AppProtocolParseOutcome {
		final i = fieldIndex(object.keys, name);
		if (i < 0)
			return success("nullable-uint:missing");
		return switch object.values[i] {
			case JNull:
				success("nullable-uint:null");
			case JNumber(value):
				if (value % 1 != 0) fail("expected_integer", path,
					"expected JSON integer") else if (value < 0) fail("expected_uint", path, "expected unsigned JSON integer") else success("nullable-uint");
			case _:
				fail("expected_nullable_integer", path, "expected JSON integer or null");
		}
	}
	...
	static function optionalBoolValue(object:ProtocolObjectField, name:String, path:String):ProtocolBoolField {
		final i = fieldIndex(object.keys, name);
		if (i < 0)
			return ProtocolBoolField.success(false);
		return switch object.values[i] {
			case JBool(value):
	...
	static function validateOptionalNullableBool(object:ProtocolObjectField, name:String, path:String):AppProtocolParseOutcome {
		final i = fieldIndex(object.keys, name);
		if (i < 0)
			return success("nullable-bool:missing");
		return switch object.values[i] {
			case JBool(_) | JNull:
	...
	static function validateOptionalStringArray(object:ProtocolObjectField, name:String, path:String, nullable:Bool):AppProtocolParseOutcome {
		final i = fieldIndex(object.keys, name);
		if (i < 0)
			return success("string-array:missing");
		return switch object.values[i] {
	...
	static function validateOptionalThreadListCwd(object:ProtocolObjectField, name:String, path:String):AppProtocolParseOutcome {
		final i = fieldIndex(object.keys, name);
		if (i < 0)
			return success("thread-list-cwd:missing");
		return switch object.values[i] {
			case JNull | JString(_):
	...
	static function validateOptionalThreadSourceKindArray(object:ProtocolObjectField, name:String, path:String):AppProtocolParseOutcome {
		return validateOptionalNullableStringEnumArray(object, name, path, [
			"cli",
			"vscode",
			"exec",
			"appServer",
	...
	static function validateOptionalNullableStringMap(object:ProtocolObjectField, name:String, path:String):AppProtocolParseOutcome {
		final i = fieldIndex(object.keys, name);
		if (i < 0)
			return success("nullable-string-map:missing");
		return switch object.values[i] {
			case JNull:
	...
	static function validateOptionalNullableObject(object:ProtocolObjectField, name:String, path:String):AppProtocolParseOutcome {
		final i = fieldIndex(object.keys, name);
		if (i < 0)
			return success("nullable-object:missing");
		return switch object.values[i] {
	...
	static function validateOptionalArrayOrNull(object:ProtocolObjectField, name:String, path:String):AppProtocolParseOutcome {
		final i = fieldIndex(object.keys, name);
		if (i < 0)
			return success("nullable-array:missing");
		return switch object.values[i] {
			case JArray(_) | JNull:
	...
	static function requiredId(keys:Array<String>, values:Array<Value>, path:String):ProtocolValueField {
		...
		if (i < 0)
			return ProtocolValueField.failure("missing_field", path, "required id is missing");
		return switch values[i] {
	...
	static function requiredValue(keys:Array<String>, values:Array<Value>, name:String, path:String):ProtocolValueField {
		final i = fieldIndex(keys, name);
		if (i < 0)
			return ProtocolValueField.failure("missing_field", path, "required field is missing");
	...
	static function optionalValue(keys:Array<String>, values:Array<Value>, name:String):ProtocolValueField {
		final i = fieldIndex(keys, name);
		if (i < 0)
			return ProtocolValueField.failure("missing_field", "$." + name, "optional field is missing");
		return ProtocolValueField.success(values[i]);
	}
	...
	static function requiredObjectField(keys:Array<String>, values:Array<Value>, name:String, path:String):ProtocolObjectField {
		final value = requiredValue(keys, values, name, path);
		if (!value.ok)
			return ProtocolObjectField.failure(value.errorCode, value.errorPath, value.errorMessage);
	...
	static function requiredArray(keys:Array<String>, values:Array<Value>, name:String, path:String):ProtocolArrayField {
		final value = requiredValue(keys, values, name, path);
		if (!value.ok)
			return ProtocolArrayField.failure(value.errorCode, value.errorPath, value.errorMessage);
		return switch value.value {
			case JArray(entries):
