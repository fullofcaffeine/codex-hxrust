...
class ThreadGoal {
	...
	public function new(threadId:String, objective:String, status:String, hasTokenBudget:Bool, tokenBudget:Int, tokensUsed:Int, timeUsedSeconds:Int,
	...
	function baseJson(includeNullBudget:Bool):String {
		var json = "{" + "\"threadId\":" + quote(threadId) + ",\"objective\":" + quote(objective) + ",\"status\":" + quote(status);
		if (hasTokenBudget) {
			json = json + ",\"tokenBudget\":" + Std.string(tokenBudget);
		} else if (includeNullBudget) {
			json = json + ",\"tokenBudget\":null";
		}
		return json + ",\"tokensUsed\":" + Std.string(tokensUsed) + ",\"timeUsedSeconds\":" + Std.string(timeUsedSeconds) + ",\"createdAt\":"
			+ Std.string(createdAt) + ",\"updatedAt\":" + Std.string(updatedAt) + "}";
	}
		...
		return switch value {
			case JObject(keys, values):
				...
				if (!threadId.ok || !objective.ok || !status.ok || !tokenBudget.ok || !tokensUsed.ok || !timeUsedSeconds.ok || !createdAt.ok || !updatedAt.ok) {
					...
					ThreadGoalRead.success(new ThreadGoal(threadId.value, objective.value, status.value, tokenBudget.present, tokenBudget.value,
						tokensUsed.value, timeUsedSeconds.value, createdAt.value, updatedAt.value));
				}
			case _:
