...
class ReplayStateSnapshotReplayGate {
	public static function run():ReplayStateSnapshotReplayReport {
		final sourceReport = ReplayStateKeyboardStateGate.run();
		final replays = replay(sourceReport);
		final snapshots = replayedSnapshots(replays);

		return new ReplayStateSnapshotReplayReport({
	...
	static function replay(report:ReplayStateKeyboardStateReport):Array<RecoveryRenderSnapshotReplay> {
		return [
	...
	static function entry(report:ReplayStateKeyboardStateReport, replayIndex:Int, sourceSequence:Int, intent:RecoveryKeyboardIntentKind,
			selectedThreadId:String, expectedMarker:String, expectedFooter:String):RecoveryRenderSnapshotReplay {
