...
import codexhx.runtime.tui.resume.host.CompletionInputRenderIntent;
import codexhx.runtime.tui.resume.host.CompletionInputRenderIntentKind;

class SeventhInputRenderIntentPlanner {
	...
	public function plan(report:SeventhInputAdmissionReport):CompletionInputRenderIntent {
		...
		final out = new CompletionInputRenderIntent({
			kind: ready ? CompletionInputRenderIntentKind.LocalRenderRequested : CompletionInputRenderIntentKind.RenderIntentRejected,
			sourceIntent: CompletionInputIntentKind.ConfirmRecoveredSelection,
			...
			reason: ready ? "post_completion_post_render_replay_aware_rendered_state_seventh_cycle_input_local_render_intent_requested" : "post_completion_post_render_replay_aware_rendered_state_seventh_cycle_input_render_intent_rejected"
		});
		nextSequence = nextSequence + 1;
		log.push("postRenderReplayAwareRenderedStateSeventhCycleIntent:" + out.summary());
		return out;
	}
