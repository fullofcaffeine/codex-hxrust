...
class ReplayAwareScheduledRenderGate {
	public static function run():ReplayAwareScheduledRenderReport {
		final schedulingReport = ReplayAwareRenderRequestGate.run();
		final renderer = new DeterministicTerminalRenderer();
		final executor = new ReplayAwareScheduledRenderExecutor();
		final execution = executor.execute(schedulingReport, renderer);
		return new ReplayAwareScheduledRenderReport({
