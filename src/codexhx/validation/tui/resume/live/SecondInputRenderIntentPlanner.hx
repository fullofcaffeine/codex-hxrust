...
class SecondInputRenderIntentPlanner {
	...
	public function plan(report:SecondInputAdmissionReport):CompletionInputRenderIntent {
		...
		final out = new CompletionInputRenderIntent({
			...
			reason: ready ? "post_completion_post_render_replay_aware_rendered_state_second_cycle_input_local_render_intent_requested" : "post_completion_post_render_replay_aware_rendered_state_second_cycle_input_render_intent_rejected"
		});
		nextSequence = nextSequence + 1;
		log.push("postRenderReplayAwareRenderedStateSecondCycleIntent:" + out.summary());
		return out;
	}
