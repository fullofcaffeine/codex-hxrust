...
class ReplayAwareSnapshotReplayGate {
	public static function run():ReplayAwareSnapshotReplayReport {
		final sourceReport = ReplayAwareKeyboardStateGate.run();
		final replays = replay(sourceReport);
		final snapshots = replayedSnapshots(replays);

		return new ReplayAwareSnapshotReplayReport({
	...
	static function replay(report:ReplayAwareKeyboardStateReport):Array<RecoveryRenderSnapshotReplay> {
		return [
	...
	static function entry(report:ReplayAwareKeyboardStateReport, replayIndex:Int, sourceSequence:Int, intent:RecoveryKeyboardIntentKind,
