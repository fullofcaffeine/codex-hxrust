...
class SecondKeyboardReadinessGate {
	public static function run():SecondKeyboardReadinessReport {
		final handoffReport = ReplayStateExecutionHandoffGate.run();
		final policy = new SecondKeyboardReadinessPolicy();
		final readiness = policy.admit(handoffReport, [RecoveryKeyboardIntentKind.MoveDown, RecoveryKeyboardIntentKind.MoveUp]);
		return new SecondKeyboardReadinessReport({
