...
import codexhx.runtime.tui.resume.host.RefreshReplayDeliveryKind;
...
class RefreshReplayDeliveryGate {
	public static function run():RefreshReplayDeliveryReport {
		...
		final pagePoll = pump.forward(StreamEvent.pageResult(1, "typed-refresh-replay-recovery-page", threadListResultJson("replay")));
		...
		return new RefreshReplayDeliveryReport({
			deliveryIntentCount: deliveries.length,
			pendingInteractiveReplayDelivered: countKind(deliveries, RefreshReplayDeliveryKind.PendingInteractiveReplay) == 5,
			sideParentStatusDelivered: countKind(deliveries, RefreshReplayDeliveryKind.SideParentStatus) == 5,
			activeThreadStatusDelivered: countKind(deliveries, RefreshReplayDeliveryKind.ActiveThreadStatus) == 5,
	...
	static function buildEnvelopes(pump:EventPump, registry:DeterministicTypedPendingRequestRegistry, builder:EnvelopeBuilder, envelopes:Array<Envelope>,
		...
		envelopes.push(resolveKeyed(pump, registry, builder, forwardPolls, PendingRequestClassKind.ExecApproval, "typed-replay-exec-1", "approval-1",
			"turn-1", "call-1", "", "", "item/commandExecution/requestApproval;approval=approval-1"));
		envelopes.push(resolveKeyed(pump, registry, builder, forwardPolls, PendingRequestClassKind.FileChangeApproval, "typed-replay-file-2", "patch-1",
			"turn-1", "patch-1", "", "", "item/fileChange/requestApproval;item=patch-1"));
		envelopes.push(resolveKeyed(pump, registry, builder, forwardPolls, PendingRequestClassKind.PermissionsApproval, "typed-replay-permissions-3",
		...
		final unsupported = registry.register(PendingRequestClassKind.DynamicToolCall, "typed-replay-dynamic-6", "tool-6", "turn-4", "tool-6", "", "",
	...
	static function applyDelivery(state:ResumePickerState, delivery:RefreshReplayDelivery):Void {
	...
	static function deliveryOrderPreserved(deliveries:Array<RefreshReplayDelivery>):Bool {
	...
	static function ignoredApplicationsHaveNoDelivery(applications:Array<RefreshApplication>, deliverySummaries:Array<String>,
	...
	static function countKind(deliveries:Array<RefreshReplayDelivery>, kind:RefreshReplayDeliveryKind):Int {
		var count = 0;
		for (delivery in deliveries)
