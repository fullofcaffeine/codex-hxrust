...
class FifthSnapshotReplayGate {
	public static function run():FifthSnapshotReplayReport {
		final sourceReport = FifthKeyboardStateGate.run();
		final replays = replay(sourceReport);
		final snapshots = replayedSnapshots(replays);

		return new FifthSnapshotReplayReport({
	...
	static function replay(report:FifthKeyboardStateReport):Array<RecoveryRenderSnapshotReplay> {
		return [
	...
	static function entry(report:FifthKeyboardStateReport, replayIndex:Int, sourceSequence:Int, intent:RecoveryKeyboardIntentKind, selectedThreadId:String,
