...
class SecondInputAdmissionGate {
	public static function run():SecondInputAdmissionReport {
		final completionReport = SecondCompletionHandoffGate.run();
		final policy = new SecondInputAdmissionPolicy();
		final admission = policy.admit(completionReport, CompletionInputIntentKind.ConfirmRecoveredSelection);
		return new SecondInputAdmissionReport({
