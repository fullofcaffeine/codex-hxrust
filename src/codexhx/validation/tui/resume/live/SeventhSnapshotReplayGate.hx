...
class SeventhSnapshotReplayGate {
	public static function run():SeventhSnapshotReplayReport {
		final sourceReport = SeventhKeyboardStateGate.run();
		final replays = replay(sourceReport);
		final snapshots = replayedSnapshots(replays);

		return new SeventhSnapshotReplayReport({
	...
	static function replay(report:SeventhKeyboardStateReport):Array<RecoveryRenderSnapshotReplay> {
		return [
	...
	static function entry(report:SeventhKeyboardStateReport, replayIndex:Int, sourceSequence:Int, intent:RecoveryKeyboardIntentKind, selectedThreadId:String,
