...
class ThirdSnapshotReplayGate {
	public static function run():ThirdSnapshotReplayReport {
		final sourceReport = ThirdKeyboardStateGate.run();
		final replays = replay(sourceReport);
		final snapshots = replayedSnapshots(replays);

		return new ThirdSnapshotReplayReport({
	...
	static function replay(report:ThirdKeyboardStateReport):Array<RecoveryRenderSnapshotReplay> {
		return [
	...
	static function entry(report:ThirdKeyboardStateReport, replayIndex:Int, sourceSequence:Int, intent:RecoveryKeyboardIntentKind, selectedThreadId:String,
