...
class NinthKeyboardStateGate {
	static final FinalFooter = "footer keyboard move_up selected=0 selectedThread=thread-surface-a";

	public static function run():NinthKeyboardStateReport {
		final readinessReport = NinthKeyboardReadinessGate.run();
		...
		return new NinthKeyboardStateReport({
	...
	static function decisionsFrom(readinessReport:NinthKeyboardReadinessReport):Array<RecoveryKeyboardDecision> {
	...
	static function decision(ready:Bool, intent:RecoveryKeyboardIntentKind, sequence:Int, selectedBefore:Int, selectedAfter:Int, threadBefore:String,
			threadAfter:String, rejectedThreadAfter:String, readinessReport:NinthKeyboardReadinessReport):RecoveryKeyboardDecision {
