...
class ReplayAwareRenderRequestGate {
	public static function run():ReplayAwareRenderRequestReport {
		final renderIntentReport = ReplayAwareInputRenderIntentGate.run();
		final frameScheduler = new DeterministicFrameScheduler();
		final requestScheduler = new ReplayAwareRenderRequestScheduler();
		final schedule = requestScheduler.schedule(renderIntentReport, frameScheduler);
		return new ReplayAwareRenderRequestReport({
