...
class SecondSnapshotReplayGate {
	public static function run():SecondSnapshotReplayReport {
		final sourceReport = SecondKeyboardStateGate.run();
		final replays = replay(sourceReport);
		final snapshots = replayedSnapshots(replays);

		return new SecondSnapshotReplayReport({
	...
	static function replay(report:SecondKeyboardStateReport):Array<RecoveryRenderSnapshotReplay> {
		return [
	...
	static function entry(report:SecondKeyboardStateReport, replayIndex:Int, sourceSequence:Int, intent:RecoveryKeyboardIntentKind, selectedThreadId:String,
