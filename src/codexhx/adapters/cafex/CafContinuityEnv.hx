...
class CafContinuityEnv {
	...
	function new(sessionReceiptPath:String, turnReceiptPath:String, successorRunId:String, predecessorSessionId:String, actualContinuityMode:String,
			continuityLoaded:Bool, receiptSource:String) {
	...
	public static function parse(sessionReceiptPath:String, turnReceiptPath:String, successorRunId:String,
			predecessorSessionId:String):CafContinuityEnvParseOutcome {
		final successor = StringTools.trim(successorRunId);
		final predecessor = StringTools.trim(predecessorSessionId);

		if (successor.length == 0)
			return CafContinuityEnvParseOutcome.failure("missing_successor_run_id", "CAF_CODEX_SUCCESSOR_RUN_ID is required");
		if (!validMetadataAtom(successor))
			return CafContinuityEnvParseOutcome.failure("invalid_successor_run_id", "CAF_CODEX_SUCCESSOR_RUN_ID contains unsupported characters");
		...
		return CafContinuityEnvParseOutcome.success(new CafContinuityEnv(sessionReceiptPath, turnReceiptPath, successor, predecessor,
			loaded ? "resume_same_conversation" : "fresh_unlinked", loaded, loaded ? "resume" : "startup"));
	}

	public static function fromProcess():CafContinuityEnvParseOutcome {
		return parse(env("CAF_CODEX_SESSION_RECEIPT_PATH"), env("CAF_CODEX_TURN_RECEIPT_PATH"), env("CAF_CODEX_SUCCESSOR_RUN_ID"),
	...
	public function metadataJson():String {
		return "{" + "\"schema\":\"codex-hxrust.caf-continuity-metadata.v1\"" + ",\"successorRunId\":" + quote(successorRunId)
			+ ",\"predecessorSessionId\":" + quote(predecessorSessionId) + ",\"actualContinuityMode\":" + quote(actualContinuityMode)
			+ ",\"continuityLoaded\":" + bool(continuityLoaded) + ",\"receiptSource\":" + quote(receiptSource) + "}";
	}
