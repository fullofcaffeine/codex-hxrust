...
class CafActiveLaneWriter {
	...
	public static function maybeWriteActiveLane(env:CafActiveLaneEnv, conversationId:String, nativeLiveStatus:CafNativeLiveStatus, nativeProcessId:Int,
			writtenAt:String):CafReceiptWriteOutcome {
		if (env.activeLanePath.length == 0)
			return CafReceiptWriteOutcome.skipped("missing_active_lane_path");
		final runId = activeRunId(env);
		if (runId.reason.length > 0)
			return CafReceiptWriteOutcome.skipped(runId.reason);
		if (env.ownerPid.length == 0)
			return CafReceiptWriteOutcome.skipped("missing_owner_pid");

		final ownerPid = Std.parseInt(env.ownerPid);
		if (ownerPid == null || ownerPid <= 0)
			return CafReceiptWriteOutcome.skipped("invalid_owner_pid");
		if (ownerPid != nativeProcessId)
			return CafReceiptWriteOutcome.skipped("owner_pid_mismatch");
		if (env.wakeRequestsDir.length == 0)
			return CafReceiptWriteOutcome.skipped("missing_wake_requests_dir");
		if (env.wakeReceiptsDir.length == 0)
			return CafReceiptWriteOutcome.skipped("missing_wake_receipts_dir");
	...
	static function activeRunId(env:CafActiveLaneEnv):RunIdRead {
		...
		if (env.runId.length > 0)
			return RunIdRead.success(env.runId);
		if (env.successorRunId.length > 0)
			return RunIdRead.success(env.successorRunId);
		return RunIdRead.skipped("missing_run_id");
	}

	static function activeLaneJson(env:CafActiveLaneEnv, runId:String, ownerPid:Int, nativeProcessId:Int, conversationId:String,
			nativeLiveStatus:CafNativeLiveStatus, writtenAt:String):String {
		var json = "{\n"
			+ "  \"schema\": "
			+ quote(activeLaneSchema)
			+ ",\n"
			...
			+ stringArray(runtimeRequestSchemas())
			+ ",\n"
			+ "  \"runtimeReceiptSchemas\": "
			+ stringArray(runtimeReceiptSchemas())
			+ ",\n"
			+ "  \"runtimeControlSchemas\": "
			+ stringArray(runtimeControlSchemas())
			+ ",\n"
			+ "  \"continuityGenerationId\": "
			+ quote(env.continuityGenerationId)
			+ ",\n"
			+ "  \"wakeRequestsDir\": "
			+ quote(env.wakeRequestsDir)
			+ ",\n"
			+ "  \"wakeReceiptsDir\": "
			+ quote(env.wakeReceiptsDir)
			+ ",\n"
			+ "  \"pendingInputSnapshotPath\": "
			+ quote(env.pendingInputSnapshotPath)
			+ ",\n"
			+ "  \"conversationId\": "
