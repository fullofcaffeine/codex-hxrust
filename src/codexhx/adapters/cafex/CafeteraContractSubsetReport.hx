...
class CafeteraContractSubsetReport {
	...
	static function coveredContracts():Array<CafeteraContractSubsetContract> {
		return [
			new CafeteraContractSubsetContract("hxcx-5.1-caf-session-turn-receipts", "HXCX-5.1", "pass", "fixture_pass",
				"Caf session and turn receipt writer", "harness/check-caf-receipts.sh", [
			...
			new CafeteraContractSubsetContract("hxcx-5.2-caf-effort-wake-bridge", "HXCX-5.2", "pass", "fixture_pass",
				"Caf effort, wake, and invalid mode directory bridge", "harness/check-caf-bridge.sh", [
			...
			new CafeteraContractSubsetContract("hxcx-5.3-caf-goal-tool-flow", "HXCX-5.3", "pass", "fixture_pass",
				"Minimal Caf goal DTO, state, and tool flow", "harness/check-goals.sh", [
			...
			new CafeteraContractSubsetContract("hxcx-8.1-caf-goal-apply-bridge", "HXCX-8.1", "pass", "fixture_pass",
				"Caf goal-apply request and receipt bridge", "harness/check-caf-bridge.sh", [
			...
			new CafeteraContractSubsetContract("hxcx-8.3-caf-queue-reconcile-bridge", "HXCX-8.3", "pass", "fixture_pass",
				"Caf queue-reconcile request and receipt bridge rails", "harness/check-caf-bridge.sh", [
			...
			new CafeteraContractSubsetContract("hxcx-8.4-caf-mode-apply-bridge-rails", "HXCX-8.4", "pass", "fixture_pass",
				"Caf mode-apply request and next-turn receipt bridge rails", "harness/check-caf-bridge.sh", [
			...
			new CafeteraContractSubsetContract("hxcx-8.2-caf-active-lane-capability", "HXCX-8.2", "pass", "fixture_pass",
				"Caf active-lane capability and native live-status DTO writer", "harness/check-caf-active-lane.sh", [
			...
			new CafeteraContractSubsetContract("hxcx-5.4-caf-continuity-metadata", "HXCX-5.4", "pass", "fixture_pass",
				"Wake/restart successor and predecessor continuity metadata", "harness/check-caf-continuity.sh", [
	...
	static function knownGaps():Array<CafeteraContractSubsetGap> {
		return [
			new CafeteraContractSubsetGap("full-cafetera-codex-module-suite", "gap", "unsupported_full_cafetera_cli",
			...
			new CafeteraContractSubsetGap("live-native-tui-event-loop", "gap", "unsupported_live_tui_runtime",
			...
			new CafeteraContractSubsetGap("native-restart-cutover", "gap", "unsupported_native_restart_cutover",
			...
			new CafeteraContractSubsetGap("caf-mode-apply-runtime", "gap", "unsupported_mode_apply_runtime",
			...
			new CafeteraContractSubsetGap("credentialed-live-model-runtime", "gap", "unsupported_live_model_runtime",
...
class CafeteraContractSubsetContract {
	...
	public function new(id:String, bead:String, status:String, classification:String, contract:String, harness:String, sourceFixtures:Array<String>,
			notes:String) {
		this.id = id;
		this.bead = bead;
		this.status = status;
