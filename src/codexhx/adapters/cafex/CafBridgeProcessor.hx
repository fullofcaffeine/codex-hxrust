...
class CafBridgeProcessor {
	...
	public static function processOnce(requestsDir:String, receiptsDir:String, writtenAt:String):CafBridgeProcessOutcome {
		if (requestsDir.length == 0)
			return CafBridgeProcessOutcome.success(0, 0);
	...
	static function modeReceipt(requestId:String, request:Value, writtenAt:String):String {
		...
		if (normalizedMode.length == 0)
			return modeUnsupportedReceipt(requestId, request, writtenAt);

		return "{\n"
			+ "  \"schema\": "
			+ quote(modeReceiptSchema)
			+ ",\n"
			+ "  \"requestId\": "
			+ quote(requestId)
			+ ",\n"
			...
			+ "  \"continuationQueued\": "
			+ bool(hasNonEmptyString(request, "continuationPrompt"))
			+ ",\n"
	...
	static function queueReconcileReceipt(requestId:String, request:Value, writtenAt:String):String {
		return "{\n"
			+ "  \"schema\": "
			+ quote(queueReconcileReceiptSchema)
			+ ",\n"
			+ "  \"requestId\": "
			+ quote(requestId)
			+ ",\n"
	...
	static function requestIdFor(object:Value, entry:String):String {
		...
		if (requestId.ok && requestId.value.length > 0)
			return requestId.value;
		if (StringTools.endsWith(entry, ".json"))
			return entry.substr(0, entry.length - 5);
		return entry;
	}
	...
	static function firstEnv(first:String, second:String, ?third:String, ?fourth:String):String {
		...
		if (third != null) {
			...
			if (thirdValue != null && thirdValue.length > 0)
				return thirdValue;
		}
		if (fourth != null) {
			final fourthValue = Sys.getEnv(fourth);
			if (fourthValue != null && fourthValue.length > 0)
				return fourthValue;
		}
		return "";
	}
