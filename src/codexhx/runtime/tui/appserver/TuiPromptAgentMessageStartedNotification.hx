...
/**
	Typed `item/started` notification for the fake assistant message item.
**/
class TuiPromptAgentMessageStartedNotification {
	...
	public function new(threadId:ThreadId, turnId:TurnId, itemId:ItemId, text:String, startedAtMs:Float) {
		this.threadId = threadId;
		this.turnId = turnId;
		this.itemId = itemId;
		this.text = text == null ? "" : text;
		this.startedAtMs = startedAtMs < 0 ? 0 : startedAtMs;
	}

	public static function fromDelta(delta:TuiPromptAgentMessageDeltaNotification):TuiPromptAgentMessageStartedNotification {
		return new TuiPromptAgentMessageStartedNotification(delta.threadId, delta.turnId, delta.itemId, delta.delta, 1000);
	}

	public function methodText():String {
		return TuiPromptJsonRpcNotificationMethod.ItemStarted.text();
	}
	...
	public function paramsValue():Value {
		return JObject(["threadId", "turnId", "item", "startedAtMs"], [
