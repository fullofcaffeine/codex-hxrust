...
class TuiPromptJsonRpcExchangeOutcome {
	...
	public static function accepted(response:TuiPromptJsonRpcResponse, notifications:Array<TuiPromptJsonRpcNotification>,
			streamNotifications:Array<TuiPromptJsonRpcStreamNotification>, events:Array<TuiAppServerEvent>):TuiPromptJsonRpcExchangeOutcome {
		return new TuiPromptJsonRpcExchangeOutcome(true, "accepted", response, notifications, streamNotifications, events);
	}
	...
	public function streamNotificationCount():Int {
		return streamNotificationsValue.length;
	}

	public function streamNotificationAt(index:Int):Null<TuiPromptJsonRpcStreamNotification> {
		if (index < 0 || index >= streamNotificationsValue.length)
			return null;
		return streamNotificationsValue[index];
	}
