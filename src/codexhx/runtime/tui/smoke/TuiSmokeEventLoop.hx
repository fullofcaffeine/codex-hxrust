...
private typedef TuiSmokeTextAreaState = {
	var text:String;
	var cursor:Int;
...
private typedef TuiSmokeTextAreaRange = {
	final start:Int;
	final end:Int;
	final name:String;
}

private typedef TuiSmokeTextAreaCursor = {
	final x:Int;
	final y:Int;
	final effectiveScroll:Int;
}
...
class TuiSmokeEventLoop {
	public static function run(request:TuiSmokeLoopRequest):TuiSmokeLoopOutcome {
		...
		for (event in request.events) {
			...
			switch event.kind {
				...
				case TuiSmokeEventKind.TextArea:
					if (!traceTextArea(event.textArea, trace)) {
						exit = TuiSmokeExitKind.Rejected;
						running = false;
					}
		...
		for (action in plan.actions) {
			switch action.kind {
				...
				case TuiSmokeChatWidgetInterruptQuitActionKind.InterruptKey:
					trace.push("tui.chat_widget_interrupt_quit.interrupt_key=" + action.key + ":route=" + action.interruptKeyRoute + ":binding="
						+ action.interruptBindingMatched + ":pending_steers=" + action.pendingSteersBefore + "->" + action.pendingSteersAfter
						+ ":submit_after=" + action.submitPendingSteersAfterInterrupt + ":review=" + action.reviewMode + ":task=" + action.taskRunning
						+ ":warning=" + action.reviewWarningInserted + ":queued_after=" + action.queuedSteerSubmittedAfterInterrupt + ":cancel_edit="
						+ action.cancelEditCleared + ":redraw=" + action.requestRedraw + ":submitted=" + action.interruptSubmitted);
				...
				case TuiSmokeChatWidgetInterruptQuitActionKind.Failure:
					if (action.failureCode == "live_chat_widget_quit_shortcut_state_effects_rejected"
						...
						|| action.failureCode == "live_chat_widget_interrupt_key_routing_effects_rejected") {
	...
	static function traceSideConversation(plan:TuiSmokeSideConversationPlan, trace:Array<String>):Bool {
		...
		for (action in plan.actions) {
			switch action.kind {
				...
				case TuiSmokeSideConversationActionKind.ActiveThreadEventDispatch:
					trace.push("tui.side_conversation.active_thread_event=" + "thread=" + action.activeThreadId + ":kind=" + action.activeEventKind
						+ ":event=" + action.eventId + ":notification=" + action.notificationDelivered + ":collab=" + action.collabReceiverCached
						+ ":pending_request=" + action.pendingRequestRegistered + ":request=" + action.requestDelivered + ":skip=" + action.requestSkipped
	...
	static function traceTextArea(plan:TuiSmokeTextAreaPlan, trace:Array<String>):Bool {
		if (plan == null || plan.allowTerminalMutation || plan.allowRatatuiBuffer || plan.allowClipboardMutation || plan.allowFilesystemMutation
			|| plan.allowNetwork || plan.allowModelCall || plan.allowAppServerDelivery || !plan.enabled()) {
			trace.push("tui.textarea.rejected=live_or_missing");
			return false;
		}
		trace.push("tui.textarea.plan=headless");
		for (action in plan.actions) {
			switch action.kind {
				case TuiSmokeTextAreaActionKind.Failure:
					trace.push("tui.textarea.failure=" + action.failureCode + ":no_terminal=" + action.noTerminalMutation + ":no_buffer="
						+ action.noRatatuiBuffer + ":no_clipboard=" + action.noClipboardMutation + ":no_fs=" + action.noFilesystemMutation + ":no_network="
						+ action.noNetwork + ":no_model=" + action.noModelCall + ":no_app_server=" + action.noAppServerDelivery + ":unsupported="
						+ action.unsupportedRejected);
				case _:
					final actual = textAreaActionTrace(action);
					if (actual != action.expectedTrace) {
						trace.push("tui.textarea." + action.kind + "_mismatch=" + action.name + ":" + actual);
						return false;
					}
					trace.push("tui.textarea." + action.kind + "=" + action.name + ":" + actual);
			}
		}
		return true;
	}

	static function textAreaActionTrace(action:TuiSmokeTextAreaAction):String {
		return switch action.kind {
			case TuiSmokeTextAreaActionKind.ReplaceClear:
				final state = textAreaState(action.text, action.cursor, action.killBuffer, action.vimEnabled, action.vimMode);
				textAreaSetTextClearingElements(state, action.newText);
				textAreaSummary(state) + ":wrap_cache=cleared:preferred_col=cleared";
			case TuiSmokeTextAreaActionKind.ReplaceElements:
				final state = textAreaState(action.text, action.cursor, action.killBuffer, action.vimEnabled, action.vimMode);
				textAreaSetTextWithElements(state, action.newText, action.elements);
				textAreaSummary(state) + ":wrap_cache=cleared";
			case TuiSmokeTextAreaActionKind.InsertReplace:
				final state = textAreaState(action.text, action.cursor, action.killBuffer, action.vimEnabled, action.vimMode);
				textAreaInsertAt(state, action.insertPos, action.insertText);
				final afterInsert = textAreaSummary(state);
				textAreaReplaceRange(state, action.rangeStart, action.rangeEnd, action.replaceText);
				"after_insert=" + afterInsert + ":after_replace=" + textAreaSummary(state);
			case TuiSmokeTextAreaActionKind.CursorBoundary:
				final state = textAreaState(action.text, action.cursor, action.killBuffer, action.vimEnabled, action.vimMode);
				textAreaSetTextClearingElements(state, action.newText);
				textAreaInsertAt(state, state.cursor, action.insertText);
				textAreaSummary(state) + ":boundary_safe=true";
			case TuiSmokeTextAreaActionKind.KillPreserve:
				final state = textAreaState(action.text, action.cursor, action.text, action.vimEnabled, action.vimMode);
				state.text = "";
				state.cursor = 0;
				textAreaSetTextClearingElements(state, action.newText);
				textAreaSetTextClearingElements(state, "");
				textAreaInsertAt(state, state.cursor, state.killBuffer);
				textAreaSummary(state) + ":kill_buffer=" + traceText(state.killBuffer);
			case TuiSmokeTextAreaActionKind.PasteBurstMode:
				final state = textAreaState(action.text, action.cursor, action.killBuffer, action.vimEnabled, action.vimMode);
				"vim="
				+ state.vimEnabled
				+ ":mode="
				+ state.vimMode
				+ ":allows_paste_burst="
				+ textAreaAllowsPasteBurst(state);
			case TuiSmokeTextAreaActionKind.WrapCursor:
				final state = textAreaState(action.text, action.cursor, action.killBuffer, action.vimEnabled, action.vimMode);
				final lines = textAreaWrappedLines(state.text, action.width);
				final cursor = textAreaCursorPos(state, action.width, action.areaHeight, action.scroll, action.areaX, action.areaY);
				"height="
				+ lines.length
				+ ":lines="
				+ textAreaLinesTrace(lines)
				+ ":cursor="
				+ cursor.x
				+ ","
				+ cursor.y
			...
			case TuiSmokeTextAreaActionKind.ElementMutation:
				textAreaElementMutationTrace();
			case TuiSmokeTextAreaActionKind.ElementRange:
	...
	static function textAreaState(text:String, cursor:Int, killBuffer:String, vimEnabled:Bool, vimMode:String):TuiSmokeTextAreaState {
		final state:TuiSmokeTextAreaState = {
			text: text,
			cursor: 0,
			elements: [],
		...
		textAreaSetCursor(state, cursor);
		return state;
	}

	static function textAreaSetTextClearingElements(state:TuiSmokeTextAreaState, text:String):Void {
		state.text = text;
	...
	static function textAreaSetTextWithElements(state:TuiSmokeTextAreaState, text:String, elements:Array<TuiSmokeTextAreaElement>):Void {
		state.text = text;
		state.cursor = intMin(state.cursor, textAreaByteLen(state.text));
		final next:Array<TuiSmokeTextAreaRange> = [];
		for (element in elements) {
			var start = intMin(element.start, textAreaByteLen(state.text));
			var end = intMin(element.end, textAreaByteLen(state.text));
			start = textAreaClampByteToBoundary(state.text, start);
			end = textAreaClampByteToBoundary(state.text, end);
			if (start < end)
				textAreaInsertSortedRange(next, {start: start, end: end, name: element.name});
		}
		state.elements = next;
		state.cursor = textAreaClampByteToBoundary(state.text, state.cursor);
	}

	static function textAreaInsertSortedRange(ranges:Array<TuiSmokeTextAreaRange>, range:TuiSmokeTextAreaRange):Void {
		var index = 0;
		while (index < ranges.length && ranges[index].start <= range.start)
			index++;
		ranges.insert(index, range);
	}

	static function textAreaSetCursor(state:TuiSmokeTextAreaState, cursor:Int):Void {
		final boundary = textAreaClampByteToBoundary(state.text, intClamp(cursor, 0, textAreaByteLen(state.text)));
		state.cursor = textAreaClampPosToNearestAtomicBoundary(state, boundary);
	}

	static function textAreaInsertAt(state:TuiSmokeTextAreaState, pos:Int, insert:String):Void {
		final safePos = textAreaClampPosForInsertion(state, pos);
		final charPos = textAreaByteToCharIndex(state.text, safePos);
		state.text = state.text.substr(0, charPos) + insert + state.text.substr(charPos);
		final insertedLen = textAreaByteLen(insert);
		if (safePos <= state.cursor)
			state.cursor += insertedLen;
		textAreaShiftElements(state, safePos, 0, insertedLen);
	}
	...
	static function textAreaReplaceRange(state:TuiSmokeTextAreaState, start:Int, end:Int, replacement:String):Void {
		...
		final insertedLen = textAreaByteLen(replacement);
		state.text = state.text.substr(0, startChar) + replacement + state.text.substr(endChar);
		textAreaUpdateElementsAfterReplace(state, safeStart, safeEnd, insertedLen);
		if (state.cursor <= safeEnd && state.cursor >= safeStart) {
			state.cursor = safeStart + insertedLen;
		} else if (state.cursor > safeEnd) {
			state.cursor = state.cursor + insertedLen - removedLen;
		}
		state.cursor = intMin(state.cursor, textAreaByteLen(state.text));
		state.cursor = textAreaClampByteToBoundary(state.text, state.cursor);
	}
	...
	static function textAreaElementMutationTrace():String {
		final state = textAreaState("pre post", "pre ".length, "", false, "insert");
		textAreaInsertElement(state, "<tool>", "tool-call");
		final inserted = textAreaSummary(state);
		final payloads = textAreaElementPayloadsTrace(state);
		final namedBefore = textAreaNamedElementRangeTrace(state, "tool-call");
		final replacedPayload = textAreaReplaceElementPayload(state, "<tool>", "<done>");
		final afterPayload = textAreaSummary(state);
		final updatedNamed = textAreaUpdateNamedElementById(state, "tool-call", "[done]");
		final afterUpdate = textAreaSummary(state);
		final namedAfter = textAreaNamedElementRangeTrace(state, "tool-call");
		final replacedById = textAreaReplaceElementById(state, "tool-call", "done");
		final afterReplaceId = textAreaSummary(state);
		return "inserted=" + inserted + ":payloads=" + payloads + ":named_before=" + namedBefore + ":replace_payload=" + replacedPayload + ":after_payload="
			+ afterPayload + ":update_named=" + updatedNamed + ":after_update=" + afterUpdate + ":named_after=" + namedAfter + ":replace_by_id="
			+ replacedById + ":after_replace_id=" + afterReplaceId;
	}

	static function textAreaElementRangeTrace():String {
		final state = textAreaState("ab cd ef", 0, "", false, "insert");
		final late = textAreaAddElementRange(state, 6, 8, "late");
		final early = textAreaAddElementRange(state, 0, 2, "early");
		final duplicate = textAreaAddElementRange(state, 0, 2, "duplicate");
		final overlap = textAreaAddElementRange(state, 1, 4, "overlap");
		final clamped = textAreaAddElementRange(state, 99, 99, "invalid");
		final ordered = textAreaElementTrace(state.elements);
		final removed = textAreaRemoveElementRange(state, 0, 2);
		final removeMissing = textAreaRemoveElementRange(state, 0, 2);
		return "added=" + late + "," + early + ":duplicate=" + duplicate + ":overlap=" + overlap + ":clamped=" + clamped + ":ordered=" + ordered
			+ ":removed=" + removed + ":missing=" + removeMissing + ":after=" + textAreaSummary(state);
	}

	static function textAreaElementBoundaryTrace():String {
		final state = textAreaState("a<el>b", 0, "", false, "insert");
		textAreaAddElementRange(state, 1, 5, "atom");
		textAreaSetCursor(state, 3);
		final clampedCursor = state.cursor;
		textAreaInsertAt(state, 4, "X");
		final afterInsert = textAreaSummary(state);
		final replace = textAreaState("ab<wide>cd", 0, "", false, "insert");
		textAreaAddElementRange(replace, 2, 8, "wide");
		textAreaReplaceRange(replace, 4, 9, "Z");
		return "cursor=" + clampedCursor + ":after_insert=" + afterInsert + ":prev=" + textAreaPrevAtomicBoundary(state, 6) + ":next="
	...
	static function textAreaInsertElement(state:TuiSmokeTextAreaState, payload:String, name:String):Void {
		...
		final end = start + textAreaByteLen(payload);
		textAreaInsertSortedRange(state.elements, {start: start, end: end, name: name});
		textAreaSetCursor(state, end);
	}

	static function textAreaElementPayloadsTrace(state:TuiSmokeTextAreaState):String {
		if (state.elements.length == 0)
			return "<none>";
		final payloads:Array<String> = [];
		for (element in state.elements)
			payloads.push(traceText(textAreaSlice(state.text, element.start, element.end)));
		return payloads.join("|");
	}

	static function textAreaNamedElementRangeTrace(state:TuiSmokeTextAreaState, name:String):String {
		for (element in state.elements) {
	...
	static function textAreaReplaceElementPayload(state:TuiSmokeTextAreaState, oldPayload:String, newPayload:String):Bool {
		for (element in state.elements) {
			if (textAreaSlice(state.text, element.start, element.end) == oldPayload) {
				final start = element.start;
				final end = element.end;
				final name = element.name;
				textAreaReplaceRange(state, start, end, newPayload);
				textAreaInsertSortedRange(state.elements, {start: start, end: start + textAreaByteLen(newPayload), name: name});
				return true;
			}
		}
		return false;
	...
	static function textAreaReplaceElementById(state:TuiSmokeTextAreaState, name:String, replacement:String):Bool {
		final element = textAreaFindNamedElement(state, name);
		if (element == null)
			return false;
		textAreaReplaceRange(state, element.start, element.end, replacement);
		return true;
	}

	static function textAreaUpdateNamedElementById(state:TuiSmokeTextAreaState, name:String, replacement:String):Bool {
		final element = textAreaFindNamedElement(state, name);
		if (element == null)
			return false;
		final start = element.start;
		textAreaReplaceRange(state, element.start, element.end, replacement);
		textAreaInsertSortedRange(state.elements, {start: start, end: start + textAreaByteLen(replacement), name: name});
		return true;
	}

	static function textAreaFindNamedElement(state:TuiSmokeTextAreaState, name:String):Null<TuiSmokeTextAreaRange> {
		for (element in state.elements) {
			if (element.name == name)
				return element;
		}
	...
	static function textAreaAddElementRange(state:TuiSmokeTextAreaState, start:Int, end:Int, name:String):Bool {
		...
		textAreaInsertSortedRange(state.elements, {start: safeStart, end: safeEnd, name: name});
		return true;
	}

	static function textAreaRemoveElementRange(state:TuiSmokeTextAreaState, start:Int, end:Int):Bool {
		...
		for (idx in 0...state.elements.length) {
			final element = state.elements[idx];
			if (element.start == safeStart && element.end == safeEnd) {
	...
	static function textAreaClampPosForInsertion(state:TuiSmokeTextAreaState, pos:Int):Int {
		final boundary = textAreaClampByteToBoundary(state.text, intClamp(pos, 0, textAreaByteLen(state.text)));
		return textAreaClampPosToNearestAtomicBoundary(state, boundary);
	}

	static function textAreaClampPosToNearestAtomicBoundary(state:TuiSmokeTextAreaState, pos:Int):Int {
		for (element in state.elements) {
			if (pos > element.start && pos < element.end) {
				final startDistance = pos - element.start;
				final endDistance = element.end - pos;
				return startDistance <= endDistance ? element.start : element.end;
			}
		}
		return pos;
	}

	static function textAreaSlice(text:String, start:Int, end:Int):String {
	...
	static function textAreaShiftElements(state:TuiSmokeTextAreaState, at:Int, removed:Int, inserted:Int):Void {
		final diff = inserted - removed;
		final shifted:Array<TuiSmokeTextAreaRange> = [];
		for (element in state.elements) {
			if (element.end <= at) {
				shifted.push(element);
			} else if (element.start >= at + removed) {
				shifted.push({start: element.start + diff, end: element.end + diff, name: element.name});
			}
		}
		state.elements = shifted;
	}

	static function textAreaUpdateElementsAfterReplace(state:TuiSmokeTextAreaState, start:Int, end:Int, inserted:Int):Void {
		final diff = inserted - (end - start);
		final next:Array<TuiSmokeTextAreaRange> = [];
		for (element in state.elements) {
			if (element.end <= start) {
				next.push(element);
			} else if (element.start >= end) {
				next.push({start: element.start + diff, end: element.end + diff, name: element.name});
			}
		}
		state.elements = next;
	}

	static function textAreaAllowsPasteBurst(state:TuiSmokeTextAreaState):Bool {
		return !state.vimEnabled || state.vimMode == "insert";
	}

	static function textAreaWrappedLines(text:String, width:Int):Array<TuiSmokeTextAreaRange> {
		final safeWidth = intMax(1, width);
		final out:Array<TuiSmokeTextAreaRange> = [];
		var lineStart = 0;
		var lineLen = 0;
		var bytePos = 0;
		var idx = 0;
		while (idx < text.length) {
			final ch = text.charAt(idx);
			final chBytes = textAreaCharByteLen(text, idx);
			if (ch == "\n") {
				out.push({start: lineStart, end: bytePos, name: ""});
				bytePos += chBytes;
				lineStart = bytePos;
				lineLen = 0;
			} else {
				if (lineLen >= safeWidth) {
					out.push({start: lineStart, end: bytePos, name: ""});
					lineStart = bytePos;
					lineLen = 0;
				}
	...
	static function textAreaCursorPos(state:TuiSmokeTextAreaState, width:Int, areaHeight:Int, scroll:Int, areaX:Int, areaY:Int):TuiSmokeTextAreaCursor {
		final lines = textAreaWrappedLines(state.text, width);
		var lineIndex = 0;
		for (idx in 0...lines.length) {
			if (lines[idx].start <= state.cursor)
				lineIndex = idx;
		}
		final maxScroll = intMax(0, lines.length - intMax(1, areaHeight));
		var effectiveScroll = intClamp(scroll, 0, maxScroll);
		if (lineIndex < effectiveScroll)
			effectiveScroll = lineIndex;
		if (lineIndex >= effectiveScroll + areaHeight)
			effectiveScroll = lineIndex - areaHeight + 1;
		effectiveScroll = intClamp(effectiveScroll, 0, maxScroll);
		final line = lines[lineIndex];
		final x = areaX + intMax(0, state.cursor - line.start);
		final y = areaY + intClamp(lineIndex - effectiveScroll, 0, intMax(0, areaHeight - 1));
		return {x: x, y: y, effectiveScroll: effectiveScroll};
	}

	static function textAreaByteLen(text:String):Int {
		var total = 0;
		for (idx in 0...text.length)
			total += textAreaCharByteLen(text, idx);
		return total;
	}

	static function textAreaCharByteLen(text:String, idx:Int):Int {
		final code = text.charCodeAt(idx);
		if (code <= 0x7F)
			return 1;
		if (code <= 0x7FF)
			return 2;
	...
	static function textAreaByteToCharIndex(text:String, bytePos:Int):Int {
		var bytes = 0;
		for (idx in 0...text.length) {
			final next = bytes + textAreaCharByteLen(text, idx);
			if (bytePos <= bytes)
	...
	static function textAreaClampByteToBoundary(text:String, bytePos:Int):Int {
		final target = intClamp(bytePos, 0, textAreaByteLen(text));
		var bytes = 0;
		var best = 0;
		for (idx in 0...text.length) {
			final next = bytes + textAreaCharByteLen(text, idx);
			if (target == bytes || target == next)
				return target;
			if (target < next)
				return best;
			best = next;
	...
	static function textAreaSummary(state:TuiSmokeTextAreaState):String {
		return "text=" + traceText(state.text) + ":cursor=" + state.cursor + ":elements=" + textAreaElementTrace(state.elements);
	}

	static function textAreaElementTrace(elements:Array<TuiSmokeTextAreaRange>):String {
		if (elements.length == 0)
			return "<none>";
		final out:Array<String> = [];
		for (element in elements)
			out.push(element.start + "-" + element.end + ":" + traceText(element.name));
		return out.join("|");
	}

	static function textAreaLinesTrace(lines:Array<TuiSmokeTextAreaRange>):String {
		if (lines.length == 0)
			return "<none>";
		final out:Array<String> = [];
		for (line in lines)
			out.push(line.start + "-" + line.end);
		return out.join("|");
	}
	...
	static function traceComposerEditing(plan:TuiSmokeComposerEditingPlan, trace:Array<String>):Bool {
		...
		for (action in plan.actions) {
			switch action.kind {
				...
				case TuiSmokeComposerEditingActionKind.TextareaRoute:
					trace.push("tui.composer_editing.textarea_route=" + action.keyName + ":input_enabled=" + action.inputEnabled + ":release_ignored="
						+ action.releaseIgnored + ":popup=" + action.popupTransitionText() + ":text=" + action.inputText + "->" + action.outputText
						+ ":cursor=" + action.cursorTransitionText() + ":mode=" + action.modeBefore + "->" + action.modeAfter + ":textarea="
				...
				case TuiSmokeComposerEditingActionKind.ImagePlaceholderEdit:
					trace.push("tui.composer_editing.image_placeholder_edit=" + action.keyName + ":text=" + action.inputText + "->" + action.outputText
						+ ":placeholder=" + action.placeholderBefore + "->" + action.placeholderAfter + ":local=" + action.localImageTransitionText()
				...
				case TuiSmokeComposerEditingActionKind.MentionsV2FileSelect:
					trace.push("tui.composer_editing.mentions_v2_file_select=" + action.keyName + ":selected=" + action.selectedPath + ":token="
						+ action.tokenBefore + "->" + action.tokenAfter + ":text=" + action.inputText + "->" + action.outputText + ":placeholder="
						+ action.placeholderBefore + "->" + action.placeholderAfter + ":local=" + action.localImageTransitionText() + ":remote="
						+ action.remoteImageBefore + "->" + action.remoteImageAfter + ":elements=" + action.elementTransitionText() + ":payloads="
						+ action.elementPayloads + ":popup=" + action.popupTransitionText() + ":redraw=" + action.needsRedraw + ":cursor="
						+ action.cursorTransitionText() + ":clamped=" + action.cursorClamped + ":retained=" + action.attachmentsRetained);
				case TuiSmokeComposerEditingActionKind.MentionsV2ToolSelect:
					trace.push("tui.composer_editing.mentions_v2_tool_select=" + action.keyName + ":insert=" + action.insertText + ":path="
						+ action.bindingPath + ":token=" + action.tokenBefore + "->" + action.tokenAfter + ":text=" + action.inputText + "->"
						+ action.outputText + ":bindings=" + action.mentionBindingTransitionText() + ":elements=" + action.elementTransitionText()
						+ ":payloads=" + action.elementPayloads + ":pending=" + action.pendingPasteTransitionText() + ":popup="
						+ action.popupTransitionText() + ":redraw=" + action.needsRedraw + ":cursor=" + action.cursorTransitionText() + ":clamped="
						+ action.cursorClamped + ":retained=" + action.attachmentsRetained);
