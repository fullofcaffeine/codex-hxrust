...
class TuiStoryReplaySummary {
    ...
    public function add(record:TuiStoryRecord):Void {
        ...
        switch record.kind {
            ...
            case TuiStoryKind.KeyEvent:
                ...
                if (record.keyEvent != null) typedText = typedText + record.keyEvent.typedCharacter();
    ...
    public function fingerprint():String {
        return "records=" + Std.string(records)
            + "|sessions=" + Std.string(sessionStarts) + "/" + Std.string(sessionEnds)
            + "|app=" + Std.string(appEvents) + ":" + variants.join(",")
            + "|keys=" + Std.string(keyEvents) + ":" + escapeText(typedText)
            + "|codex=" + Std.string(codexEvents) + ":" + codexTypes.join(",")
            + "|history=" + Std.string(insertHistoryEvents) + "/" + Std.string(insertedLines)
            + "|tasks=" + Std.string(taskStarted) + "/" + Std.string(taskCompleted)
            + "|deltas=" + Std.string(agentMessageDeltaCount) + "/" + Std.string(reasoningDeltaCount)
            + "|assistant=" + escapeText(assistantPreview)
            + "|reasoning=" + escapeText(reasoningPreview)
            + "|ops=" + Std.string(operations)
            + "|shutdown=" + Std.string(shutdownComplete);
    }
