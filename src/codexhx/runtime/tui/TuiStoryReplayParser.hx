...
class TuiStoryReplayParser {
	...
	public static function parseLine(line:String, lineNumber:Int):TuiStoryParseOutcome {
		...
		if (!parsed.ok)
			return TuiStoryParseOutcome.failure(parsed.errorCode, parsed.errorPath, parsed.errorMessage);

		if (hasField(parsed.value, "schema"))
			return TuiStoryParseOutcome.success(null);

		final dir = requiredString(parsed.value, "dir");
		if (dir == null)
			return failedRequired(lineNumber, "dir", "expected direction string");
		if (!TuiStoryDirection.isValid(dir))
			return TuiStoryParseOutcome.failure("invalid_direction", path(lineNumber, "dir"), "unsupported story direction");

		final kind = requiredString(parsed.value, "kind");
		if (kind == null)
			return failedRequired(lineNumber, "kind", "expected kind string");
		if (!TuiStoryKind.isValid(kind))
			return TuiStoryParseOutcome.failure("invalid_kind", path(lineNumber, "kind"), "unsupported story kind");

		final ts = requiredString(parsed.value, "ts");
		if (ts == null)
			return failedRequired(lineNumber, "ts", "expected timestamp string");
		...
		return switch kind {
			...
			case TuiStoryKind.AppEvent:
				...
				if (variant == null) failedRequired(lineNumber, "variant",
					"expected app event variant"); else record(lineNumber, dir, kind, ts, variant, null, CodexStoryMessageType.Unsupported, "", 0, "", "");
			case TuiStoryKind.KeyEvent:
				final event = requiredString(parsed.value, "event");
				if (event == null) failedRequired(lineNumber, "event",
					"expected key event string"); else record(lineNumber, dir, kind, ts, "", TuiStoryKeyEvent.parse(event), CodexStoryMessageType.Unsupported,
					"", 0, "", "");
			...
			case TuiStoryKind.InsertHistory:
				...
				if (lines < 0) failedRequired(lineNumber, "lines",
					"expected non-negative insert_history line count"); else record(lineNumber, dir, kind, ts, "", null, CodexStoryMessageType.Unsupported,
					"", lines, "", "");
	...
	static function parseSessionStart(value:Value, lineNumber:Int, dir:String, ts:String):TuiStoryParseOutcome {
		...
		if (cwd == null)
			return failedRequired(lineNumber, "cwd", "expected cwd string");
		final model = requiredString(value, "model");
		if (model == null)
			return failedRequired(lineNumber, "model", "expected model string");
		return record(lineNumber, dir, TuiStoryKind.SessionStart, ts, "", null, CodexStoryMessageType.Unsupported, "", 0, "",
			"cwd:<normalized>|model:<normalized>");
	}
	...
	static function parseCodexEvent(value:Value, lineNumber:Int, dir:String, ts:String):TuiStoryParseOutcome {
		...
		if (payload == null)
			return failedRequired(lineNumber, "payload", "expected codex event payload");
		final msg = requiredObject(payload, "msg");
		if (msg == null)
			return failedRequired(lineNumber, "payload.msg", "expected codex event msg");
		final rawType = requiredString(msg, "type");
		if (rawType == null)
			return failedRequired(lineNumber, "payload.msg.type", "expected codex event type");
	...
	static function parseOperation(value:Value, lineNumber:Int, dir:String, ts:String):TuiStoryParseOutcome {
		final payload = requiredObject(value, "payload");
		if (payload == null)
			return failedRequired(lineNumber, "payload", "expected op payload");
		final opType = requiredString(payload, "type");
		if (opType == null)
			return failedRequired(lineNumber, "payload.type", "expected op type");
		return record(lineNumber, dir, TuiStoryKind.Operation, ts, "", null, CodexStoryMessageType.Unsupported, "", 0, opType, "");
	}

	static function record(lineNumber:Int, dir:String, kind:String, ts:String, appVariant:String, keyEvent:TuiStoryKeyEvent,
			codexEventType:CodexStoryMessageType, textDelta:String, historyLines:Int, operationType:String, sourceId:String):TuiStoryParseOutcome {
		return TuiStoryParseOutcome.success(new TuiStoryRecord(lineNumber, cast dir, cast kind, normalizeTimestamp(ts), appVariant, keyEvent, codexEventType,
			textDelta, historyLines, operationType, sourceId));
	}
