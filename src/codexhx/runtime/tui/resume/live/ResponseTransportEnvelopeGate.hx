...
class ResponseTransportEnvelopeGate {
	...
	public static function run():ResponseTransportEnvelopeReport {
		...
		final resolveCommand = requestHandle.dispatch(RequestResponseIntent.resolved(RESOLVE_REQUEST_ID,
		...
		final rejectCommand = requestHandle.dispatch(RequestResponseIntent.refused(REJECT_REQUEST_ID,
		...
		final localRefusalCommand = requestHandle.dispatch(RequestResponseIntent.resolved(REFUSAL_REQUEST_ID,
		...
		final sendFailedCommand = requestHandle.dispatch(RequestResponseIntent.refused(SEND_FAILED_REQUEST_ID,
		...
		final pagePoll = pump.forward(StreamEvent.pageResult(1, "response-transport-envelope-recovery-page", threadListResultJson("transport")));
		...
		return new ResponseTransportEnvelopeReport({
			resolveEnvelopeRecorded: resolveEnvelope.kind == ResponseTransportEnvelopeKind.ResolveResult
			&& containsOne(envelopeSummaries, "\"result\":{\"answer\":\"continue\"}"),
			rejectEnvelopeRecorded: rejectEnvelope.kind == ResponseTransportEnvelopeKind.RejectError
			&& containsOne(envelopeSummaries, "\"error\":{\"code\":\"unsupported_in_fixture\""),
			localRefusalEnvelopeRecorded: localRefusalEnvelope.kind == ResponseTransportEnvelopeKind.LocalSerializationRefusal
			&& localRefusalEnvelope.localOnly
			&& containsOne(envelopeSummaries, "missing_response_payload"),
			sendFailureEnvelopeRecorded: sendFailedEnvelope.kind == ResponseTransportEnvelopeKind.SendFailure
	...
	static function applyEnvelope(state:ResumePickerState, envelope:ResponseTransportEnvelope, status:String):Void {
		state.inlineErrorShown = envelope.kind == ResponseTransportEnvelopeKind.SendFailure;
		...
		state.loaderEventDetail = DiagnosticSummary.render([
			...
			DiagnosticSummary.boolValue("liveTransport", envelope.liveTransportAttempted),
