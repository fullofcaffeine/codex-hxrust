...
class ResumePickerAppServerTypedResponseDispatchOrderingRefreshRenderGate {
	public static function run():ResumePickerAppServerTypedResponseDispatchOrderingRefreshRenderGateReport {
		...
		envelopes.push(resolveKeyed(pump, registry, builder, events, forwardPolls, ResumePickerAppServerPendingRequestClassKind.ExecApproval,
			"typed-dispatch-exec-1", "approval-1", "turn-1", "call-1", "", "", "item/commandExecution/requestApproval;approval=approval-1"));
		envelopes.push(resolveKeyed(pump, registry, builder, events, forwardPolls, ResumePickerAppServerPendingRequestClassKind.FileChangeApproval,
			"typed-dispatch-file-2", "patch-1", "turn-1", "patch-1", "", "", "item/fileChange/requestApproval;item=patch-1"));
		envelopes.push(resolveKeyed(pump, registry, builder, events, forwardPolls, ResumePickerAppServerPendingRequestClassKind.PermissionsApproval,
		...
		registry.register(ResumePickerAppServerPendingRequestClassKind.UserInput, userInputHost.requestId, "turn-2", "turn-2", "tool-1", "", "",
			userInputHost.detail);
		...
		final unsupportedEvent = registry.register(ResumePickerAppServerPendingRequestClassKind.DynamicToolCall, "typed-dispatch-dynamic-6", "tool-6",
			"turn-4", "tool-6", "", "", "item/tool/call;dynamic=true");
		events.push(unsupportedEvent);
		envelopes.push(builder.resolve(unsupportedEvent));

		final missingEvent = registry.resolveKey(ResumePickerAppServerPendingRequestClassKind.ExecApproval, "missing-approval");
		...
		final pagePoll = pump.forward(ResumePickerAppServerStreamEvent.pageResult(1, "typed-dispatch-recovery-page", threadListResultJson("dispatch")));
		...
		return new ResumePickerAppServerTypedResponseDispatchOrderingRefreshRenderGateReport({
			responseOrderPreserved: orderPreserved(outcomes),
			supportedRefreshScheduled: supportedRefreshScheduled(outcomes),
			unsupportedRejectNoRefresh: outcomeHas(outcomes, ResumePickerAppServerTypedResponseDispatchOutcomeKind.RejectSent, "typed-dispatch-dynamic-6",
				false),
			missingNoopNoRefresh: outcomeHas(outcomes, ResumePickerAppServerTypedResponseDispatchOutcomeKind.LocalNoop, "", false),
			lateDuplicateRefused: outcomeHas(outcomes, ResumePickerAppServerTypedResponseDispatchOutcomeKind.LateDuplicateRefused, "typed-dispatch-exec-1",
				false),
	...
	static function resolveKeyed(pump:ResumePickerAppServerEventPump, registry:DeterministicResumePickerAppServerTypedPendingRequestRegistry,
			builder:DeterministicResumePickerAppServerTypedResponseEnvelopeBuilder, events:Array<ResumePickerAppServerTypedPendingRequestEvent>,
			forwardPolls:Array<String>, requestClass:ResumePickerAppServerPendingRequestClassKind, requestId:String, key:String, turnId:String, itemId:String,
			serverName:String, mcpRequestId:String, detail:String):ResumePickerAppServerTypedResponseEnvelope {
	...
	static function applyOutcome(state:ResumePickerState, outcome:ResumePickerAppServerTypedResponseDispatchOutcome):Void {
		state.inlineErrorShown = outcome.kind == ResumePickerAppServerTypedResponseDispatchOutcomeKind.RejectSent
			|| outcome.kind == ResumePickerAppServerTypedResponseDispatchOutcomeKind.LateDuplicateRefused;
	...
	static function eventSummaries(events:Array<ResumePickerAppServerTypedPendingRequestEvent>):Array<String> {
		final out:Array<String> = [];
		for (event in events)
	...
	static function orderPreserved(outcomes:Array<ResumePickerAppServerTypedResponseDispatchOutcome>):Bool {
		var i = 0;
	...
	static function supportedRefreshScheduled(outcomes:Array<ResumePickerAppServerTypedResponseDispatchOutcome>):Bool {
		var count = 0;
		for (outcome in outcomes) {
	...
	static function outcomeHas(outcomes:Array<ResumePickerAppServerTypedResponseDispatchOutcome>, kind:ResumePickerAppServerTypedResponseDispatchOutcomeKind,
			requestId:String, refreshScheduled:Bool):Bool {
		for (outcome in outcomes)
