...
class FifthKeyboardStateGate {
	static final FinalFooter = "footer keyboard move_up selected=0 selectedThread=thread-surface-a";

	public static function run():FifthKeyboardStateReport {
		final readinessReport = FifthKeyboardReadinessGate.run();
		...
		return new FifthKeyboardStateReport({
	...
	static function decisionsFrom(readinessReport:FifthKeyboardReadinessReport):Array<RecoveryKeyboardDecision> {
	...
	static function decision(ready:Bool, intent:RecoveryKeyboardIntentKind, sequence:Int, selectedBefore:Int, selectedAfter:Int, threadBefore:String,
			threadAfter:String, rejectedThreadAfter:String, readinessReport:FifthKeyboardReadinessReport):RecoveryKeyboardDecision {
