...
class ReplayStateRenderRequestGate {
	public static function run():ReplayStateRenderRequestReport {
		final renderIntentReport = ReplayStateInputRenderIntentGate.run();
		final frameScheduler = new DeterministicFrameScheduler();
		final requestScheduler = new ReplayStateRenderRequestScheduler();
		final schedule = requestScheduler.schedule(renderIntentReport, frameScheduler);
		return new ReplayStateRenderRequestReport({
