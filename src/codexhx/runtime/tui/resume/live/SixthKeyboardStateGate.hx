...
class SixthKeyboardStateGate {
	static final FinalFooter = "footer keyboard move_up selected=0 selectedThread=thread-surface-a";

	public static function run():SixthKeyboardStateReport {
		final readinessReport = SixthKeyboardReadinessGate.run();
		...
		return new SixthKeyboardStateReport({
	...
	static function decisionsFrom(readinessReport:SixthKeyboardReadinessReport):Array<RecoveryKeyboardDecision> {
	...
	static function decision(ready:Bool, intent:RecoveryKeyboardIntentKind, sequence:Int, selectedBefore:Int, selectedAfter:Int, threadBefore:String,
			threadAfter:String, rejectedThreadAfter:String, readinessReport:SixthKeyboardReadinessReport):RecoveryKeyboardDecision {
