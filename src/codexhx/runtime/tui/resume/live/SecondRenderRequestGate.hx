...
class SecondRenderRequestGate {
	public static function run():SecondRenderRequestReport {
		final renderIntentReport = SecondInputRenderIntentGate.run();
		final frameScheduler = new DeterministicFrameScheduler();
		final requestScheduler = new SecondRenderRequestScheduler();
		final schedule = requestScheduler.schedule(renderIntentReport, frameScheduler);
		return new SecondRenderRequestReport({
