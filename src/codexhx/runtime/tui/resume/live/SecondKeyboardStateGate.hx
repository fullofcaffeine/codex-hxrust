...
class SecondKeyboardStateGate {
	static final FinalFooter = "footer keyboard move_up selected=0 selectedThread=thread-surface-a";

	public static function run():SecondKeyboardStateReport {
		final readinessReport = SecondKeyboardReadinessGate.run();
		...
		return new SecondKeyboardStateReport({
	...
	static function decisionsFrom(readinessReport:SecondKeyboardReadinessReport):Array<RecoveryKeyboardDecision> {
	...
	static function decision(ready:Bool, intent:RecoveryKeyboardIntentKind, sequence:Int, selectedBefore:Int, selectedAfter:Int, threadBefore:String,
			threadAfter:String, rejectedThreadAfter:String, readinessReport:SecondKeyboardReadinessReport):RecoveryKeyboardDecision {
