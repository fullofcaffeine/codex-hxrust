...
class ReplayStateKeyboardReadinessGate {
	public static function run():ReplayStateKeyboardReadinessReport {
		final handoffReport = ReplayStateHandoffGate.run();
		final policy = new ReplayStateKeyboardReadinessPolicy();
		final readiness = policy.admit(handoffReport, [RecoveryKeyboardIntentKind.MoveDown, RecoveryKeyboardIntentKind.MoveUp]);
		return new ReplayStateKeyboardReadinessReport({
