...
class EighthSnapshotReplayGate {
	public static function run():EighthSnapshotReplayReport {
		final sourceReport = EighthKeyboardStateGate.run();
		final replays = replay(sourceReport);
		final snapshots = replayedSnapshots(replays);

		return new EighthSnapshotReplayReport({
	...
	static function replay(report:EighthKeyboardStateReport):Array<RecoveryRenderSnapshotReplay> {
		return [
	...
	static function entry(report:EighthKeyboardStateReport, replayIndex:Int, sourceSequence:Int, intent:RecoveryKeyboardIntentKind, selectedThreadId:String,
