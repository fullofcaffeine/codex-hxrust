...
class PostRenderStateHandoffGate {
	public static function run():PostRenderStateHandoffReport {
		final executionReport = PostRenderScheduledRenderGate.run();
		final policy = new PostRenderStateHandoffPolicy();
		final handoff = policy.handoff(executionReport);
		return new PostRenderStateHandoffReport({
