...
class SixthSnapshotReplayGate {
	public static function run():SixthSnapshotReplayReport {
		final sourceReport = SixthKeyboardStateGate.run();
		final replays = replay(sourceReport);
		final snapshots = replayedSnapshots(replays);

		return new SixthSnapshotReplayReport({
	...
	static function replay(report:SixthKeyboardStateReport):Array<RecoveryRenderSnapshotReplay> {
		return [
	...
	static function entry(report:SixthKeyboardStateReport, replayIndex:Int, sourceSequence:Int, intent:RecoveryKeyboardIntentKind, selectedThreadId:String,
