...
class FourthSnapshotReplayGate {
	public static function run():FourthSnapshotReplayReport {
		final sourceReport = FourthKeyboardStateGate.run();
		final replays = replay(sourceReport);
		final snapshots = replayedSnapshots(replays);

		return new FourthSnapshotReplayReport({
	...
	static function replay(report:FourthKeyboardStateReport):Array<RecoveryRenderSnapshotReplay> {
		return [
	...
	static function entry(report:FourthKeyboardStateReport, replayIndex:Int, sourceSequence:Int, intent:RecoveryKeyboardIntentKind, selectedThreadId:String,
