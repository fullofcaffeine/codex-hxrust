...
class ReplayStateHandoffGate {
	public static function run():ReplayStateHandoffReport {
		final executionReport = ReplayAwareScheduledRenderGate.run();
		final policy = new ReplayStateHandoffPolicy();
		final handoff = policy.handoff(executionReport);
		return new ReplayStateHandoffReport({
