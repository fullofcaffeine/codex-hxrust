...
class DeterministicResumePickerAppServerTypedResponseRecoveryPostCompletionPostRenderReplayAwareInputAdmissionPolicy {
	...
	public function admit(report:ResumePickerAppServerTypedResponseRecoveryPostCompletionPostRenderReplayAwareReplayCompletionHandoffRenderGateReport,
		...
		final out = new ResumePickerAppServerTypedResponseRecoveryPostCompletionInputAdmission({
			...
			reason: ready ? "post_completion_post_render_replay_aware_input_admitted_after_replay" : "post_completion_post_render_replay_aware_input_rejected_before_replay"
		});
		nextSequence = nextSequence + 1;
		log.push("postRenderReplayAwareInput:" + out.summary());
		return out;
	}
