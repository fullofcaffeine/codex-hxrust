...
class SeventhKeyboardReadinessGate {
	public static function run():SeventhKeyboardReadinessReport {
		final handoffReport = SixthExecutionHandoffGate.run();
		final policy = new SeventhKeyboardReadinessPolicy();
		final readiness = policy.admit(handoffReport, [RecoveryKeyboardIntentKind.MoveDown, RecoveryKeyboardIntentKind.MoveUp]);
		return new SeventhKeyboardReadinessReport({
