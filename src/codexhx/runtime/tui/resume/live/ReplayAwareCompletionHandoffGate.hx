...
class ReplayAwareCompletionHandoffGate {
	public static function run():ReplayAwareCompletionHandoffReport {
		final replayReport = ReplayAwareSnapshotReplayGate.run();
		final policy = new ReplayAwareCompletionHandoffPolicy();
		final handoff = policy.handoff(replayReport);
		return new ReplayAwareCompletionHandoffReport({
