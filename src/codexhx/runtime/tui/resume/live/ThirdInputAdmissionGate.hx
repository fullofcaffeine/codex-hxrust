...
class ThirdInputAdmissionGate {
	public static function run():ThirdInputAdmissionReport {
		final completionReport = ThirdCompletionHandoffGate.run();
		final policy = new ThirdInputAdmissionPolicy();
		final admission = policy.admit(completionReport, CompletionInputIntentKind.ConfirmRecoveredSelection);
		return new ThirdInputAdmissionReport({
