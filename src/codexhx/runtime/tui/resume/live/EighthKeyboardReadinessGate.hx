...
class EighthKeyboardReadinessGate {
	public static function run():EighthKeyboardReadinessReport {
		final handoffReport = SeventhExecutionHandoffGate.run();
		final policy = new EighthKeyboardReadinessPolicy();
		final readiness = policy.admit(handoffReport, [RecoveryKeyboardIntentKind.MoveDown, RecoveryKeyboardIntentKind.MoveUp]);
		return new EighthKeyboardReadinessReport({
