...
class PostRenderKeyboardReadinessGate {
	public static function run():PostRenderKeyboardReadinessReport {
		final handoffReport = CompletionStateHandoffGate.run();
		final policy = new PostRenderKeyboardReadinessPolicy();
		final readiness = policy.admit(handoffReport, [RecoveryKeyboardIntentKind.MoveDown, RecoveryKeyboardIntentKind.MoveUp]);
		return new PostRenderKeyboardReadinessReport({
