package codexhx.runtime.tui.resume.live.recovery.ninth;
...
class CompletionHandoffPolicy {
	...
	public function handoff(report:SnapshotReplayReport):RecoveryReplayCompletionHandoff {
		...
		final out = new RecoveryReplayCompletionHandoff({
			...
			reason: completed ? "post_completion_post_render_replay_aware_rendered_state_ninth_cycle_replay_completed_for_next_slice" : "post_completion_post_render_replay_aware_rendered_state_ninth_cycle_replay_completion_rejected"
		});
		log.push("post-render-replay-aware-rendered-state-ninth-cycle-completion:" + out.summary());
		return out;
	}
