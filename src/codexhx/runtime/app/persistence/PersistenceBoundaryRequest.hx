...
class PersistenceBoundaryRequest {
    ...
    public function new(
        ...
        stateDbRequested:Bool,
        logDbRequested:Bool,
        reconcileRolloutRequested:Bool,
        persistThreadRequested:Bool
    ) {
    ...
    public function evaluate():PersistenceBoundaryOutcome {
        ...
        if (!metadataOutcome.ok) return metadataOutcome;

        final nativeFeatures = requestedNativeFeatures();
        if (backend == PersistenceBackendKind.NativeSqlite) {
            if (nativeFeatures.length == 0) nativeFeatures.push(PersistentFeatureKind.StateDbHandle);
            return PersistenceBoundaryOutcome.success("native_boundary_required", "native SQLite/log persistence must be implemented as a generic metal Rust boundary", nativeFeatures, metadata.summary());
        }
        ...
        return PersistenceBoundaryOutcome.success("portable_metadata_only", "portable Haxe may validate rollout metadata, not production persistence effects", nativeFeatures, metadata.summary());
    }

    function requestedNativeFeatures():Array<String> {
        final features:Array<String> = [];
        if (stateDbRequested) features.push(PersistentFeatureKind.StateDbHandle);
        if (logDbRequested) features.push(PersistentFeatureKind.LogDbLayer);
        if (reconcileRolloutRequested) features.push(PersistentFeatureKind.ReconcileRollout);
        if (persistThreadRequested) features.push(PersistentFeatureKind.PersistThread);
        return features;
    }
}
