...
class ThreadPersistenceMetadata {
    ...
    public function new(
        ...
        rolloutItemKinds:Array<String>,
        includeHistory:Bool,
        archived:Bool,
        goalStateRequested:Bool
    ) {
    ...
    public function validate():PersistenceBoundaryOutcome {
        if (threadId == null) return PersistenceBoundaryOutcome.failure("invalid_thread_id", "thread id must be a UUID");
        if (sessionId == null) return PersistenceBoundaryOutcome.failure("invalid_session_id", "session id must be a UUID");
        if (rolloutPath == null) return PersistenceBoundaryOutcome.failure("invalid_rollout_path", "rollout path must be a non-empty scalar");
        ...
        if (historyItemCount < 0) return PersistenceBoundaryOutcome.failure("invalid_history_count", "history item count must be non-negative");
        if (persistedItemCount < 0) return PersistenceBoundaryOutcome.failure("invalid_persisted_count", "persisted item count must be non-negative");
    ...
    public function summary():String {
        return "thread=" + threadId.toString()
            ...
            + ";rollout=" + rolloutPath.toString()
            + ";history=" + Std.string(historyItemCount)
            + ";persisted=" + Std.string(persistedItemCount)
            + ";kinds=" + rolloutItemKinds.join("|")
            + ";includeHistory=" + boolText(includeHistory)
            + ";archived=" + boolText(archived)
            + ";goalState=" + boolText(goalStateRequested);
    }
