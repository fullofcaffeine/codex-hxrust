...
class InMemoryAppServerClient {
    ...
    public function send(command:CodexRuntimeCommand):RuntimeClientOutcome {
        if (command.kind != CodexRuntimeCommandKind.AppRequest) {
            return RuntimeClientOutcome.failure("unsupported_command_kind", "send accepts only app request commands", command.requestId, command.method, pendingTotal);
        }
        if (pending.exists(command.requestId)) {
            return RuntimeClientOutcome.failure("duplicate_request_id", "request id already has a pending response", command.requestId, command.method, pendingTotal);
        }
    ...
    public function complete(command:CodexRuntimeCommand):RuntimeClientOutcome {
        if (command.kind != CodexRuntimeCommandKind.CompleteResponse) {
            return RuntimeClientOutcome.failure("unsupported_command_kind", "complete accepts only response completion commands", command.requestId, command.method, pendingTotal);
        }
        ...
        if (original.method != command.method) {
            return RuntimeClientOutcome.failure("response_method_mismatch", "response method differs from pending request method", command.requestId, command.method, pendingTotal);
        }
    ...
    public function fail(command:CodexRuntimeCommand):RuntimeClientOutcome {
        if (command.kind != CodexRuntimeCommandKind.FailResponse) {
            return RuntimeClientOutcome.failure("unsupported_command_kind", "fail accepts only response failure commands", command.requestId, command.method, pendingTotal);
        }
        ...
        if (original.method != command.method) {
            return RuntimeClientOutcome.failure("response_method_mismatch", "failure method differs from pending request method", command.requestId, command.method, pendingTotal);
        }
    ...
    function wrapRequest(command:CodexRuntimeCommand):String {
        return "{\"id\":" + quote("runtime-request-" + command.requestId) + ",\"kind\":\"request\",\"method\":" + quote(command.method)
            + ",\"message\":{\"jsonrpc\":\"2.0\",\"id\":" + quote(command.requestId) + ",\"method\":" + quote(command.method)
