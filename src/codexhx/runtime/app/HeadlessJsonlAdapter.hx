...
class HeadlessJsonlAdapter {
	...
	function dispatchJsonRpcRequest(request:Value):Array<String> {
		...
		if (!method.ok) {
			return [
				jsonRpcError(requestId.value, -32600, "missing_method", "request method is required")
			];
		}
	...
	function start(commandId:String):Array<String> {
		started = true;
		currentStatus = "idle";
		return ["{\"command\":\"start\",\"id\":"
			+ quote(commandId)
			+ ",\"kind\":\"response\",\"ok\":true,\"sessionId\":\""
			+ sessionId
	...
	function submit(commandId:String, commandValue:Value):Array<String> {
		if (!started) {
			return [
				errorLine(commandId, "submit", "thread_not_started", "start must be called before submit")
			];
		}
		...
		return ["{\"command\":\"submit\",\"id\":"
			+ quote(commandId)
			+ ",\"kind\":\"response\",\"ok\":"
			+ bool(lastOutcome.ok)
			+ ",\"terminalState\":"
			+ quote(lastOutcome.terminalState)
			+ ",\"threadId\":\""
			+ threadId
			+ "\",\"turnId\":"
			+ quote(currentTurnId)
	...
	function status(commandId:String):Array<String> {
		return ["{\"command\":\"status\",\"id\":"
			+ quote(commandId)
			+ ",\"kind\":\"response\",\"ok\":true,\"status\":"
			+ quote(currentStatus)
			+ ",\"threadId\":\""
			+ threadId
			+ "\",\"turnId\":"
	...
	function transcript(commandId:String):Array<String> {
		if (lastOutcome == null) {
			return [
				errorLine(commandId, "transcript", "missing_transcript", "no transcript is available")
			];
		}
		...
		for (event in lastOutcome.events) {
			lines.push("{\"command\":\"transcript\",\"event\":"
				+ event.canonicalJson()
				+ ",\"id\":"
				+ quote(commandId)
				+ ",\"index\":"
				+ Std.string(i)
				+ ",\"kind\":\"transcript_event\",\"ok\":true,\"threadId\":\""
				+ threadId
				+ "\",\"turnId\":"
				+ quote(currentTurnId)
				+ "}");
	...
	function appTurnStart(requestIdJson:String, request:Value):Array<String> {
		if (!started) {
			return [
				jsonRpcError(requestIdJson, -32000, "thread_not_started", "thread/start must be called before turn/start")
			];
		}

		final params = requiredObject(request, "params");
		if (!params.ok)
			return [jsonRpcError(requestIdJson, -32602, params.value, params.errorMessage)];

		final thread = requiredString(params.objectValue, "threadId");
		if (!thread.ok)
			return [
				jsonRpcError(requestIdJson, -32602, "missing_thread_id", "turn/start params.threadId is required")
			];
		if (thread.value != threadId)
			return [jsonRpcError(requestIdJson, -32000, "unknown_thread", "thread id is not loaded")];

		final prompt = textInput(params.objectValue);
		if (!prompt.ok)
			return [jsonRpcError(requestIdJson, -32602, prompt.value, prompt.errorMessage)];
	...
	function appTurnInterrupt(requestIdJson:String, request:Value):Array<String> {
		if (!started) {
			return [
				jsonRpcError(requestIdJson, -32000, "thread_not_started", "thread/start must be called before turn/interrupt")
			];
		}
		...
		if (!thread.ok)
			return [
				jsonRpcError(requestIdJson, -32602, "missing_thread_id", "turn/interrupt params.threadId is required")
			];
		if (thread.value != threadId)
			return [jsonRpcError(requestIdJson, -32000, "unknown_thread", "thread id is not loaded")];
		...
		if (!turn.ok)
			return [
				jsonRpcError(requestIdJson, -32602, "missing_turn_id", "turn/interrupt params.turnId is required")
			];
		if (turn.value != currentTurnId || currentStatus != "active") {
			return [
				jsonRpcError(requestIdJson, -32000, "turn_not_active", "no active turn to interrupt")
			];
		}
	...
	function appThreadRead(requestIdJson:String, request:Value):Array<String> {
		if (!started) {
			return [
				jsonRpcError(requestIdJson, -32000, "thread_not_started", "thread/start must be called before thread/read")
			];
		}
		...
		if (!thread.ok)
			return [
				jsonRpcError(requestIdJson, -32602, "missing_thread_id", "thread/read params.threadId is required")
			];
		if (thread.value != threadId)
			return [jsonRpcError(requestIdJson, -32000, "unknown_thread", "thread id is not loaded")];
		...
		if (!includeTurns.ok)
			return [
				jsonRpcError(requestIdJson, -32602, "expected_include_turns_bool", "thread/read params.includeTurns must be a boolean")
			];

		return [
			jsonRpcResponse(requestIdJson, "{\"thread\":" + threadJson(includeTurns.boolValue) + "}")
		];
	}
	...
	function errorLine(commandId:String, command:String, code:String, message:String):String {
		return "{\"command\":" + quote(command) + ",\"errorCode\":" + quote(code) + ",\"errorMessage\":" + quote(message) + ",\"id\":" + quote(commandId)
			+ ",\"kind\":\"error\",\"ok\":false}";
	}
	...
	function itemStartedNotification(itemJson:String):String {
		return jsonRpcNotification("item/started",
			"{\"item\":"
			+ itemJson
			+ ",\"startedAtMs\":"
			+ Std.string(itemStartedAtMs)
			+ ",\"threadId\":\""
			+ threadId
			+ "\",\"turnId\":"
			+ quote(currentTurnId)
	...
	function itemCompletedNotification(itemJson:String):String {
		return jsonRpcNotification("item/completed",
			"{\"completedAtMs\":"
			+ Std.string(itemCompletedAtMs)
			+ ",\"item\":"
			+ itemJson
			+ ",\"threadId\":\""
			+ threadId
	...
	function agentMessageDeltaNotification(delta:String):String {
		return jsonRpcNotification("item/agentMessage/delta",
			"{\"delta\":"
			+ quote(delta)
			+ ",\"itemId\":\"agent-"
			+ currentTurnId
	...
	function rawResponseItemCompletedNotification(itemJson:String):String {
		return jsonRpcNotification("rawResponseItem/completed",
			"{\"item\":"
			+ itemJson
			+ ",\"threadId\":\""
			+ threadId
	...
	function errorNotification(willRetry:Bool):String {
		return jsonRpcNotification("error",
			"{\"error\":"
			+ turnErrorJson()
			+ ",\"threadId\":\""
			+ threadId
	...
	function failedErrorCode():String {
		if (lastOutcome == null)
			return "other";
		if (lastOutcome.errorCode.length > 0)
			return lastOutcome.errorCode;
		for (event in lastOutcome.events) {
			if (event.kind == "response_failed" && event.errorCode.length > 0)
	...
	function failedErrorMessage():String {
		if (lastOutcome == null)
			return "turn failed";
		if (lastOutcome.errorMessage.length > 0)
			return lastOutcome.errorMessage;
		for (event in lastOutcome.events) {
			if (event.kind == "response_failed" && event.errorMessage.length > 0)
	...
	function jsonRpcError(requestIdJson:String, code:Int, errorCode:String, message:String):String {
		return "{\"error\":{\"code\":" + Std.string(code) + ",\"data\":{\"errorCode\":" + quote(errorCode) + "},\"message\":" + quote(message) + "},\"id\":"
			+ requestIdJson + ",\"jsonrpc\":\"2.0\"}";
	}
	...
	static function textInput(params:Value):StringField {
		return switch params {
			case JObject(keys, values):
				...
				if (i < 0)
					return StringField.error("missing_input", "turn/start params.input is required");
				switch values[i] {
					case JArray(entries):
						...
						while (j < entries.length) {
							switch entries[j] {
								case JObject(inputKeys, inputValues):
									...
									if (typeIndex < 0)
										return StringField.error("missing_input_type", "turn/start text input type is required");
									...
									if (inputType != "text")
										return StringField.error("unsupported_user_input", "only text user input is supported");
									final textIndex = fieldIndex(inputKeys, "text");
									if (textIndex < 0)
										return StringField.error("missing_input_text", "turn/start text input is required");
