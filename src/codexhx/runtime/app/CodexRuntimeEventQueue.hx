...
class CodexRuntimeEventQueue {
    ...
    public function readEvent():RuntimeEventReadOutcome {
        if (events.length == 0) return RuntimeEventReadOutcome.empty();
    ...
    function pushRequired(event:CodexRuntimeEvent):RuntimeQueuePushOutcome {
        ...
        if (lagOutcome != null && !lagOutcome.ok) return lagOutcome;
    ...
    function pushBestEffort(event:CodexRuntimeEvent):RuntimeQueuePushOutcome {
        ...
        if (lagOutcome != null && !lagOutcome.ok) {
            ...
            return RuntimeQueuePushOutcome.dropped("best_effort_dropped", "best-effort event dropped while lag marker was pending", events.length, skippedBestEffort);
        }
        if (events.length >= capacity) {
            skippedBestEffort = skippedBestEffort + 1;
            return RuntimeQueuePushOutcome.dropped("best_effort_dropped", "best-effort event dropped because consumer queue is full", events.length, skippedBestEffort);
        }
        events.push(event);
        return RuntimeQueuePushOutcome.queued(event, events.length, skippedBestEffort);
    }
    ...
    function flushLagMarker(requiredEventFollows:Bool):RuntimeQueuePushOutcome {
        if (skippedBestEffort == 0) return null;
        if (events.length >= capacity) {
