...
class BootstrapClientInfo {
	...
	public function validate():BootstrapValidationOutcome {
		if (trim(name).length == 0)
			return BootstrapValidationOutcome.failure("invalid_client_name", "clientInfo.name must be non-empty");
		if (trim(version).length == 0)
			return BootstrapValidationOutcome.failure("invalid_client_version", "clientInfo.version must be non-empty");
		if (hasTitle && trim(title).length == 0)
			return BootstrapValidationOutcome.failure("invalid_client_title", "clientInfo.title must be non-empty when present");
