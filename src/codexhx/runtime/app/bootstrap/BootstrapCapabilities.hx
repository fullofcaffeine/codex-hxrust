...
class BootstrapCapabilities {
	...
	public function validate():BootstrapValidationOutcome {
		...
		for (method in optOutNotificationMethods) {
			...
			if (method.indexOf("/") < 0) {
				return BootstrapValidationOutcome.failure("invalid_opt_out_notification",
					"opt-out notification method must use an exact app-server method name");
			}
	...
	public function toJson():String {
		return "{\"experimentalApi\":" + boolJson(experimentalApi) + ",\"optOutNotificationMethods\":" + stringArrayJson(optOutNotificationMethods)
