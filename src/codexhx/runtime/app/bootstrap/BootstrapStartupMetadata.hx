...
class BootstrapStartupMetadata {
    ...
    public function validate():BootstrapValidationOutcome {
        if (StringTools.trim(model).length == 0) return BootstrapValidationOutcome.failure("invalid_startup_model", "startup model must be non-empty");
        if (StringTools.trim(modelProvider).length == 0) return BootstrapValidationOutcome.failure("invalid_startup_model_provider", "startup model provider must be non-empty");
    ...
    public function toJson():String {
        ...
        return "{\"account\":{\"authMode\":" + nullable(accountAuthMode)
            + ",\"email\":" + nullable(accountEmail)
            + ",\"planType\":" + nullable(accountPlanType)
            + "},\"configWarnings\":[" + warnings.join(",") + "]"
            + ",\"model\":{\"id\":" + JsonScalar.quote(model)
