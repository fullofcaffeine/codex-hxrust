...
class CodexBootstrapSession {
    public static function bootstrap(
        ...
        response:BootstrapInitializeResponse,
        metadata:BootstrapStartupMetadata
        ...
        final summary = Std.string(mode) + ":"
            + params.clientInfo.name + ":"
            + response.platformFamily + "/"
            + response.platformOs + ":"
            + response.serverVersion() + ":warnings="
            + Std.string(metadata.configWarnings.length);

        return BootstrapValidationOutcome.completed(new BootstrapReport(
            mode,
