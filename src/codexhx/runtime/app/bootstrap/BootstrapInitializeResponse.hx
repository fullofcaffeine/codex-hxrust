...
class BootstrapInitializeResponse {
    ...
    public function validate():BootstrapValidationOutcome {
        if (StringTools.trim(userAgent).length == 0) return BootstrapValidationOutcome.failure("invalid_user_agent", "userAgent must be non-empty");
        if (!isAbsolutePath(codexHome)) return BootstrapValidationOutcome.failure("invalid_codex_home", "codexHome must be absolute");
        ...
        if (StringTools.trim(platformOs).length == 0) return BootstrapValidationOutcome.failure("invalid_platform_os", "platformOs must be non-empty");
        return BootstrapValidationOutcome.success("initialize-response");
    }

    public function toJson():String {
        return "{\"codexHome\":" + quote(codexHome)
            + ",\"platformFamily\":" + quote(platformFamily)
            + ",\"platformOs\":" + quote(platformOs)
            + ",\"userAgent\":" + quote(userAgent) + "}";
    }

    public function serverVersion():String {
        final slash = userAgent.indexOf("/");
        if (slash < 0 || slash + 1 >= userAgent.length) return "";
    ...
    static function isAbsolutePath(path:String):Bool {
        ...
        if (path.length >= 3 && path.charAt(1) == ":" && (path.charAt(2) == "\\" || path.charAt(2) == "/")) return true;
        return false;
    }
