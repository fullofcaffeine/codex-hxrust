...
class ThreadReadIdleGoalProgressAccountingOutcome {
	...
	function new(
		...
		dbCallAttempted:Bool,
		dbOutcomeKind:String,
		progressUpdated:Bool,
		progressReturned:Bool,
		goalId:String,
		status:String,
		wallClockAccountedSeconds:Int,
		tokenDeltaAccounted:Int,
		activeGoalCleared:Bool,
		idleBaselineReset:Bool,
		budgetLimitReportCleared:Bool,
		threadGoalUpdatedEmitted:Bool,
		emittedTurnId:String,
	...
	public static function missingSnapshot():ThreadReadIdleGoalProgressAccountingOutcome {
		return new ThreadReadIdleGoalProgressAccountingOutcome(
			true,
			"idle_progress_snapshot_missing",
			false,
			false,
			"none",
			false,
			...
			"",
			false,
			"account_idle_goal_progress->idle_progress_snapshot/none->return_none",
	...
	public static function stateError(kind:String, code:String):ThreadReadIdleGoalProgressAccountingOutcome {
		return new ThreadReadIdleGoalProgressAccountingOutcome(
			...
			"",
			false,
			"account_idle_goal_progress->idle_progress_snapshot->account_thread_goal_usage/error:" + code,
	...
	public static function unchanged(expectedGoalId:String):ThreadReadIdleGoalProgressAccountingOutcome {
		return new ThreadReadIdleGoalProgressAccountingOutcome(
			true,
			"state_accounting_unchanged_idle_reset",
			true,
			true,
			ThreadReadGoalAccountingDbOutcomeKind.Unchanged,
			false,
			false,
			expectedGoalId,
			"",
			...
			"",
			false,
			"account_idle_goal_progress->idle_progress_snapshot:" + expectedGoalId + "->account_thread_goal_usage/unchanged->reset_idle_progress_baseline_and_clear_active_goal->return_none",
	...
	public static function updated(
		...
		terminalMetricRecorded:Bool,
		disposition:String
	):ThreadReadIdleGoalProgressAccountingOutcome {
		return new ThreadReadIdleGoalProgressAccountingOutcome(
			true,
			code,
			true,
			true,
			ThreadReadGoalAccountingDbOutcomeKind.Updated,
			true,
			true,
			goalId,
			status,
			...
			"account_idle_goal_progress->idle_progress_snapshot:" + goalId
				+ "->account_thread_goal_usage/updated:token_delta=0"
				+ "->mark_idle_progress_accounted_for_status:" + status + "/" + disposition
	...
	public function summary():String {
		return "ok=" + (ok ? "true" : "false")
			+ ";code=" + code
			+ ";snapshot=" + (idleProgressSnapshotAvailable ? "true" : "false")
			+ ";dbCall=" + (dbCallAttempted ? "true" : "false")
			+ ";dbOutcome=" + dbOutcomeKind
			+ ";updated=" + (progressUpdated ? "true" : "false")
			+ ";returned=" + (progressReturned ? "true" : "false")
			+ ";goalId=" + goalId
			+ ";status=" + status
			+ ";seconds=" + Std.string(wallClockAccountedSeconds)
			+ ";tokens=" + Std.string(tokenDeltaAccounted)
			+ ";activeCleared=" + (activeGoalCleared ? "true" : "false")
			+ ";idleBaselineReset=" + (idleBaselineReset ? "true" : "false")
			+ ";budgetReportCleared=" + (budgetLimitReportCleared ? "true" : "false")
			+ ";event=" + (threadGoalUpdatedEmitted ? "true" : "false")
			+ ";emittedTurnId=" + emittedTurnId
