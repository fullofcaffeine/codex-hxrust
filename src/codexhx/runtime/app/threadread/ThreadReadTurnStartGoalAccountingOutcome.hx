...
class ThreadReadTurnStartGoalAccountingOutcome {
	...
	function new(
		...
		accountTokens:Bool,
		clearCurrentTurnGoalCalled:Bool,
		storedGoalLookupAttempted:Bool,
		storedGoalLookupOutcomeKind:String,
		storedGoalMarkedActive:Bool,
		currentTurnGoalId:String,
		wallClockActiveGoalId:String,
		budgetLimitReportCleared:Bool,
		stateLookupError:Bool,
		sequence:String,
		message:String
	) {
		this.ok = ok;
	...
	public static function runtimeMissing():ThreadReadTurnStartGoalAccountingOutcome {
		return new ThreadReadTurnStartGoalAccountingOutcome(
			true,
			...
			false,
			false,
			"on_turn_start->goal_runtime_handle/none->return",
			"turn start skipped because the goal runtime was unavailable"
		);
	}
	...
	public static function runtimeDisabled():ThreadReadTurnStartGoalAccountingOutcome {
		return new ThreadReadTurnStartGoalAccountingOutcome(
			...
			false,
			false,
			"on_turn_start->runtime_disabled->return",
			"turn start skipped because the goal runtime was disabled"
		);
	}
	...
	public static function planMode(turnId:String):ThreadReadTurnStartGoalAccountingOutcome {
		return new ThreadReadTurnStartGoalAccountingOutcome(
			true,
			"plan_mode_current_goal_cleared",
			true,
			true,
			true,
			false,
			...
			true,
			false,
			"on_turn_start:" + turnId + "->start_turn:account_tokens=false->clear_current_turn_goal->return",
	...
	public static function missingGoal():ThreadReadTurnStartGoalAccountingOutcome {
		return new ThreadReadTurnStartGoalAccountingOutcome(
			...
			false,
			false,
			"on_turn_start->start_turn:account_tokens=true->get_thread_goal/none->return",
			"no stored goal was available to mark active for this turn"
		);
	}
	...
	public static function nonActiveGoal(status:String):ThreadReadTurnStartGoalAccountingOutcome {
		return new ThreadReadTurnStartGoalAccountingOutcome(
			...
			false,
			false,
			"on_turn_start->start_turn:account_tokens=true->get_thread_goal:" + status + "->return",
			"stored goal status is not active or budget-limited"
		);
	}
	...
	public static function markedActive(goalId:String, status:String, budgetLimitReportCleared:Bool):ThreadReadTurnStartGoalAccountingOutcome {
		return new ThreadReadTurnStartGoalAccountingOutcome(
			true,
			status == "budgetLimited" ? "budget_limited_goal_marked_active_for_turn" : "active_goal_marked_active_for_turn",
			true,
			true,
			true,
			true,
			...
			false,
			"on_turn_start->start_turn:account_tokens=true->get_thread_goal:" + status + "->mark_turn_goal_active:" + goalId,
	...
	public function summary():String {
		return "ok=" + (ok ? "true" : "false")
			...
			+ ";startTurn=" + (startTurnCalled ? "true" : "false")
			+ ";accountTokens=" + (accountTokens ? "true" : "false")
			+ ";clearCurrentTurnGoal=" + (clearCurrentTurnGoalCalled ? "true" : "false")
			+ ";lookupAttempted=" + (storedGoalLookupAttempted ? "true" : "false")
			+ ";lookupOutcome=" + storedGoalLookupOutcomeKind
			+ ";markedActive=" + (storedGoalMarkedActive ? "true" : "false")
			+ ";currentTurnGoalId=" + currentTurnGoalId
			+ ";wallClockActiveGoalId=" + wallClockActiveGoalId
			+ ";budgetReportCleared=" + (budgetLimitReportCleared ? "true" : "false")
			+ ";stateLookupError=" + (stateLookupError ? "true" : "false")
			+ ";sequence=" + sequence
			+ ";message=" + message;
	}
}
