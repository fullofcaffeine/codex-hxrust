...
class ThreadReadTryStartTurnIfIdleOutcome {
	...
	function new(
		...
		reservationCleared:Bool,
		pendingInputInjected:Bool,
		regularTaskStarted:Bool,
	...
	public static function makeAccepted(returnedItemSummary:String):ThreadReadTryStartTurnIfIdleOutcome {
		return new ThreadReadTryStartTurnIfIdleOutcome(
			true,
			"accepted_regular_turn_started",
			true,
			false,
			ThreadReadTryStartTurnIfIdleRejectionReason.None,
			true,
			false,
			true,
			true,
			false,
			returnedItemSummary,
			"steering/item->idle/reserve->pending/input/extend->regular/task/start",
	...
	public static function makeRejected(
		...
		final sequence = reservationCreated
			? "steering/item->idle/reserve->idle/reservation/clear->reject:" + reason
			: "steering/item->reject:" + reason;
		return new ThreadReadTryStartTurnIfIdleOutcome(
			true,
			code,
			false,
	...
	public function summary():String {
		return "ok=" + (ok ? "true" : "false")
			...
			+ ";reason=" + reason
			+ ";reservationCreated=" + (reservationCreated ? "true" : "false")
			+ ";reservationCleared=" + (reservationCleared ? "true" : "false")
			+ ";pendingInputInjected=" + (pendingInputInjected ? "true" : "false")
			+ ";regularTaskStarted=" + (regularTaskStarted ? "true" : "false")
			+ ";returnedItemUnchanged=" + (returnedItemUnchanged ? "true" : "false")
			+ ";returnedItemSummary=" + returnedItemSummary
			+ ";sequence=" + sequence
			+ ";message=" + message;
	}
}
