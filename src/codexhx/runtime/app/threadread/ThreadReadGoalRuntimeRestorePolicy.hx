...
class ThreadReadGoalRuntimeRestorePolicy {
	...
	public static function build(request:ThreadReadGoalRuntimeRestoreRequest):ThreadReadGoalRuntimeRestoreOutcome {
		if (!request.runtimePresent) return ThreadReadGoalRuntimeRestoreOutcome.runtimeMissing();
		if (!request.runtimeEnabled) return ThreadReadGoalRuntimeRestoreOutcome.disabled(request.previousActiveGoalId);
		if (!request.stateReadOk) return ThreadReadGoalRuntimeRestoreOutcome.failure(request.previousActiveGoalId, request.stateReadErrorCode);
		...
		if (request.storedGoal == null) {
			return ThreadReadGoalRuntimeRestoreOutcome.cleared(
				"stored_goal_missing_cleared",
				request.previousActiveGoalId,
				"missing stored goal clears active goal accounting"
			);
		}

		return ThreadReadGoalRuntimeRestoreOutcome.cleared(
			"stored_goal_not_active_cleared",
			request.previousActiveGoalId,
