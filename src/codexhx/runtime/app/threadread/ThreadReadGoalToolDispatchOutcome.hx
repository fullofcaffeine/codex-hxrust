...
class ThreadReadGoalToolDispatchOutcome {
	...
	function new(kind:ThreadReadGoalToolKind, spec:ThreadReadGoalToolSpec, ok:Bool, code:String, threadId:String, turnId:String, callId:String,
			functionCallErrorKind:String, errorMessage:String, goalPresent:Bool, hasRemainingTokens:Bool, remainingTokens:Int, hasCompletionBudgetReport:Bool,
			eventEmitted:Bool, getOutcome:ThreadReadGetGoalToolOutcome, createOutcome:ThreadReadCreateGoalToolOutcome,
			updateOutcome:ThreadReadUpdateGoalToolOutcome, sequence:String) {
		this.kind = kind;
		this.toolName = kind;
		this.spec = spec;
	...
	public static function fromGet(outcome:ThreadReadGetGoalToolOutcome):ThreadReadGoalToolDispatchOutcome {
		return new ThreadReadGoalToolDispatchOutcome(ThreadReadGoalToolKind.Get, ThreadReadGoalToolSpec.fromKind(ThreadReadGoalToolKind.Get), outcome.ok,
			outcome.code, outcome.threadId, "", "", outcome.functionCallErrorKind, outcome.errorMessage, outcome.goalPresent, outcome.hasRemainingTokens,
			outcome.remainingTokens, outcome.hasCompletionBudgetReport, outcome.eventsEmitted, outcome, null, null,
	...
	public static function fromCreate(outcome:ThreadReadCreateGoalToolOutcome):ThreadReadGoalToolDispatchOutcome {
		return new ThreadReadGoalToolDispatchOutcome(ThreadReadGoalToolKind.Create, ThreadReadGoalToolSpec.fromKind(ThreadReadGoalToolKind.Create),
			outcome.ok, outcome.code, outcome.threadId, outcome.turnId, "", outcome.functionCallErrorKind, outcome.errorMessage, outcome.goalPresent,
			outcome.hasRemainingTokens, outcome.remainingTokens, outcome.hasCompletionBudgetReport, outcome.eventEmitted, null, outcome, null,
	...
	public static function fromUpdate(outcome:ThreadReadUpdateGoalToolOutcome):ThreadReadGoalToolDispatchOutcome {
		return new ThreadReadGoalToolDispatchOutcome(ThreadReadGoalToolKind.Update, ThreadReadGoalToolSpec.fromKind(ThreadReadGoalToolKind.Update),
			outcome.ok, outcome.code, outcome.threadId, outcome.turnId, outcome.callId, outcome.functionCallErrorKind, outcome.errorMessage,
			outcome.goalPresent, outcome.hasRemainingTokens, outcome.remainingTokens, outcome.hasCompletionBudgetReport, outcome.eventEmitted, null, null,
			outcome, "GoalToolExecutor::handle->GoalToolKind::Update->" + outcome.sequence);
	}
	...
	public static function malformedRequest(kind:ThreadReadGoalToolKind):ThreadReadGoalToolDispatchOutcome {
		...
		return new ThreadReadGoalToolDispatchOutcome(kind, spec, false, "goal_tool_dispatch_request_error", "", "", "", "respond_to_model",
			"dispatch request did not carry the payload for "
			+ kind, false, false, 0, false, false, null, null, null,
			"GoalToolExecutor::handle->"
			+ kind
			+ "->missing_typed_request");
	}

	public function delegateSummary():String {
		if (getOutcome != null)
			return getOutcome.summary();
		if (createOutcome != null)
			return createOutcome.summary();
		if (updateOutcome != null)
			return updateOutcome.summary();
		return "";
	}
