...
class ThreadReadToolFinishGoalProgressAdmissionOutcome {
	...
	function new(
		...
		toolAttemptCounts:Bool,
		updateGoalSelfTool:Bool,
		admitted:Bool,
	...
	public static function runtimeMissing(request:ThreadReadToolFinishGoalProgressAdmissionRequest):ThreadReadToolFinishGoalProgressAdmissionOutcome {
		return skipped(
			...
			false,
			false,
			"on_tool_finish->goal_runtime_handle/none->return",
			"tool-finish goal progress skipped because the goal runtime was unavailable"
		);
	}

	public static function runtimeDisabled(request:ThreadReadToolFinishGoalProgressAdmissionRequest, toolAttemptCounts:Bool, updateGoalSelfTool:Bool):ThreadReadToolFinishGoalProgressAdmissionOutcome {
		return skipped(
			...
			updateGoalSelfTool,
			false,
			"on_tool_finish->runtime_disabled->return",
			"tool-finish goal progress skipped because the goal runtime was disabled"
		);
	}

	public static function outcomeNotCounted(request:ThreadReadToolFinishGoalProgressAdmissionRequest, toolAttemptCounts:Bool, updateGoalSelfTool:Bool):ThreadReadToolFinishGoalProgressAdmissionOutcome {
		return skipped(
			"tool_outcome_not_counted_skip",
			request,
			toolAttemptCounts,
			updateGoalSelfTool,
			true,
	...
	public static function updateGoalSelfToolSkip(request:ThreadReadToolFinishGoalProgressAdmissionRequest, toolAttemptCounts:Bool):ThreadReadToolFinishGoalProgressAdmissionOutcome {
		return skipped(
			"update_goal_self_tool_skip",
			request,
			toolAttemptCounts,
			true,
			true,
	...
	public static function accountingMissing(request:ThreadReadToolFinishGoalProgressAdmissionRequest):ThreadReadToolFinishGoalProgressAdmissionOutcome {
		return makeAdmitted(
			"accounting_outcome_missing_warn",
			request,
			false,
			"accounting_outcome_missing",
			false,
			false,
			true,
	...
	public static function accountingFailed(request:ThreadReadToolFinishGoalProgressAdmissionRequest, accountingCode:String):ThreadReadToolFinishGoalProgressAdmissionOutcome {
		return makeAdmitted(
			...
			false,
			true,
			"on_tool_finish->account_active_goal_progress:" + request.callId + "/error:" + accountingCode + "->warn->return",
			"active goal progress accounting failed after tool finish"
		);
	}

	public static function noProgress(request:ThreadReadToolFinishGoalProgressAdmissionRequest, accountingCode:String):ThreadReadToolFinishGoalProgressAdmissionOutcome {
		return makeAdmitted(
			"admitted_no_active_progress",
			request,
			true,
			accountingCode,
			false,
	...
	public static function progressReturnedOutcome(
		request:ThreadReadToolFinishGoalProgressAdmissionRequest,
		accountingCode:String,
		budgetLimitedProgress:Bool
	):ThreadReadToolFinishGoalProgressAdmissionOutcome {
		return makeAdmitted(
			budgetLimitedProgress ? "admitted_budget_limited_progress" : "admitted_progress",
			request,
			true,
			accountingCode,
			true,
			budgetLimitedProgress,
			false,
			"on_tool_finish->account_active_goal_progress:" + request.callId + "/ok:progress->"
	...
	static function skipped(
		...
		return new ThreadReadToolFinishGoalProgressAdmissionOutcome(
			true,
			code,
	...
	static function makeAdmitted(
		...
		return new ThreadReadToolFinishGoalProgressAdmissionOutcome(
			...
			true,
			"active_only",
			ThreadReadGoalAccountingDisposition.KeepActive,
			request.callId,
			accountingOk,
			accountingCode,
			progressReturned,
	...
	public function summary():String {
		return "ok=" + (ok ? "true" : "false")
			...
			+ ";toolName=" + toolName
			+ ";outcome=" + outcomeKind
			+ ";completedSuccess=" + (completedSuccess ? "true" : "false")
			+ ";failedHandlerExecuted=" + (failedHandlerExecuted ? "true" : "false")
			+ ";toolAttemptCounts=" + (toolAttemptCounts ? "true" : "false")
			+ ";updateGoalSelfTool=" + (updateGoalSelfTool ? "true" : "false")
			...
			+ ";budgetLimitedProgress=" + (budgetLimitedProgress ? "true" : "false")
