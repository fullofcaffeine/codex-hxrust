...
class ThreadReadGoalTokenUsageRecordOutcome {
	...
	function new(
		...
		turnKnown:Bool,
		accountTokens:Bool,
		currentUsageUpdated:Bool,
		recorded:Bool,
		turnDelta:Int,
		threadUnflushedDelta:Int,
		goalChargeDelta:Int,
		ignoredReasoningOutputTokens:Bool,
		ignoredTotalTokens:Bool,
		sequence:String,
		message:String
	) {
		this.ok = ok;
	...
	public static function runtimeMissing(turnStoreLevelId:String, currentTurnId:String):ThreadReadGoalTokenUsageRecordOutcome {
		return new ThreadReadGoalTokenUsageRecordOutcome(
			true,
			...
			false,
			false,
			"on_token_usage->goal_runtime_handle/none->return",
	...
	public static function runtimeDisabled(turnStoreLevelId:String, currentTurnId:String):ThreadReadGoalTokenUsageRecordOutcome {
		return new ThreadReadGoalTokenUsageRecordOutcome(
			...
			false,
			false,
			"on_token_usage->runtime_disabled->return",
	...
	public static function unknownTurn(turnStoreLevelId:String, currentTurnId:String):ThreadReadGoalTokenUsageRecordOutcome {
		return new ThreadReadGoalTokenUsageRecordOutcome(
			...
			false,
			false,
			"on_token_usage->record_token_usage:" + turnStoreLevelId + "->turns.get_mut/none->return_none",
			"token usage contribution skipped because no accounting record exists for the turn"
		);
	}

	public static function notAccounted(
		code:String,
		turnStoreLevelId:String,
		currentTurnId:String,
		accountTokens:Bool,
		...
		ignoredReasoningOutputTokens:Bool,
		ignoredTotalTokens:Bool
	):ThreadReadGoalTokenUsageRecordOutcome {
		...
		return new ThreadReadGoalTokenUsageRecordOutcome(
			...
			0,
			goalChargeDelta,
			ignoredReasoningOutputTokens,
			ignoredTotalTokens,
			"on_token_usage->record_token_usage:" + turnStoreLevelId + "->current_token_usage=total->" + reason + "->return_none",
	...
	public static function recordedDelta(
		turnStoreLevelId:String,
		currentTurnId:String,
		turnDelta:Int,
		threadUnflushedDelta:Int,
		...
		return new ThreadReadGoalTokenUsageRecordOutcome(
			...
			true,
			turnDelta,
			threadUnflushedDelta,
			turnDelta,
			ignoredReasoningOutputTokens,
			ignoredTotalTokens,
			"on_token_usage->record_token_usage:" + turnStoreLevelId + "->current_token_usage=total->RecordedTokenDelta(turn_delta=" + Std.string(turnDelta) + ",thread_unflushed_delta=" + Std.string(threadUnflushedDelta) + ")",
	...
	public function summary():String {
		return "ok=" + (ok ? "true" : "false")
			...
			+ ";turnStoreLevelId=" + turnStoreLevelId
			+ ";currentTurnId=" + currentTurnId
			+ ";recordAttempted=" + (recordAttempted ? "true" : "false")
			+ ";turnKnown=" + (turnKnown ? "true" : "false")
			+ ";accountTokens=" + (accountTokens ? "true" : "false")
			+ ";currentUsageUpdated=" + (currentUsageUpdated ? "true" : "false")
			+ ";recorded=" + (recorded ? "true" : "false")
			+ ";turnDelta=" + Std.string(turnDelta)
			+ ";threadUnflushedDelta=" + Std.string(threadUnflushedDelta)
			+ ";goalChargeDelta=" + Std.string(goalChargeDelta)
			+ ";ignoredReasoning=" + (ignoredReasoningOutputTokens ? "true" : "false")
			+ ";ignoredTotal=" + (ignoredTotalTokens ? "true" : "false")
			+ ";sequence=" + sequence
			+ ";message=" + message;
	}
}
