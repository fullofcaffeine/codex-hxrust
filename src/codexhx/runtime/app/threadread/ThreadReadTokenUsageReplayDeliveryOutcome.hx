...
class ThreadReadTokenUsageReplayDeliveryOutcome {
	...
	public static function makeDelivered(operation:ThreadReadTokenUsageReplayDeliveryOperation, connectionId:String):ThreadReadTokenUsageReplayDeliveryOutcome {
		return new ThreadReadTokenUsageReplayDeliveryOutcome(
			true,
			"delivered",
			operation,
			true,
			...
			"restored token usage notification delivered to requesting connection"
		);
	}

	public static function makeSkipped(operation:ThreadReadTokenUsageReplayDeliveryOperation, code:String, message:String):ThreadReadTokenUsageReplayDeliveryOutcome {
		return new ThreadReadTokenUsageReplayDeliveryOutcome(
			true,
			code,
			operation,
	...
	public static function failure(operation:ThreadReadTokenUsageReplayDeliveryOperation, code:String, message:String):ThreadReadTokenUsageReplayDeliveryOutcome {
		return new ThreadReadTokenUsageReplayDeliveryOutcome(
			false,
			code,
			operation,
	...
	public function recipientSummary():String {
		return delivered
			? "targeted=" + Std.string(targetedConnectionCount) + ";connection=" + connectionId + ";broadcast=" + (broadcast ? "true" : "false")
			: "targeted=0;connection=none;broadcast=false";
	}
