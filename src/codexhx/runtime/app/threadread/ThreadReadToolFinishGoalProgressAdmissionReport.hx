...
class ThreadReadToolFinishGoalProgressAdmissionReport {
	...
	public function admittedCount():Int {
		var count = 0;
		for (outcome in outcomes) if (outcome.admitted) count = count + 1;
		return count;
	}
	...
	public function accountingAttemptedCount():Int {
		var count = 0;
		for (outcome in outcomes) if (outcome.accountingAttempted) count = count + 1;
		return count;
	}

	public function warningCount():Int {
		var count = 0;
		for (outcome in outcomes) if (outcome.warningLogged) count = count + 1;
		return count;
	}

	public function budgetLimitedProgressCount():Int {
		var count = 0;
		for (outcome in outcomes) if (outcome.budgetLimitedProgress) count = count + 1;
		return count;
	}
	...
	public function summary():String {
		...
		return "schema=" + schema
			...
			+ ";admitted=" + Std.string(admittedCount())
			+ ";skipped=" + Std.string(skippedCount())
			+ ";accountingAttempted=" + Std.string(accountingAttemptedCount())
			+ ";warnings=" + Std.string(warningCount())
			+ ";budgetLimitedProgress=" + Std.string(budgetLimitedProgressCount())
			+ ";outcomes=[" + parts.join("##") + "]";
	}
}
