...
class ThreadReadBudgetLimitGoalSteeringPolicy {
	...
	public static function build(request:ThreadReadBudgetLimitGoalSteeringRequest):ThreadReadBudgetLimitGoalSteeringOutcome {
		if (!request.progressOk) {
			return ThreadReadBudgetLimitGoalSteeringOutcome.failure("progress_accounting_failed", false, "item=none",
		...
		if (!request.progressAvailable || request.goal == null) {
			return ThreadReadBudgetLimitGoalSteeringOutcome.makeSkipped("active_progress_missing_skip", false, false, false, "item=none",
				"tool/finish->account_active_goal_progress->none->skip", "no active goal progress was available for this tool finish");
		}

		if (request.goal.status != ThreadGoalStatus.BudgetLimited) {
			return ThreadReadBudgetLimitGoalSteeringOutcome.makeSkipped("goal_not_budget_limited_skip", true, false, false, "item=none",
		...
		if (request.budgetLimitAlreadyReported) {
			return ThreadReadBudgetLimitGoalSteeringOutcome.makeSkipped("budget_limit_already_reported_skip", true, true, true, "item=none",
		...
		final steering = ThreadReadGoalSteeringBuilder.build(new ThreadReadGoalSteeringRequest(ThreadReadGoalSteeringItemKind.BudgetLimit, request.goal, null,
			false));
		final injection = ThreadReadActiveTurnGoalSteeringInjectionPolicy.build(new ThreadReadActiveTurnGoalSteeringInjectionRequest(steering,
			request.threadManagerAvailable, request.liveThreadAvailable, request.activeTurnRunning));
