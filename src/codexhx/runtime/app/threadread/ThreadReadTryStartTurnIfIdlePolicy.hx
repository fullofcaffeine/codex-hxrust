...
class ThreadReadTryStartTurnIfIdlePolicy {
	...
	public static function build(request:ThreadReadTryStartTurnIfIdleRequest):ThreadReadTryStartTurnIfIdleOutcome {
		if (request.inputEmpty)
			return ThreadReadTryStartTurnIfIdleOutcome.makeEmptyInputNoop();
		...
		if (request.steeringOutcome == null || !request.steeringOutcome.ok || !request.steeringOutcome.emitted || request.steeringOutcome.item == null) {
			return ThreadReadTryStartTurnIfIdleOutcome.failure("steering_item_not_available", itemSummary,
				"try_start_turn_if_idle requires an emitted steering response item");
		}

		if (request.pendingTriggerTurnBeforeReservation) {
			return reject(ThreadReadTryStartTurnIfIdleRejectionReason.PendingTriggerTurn, false, itemSummary,
				"pending trigger-turn mailbox is prioritized before automatic idle work");
		}

		if (request.collaborationPlanModeBeforeReservation) {
			return reject(ThreadReadTryStartTurnIfIdleRejectionReason.PlanMode, false, itemSummary,
		...
		if (request.activeTaskKind == ThreadReadTryStartTurnIfIdleActiveTaskKind.Regular) {
			return reject(ThreadReadTryStartTurnIfIdleRejectionReason.Busy, false, itemSummary,
		...
		if (request.pendingTriggerTurnAfterReservation) {
			return reject(ThreadReadTryStartTurnIfIdleRejectionReason.PendingTriggerTurn, true, itemSummary,
		...
		if (request.collaborationPlanModeAfterTurnContext) {
			return reject(ThreadReadTryStartTurnIfIdleRejectionReason.PlanMode, true, itemSummary,
		...
		if (request.reservationLostBeforeStart) {
			return reject(ThreadReadTryStartTurnIfIdleRejectionReason.Busy, true, itemSummary, "idle reservation was lost before task start");
		}

		return ThreadReadTryStartTurnIfIdleOutcome.makeAccepted(itemSummary);
	}

	static function reject(reason:ThreadReadTryStartTurnIfIdleRejectionReason, reservationCreated:Bool, itemSummary:String,
