...
class ThreadReadCreateGoalToolReport {
	...
	public function previewWarningCount():Int {
		var count = 0;
		for (outcome in outcomes)
			if (outcome.previewWarningLogged)
				count = count + 1;
		return count;
	}
	...
	public function summary():String {
		...
		return "schema=" + schema + ";cases=" + Std.string(outcomes.length) + ";success=" + Std.string(successCount()) + ";errors="
			+ Std.string(errorCount()) + ";previewWarnings=" + Std.string(previewWarningCount()) + ";eventsEmitted=" + Std.string(eventEmittedCount())
			+ ";insertAttempted=" + Std.string(insertAttemptedCount()) + ";outcomes=[" + parts.join("##") + "]";
	}
}
