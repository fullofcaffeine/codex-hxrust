...
class ThreadReadResumeGoalSnapshotPolicy {
	...
	public static function plan(request:ThreadReadResumeGoalSnapshotRequest):ThreadReadResumeGoalSnapshotOutcome {
		if (!request.responseReady) {
			return ThreadReadResumeGoalSnapshotOutcome.failure(request.operation, "response_not_ready",
		...
		if (request.tokenUsageDelivery == null || !request.tokenUsageDelivery.ok) {
			return ThreadReadResumeGoalSnapshotOutcome.failure(request.operation, "token_usage_delivery_not_settled",
		...
		if (request.operation == ThreadReadTokenUsageReplayDeliveryOperation.Fork) {
			return ThreadReadResumeGoalSnapshotOutcome.makeSkipped(request.operation, "skipped_fork_has_no_resume_goal_snapshot", baseSequence(request),
				"upstream fork delivery replays token usage but does not emit a resume goal snapshot");
		}
		if (!request.goalsFeatureEnabled) {
			return ThreadReadResumeGoalSnapshotOutcome.makeSkipped(request.operation, "skipped_goals_feature_disabled", baseSequence(request),
				"goals feature disabled; upstream returns before snapshot and idle continuation");
		}
		if (!request.stateDbAvailable) {
			return ThreadReadResumeGoalSnapshotOutcome.makeSkipped(request.operation, "skipped_state_db_unavailable", baseSequence(request),
		...
		if (request.goal == null) {
			return ThreadReadResumeGoalSnapshotOutcome.cleared(request.operation, appendStep(baseSequence(request), "thread/goal/cleared"), pendingPoint);
		}
		final intent = continuationIntentForStatus(request.goal.status);
		return ThreadReadResumeGoalSnapshotOutcome.updated(request.operation, appendStep(baseSequence(request), "thread/goal/updated"), intent, pendingPoint,
	...
	static function continuationIntentForStatus(status:String):ThreadReadResumeGoalContinuationIntent {
		if (status == ThreadGoalStatus.Active)
			return ThreadReadResumeGoalContinuationIntent.EmitIdleLifecycle;
		if (ThreadGoalStatus.isTerminal(status))
