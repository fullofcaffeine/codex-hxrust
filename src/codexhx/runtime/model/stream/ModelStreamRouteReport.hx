...
class ModelStreamRouteReport {
	...
	public function cancelledCount():Int {
		var count = 0;
		for (outcome in outcomes) if (outcome.cancelled) count = count + 1;
		return count;
	}

	public function envelopeDeniedCount():Int {
		var count = 0;
		for (outcome in outcomes) if (outcome.code == "model_request_envelope_denied") count = count + 1;
		return count;
	}
	...
	public function summary():String {
		...
		return "schema=" + schema
			...
			+ ";cancelled=" + Std.string(cancelledCount())
			+ ";envelopeDenied=" + Std.string(envelopeDeniedCount())
			+ ";outcomes=[" + parts.join("##") + "]";
	}
}
