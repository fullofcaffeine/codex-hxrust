...
class MockModelStreamParser {
		...
		return switch eventType {
			case "response.created":
				...
				if (!response.ok) response.toOutcome() else {
					...
					if (!id.ok)
						fail(id.errorCode, id.errorPath, id.errorMessage)
					else
						one(new ModelStreamEvent("response_created", id.value, "", "", "", "", 0));
				}
			case "response.output_text.delta":
				final delta = CodexJson.stringField(value, "delta", path + ".data");
				if (!delta.ok) fail(delta.errorCode, delta.errorPath,
					delta.errorMessage) else one(new ModelStreamEvent("assistant_delta", "", "", delta.value, "", "", 0));
	...
	static function mapOutputItemDone(value:Value, path:String):ModelStreamParseOutcome {
		...
		if (!itemType.ok)
			return fail(itemType.errorCode, itemType.errorPath, itemType.errorMessage);
		if (itemType.value != "message")
			return fail("unsupported_output_item", path + ".data.item.type", "only assistant message output items are supported");

		final role = CodexJson.stringField(item.value, "role", path + ".data.item");
		if (!role.ok)
			return fail(role.errorCode, role.errorPath, role.errorMessage);
		if (role.value != "assistant")
			return fail("unsupported_output_role", path + ".data.item.role", "only assistant output items are supported");

		final id = CodexJson.stringField(item.value, "id", path + ".data.item");
		if (!id.ok)
			return fail(id.errorCode, id.errorPath, id.errorMessage);
		...
		for (entry in content.values) {
			...
			if (!entryObject.ok)
				return entryObject.toOutcome();
			final contentType = CodexJson.stringField(entry, "type", entryPath);
			if (!contentType.ok)
				return fail(contentType.errorCode, contentType.errorPath, contentType.errorMessage);
			if (contentType.value == "output_text") {
				final textField = CodexJson.stringField(entry, "text", entryPath);
				if (!textField.ok)
					return fail(textField.errorCode, textField.errorPath, textField.errorMessage);
	...
	static function mapCompleted(value:Value, path:String):ModelStreamParseOutcome {
		...
		if (usage.ok) {
			...
			if (!total.ok)
				return fail(total.errorCode, total.errorPath, total.errorMessage);
	...
	static function mapFailed(value:Value, path:String):ModelStreamParseOutcome {
		...
		if (!code.ok)
			return fail(code.errorCode, code.errorPath, code.errorMessage);
		final message = CodexJson.stringField(error.value, "message", path + ".data.response.error");
		if (!message.ok)
			return fail(message.errorCode, message.errorPath, message.errorMessage);
