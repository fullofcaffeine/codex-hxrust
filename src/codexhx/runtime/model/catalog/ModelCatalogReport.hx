...
class ModelCatalogReport {
	...
	public function hiddenDeniedCount():Int {
		var count = 0;
		for (outcome in outcomes) if (outcome.code == "hidden_model") count = count + 1;
		return count;
	}

	public function liveFetchDeniedCount():Int {
		var count = 0;
		for (outcome in outcomes) if (outcome.code == "live_model_fetch_disabled") count = count + 1;
		return count;
	}

	public function apiFilteredCount():Int {
		var count = 0;
		for (outcome in outcomes) count = count + outcome.apiFilteredCount;
		return count;
	}
	...
	public function summary():String {
		...
		return "schema=" + schema
			...
			+ ";hiddenDenied=" + Std.string(hiddenDeniedCount())
			+ ";liveFetchDenied=" + Std.string(liveFetchDeniedCount())
