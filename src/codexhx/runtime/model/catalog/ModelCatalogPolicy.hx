...
class ModelCatalogPolicy {
	...
	public static function build(request:ModelCatalogRequest):ModelCatalogOutcome {
		...
		if (admission == null || !admission.ok) {
			return ModelCatalogOutcome.denied(
				request,
				admission,
				...
				capabilitySummary,
				emptySummaries,
				admission == null ? "missing provider admission request" : admission.errorMessage,
		...
		if (liveFetchAttempted && !request.allowLiveFetch) {
			return ModelCatalogOutcome.denied(
				...
				capabilitySummary,
				emptySummaries,
				"live model catalog refresh is disabled for this credential-free fixture gate",
		...
		if (StringTools.trim(request.requestedModelId).length > 0) {
			...
			if (requested == null) {
				return ModelCatalogOutcome.denied(
					...
					capabilitySummary,
					summaries,
					"requested model is not available for the active provider/auth mode",
					"model_catalog->find_requested_model:not_found"
				);
			}
			if (!request.includeHidden && requested.hidden()) {
				return ModelCatalogOutcome.denied(
					...
					capabilitySummary,
					summaries,
					"requested model is hidden from the picker without include_hidden",
					"model_catalog->app_server_filter:hidden"
				);
			}
			return ModelCatalogOutcome.accepted(
				request,
				admission,
				liveFetchAttempted,
				requested,
				defaultModel == null ? "" : defaultModel.modelId,
				request.catalog.length,
				providerModels.length,
				visibleModels.length,
				hiddenCount,
		...
		if (defaultModel == null) {
			return ModelCatalogOutcome.denied(
				...
				capabilitySummary,
				summaries,
				"no models are available for the active provider/auth mode",
				"model_catalog->default_model:none"
			);
		}
