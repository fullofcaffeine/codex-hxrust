...
class ProviderAdmissionPolicy {
	...
	public static function build(request:ProviderAdmissionRequest):ProviderAdmissionOutcome {
		if (request.provider == null || !request.provider.valid()) {
			return ProviderAdmissionOutcome.denied(
				request,
		...
		if (request.model.providerId != request.provider.providerId) {
			return ProviderAdmissionOutcome.denied(
				...
				"requested model belongs to " + request.model.providerId + " but active provider is " + request.provider.providerId,
		...
		if (!providerShape.ok) {
			return ProviderAdmissionOutcome.denied(
				request,
				"provider_auth_shape_conflict",
				providerShape.errorMessage,
		...
		if (request.networkKind == ProviderAdmissionNetworkKind.LiveNetwork && !request.liveNetworkAllowed) {
			return ProviderAdmissionOutcome.denied(
				...
				"live provider traffic is disabled for this credential-free fixture gate",
				"provider_admission->network_policy:live_network_disabled"
			);
		}
		if (request.provider.hasAwsAuth) return admitAws(request);
		if (request.provider.requiresOpenAiAuth) return admitOpenAiAuth(request);
		return admitProviderScoped(request);
	}

	static function admitAws(request:ProviderAdmissionRequest):ProviderAdmissionOutcome {
		if (request.accountKind != ProviderAdmissionAccountKind.AmazonBedrock || request.credentialKind != ProviderAdmissionCredentialKind.Aws) {
			return ProviderAdmissionOutcome.denied(
				request,
				"aws_auth_required",
				"Amazon Bedrock providers require AWS auth account state",
				"provider_admission->aws_account_state:error"
			);
		}
		return ProviderAdmissionOutcome.admitted(
			request,
			false,
			"aws_configured",
			admittedNetworkKind(request),
			"amazon_bedrock",
	...
	static function admitOpenAiAuth(request:ProviderAdmissionRequest):ProviderAdmissionOutcome {
		if (!openAiAccountKind(request.accountKind) || request.credentialKind != ProviderAdmissionCredentialKind.OpenAiAuth || !request.hasCredentialMaterial) {
			return ProviderAdmissionOutcome.denied(
				request,
				"missing_openai_auth",
		...
		return ProviderAdmissionOutcome.admitted(
			...
			"present",
			admittedNetworkKind(request),
			accountVisible(request.accountKind),
