...
class ProviderAdmissionReport {
	...
	public function liveNetworkDeniedCount():Int {
		var count = 0;
		for (outcome in outcomes)
			if (outcome.code == "live_network_disabled")
				count = count + 1;
		return count;
	}
	...
	public function redactedCredentialCount():Int {
		var count = 0;
		for (outcome in outcomes)
			if (outcome.credentialBucket == "present" || outcome.credentialBucket == "configured")
				count = count + 1;
		return count;
	}
	...
	public function summary():String {
		...
		return "schema=" + schema + ";cases=" + Std.string(outcomes.length) + ";success=" + Std.string(successCount()) + ";errors="
			+ Std.string(errorCount()) + ";credentialsAccepted=" + Std.string(credentialAcceptedCount()) + ";redactedCredentials="
			+ Std.string(redactedCredentialCount()) + ";liveNetworkDenied=" + Std.string(liveNetworkDeniedCount()) + ";outcomes=[" + parts.join("##") + "]";
	}
}
