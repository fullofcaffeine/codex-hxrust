...
class ModelRequestEnvelopeReport {
	...
	public function liveRouteDeniedCount():Int {
		var count = 0;
		for (outcome in outcomes)
			if (outcome.code == "live_network_disabled" || outcome.code == "websocket_unsupported")
				count = count + 1;
		return count;
	}
	...
	public function planDeniedCount():Int {
		var count = 0;
		for (outcome in outcomes)
			if (outcome.code == "turn_model_plan_denied")
				count = count + 1;
		return count;
	}
	...
	public function summary():String {
		...
		return "schema=" + schema + ";cases=" + Std.string(outcomes.length) + ";success=" + Std.string(successCount()) + ";errors="
			+ Std.string(errorCount()) + ";liveRouteDenied=" + Std.string(liveRouteDeniedCount()) + ";planDenied=" + Std.string(planDeniedCount())
			+ ";outcomes=[" + parts.join("##") + "]";
	}
}
