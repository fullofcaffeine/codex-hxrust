...
class ModelRequestEnvelopePolicy {
	...
	public static function build(request:ModelRequestEnvelopeRequest):ModelRequestEnvelopeOutcome {
		...
		if (plan == null || !plan.ok) {
			return ModelRequestEnvelopeOutcome.denied(
				request,
				plan,
				"turn_model_plan_denied",
				plan == null ? "missing turn model plan" : plan.errorMessage,
		...
		if (request.routeIntent != ModelRequestRouteIntent.PlanOnly && !request.liveNetworkAllowed) {
			return ModelRequestEnvelopeOutcome.denied(
				...
				"fixture gate refused live model route before transport",
		...
		if (request.routeIntent != ModelRequestRouteIntent.PlanOnly && !request.hasCredentialMaterial) {
			return ModelRequestEnvelopeOutcome.denied(
				...
				"live model route requires credential material",
				"model_request_envelope->route:" + request.routeIntent + "->credential:missing"
			);
		}
		return ModelRequestEnvelopeOutcome.accepted(
			request,
			plan,
	...
	static function selectedModel(request:ModelRequestEnvelopeRequest, plan:TurnModelPlanOutcome):ModelCatalogEntry {
		for (model in request.planRequest.catalogRequest.catalog) {
			if (model.providerId == plan.providerId && model.modelId == plan.selectedModelId) return model;
		}
		return null;
	}
	...
	static function envelope(
		...
		final serviceTier = prompt.requestedServiceTier.length > 0
			? prompt.requestedServiceTier
			: (model.defaultServiceTier.length > 0 ? model.defaultServiceTier : "none");
		return new ModelRequestEnvelope(
			model.modelId,
			provider.providerId,
			routeName(request.routeIntent),
			"responses",
			true,
			isAzureResponsesEndpoint(provider.baseUrl),
			prompt.parallelToolCalls && !model.useResponsesLite,
			reasoningIncluded,
			reasoningIncluded ? (model.useResponsesLite ? "all_turns" : "default") : "none",
			reasoningIncluded,
			textControls,
			serviceTier,
			true,
			true,
			prompt.enableRequestCompression,
			false,
			prompt.toolSpecCount,
			prompt.inputItemCount
		);
	}

	static function routeName(intent:ModelRequestRouteIntent):String {
		if (intent == ModelRequestRouteIntent.ResponsesHttp) return "responses_http";
		if (intent == ModelRequestRouteIntent.ResponsesWebsocket) return "responses_websocket";
		return "request_envelope_only";
	}

	static function textControls(prompt:ModelRequestPromptEnvelope):String {
		if (prompt.outputSchemaPresent) return prompt.outputSchemaStrict ? "json_schema_strict" : "json_schema";
		if (prompt.supportsVerbosity && prompt.requestedVerbosity.length > 0) return "verbosity:" + prompt.requestedVerbosity;
		return "none";
	}
