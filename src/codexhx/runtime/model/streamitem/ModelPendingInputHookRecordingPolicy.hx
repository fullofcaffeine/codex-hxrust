...
class ModelPendingInputHookRecordingPolicy {
	public static function record(request:ModelPendingInputHookRecordingRequest):ModelPendingInputHookRecordingOutcome {
		if (request == null) return failure("", "missing pending input hook recording request");
		final drain = request.drainOutcome;
		if (drain == null || !drain.ok) return failure(request.requestId, "pending input hook recording requires a drain outcome");
		...
		for (item in request.items) {
			...
			if (stopped) {
				...
				if (item.isNonEmptyUserInput()) acceptedUserInput = true;
				if (item.isUserInput()) {
		...
		return new ModelPendingInputHookRecordingOutcome(
			true,
			"pending_input_hook_recording_modeled",
			request.requestId,
			drain.requestId,
	...
	static function skipped(request:ModelPendingInputHookRecordingRequest, errorMessage:String):ModelPendingInputHookRecordingOutcome {
		...
		return new ModelPendingInputHookRecordingOutcome(
			...
			request.requestId,
			drain.requestId,
			ModelPendingInputHookRecordingDecisionKind.Skipped,
			0,
			false,
			false,
			0,
	...
	static function failure(requestId:String, errorMessage:String):ModelPendingInputHookRecordingOutcome {
		return new ModelPendingInputHookRecordingOutcome(
			false,
			"pending_input_hook_recording_failed",
			requestId,
			"",
			ModelPendingInputHookRecordingDecisionKind.Skipped,
			0,
