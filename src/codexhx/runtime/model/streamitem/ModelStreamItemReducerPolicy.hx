...
class ModelStreamItemReducerPolicy {
	...
	public static function build(request:ModelStreamItemReducerRequest):ModelStreamItemReducerOutcome {
		...
		if (route == null || !route.ok) {
			return ModelStreamItemReducerOutcome.denied(
				request,
				route,
				route == null ? "missing stream route" : route.errorMessage,
		...
		for (event in request.events) {
			...
			if (event.kind == ModelStreamItemEventKind.OutputItemAdded) {
				...
				runtimeEvents.push(new ModelStreamRuntimeEvent(
					ModelStreamRuntimeEventKind.AgentMessageContentDelta,
					activeItem.kind,
					activeItem.itemId,
					"",
					"",
				...
				if (activeItem == null || !activeStreaming) {
					return ModelStreamItemReducerOutcome.failed(request, route, runtimeEvents, "ReasoningSummaryDelta without active item", sequence.join("->"));
				}
				runtimeEvents.push(new ModelStreamRuntimeEvent(
					ModelStreamRuntimeEventKind.ReasoningContentDelta,
					activeItem.kind,
					activeItem.itemId,
					"",
					"",
				...
				if (activeItem == null || !activeStreaming) {
					return ModelStreamItemReducerOutcome.failed(request, route, runtimeEvents, "ReasoningRawContentDelta without active item", sequence.join("->"));
				}
				if (request.showRawReasoning) {
					runtimeEvents.push(new ModelStreamRuntimeEvent(
						ModelStreamRuntimeEventKind.ReasoningRawContentDelta,
						activeItem.kind,
						activeItem.itemId,
						"",
						"",
				...
				runtimeEvents.push(new ModelStreamRuntimeEvent(
					ModelStreamRuntimeEventKind.StreamCompleted,
					ModelStreamOutputItemKind.Unknown,
					"",
					"",
					"",
					"",
					event.endTurn ? "end_turn" : "follow_up",
					0
				));
				if (!event.endTurn) needsFollowUp = true;
			}
		}

		return ModelStreamItemReducerOutcome.reduced(
			request,
			route,
			runtimeEvents,
			startedCount,
	...
	static function isToolCall(item:ModelStreamOutputItem):Bool {
		return item != null && (
			item.kind == ModelStreamOutputItemKind.FunctionCall
			|| item.kind == ModelStreamOutputItemKind.CustomToolCall
		);
	}

	static function finalizedItem(item:ModelStreamOutputItem, planMode:Bool):ModelStreamOutputItem {
		if (item.kind != ModelStreamOutputItemKind.AssistantMessage) return item;
		return new ModelStreamOutputItem(
			item.kind,
			item.itemId,
			item.role,
			stripHiddenAssistantMarkup(item.text, planMode),
			item.phase,
			item.summary,
			item.rawContent,
			item.callId,
	...
	static function itemCompleted(item:ModelStreamOutputItem):ModelStreamRuntimeEvent {
		return new ModelStreamRuntimeEvent(
			ModelStreamRuntimeEventKind.ItemCompleted,
			item.kind,
			item.itemId,
			item.callId,
			toolDisplayName(item),
			"",
			item.kind == ModelStreamOutputItemKind.Reasoning ? item.summary.join("|") : item.text,
			0
		);
	}
	...
	static function toolCallQueued(item:ModelStreamOutputItem):ModelStreamRuntimeEvent {
		return new ModelStreamRuntimeEvent(
			ModelStreamRuntimeEventKind.ToolCallQueued,
			item.kind,
			item.itemId,
			item.callId,
			toolDisplayName(item),
	...
	static function toolDisplayName(item:ModelStreamOutputItem):String {
		if (item == null) return "";
		if (item.namespace.length > 0) return item.namespace + "." + item.toolName;
	...
	static function stripHiddenAssistantMarkup(text:String, planMode:Bool):String {
		...
		if (!planMode) return withoutCitations;
