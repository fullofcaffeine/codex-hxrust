...
class ModelThreadSideParentPendingPolicy {
	public static function apply(request:ModelThreadSideParentPendingRequest):ModelThreadSideParentPendingOutcome {
		if (request == null) return failure("", "", "missing thread side-parent pending request");
		final activeTurnRequestId = request.activeTurnOutcome == null ? "" : request.activeTurnOutcome.requestId;
		if (request.activeTurnOutcome == null) return failure(request.requestId, "", "missing thread active-turn outcome");
		if (!request.activeTurnOutcome.ok) return failure(request.requestId, activeTurnRequestId, "thread active-turn outcome was not successful");
		...
		switch request.eventKind {
			case RequestQueued:
				if (request.requestAddsUserInput) userInputCount = userInputCount + 1;
				if (request.requestAddsApproval) approvalCount = approvalCount + 1;
		...
		final requestStatusFallbackApplied = storeStatus == ModelThreadSideParentStatusKind.None && fallbackStatus != ModelThreadSideParentStatusKind.None;
		...
		return new ModelThreadSideParentPendingOutcome(
			true,
			"thread_side_parent_pending_status_modeled",
			request.requestId,
			activeTurnRequestId,
			request.eventKind,
			decisionKind,
			...
			approvalCount,
			approvalCount > 0,
			userInputCount > 0 && approvalCount > 0 && statusAfter == ModelThreadSideParentStatusKind.NeedsInput,
			requestStatusFallbackApplied,
			resolvedRequestRemoved,
			evictedRequestRemoved,
			threadClosedCleared,
			request.eventOrderIndex == request.previousEventCount + 1,
			request.activeTurnOutcome.liveNetworkAttempted,
			request.activeTurnOutcome.realFilesystemMutated,
	...
	static function statusFromCounts(userInputCount:Int, approvalCount:Int):ModelThreadSideParentStatusKind {
		if (userInputCount > 0) return ModelThreadSideParentStatusKind.NeedsInput;
		if (approvalCount > 0) return ModelThreadSideParentStatusKind.NeedsApproval;
	...
	static function decisionForStatus(
		status:ModelThreadSideParentStatusKind,
		requestStatusFallbackApplied:Bool
	):ModelThreadSideParentPendingDecisionKind {
		if (requestStatusFallbackApplied) return ModelThreadSideParentPendingDecisionKind.UsedRequestStatusFallback;
	...
	static function failure(requestId:String, activeTurnRequestId:String, errorMessage:String):ModelThreadSideParentPendingOutcome {
		return new ModelThreadSideParentPendingOutcome(
			false,
			"thread_side_parent_pending_status_failed",
			requestId,
			activeTurnRequestId,
			ModelThreadSideParentPendingEventKind.StatusRefresh,
			ModelThreadSideParentPendingDecisionKind.PreservedNoPending,
