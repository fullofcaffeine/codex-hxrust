...
class ModelPostDrainEmissionPolicy {
	public static function project(request:ModelPostDrainEmissionRequest):ModelPostDrainEmissionOutcome {
		if (request == null) return failure("", "missing post-drain emission request");
		final drain = request.drainOutcome;
		if (drain == null || !drain.ok) return failure(request.requestId, "post-drain emission requires a valid drain outcome");
		...
		final cancellationKind = request.cancellationRequestedAfterDrain
			? ModelPostDrainCancellationKind.AfterDrainBeforeTurnDiff
		...
		return new ModelPostDrainEmissionOutcome(
			true,
			...
			tokenPending,
			tokenProjected,
			request.tokenInfoAvailable,
			!fatalDrain,
			turnDiffPending,
			turnDiffTrackerRead,
			request.unifiedDiffAvailable,
			turnDiffProjected,
			request.cancellationRequestedAfterDrain && turnDiffPending,
			turnDiffSkippedNoDiff,
			samplingOutcomeReturned,
			drain.liveNetworkAttempted,
			drain.realFilesystemMutated,
			drain.toolExecutedOutsideFixture,
			fatalDrain ? "fatal drain prevents post-drain emission" : ""
		);
	}
