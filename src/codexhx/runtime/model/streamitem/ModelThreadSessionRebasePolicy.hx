...
class ModelThreadSessionRebasePolicy {
	public static function rebase(request:ModelThreadSessionRebaseRequest):ModelThreadSessionRebaseOutcome {
		if (request == null) return failure("", "", "missing thread session rebase request");
		final evictionRequestId = request.evictionOutcome == null ? "" : request.evictionOutcome.requestId;
		if (request.evictionOutcome == null) return failure(request.requestId, "", "missing thread buffered request eviction outcome");
		if (!request.evictionOutcome.ok) return failure(request.requestId, evictionRequestId, "thread buffered request eviction outcome was not successful");
		...
		final snapshotRequestReplayed = requestEvent && survives && request.snapshotFilterChecked && pendingReplayStatePreserved && !request.serverResolutionRecordedBefore;
		final resolvedRequestFiltered = requestEvent && survives && request.snapshotFilterChecked && request.serverResolutionRecordedBefore && !snapshotRequestReplayed;
		final bufferEventCountAfter = dropped && request.bufferEventCountBefore > 0 ? request.bufferEventCountBefore - 1 : request.bufferEventCountBefore;
		...
		return new ModelThreadSessionRebaseOutcome(
			true,
			"thread_session_rebase_modeled",
			request.requestId,
			evictionRequestId,
			rebaseKind,
			request.rebaseEventKind,
			survives,
			dropped,
			pendingReplayStatePreserved,
			snapshotRequestReplayed,
			resolvedRequestFiltered,
			bufferEventCountAfter,
			orderingPreserved,
			request.evictionOutcome.liveNetworkAttempted,
			request.evictionOutcome.realFilesystemMutated,
	...
	static function rebaseKind(
		eventKind:ModelThreadSessionRebaseEventKind,
		resolvedRequestFiltered:Bool
	):ModelThreadSessionRebaseKind {
		if (resolvedRequestFiltered) return ModelThreadSessionRebaseKind.FilteredResolvedRequest;
	...
	static function failure(requestId:String, evictionRequestId:String, errorMessage:String):ModelThreadSessionRebaseOutcome {
		return new ModelThreadSessionRebaseOutcome(
			false,
			"thread_session_rebase_failed",
			requestId,
			evictionRequestId,
			ModelThreadSessionRebaseKind.DroppedOrdinaryNotification,
			ModelThreadSessionRebaseEventKind.OrdinaryNotification,
			false,
			false,
			false,
			false,
