...
class ModelResizeReflowSchedulingPolicy {
	...
	public static function apply(request:ModelResizeReflowSchedulingRequest):ModelResizeReflowSchedulingOutcome {
		if (request == null)
			return failure("", "", "missing resize reflow scheduling request");
		final resizeReflowRequestId = request.resizeReflowOutcome == null ? "" : request.resizeReflowOutcome.requestId;
		if (request.resizeReflowOutcome == null)
			return failure(request.requestId, "", "missing terminal resize reflow outcome");
		if (!request.resizeReflowOutcome.ok)
			return failure(request.requestId, resizeReflowRequestId, "terminal resize reflow outcome was not successful");
		...
		final reflowNeededForWidth = !widthInitialized
			&& request.previousReflowWidth != request.currentWidth
			&& request.previousPendingWidth != request.currentWidth;
	...
	static function decisionKind(enabled:Bool, shouldRebuildTranscript:Bool, heightChanged:Bool,
			reflowNeededForWidth:Bool):ModelResizeReflowSchedulingDecisionKind {
		if (!shouldRebuildTranscript)
			return ModelResizeReflowSchedulingDecisionKind.UnchangedSizeNoOp;
		if (!enabled && reflowNeededForWidth)
			return ModelResizeReflowSchedulingDecisionKind.DisabledWidthChangeCleared;
		if (reflowNeededForWidth)
			return ModelResizeReflowSchedulingDecisionKind.WidthChangeScheduled;
		if (heightChanged)
			return ModelResizeReflowSchedulingDecisionKind.HeightChangeScheduled;
		return ModelResizeReflowSchedulingDecisionKind.UnchangedSizeNoOp;
	}
