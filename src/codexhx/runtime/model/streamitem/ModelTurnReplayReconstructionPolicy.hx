...
class ModelTurnReplayReconstructionPolicy {
	public static function reconstruct(request:ModelTurnReplayReconstructionRequest):ModelTurnReplayReconstructionOutcome {
		if (request == null) return failure("", "", ModelTurnReplayKind.ThreadSnapshot, ModelTurnReplayTargetKind.MissingNoop, "", "missing turn replay reconstruction request");
		...
		if (!request.projectionOutcome.ok) {
			return failure(request.requestId, request.projectionOutcome.requestId, request.replayKind, request.targetKind, request.terminalTurnId, "terminal projection outcome was not successful");
		}
		...
		final completedTerminal = status == ModelTurnTerminalProjectedStatusKind.Completed || status == ModelTurnTerminalProjectedStatusKind.Failed;
		...
		return new ModelTurnReplayReconstructionOutcome(
			true,
			"turn_replay_reconstruction_modeled",
			request.requestId,
			request.projectionOutcome.requestId,
			request.replayKind,
			request.targetKind,
			terminalTurnId,
			status,
			currentClosed,
			currentMarkedTerminal,
			targetHistorical,
			activePreserved,
			fallbackActive,
			missingNoop,
			request.projectionOutcome.threadHistoryFailedStatusPreserved,
			terminalNotification,
			replayKindAttached,
			inProgressStart,
			resumeStartSuppressed,
			liveSuppressed,
			request.projectionOutcome.lastAgentMessagePropagatedToCoreStatus,
			request.projectionOutcome.liveNetworkAttempted,
			request.projectionOutcome.realFilesystemMutated,
			request.projectionOutcome.toolExecutedOutsideFixture,
			missingNoop ? "terminal event had no active or historical replay target" : ""
		);
	}
	...
	static function failure(
		...
		return new ModelTurnReplayReconstructionOutcome(
			false,
			"turn_replay_reconstruction_failed",
			requestId,
			projectionRequestId,
			replayKind,
			targetKind,
