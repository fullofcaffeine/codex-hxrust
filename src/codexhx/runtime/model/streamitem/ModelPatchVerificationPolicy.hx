...
class ModelPatchVerificationPolicy {
	public static function verify(request:ModelPatchVerificationRequest):ModelPatchVerificationOutcome {
		if (request == null)
			return failure("", "missing_patch_verification_request", "missing patch verification request");
		...
		if (patchInput.length == 0)
			return failure(request.requestId, "missing_apply_patch_input", "missing queued apply_patch input for callId=" + request.callId);

		final parse = parsePatch(request.callId, patchInput);
		if (parse == null)
			return failure(request.requestId, "invalid_apply_patch_input", "apply_patch input could not be parsed");

		final verificationError = verifyChanges(parse.changes, request.files);
		if (verificationError.length > 0)
			return failure(request.requestId, "patch_verification_failed", verificationError);

		final verifiedChanges = hydrateVerifiedChanges(parse.changes, request.files);
		final begin = new ModelPatchVerificationEvent(ModelPatchVerificationEventKind.Begin, request.callId, request.turnId, request.autoApproved,
			request.desiredStatus, false, verifiedChanges, "", "");
		final success = request.desiredStatus == ModelPatchApplyStatus.Completed;
		final end = new ModelPatchVerificationEvent(ModelPatchVerificationEventKind.End, request.callId, request.turnId, request.autoApproved,
			request.desiredStatus, success, verifiedChanges, request.stdout, request.stderr);
		return new ModelPatchVerificationOutcome(true, "patch_apply_verified", request.requestId, begin, end, false, false, false, "");
	}
	...
	static function queuedApplyPatchInput(outcome:ModelStreamItemReducerOutcome, callId:String):String {
		for (event in outcome.runtimeEvents) {
			if (event.kind == ModelStreamRuntimeEventKind.ToolCallQueued && event.callId == callId && event.toolName == "apply_patch")
				return event.text;
		}
		return "";
	}
	...
	static function verifyChanges(changes:Array<ModelPatchFileChange>, files:Array<ModelPatchVirtualFile>):String {
		if (changes == null || changes.length == 0)
			return "apply_patch input has no file changes";
		for (change in changes) {
			if (change.path.length == 0)
				return "patch change path must not be empty";
	...
	static function hydrateVerifiedChanges(changes:Array<ModelPatchFileChange>, files:Array<ModelPatchVirtualFile>):Array<ModelPatchFileChange> {
		...
		for (change in changes) {
			...
			if (copy.kind == ModelPatchFileChangeKind.Delete || copy.kind == ModelPatchFileChangeKind.Update)
				copy.content = fileContent(files, copy.path);
			out.push(copy);
		}
		return out;
	}
