...
class ModelPatchTurnDiffTrackerPolicy {
	public static function update(request:ModelPatchTurnDiffTrackerRequest):ModelPatchTurnDiffTrackerOutcome {
		if (request == null)
			return failure("", "missing patch turn-diff tracker request");
		...
		final unifiedDiff = trackerValid
			&& updateKind == ModelPatchTurnDiffTrackerUpdateKind.Track ? renderUnifiedDiff(request.environmentId, request.appliedDelta.changes) : "";
		final shouldEmit = updateKind != ModelPatchTurnDiffTrackerUpdateKind.None
			&& (request.previousUnifiedDiff.length > 0 || unifiedDiff.length > 0);
		return new ModelPatchTurnDiffTrackerOutcome(true, "patch_turn_diff_tracker_updated", request.requestId, updateKind, trackerValid, shouldEmit,
			unifiedDiff, false, false, false, "");
	}
	...
	static function renderUnifiedDiff(environmentId:String, changes:Array<ModelPatchFileChange>):String {
		...
		for (change in changes) {
			...
			if (change.kind == ModelPatchFileChangeKind.Add) {
				...
				out.push("--- /dev/null");
				out.push("+++ b/" + display);
				for (line in splitLines(change.content))
					out.push("+" + line);
	...
	static function failure(requestId:String, errorMessage:String):ModelPatchTurnDiffTrackerOutcome {
		return new ModelPatchTurnDiffTrackerOutcome(false, "patch_turn_diff_tracker_failed", requestId, ModelPatchTurnDiffTrackerUpdateKind.None, false,
