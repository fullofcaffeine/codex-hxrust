...
class ModelTurnTerminalProjectionPolicy {
	public static function project(request:ModelTurnTerminalProjectionRequest):ModelTurnTerminalProjectionOutcome {
		if (request == null) return failure("", "", "", "", ModelTurnTerminalProjectionEventKind.TurnComplete, "missing turn terminal projection request");
		...
		if (!request.lifecycleOutcome.ok) {
			return failure(request.requestId, request.threadId, request.turnId, request.lifecycleOutcome.requestId, request.eventKind, "turn lifecycle outcome was not successful");
		}
	...
	static function projectComplete(request:ModelTurnTerminalProjectionRequest):ModelTurnTerminalProjectionOutcome {
		if (!request.lifecycleOutcome.turnCompleteEmitted) {
			return failure(request.requestId, request.threadId, request.turnId, request.lifecycleOutcome.requestId, request.eventKind, "lifecycle did not emit TurnComplete");
		}
		...
		return new ModelTurnTerminalProjectionOutcome(
			true,
			"turn_terminal_projection_modeled",
			request.requestId,
			request.threadId,
			projectedTurnId(request),
			request.lifecycleOutcome.requestId,
			ModelTurnTerminalProjectionEventKind.TurnComplete,
			ModelTurnTerminalProjectedStatusKind.Completed,
			...
			message.length > 0,
			false,
			!priorError && notifyUser && request.sawCopySourceThisTurn,
			!priorError,
			false,
			priorError,
			request.lifecycleOutcome.liveNetworkAttempted,
			request.lifecycleOutcome.realFilesystemMutated,
			request.lifecycleOutcome.toolExecutedOutsideFixture,
			priorError ? turnErrorMessage(request) : ""
		);
	}
	...
	static function projectAborted(request:ModelTurnTerminalProjectionRequest):ModelTurnTerminalProjectionOutcome {
		if (!request.lifecycleOutcome.turnAbortedEmitted) {
			return failure(request.requestId, request.threadId, request.turnId, request.lifecycleOutcome.requestId, request.eventKind, "lifecycle did not emit TurnAborted");
		}
		...
		return new ModelTurnTerminalProjectionOutcome(
			...
			projectedTurnId(request),
			request.lifecycleOutcome.requestId,
			ModelTurnTerminalProjectionEventKind.TurnAborted,
			coreStatus,
			ModelTurnTerminalProjectedStatusKind.Interrupted,
			ModelTurnTerminalNotificationIntentKind.AppServerTurnCompleted,
			ModelTurnTerminalNotificationIntentKind.TuiInterruptedTurn,
			true,
			request.pendingInterruptRequest,
			true,
			false,
			false,
			true,
	...
	static function lastAgentMessage(request:ModelTurnTerminalProjectionRequest):String {
		if (request.lastAgentMessageOverride.length > 0) return request.lastAgentMessageOverride;
	...
	static function turnErrorMessage(request:ModelTurnTerminalProjectionRequest):String {
		if (request.priorTurnErrorMessage.length > 0) return request.priorTurnErrorMessage;
	...
	static function abortReason(request:ModelTurnTerminalProjectionRequest):String {
		if (request.abortReason.length > 0) return request.abortReason;
		if (request.lifecycleOutcome.errorMessage.length > 0) return request.lifecycleOutcome.errorMessage;
		return "unknown";
	}
	...
	static function failure(
		...
		return new ModelTurnTerminalProjectionOutcome(
			false,
			"turn_terminal_projection_failed",
			requestId,
			threadId,
			turnId,
			lifecycleRequestId,
			eventKind,
			ModelTurnTerminalProjectedStatusKind.Errored,
			ModelTurnTerminalProjectedStatusKind.Failed,
			ModelTurnTerminalNotificationIntentKind.None,
			ModelTurnTerminalNotificationIntentKind.None,
			false,
			false,
			false,
