...
class ModelThreadSideThreadComposerHandoffPolicy {
	public static function apply(request:ModelThreadSideThreadComposerHandoffRequest):ModelThreadSideThreadComposerHandoffOutcome {
		if (request == null) return failure("", "", "", "missing thread side-thread composer handoff request");
		...
		if (request.startOutcome == null) return failure(request.requestId, "", startupRoutingRequestId, "missing thread side-thread start outcome");
		if (!request.startOutcome.ok) return failure(request.requestId, startRequestId, startupRoutingRequestId, "thread side-thread start outcome was not successful");
		...
		if (!hasMessage) {
			return success(
				request,
				startRequestId,
				startupRoutingRequestId,
				ModelThreadSideThreadComposerHandoffDecisionKind.NoUserMessageNoop,
				false,
				false,
				false,
				"",
		...
		return switch request.startOutcome.decisionKind {
			...
			case SwitchedAndSubmittedUserMessage:
				success(
					...
					startRequestId,
					startupRoutingRequestId,
					ModelThreadSideThreadComposerHandoffDecisionKind.SubmittedAfterSwitch,
					true,
					false,
					false,
					"",
	...
	static function restored(
		request:ModelThreadSideThreadComposerHandoffRequest,
	...
	static function success(
		...
		final sourceOrderingPreserved = startupRoutingComposed
			? request.startupRoutingOutcome.eventOrderingPreserved
			: request.startOutcome.eventOrderingPreserved;
		final liveNetworkAttempted = startupRoutingComposed
			? request.startupRoutingOutcome.liveNetworkAttempted
			: request.startOutcome.liveNetworkAttempted;
		final realFilesystemMutated = startupRoutingComposed
			? request.startupRoutingOutcome.realFilesystemMutated
			: request.startOutcome.realFilesystemMutated;
		final toolExecutedOutsideFixture = startupRoutingComposed
			? request.startupRoutingOutcome.toolExecutedOutsideFixture
			: request.startOutcome.toolExecutedOutsideFixture;
		return new ModelThreadSideThreadComposerHandoffOutcome(
			true,
			"thread_side_thread_composer_handoff_modeled",
			request.requestId,
			startRequestId,
			startupRoutingRequestId,
			decisionKind,
			userMessagePreserved,
			restoreAttempted,
			composerMutated,
			composerTextAfter,
			submittedAsPlainUserTurn,
			request.userMessageProvided && (restoreAttempted != submittedAsPlainUserTurn),
			request.startOutcome.sideUiSynced,
			request.startOutcome.contextLabelCleared,
			request.startOutcome.errorMessageAdded,
			request.startOutcome.runControlContinue,
			startupRoutingComposed,
			sourceOrderingPreserved && request.eventOrderIndex == request.previousEventCount + 1,
	...
	static function failure(requestId:String, startRequestId:String, startupRoutingRequestId:String, errorMessage:String):ModelThreadSideThreadComposerHandoffOutcome {
		return new ModelThreadSideThreadComposerHandoffOutcome(
			false,
			"thread_side_thread_composer_handoff_failed",
			requestId,
			startRequestId,
			startupRoutingRequestId,
			ModelThreadSideThreadComposerHandoffDecisionKind.NoUserMessageNoop,
			...
			false,
			false,
			startupRoutingRequestId.length > 0,
			false,
			false,
			false,
			false,
