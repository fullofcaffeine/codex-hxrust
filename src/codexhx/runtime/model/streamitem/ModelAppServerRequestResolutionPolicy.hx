...
class ModelAppServerRequestResolutionPolicy {
	public static function resolve(request:ModelAppServerRequestResolutionRequest):ModelAppServerRequestResolutionOutcome {
		if (request == null) return failure("", "", ModelReplayedServerRequestKind.UserInput, ModelAppServerRequestResolutionCommandKind.UserInputAnswer, "missing app-server request resolution request");
		if (request.surfaceOutcome == null) return failure(request.requestId, "", request.requestKind, request.commandKind, "missing replayed server request surface outcome");
		if (!request.surfaceOutcome.ok) return failure(request.requestId, request.surfaceOutcome.requestId, request.requestKind, request.commandKind, "replayed server request surface outcome was not successful");
		...
		return new ModelAppServerRequestResolutionOutcome(
			true,
			"app_server_request_resolution_modeled",
			request.requestId,
			request.surfaceOutcome.requestId,
			request.requestKind,
			request.commandKind,
			serialized ? payloadKind(request.commandKind) : ModelAppServerRequestResolutionPayloadKind.None,
			request.appServerRequestId,
			request.requestKey,
			request.commandKey,
			pendingRecorded,
			commandMatched,
			commandMatched,
			serialized,
			commandMatched && request.commandKind == ModelAppServerRequestResolutionCommandKind.ExecApprovalResponse,
			commandMatched && request.commandKind == ModelAppServerRequestResolutionCommandKind.FileChangeApprovalResponse,
			commandMatched && request.commandKind == ModelAppServerRequestResolutionCommandKind.PermissionsResponse,
			userInputPopped,
			queueAfter,
			userInputPopped ? request.pendingItemId : "",
			commandMatched && request.commandKind == ModelAppServerRequestResolutionCommandKind.McpElicitationResponse,
			commandMatched && notification,
			noop,
			unsupportedRejected,
			true,
			live,
	...
	static function commandMatches(request:ModelAppServerRequestResolutionRequest):Bool {
		return switch request.commandKind {
			case ModelAppServerRequestResolutionCommandKind.ExecApprovalResponse:
				request.requestKind == ModelReplayedServerRequestKind.ExecApproval && request.requestKey == request.commandKey;
			case ModelAppServerRequestResolutionCommandKind.FileChangeApprovalResponse:
				request.requestKind == ModelReplayedServerRequestKind.FileChangeApproval && request.requestKey == request.commandKey;
			case ModelAppServerRequestResolutionCommandKind.PermissionsResponse:
				request.requestKind == ModelReplayedServerRequestKind.PermissionsApproval && request.requestKey == request.commandKey;
			...
			case ModelAppServerRequestResolutionCommandKind.McpElicitationResponse:
				request.requestKind == ModelReplayedServerRequestKind.McpElicitation
					&& request.serverName == request.commandServerName
					&& request.mcpRequestId == request.commandMcpRequestId;
	...
	static function failure(
		...
		return new ModelAppServerRequestResolutionOutcome(
			false,
			"app_server_request_resolution_failed",
			requestId,
			surfaceRequestId,
			requestKind,
			commandKind,
