...
class ModelPatchApprovalDecisionPolicy {
	public static function decide(request:ModelPatchApprovalDecisionRequest):ModelPatchApprovalDecisionOutcome {
		if (request == null) return failure("", [], "missing patch approval decision request");
		...
		if (request.applicationOutcome == null || !request.applicationOutcome.ok) {
			return failure(request.requestId, approvalKeys(request.environmentId, request.verificationOutcome.endEvent.changes), "patch approval decision requires a modeled application outcome");
		}
		...
		final sandboxRetryRequested = request.sandboxDenied && request.sandboxApprovalAllowed && request.sandboxAttempt == ModelPatchSandboxAttemptKind.Sandboxed;
		return new ModelPatchApprovalDecisionOutcome(
			true,
			"patch_approval_decision_modeled",
			request.requestId,
			approvalRequired,
			...
			"auto",
			true,
			sandboxRetryRequested,
			"tool=apply_patch;input=command",
			effectiveDecision,
			keys,
			false,
			false,
	...
	static function approvalKeys(environmentId:String, changes:Array<ModelPatchFileChange>):Array<ModelPatchApprovalKey> {
		...
		for (change in changes) {
			...
			if (change.kind == ModelPatchFileChangeKind.Update && change.movePath.length > 0) pushUnique(out, environmentId, change.movePath);
		}
		return out;
	}

	static function pushUnique(keys:Array<ModelPatchApprovalKey>, environmentId:String, path:String):Void {
		for (key in keys) if (key.environmentId == environmentId && key.path == path) return;
	...
	static function failure(requestId:String, keys:Array<ModelPatchApprovalKey>, errorMessage:String):ModelPatchApprovalDecisionOutcome {
		return new ModelPatchApprovalDecisionOutcome(
			false,
			"patch_approval_decision_failed",
			requestId,
			false,
			false,
			false,
			...
			false,
			"",
			ModelPatchReviewDecision.Denied,
			keys,
			false,
			false,
			false,
