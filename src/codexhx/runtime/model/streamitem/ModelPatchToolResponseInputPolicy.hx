...
class ModelPatchToolResponseInputPolicy {
	public static function admit(request:ModelPatchToolResponseInputRequest):ModelPatchToolResponseInputOutcome {
		if (request == null) return failure("", "missing patch tool response input request");
		...
		return new ModelPatchToolResponseInputOutcome(
			true,
			"patch_tool_response_input_admitted",
			request.requestId,
			request.followUpOutcome.callId,
			request.followUpOutcome.responseKind,
			admitted ? ModelPatchToolResponseAdmissionKind.Admitted : ModelPatchToolResponseAdmissionKind.Skipped,
			admitted ? nextCount : 0,
			nextCount,
			request.followUpOutcome.outputText,
			request.followUpOutcome.success,
			request.followUpOutcome.modelNeedsFollowUp,
			admitted,
			admitted,
			false,
	...
	static function failure(requestId:String, errorMessage:String):ModelPatchToolResponseInputOutcome {
		return new ModelPatchToolResponseInputOutcome(
			false,
			"patch_tool_response_input_failed",
			requestId,
			"",
			ModelPatchToolOutputItemKind.FunctionCallOutput,
			ModelPatchToolResponseAdmissionKind.Skipped,
			0,
			0,
			"",
			false,
