...
class TurnModelPlanPolicy {
	...
	public static function build(request:TurnModelPlanRequest):TurnModelPlanOutcome {
		...
		final capabilities = request.catalogRequest == null
			|| request.catalogRequest.admissionRequest == null ? ModelProviderCapabilities.defaultConfigured() : ModelProviderCapabilities.forProvider(request.catalogRequest.admissionRequest.provider);
		final emptyPlan = disabledPlan();
		if (catalog == null || !catalog.ok) {
			return TurnModelPlanOutcome.denied(request, catalog, "model_catalog_denied", "", ModelCatalogToolMode.None, emptyPlan,
				catalog == null ? "missing model catalog request" : catalog.errorMessage, "turn_model_plan->model_catalog:error");
		}
		final selected = selectedModel(request, catalog);
		if (selected == null) {
			return TurnModelPlanOutcome.denied(request, catalog, "selected_model_missing", catalog.selectedModelId, ModelCatalogToolMode.None, emptyPlan,
				"selected catalog model metadata is missing", "turn_model_plan->selected_model_metadata:missing");
		}
		...
		if (!plan.enabled(request.requestedCapability)) {
			return TurnModelPlanOutcome.denied(request, catalog, "unsupported_tool_capability", selected.modelId, toolMode, plan,
		...
		return TurnModelPlanOutcome.accepted(request, catalog, selected.modelId, toolMode, true, plan,
			"turn_model_plan->model_catalog:ok->tool_mode:"
			+ toolMode
			+ "->capability:"
	...
	static function selectedModel(request:TurnModelPlanRequest, catalog:ModelCatalogOutcome):ModelCatalogEntry {
		for (model in request.catalogRequest.catalog) {
			if (model.providerId == catalog.providerId && model.modelId == catalog.selectedModelId)
				return model;
		}
		return null;
	}
	...
	static function effectiveToolMode(model:ModelCatalogEntry, features:TurnModelFeatureFlags):ModelCatalogToolMode {
		if (model.toolMode != ModelCatalogToolMode.None)
			return model.toolMode;
		if (features.codeModeOnly)
			return ModelCatalogToolMode.CodeModeOnly;
		if (features.codeMode)
			return ModelCatalogToolMode.CodeMode;
		return ModelCatalogToolMode.Direct;
	}

	static function capabilityPlan(model:ModelCatalogEntry, capabilities:ModelProviderCapabilities, toolMode:ModelCatalogToolMode,
			request:TurnModelPlanRequest):TurnModelCapabilityPlan {
		...
		final standaloneWebRun = namespaceTools
			...
			&& webModeOn;
		final hostedWebSearch = !model.useResponsesLite && !standaloneWebRun && capabilities.webSearch && webModeOn;
		final hostedWebAccess = hostedWebSearch ? webAccess(request.webSearchMode) : "none";
		final hostedWebTypes = hostedWebSearch
			&& model.webSearchToolType == ModelCatalogWebSearchToolType.TextAndImage ? "text,image" : "text";
		...
		final hostedImage = imageRuntime && request.features.imageGeneration && !standaloneImageVisible;
		...
		return new TurnModelCapabilityPlan(hostedWebSearch, hostedWebAccess, hostedWebTypes, standaloneWebRun, hostedImage, standaloneImageVisible,
			namespaceTools, codeModeNested, toolSearch);
	}
