...
class TurnModelPlanReport {
	...
	public function unsupportedCapabilityCount():Int {
		var count = 0;
		for (outcome in outcomes) if (outcome.code == "unsupported_tool_capability") count = count + 1;
		return count;
	}

	public function catalogDeniedCount():Int {
		var count = 0;
		for (outcome in outcomes) if (outcome.code == "model_catalog_denied") count = count + 1;
		return count;
	}
	...
	public function summary():String {
		...
		return "schema=" + schema
			...
			+ ";unsupportedCapabilities=" + Std.string(unsupportedCapabilityCount())
			+ ";catalogDenied=" + Std.string(catalogDeniedCount())
			+ ";outcomes=[" + parts.join("##") + "]";
	}
}
