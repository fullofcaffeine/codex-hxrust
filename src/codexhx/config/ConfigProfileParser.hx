...
class ConfigProfileParser {
	...
	static function build(keys:Array<String>, values:Array<Value>, object:Value):ConfigProfileParseOutcome {
		...
		if (!profileName.ok)
			return profileName.toParseOutcome("$.profileName");
		...
		if (!model.ok)
			return model.toParseOutcome("$.model");
		...
		if (!validReasoningEffort(reasoningEffort)) {
			return ConfigProfileParseOutcome.failure("invalid_reasoning_effort", "$.reasoningEffort",
				"reasoning effort must be a non-empty Codex effort value");
		}

		final approvalPolicy = requiredString(keys, values, "approvalPolicy");
		if (!approvalPolicy.ok)
			return approvalPolicy.toParseOutcome("$.approvalPolicy");
		...
		if (!sandboxMode.ok)
			return sandboxMode.toParseOutcome("$.sandboxMode");
		...
		if (!writableRoots.ok)
			return writableRoots.toParseOutcome("$.writableRoots");

		final unsupported = CodexJson.unknownFields(object, ALLOWED_FIELDS);
		unsupported.sort(compareStrings);

		return ConfigProfileParseOutcome.success(new ConfigProfile(schema, profileName.value, model.value, modelProviderId, reasoningEffort,
			approvalPolicy.value, sandboxMode.value, webSearchEnabled, imageGenerationEnabled, developerInstructions, writableRoots.values, unsupported,
			hasSecretField(keys)));
	}
	...
	static function requiredString(keys:Array<String>, values:Array<Value>, name:String):ConfigStringField {
		...
		if (i < 0)
			return ConfigStringField.failure("missing_field", "required field is missing");
	...
	static function optionalStringArray(keys:Array<String>, values:Array<Value>, name:String):ConfigStringArrayField {
		final i = fieldIndex(keys, name);
		if (i < 0)
			return ConfigStringArrayField.success([]);
		return switch values[i] {
			case JArray(entries):
