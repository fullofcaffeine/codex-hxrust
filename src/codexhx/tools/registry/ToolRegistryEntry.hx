...
class ToolRegistryEntry {
	...
	public function new(kind:String, name:String, namespace:String, tool:String, rawServerName:String, rawToolName:String, description:String,
			inputSchema:Value, deferLoading:Bool, supportsParallelToolCalls:Bool, executable:Bool, connectorId:String, connectorName:String, pluginId:String) {
		this.kind = kind;
		this.name = name;
	...
	public static function localFunction(name:String, description:String, inputSchema:Value):ToolRegistryEntry {
		...
		return new ToolRegistryEntry(ToolKind.LocalFunction, callable, "", callable, "", name, description, inputSchema, false, false, true, "", "", "");
	}

	public static function mcp(serverName:String, rawToolName:String, description:String, inputSchema:Value, connectorId:String, connectorName:String,
			supportsParallelToolCalls:Bool):ToolRegistryEntry {
		final namespace = "mcp__" + sanitizeToolPart(serverName);
		final tool = sanitizeToolPart(rawToolName);
		return new ToolRegistryEntry(ToolKind.Mcp, namespace + "__" + tool, namespace, tool, serverName, rawToolName, description, inputSchema, false,
			supportsParallelToolCalls, false, connectorId, connectorName, "");
	}
	...
	public static function dynamicTool(namespace:String, name:String, description:String, inputSchema:Value, deferLoading:Bool):ToolRegistryEntry {
		...
		return new ToolRegistryEntry(ToolKind.Dynamic, modelName, cleanNamespace, cleanTool, "", name, description, inputSchema, deferLoading, false, false,
			"", "", "");
	}
	...
	public static function sanitizeToolPart(value:String):String {
		...
		if (out == "")
			out = "tool";
		if (isAsciiDigit(out.substr(0, 1)))
			out = "_" + out;
		if (out.length > 64)
			out = out.substr(0, 64);
		return out;
	...
	static function trimUnderscores(value:String):String {
		...
		while (start < end && value.substr(start, 1) == "_")
			start = start + 1;
		while (end > start && value.substr(end - 1, 1) == "_")
			end = end - 1;
