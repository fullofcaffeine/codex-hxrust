...
class ApplyPatchDryRun {
	public static function run(patch:String, mutationRequested:Bool):ApplyPatchDryRunOutcome {
		if (mutationRequested) {
			return ApplyPatchDryRunOutcome.failure("mutation_disabled", "$.mutationRequested",
				"apply-patch mutation is disabled by default; run dry-run first");
		}
		...
		if (lines[lines.length - 1] != "*** End Patch") {
			return ApplyPatchDryRunOutcome.failure("invalid_patch_footer", "$.patch.line[" + Std.string(lines.length) + "]",
			...
			if (starts(line, "*** Add File: ")) {
				...
				while (i < end) {
					...
					if (bodyLine == "*** End of File") {
						...
						return ApplyPatchDryRunOutcome.failure("invalid_update_line", linePath(i),
							"update hunks may only contain @@, +, -, context, or EOF marker lines");
					}
				}
