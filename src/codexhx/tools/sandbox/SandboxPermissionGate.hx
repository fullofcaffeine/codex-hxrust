...
class SandboxPermissionGate {
	...
	static function validMode(mode:String):Bool {
		return mode == "read-only" || mode == "workspace-write" || mode == "external-sandbox" || mode == "danger-full-access";
	}

	static function validOperation(operation:String):Bool {
		return operation == "read" || operation == "workspace-write" || operation == "exec" || operation == "network";
	}
