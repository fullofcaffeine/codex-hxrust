...
class GoalToolHandler {
    ...
    public function handle(toolName:String, argumentsJson:String):GoalToolCallOutcome {
        ...
        if (!parsed.ok) return GoalToolCallOutcome.failure(parsed.errorCode, parsed.errorMessage);
    ...
    function handleCreate(value:Value):GoalToolCallOutcome {
        ...
        if (!objective.ok) return GoalToolCallOutcome.failure("missing_objective", "goal objective is required");
        final budget = readOptionalInt(value, "token_budget");
        if (!budget.ok) return GoalToolCallOutcome.failure("invalid_goal_budget", "goal budgets must be positive when provided");

        final outcome = store.createGoal(objective.value, budget.present, budget.value, now);
        if (!outcome.ok) return failureFromOutcome(outcome);
    ...
    function handleUpdate(value:Value):GoalToolCallOutcome {
        ...
        if (!status.ok) return GoalToolCallOutcome.failure("missing_status", "goal status is required");
        if (status.value != ThreadGoalStatus.Complete && status.value != ThreadGoalStatus.Blocked) {
            return GoalToolCallOutcome.failure(
    ...
    static function completionBudgetReport(goal:ThreadGoal, include:Bool):String {
        if (!include || goal == null || goal.status != ThreadGoalStatus.Complete) return "null";
        if (!goal.hasTokenBudget && goal.timeUsedSeconds <= 0) return "null";
        return quote("Goal achieved. Report final usage from this tool result's structured goal fields. If `goal.tokenBudget` is present, include token usage from `goal.tokensUsed` and `goal.tokenBudget`. If `goal.timeUsedSeconds` is greater than 0, summarize elapsed time in a concise, human-friendly form appropriate to the response language.");
    }
    ...
    static function parseArguments(argumentsJson:String):ToolArgsRead {
        ...
        if (!parsed.ok) return ToolArgsRead.failure("invalid_tool_arguments", "goal tool arguments are not valid JSON");
    ...
    static function readString(value:Value, name:String):ToolStringRead {
        return switch value {
            case JObject(keys, values):
                ...
                if (i < 0) return ToolStringRead.failure();
                switch values[i] {
    ...
    static function readOptionalInt(value:Value, name:String):ToolOptionalIntRead {
        return switch value {
            case JObject(keys, values):
                final i = fieldIndex(keys, name);
                if (i < 0) return ToolOptionalIntRead.absent();
                switch values[i] {
