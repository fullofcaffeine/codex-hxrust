...
class TranscriptStateStore {
	...
	public static function stateJson(outcome:OneTurnSessionOutcome):String {
		return "{" + "\"assistantText\":" + JsonScalar.quote(outcome.assistantText) + ",\"eventCount\":" + Std.string(outcome.events.length)
			+ ",\"schema\":" + JsonScalar.quote(stateSchema) + ",\"streamId\":" + JsonScalar.quote(outcome.streamId) + ",\"terminalState\":"
			+ JsonScalar.quote(outcome.terminalState) + "}\n";
	}
