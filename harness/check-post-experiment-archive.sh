...
jq -e '
  ...
  and (.finalChecks | index("harness/check-replacement-go-no-go.sh") != null)
  and (.finalChecks | index("harness/check-state-backend-spike.sh") != null)
  and (.finalChecks | index("harness/check-tool-registry.sh") != null)
  and (.finalChecks | index("harness/check-haxe-rust-upstream-repros.sh") != null)
' "$ARCHIVE" >/dev/null
...
jq -e --slurpfile u "$UPSTREAM_REPROS" '
  ...
  and (.brewConversionNotes[] | select(.target == "haxe_rust_upstream_issues").note | test("zero generic haxe.rust expected-failure fixtures"))
  and $u[0].summary.totalRepros == 0
' "$ARCHIVE" >/dev/null
