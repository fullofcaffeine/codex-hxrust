...
| Optional anonymous-field read lowering | `open_upstream` | medium | runtime/model/session | Filed as haxe.rust `haxe.rust-i8li`; local active-turn error classifier materializes all draft fields through a typed default factory while optional anonymous-field semantics are fixed generically. |
| Guarded nullable primitive unwrap output shape | `open_upstream` | medium | runtime/model/session | TUI-LIVE-54 exposed a generated-Rust quality gap where `Null<Int>` narrowed by a guard/switch still emits a dead `Null Access` dynamic throw in the non-null branch. Track as generic haxe.rust lowering; codexhx keeps typed `Int` output and avoids app-side Dynamic/raw-Rust workarounds. |
...
- haxe.rust fixes have been generic and upstreamable so far; `haxe.rust-362`, `haxe.rust-ojj`, `haxe.rust-fzl`, `haxe.rust-3f0g`, `haxe.rust-fz20`, `haxe.rust-3oju`, `haxe.rust-akfm`, `haxe.rust-i8li`, and the guarded nullable primitive unwrap output-shape gap are the current open generic regressions from live-runtime/TUI/model-catalog/planning/request-envelope/stream-route/stream-item-reducer/async-contract/process-boundary work.
