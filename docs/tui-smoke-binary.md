...
- `TuiSmokeEventLoop` models a minimal raw Codex app-loop subset: draw, resize, resize draw scheduling, resize repaint/scrollback intent, inline viewport resize sync, suspend/resume viewport restoration, event-stream pause/resume intent, terminal mode/keyboard enhancement intent, alternate-screen enable/enter/leave/clear intent, legacy and resize-reflow draw/update composition intent, status update, input update, key dispatch, and explicit app exit.
