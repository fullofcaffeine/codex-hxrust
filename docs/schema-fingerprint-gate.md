...
- request params/responses for `thread/start`, `turn/start`, `turn/interrupt`, `thread/read`, `windowsSandbox/setupStart`, `windowsSandbox/readiness`, `account/login/start`, `account/login/cancel`, `account/logout`, `account/rateLimits/read`, `account/usage/read`, `account/sendAddCreditsNudgeEmail`, `feedback/upload`, `command/exec`, `command/exec/write`, `command/exec/terminate`, and `command/exec/resize`
- `process/spawn` is tracked from upstream Rust DTO/protocol source because upstream currently does not emit standalone `v2/ProcessSpawnParams.json` or `v2/ProcessSpawnResponse.json`; the fingerprint report records this as an explicit gap while process notification schemas remain selected
