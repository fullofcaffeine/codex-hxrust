...
**Bead:** HXCX-4.48 / `codex-hxrust-19q`
...
- `ModelStreamItemEventKind` represents selected `ResponseEvent` item-added, item-done, output text delta, reasoning summary delta, reasoning raw-content delta, and completion events.
- `ModelStreamRuntimeEventKind` captures the runtime-facing item started/completed, assistant delta, reasoning delta, raw reasoning delta, tool-call queued, stream completed, route denied, and reducer error outcomes.
- `ModelStreamItemReducerPolicy` maintains active item metadata, emits deltas against that item id, strips selected hidden assistant markup at completion, and queues tool calls for follow-up without executing them.

The fixture `fixtures/hxrust/model-stream-item-reducer.v1.json` covers OpenAI assistant text deltas and completion, OpenAI reasoning summary/raw deltas, Bedrock function tool call follow-up routing, Responses Lite custom tool call follow-up routing, inherited local envelope refusal, no live traffic, and secret-free summaries.

## Non-Goals

This is not live SSE parsing, WebSocket ownership, Tokio task ownership, tool execution, tool argument diff streaming, plan-mode proposed-plan item extraction, provider auth refresh, unauthorized retry handling, inference trace persistence, realtime/audio transport, or interactive TUI implementation.
