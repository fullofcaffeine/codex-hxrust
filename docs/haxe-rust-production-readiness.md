...
haxe.rust has handled the current pressure test better than a toy compiler would: 14 of 20 pressure gaps were resolved upstream, app/test Haxe source has zero raw Rust escape matches, and the helper, headless, runtime, TUI, persistence, thread-read, goal-tool, provider admission, model catalog, turn model/tool planning, model request envelope, model stream route, model stream item reducer, and selected Cafex fixture slices compile through generated Cargo gates. But broad replacement still has hard caveats:
...
| Input | Current value |
| --- | --- |
| haxe.rust pressure gaps | 20 total, 14 resolved upstream, 6 open upstream, 0 local workarounds |
| Generic upstream repros for remaining gaps | 0 expected-failure fixtures in `../haxe.rust` |
| Raw Rust escape pressure | 0 matches across 404 Haxe source/test files |
...
- `haxe.rust-362`, `haxe.rust-ojj`, `haxe.rust-fzl`, `haxe.rust-3f0g`, `haxe.rust-fz20`, and `haxe.rust-3oju` need product-neutral compiler regressions before broad replacement evidence can be called clean.
...
4. Treat `haxe.rust-362`, `haxe.rust-ojj`, `haxe.rust-fzl`, `haxe.rust-3f0g`, `haxe.rust-fz20`, and `haxe.rust-3oju` as open compiler risks until fixed upstream.
