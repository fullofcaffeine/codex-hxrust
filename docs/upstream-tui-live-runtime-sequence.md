...
### HXCX-TUI-329 Headless Raw Codex App-Server Thread Settings Updates

Status: HXCX-TUI-329 extends `fixtures/hxrust/tui-smoke.v1.json`, `test/TuiSmokeHarness.hx`, and `harness/check-tui-smoke.sh` with typed headless evidence for ChatWidget `ThreadSettingsUpdated` notification handling. Upstream anchors are `../codex/codex-rs/tui/src/chatwidget/tests/app_server.rs` `thread_settings_updated_updates_visible_state_without_transcript` and `thread_settings_updated_preserves_default_settings_for_plan_mode`. The fixture preserves visible-thread model, reasoning effort, service tier, approval policy, approvals reviewer, active permission profile, personality, and active collaboration mode updates; exclusion of transcript-history insertion; wrong-thread ignore behavior; Plan-mode default collaboration-mode preservation; default-mask restoration to the default model/reasoning settings; and no-live/no-app-server/no-ratatui/no-model rejection evidence. This is deterministic ChatWidget app-server notification evidence only, not live app-server transport, terminal mutation, ratatui rendering, persistent DB access, network traffic, model traffic, realtime transport, or adapter-specific behavior.

### HXCX-TUI-330 Headless Raw Codex App-Server Turn State

Status: HXCX-TUI-330 extends `fixtures/hxrust/tui-smoke.v1.json`, `test/TuiSmokeHarness.hx`, and `harness/check-tui-smoke.sh` with typed headless evidence for adjacent ChatWidget app-server turn-state behavior. Upstream anchors are `../codex/codex-rs/tui/src/chatwidget/tests/app_server.rs` `live_app_server_user_message_item_completed_does_not_duplicate_rendered_prompt`, `live_app_server_turn_completed_clears_working_status_after_answer_item`, and `live_app_server_turn_started_sets_feedback_turn_id`. The fixture preserves local prompt rendering after composer submit, matching completed user-message duplicate suppression, answer item history insertion before turn completion, task-running/status-widget cleanup after completed turn, active turn id capture for feedback submission, and no-live/no-app-server/no-ratatui/no-model rejection evidence. This is deterministic ChatWidget app-server turn-state evidence only, not live app-server transport, terminal mutation, ratatui rendering, persistent DB access, network traffic, model traffic, realtime transport, or adapter-specific behavior.

### HXCX-TUI-331 Headless Raw Codex App-Server Invalid URL Elicitation

Status: HXCX-TUI-331 extends `fixtures/hxrust/tui-smoke.v1.json`, `test/TuiSmokeHarness.hx`, and `harness/check-tui-smoke.sh` with typed headless evidence for ChatWidget app-server invalid URL elicitation handling. Upstream anchor is `../codex/codex-rs/tui/src/chatwidget/tests/app_server.rs` `invalid_url_elicitation_is_declined`. The fixture preserves visible-thread/request-thread separation, server/request/turn/elicitation ids, invalid URL rejection, `SubmitThreadOp` `ResolveElicitation` `Decline` targeting the request thread, empty content/meta facts, no visible-thread mutation, no app-link/browser view, and no-live/no-app-server/no-network/no-browser/no-model rejection evidence. This is deterministic ChatWidget app-server elicitation routing evidence only, not live app-server transport, browser launch, app-link rendering, terminal mutation, ratatui rendering, persistent DB access, network traffic, model traffic, realtime transport, or adapter-specific behavior.

### HXCX-TUI-332 Headless Raw Codex App-Server Collab Spawn Cached Settings

Status: HXCX-TUI-332 extends `fixtures/hxrust/tui-smoke.v1.json`, `test/TuiSmokeHarness.hx`, and `harness/check-tui-smoke.sh` with typed headless evidence for ChatWidget app-server collab-agent spawn completion fallback behavior. Upstream anchor is `../codex/codex-rs/tui/src/chatwidget/tests/app_server.rs` `collab_spawn_end_shows_requested_model_and_effort`. The fixture preserves in-progress spawn request caching, completion payload absence of model/reasoning effort, cached requested model/effort recovery, receiver nickname/role label rendering, spawn cache cleanup, history insertion, and no-live/no-tool/no-filesystem/no-network/no-ratatui/no-model rejection evidence. This is deterministic collab-agent history evidence only, not live app-server transport, agent spawning, process execution, terminal mutation, ratatui rendering, persistent DB access, network traffic, model traffic, realtime transport, or adapter-specific behavior.

### HXCX-TUI-333 Headless Raw Codex Inactive URL Elicitation Routing

Status: HXCX-TUI-333 extends `fixtures/hxrust/tui-smoke.v1.json`, `test/TuiSmokeHarness.hx`, and `harness/check-tui-smoke.sh` with typed headless evidence for app-level inactive-thread URL elicitation routing. Upstream anchors are `../codex/codex-rs/tui/src/app/tests.rs` `inactive_thread_url_elicitation_routes_to_app_link` and `inactive_thread_invalid_url_elicitation_is_declined`. The fixture preserves inactive thread id, server/request/turn/elicitation ids, HTTPS app-link request title/description/url/target facts, invalid HTTP rejection, `SubmitThreadOp` `ResolveElicitation` `Decline` targeting the inactive thread, empty content/meta facts, no visible-thread mutation, and no-live/no-app-server/no-network/no-browser/no-model rejection evidence. This is deterministic app-level request routing evidence only, not live app-server transport, browser launch, app-link rendering, terminal mutation, ratatui rendering, persistent DB access, network traffic, model traffic, realtime transport, or adapter-specific behavior.

### HXCX-TUI-334 Headless Raw Codex Inactive Approval Badge Clear

Status: HXCX-TUI-334 extends `fixtures/hxrust/tui-smoke.v1.json`, `test/TuiSmokeHarness.hx`, and `harness/check-tui-smoke.sh` with typed headless evidence for app-level inactive-thread approval badge cleanup. Upstream anchor is `../codex/codex-rs/tui/src/app/tests.rs` `inactive_thread_approval_badge_clears_after_turn_completion_notification`. The fixture preserves main/agent thread separation, inactive exec approval request facts, `Robie [explorer]` agent-navigation label propagation, pending approval badge visibility before completion, matching `TurnCompleted` notification clearing the badge immediately, active thread preservation, and no-live/no-terminal/no-ratatui/no-filesystem/no-network/no-model rejection evidence. This is deterministic app-level approval badge state evidence only, not live app-server transport, approval popup ownership, terminal mutation, ratatui rendering, persistent DB access, filesystem mutation, network traffic, model traffic, realtime transport, or adapter-specific behavior.

### HXCX-TUI-335 Headless Raw Codex Inactive Thread-Started Session Initialization

Status: HXCX-TUI-335 extends `fixtures/hxrust/tui-smoke.v1.json`, `test/TuiSmokeHarness.hx`, and `harness/check-tui-smoke.sh` with typed headless evidence for app-level inactive `ThreadStarted` session initialization. Upstream anchor is `../codex/codex-rs/tui/src/app/tests.rs` `inactive_thread_started_notification_initializes_replay_session`. The fixture preserves primary/main and inactive agent thread separation, rollout-derived agent model and cwd, agent model-provider id, primary approval-policy inheritance, primary workspace-root rebasing from `/tmp/main` to `/tmp/agent` while preserving `/tmp/shared`, rollout path capture, `Robie [explorer]` agent-navigation metadata, idle/non-closed navigation state, and no-live/no-app-server/no-filesystem/no-terminal/no-ratatui/no-network/no-model rejection evidence. This is deterministic session-inference state evidence only, not live app-server transport, rollout JSONL filesystem reads, Tokio channel ownership, terminal mutation, ratatui rendering, persistent DB access, network traffic, model traffic, realtime transport, or adapter-specific behavior.

### HXCX-TUI-336 Headless Raw Codex Inactive Thread-Started Missing-Path Model Fallback

Status: HXCX-TUI-336 extends `fixtures/hxrust/tui-smoke.v1.json`, `test/TuiSmokeHarness.hx`, and `harness/check-tui-smoke.sh` with typed headless evidence for app-level inactive `ThreadStarted` session initialization when the server thread has no rollout path. Upstream anchor is `../codex/codex-rs/tui/src/app/tests.rs` `inactive_thread_started_notification_preserves_primary_model_when_path_missing`. The fixture preserves main/agent thread separation, absent rollout path, primary model fallback to `gpt-primary`, agent model-provider id, primary approval-policy inheritance, cwd/root rebasing to `/tmp/agent`, `Robie [explorer]` agent-navigation metadata, idle/non-closed navigation state, and no-live/no-app-server/no-filesystem/no-terminal/no-ratatui/no-network/no-model rejection evidence. This is deterministic missing-path session fallback evidence only, not live app-server transport, rollout JSONL filesystem reads, Tokio channel ownership, terminal mutation, ratatui rendering, persistent DB access, network traffic, model traffic, realtime transport, or adapter-specific behavior.

### HXCX-TUI-337 Headless Raw Codex Thread-Read Permission Profile Fallback
...
### HXCX-TUI-339 Headless Raw Codex Side Fork Config Guardrails
...
### TUI-LIVE-5 Fake App-Server Session Attach

Status: TUI-LIVE-5 adds `codexhx.runtime.tui.appserver` as the first production-shaped app-server session seam for the minimal live shell. `FakeTuiAppServerFacade` owns typed pending attach requests, active `SessionId`/`ThreadId` state, stale attach response rejection, active-thread notification filtering, and a deterministic in-process event queue; `TuiAppServerEvent` and `TuiAppServerShellEffect` model session started, thread status, assistant delta, disconnect, request registration, stale rejection, event application/ignore, and redraw intent without `Dynamic` or trace-string pass criteria. The metal haxe.rust harness `harness/check-tui-fake-app-server-session.sh` proves typed request IDs, stale response rejection, shell state mutation, rendered frame facts, headless/live backend draw/restore, and generated Cargo check/test/run without credentials. This still does not open a socket, pump async app-server streams into the scheduler, submit prompts, persist state, execute tools, call models, or render full upstream ratatui history cells.

### TUI-LIVE-6 App-Server Event Pump Into Live Shell

Status: TUI-LIVE-6 adds `TuiAppServerEventPump`, `TuiAppServerPumpPolicy`, and `TuiAppServerPumpOutcome` as the typed bridge from fake app-server notifications into the live shell redraw path. The pump drains queued `TuiAppServerEvent` values, lets `FakeTuiAppServerFacade` mutate `ChatWidgetShellState`, translates `DrawRequested` shell effects into `TerminalRedrawScheduler` requests, renders `ChatWidgetShellRenderer`, and applies terminal operations through `TerminalSchedulerRunner`. The policy exposes synchronous lossless drain, bounded backpressure, and cancellation hooks without dropping events or pretending to own async channels yet. The metal haxe.rust harness `harness/check-tui-app-server-event-pump.sh` proves assistant delta, status update, disconnect handling, queued redraw coalescing, bounded backpressure, cancellation preservation, headless/live backend draw/restore, and generated Cargo check/test/run without credentials. This still does not open a socket, submit prompt envelopes, call models, persist state, execute tools, or implement the full upstream async event loop.

### TUI-LIVE-7 Prompt Submit Envelope From Live Composer
...
### TUI-LIVE-11 Runnable Live Shell Loop

Status: TUI-LIVE-11 adds `codexhx.runtime.tui.live` as the first bounded runnable shell loop over the production terminal and fake app-server seams. `TuiLiveShellRunner` owns setup, fake session attach, initial draw, typed terminal polling, scheduler dispatch for tick/draw/resize/exit events, composer input dispatch through `TuiAppServerEventPump`, semantic agent previous/next routing, empty-composer `q` exit, and terminal restore. `TuiLiveShellRunRequest`, `TuiLiveShellRunPolicy`, and `TuiLiveShellRunOutcome` keep the API typed and CI-bounded without fixture traces. The metal gate `harness/check-tui-live-shell-runner.sh` proves interpreter and generated Cargo check/test/run coverage for prompt echo, active-agent prompt routing, Esc/Ctrl-C/q exits, restore, and live backend no-TTY fallback. This still does not open a real app-server JSON-RPC transport, call a model, persist SQLite/log state, execute tools, or render full upstream ratatui widgets.

### TUI-LIVE-12 Interactive Live Shell Demo Entry Point

Status: TUI-LIVE-12 adds `TuiLiveShellDemoMain` and `hxml/tui-live-shell-demo.hxml` as a user-runnable generated Rust entrypoint for the fake-server live shell. `LiveTerminalBackend` now exposes typed poll-timeout configuration so the native crossterm poll can block briefly in a real terminal while remaining no-TTY-safe in CI. The demo uses the same `TuiLiveShellRunner` path as TUI-LIVE-11, with a long-running bounded policy, a fake primary session, and one fake demo agent for Alt+Left/Alt+Right switching. The metal gate `harness/check-tui-live-shell-demo.sh` proves timeout clamping, no-TTY setup/draw/restore completion, generated harness check/test/run, and generated demo binary check/run. This still does not attach real app-server JSON-RPC transport, call a model, persist SQLite/log state, execute tools, or render full upstream ratatui widgets.

### TUI-LIVE-13 Prompt Transport Seam

Status: TUI-LIVE-13 adds `TuiPromptTransport`, `TuiPromptTransportOutcome`, and `EchoTuiPromptTransport` as the shell-facing prompt transport seam. `FakeTuiAppServerFacade` still validates active session/thread state and still defaults to the same fake echo behavior, but prompt-submission response events now come from the injected transport rather than hardcoded facade enqueueing. The prompt-submit gate proves accepted echo events, transport rejection that preserves the request envelope while enqueueing no fake assistant events, headless/live redraw preservation, and generated metal Cargo check/test/run. This still does not open app-server JSON-RPC sockets, parse protocol JSON, call a model, persist state, or execute tools.

### TUI-LIVE-14 JSON-RPC Prompt Request Envelope

Status: TUI-LIVE-14 makes the default prompt transport produce a typed outbound JSON-RPC `turn/start` request before returning fake echo events. `TuiPromptJsonRpcMethod`, `TuiPromptTurnStartParams`, and `TuiPromptJsonRpcRequest` keep request id, method, active thread id, and text input modeled without `Dynamic`, `cast`, raw Rust, or smoke imports. `JsonRpcTuiPromptTransport` records the request and delegates to the existing fake echo response path, so the runnable demo behavior is unchanged while the submit boundary now has an app-server-shaped request object. The prompt-submit gate parses the recorded request through `AppProtocol.parseFixtureItem` and proves method/id/params JSON facts, echo event preservation, rejection preservation, headless/live redraw preservation, and generated metal Cargo check/test/run. This still does not open a socket, own app-server process lifecycle, call a model, persist state, or execute tools.

### TUI-LIVE-15 JSON-RPC Turn Start Response Envelope

Status: TUI-LIVE-15 extends the fake live prompt transport with a typed `turn/start` response result. `TuiPromptTurnStatus`, `TuiPromptTurnStartResponse`, and `TuiPromptJsonRpcResponse` model the selected upstream `TurnStartResponse` subset with a typed turn id, `inProgress` status, full items view, and empty items list. `TuiPromptTransportOutcome` now carries an optional typed response for accepted submissions, `EchoTuiPromptTransport` emits one, and `JsonRpcTuiPromptTransport` records a parser-compatible JSON-RPC response alongside the existing request while preserving the queued fake echo events. The prompt-submit gate proves response id/result JSON facts, parses the response through `AppProtocol.parseFixtureItem`, preserves rejection behavior, and keeps the live runner/demo generated gates green. This still does not open a socket, own app-server process lifecycle, call a model, persist state, or execute tools.

### TUI-LIVE-16 Fake JSON-RPC Prompt Exchange

Status: TUI-LIVE-16 adds `TuiPromptJsonRpcExchange`, `TuiPromptJsonRpcExchangeOutcome`, and `EchoTuiPromptJsonRpcExchange` underneath the shell-facing prompt transport. `JsonRpcTuiPromptTransport` now builds a typed `turn/start` request, sends it through the exchange, records the typed JSON-RPC response, and converts the exchange outcome back into `TuiPromptTransportOutcome` for the existing live shell. The prompt-submit gate proves exchange rejection separately from shell transport rejection: rejected exchanges preserve the outbound request, record no response, return a typed refusal, and enqueue no fake events. The live runner/demo generated gates remain unchanged. This still does not open a socket, own app-server process lifecycle, call a model, persist state, or execute tools.

### TUI-LIVE-17 JSON-RPC Turn Started Notification Envelope

Status: TUI-LIVE-17 adds `TuiPromptJsonRpcNotificationMethod` and `TuiPromptJsonRpcNotification` for the first fake app-server stream notification. `EchoTuiPromptJsonRpcExchange` now emits a typed `turn/started` notification with the active thread id and the same selected turn payload as the `turn/start` response, `TuiPromptJsonRpcExchangeOutcome` carries typed notifications, and `JsonRpcTuiPromptTransport` records them alongside the request and response. The prompt-submit gate parses the notification through `AppProtocol.parseFixtureItem` and proves exchange rejection records no response, no notifications, and no fake events. This still does not open a socket, own app-server process lifecycle, call a model, persist state, or execute tools.

### TUI-LIVE-18 JSON-RPC Turn Completed Notification Envelope

Status: TUI-LIVE-18 extends the fake prompt exchange stream with a typed `turn/completed` notification after `turn/started`. `TuiPromptTurnStatus` now includes `completed`, `TuiPromptJsonRpcNotificationMethod` includes `turn/completed`, and `EchoTuiPromptJsonRpcExchange` records start/completion notifications in order while preserving the same fake shell events. The prompt-submit gate parses both notifications through `AppProtocol.parseFixtureItem`, proves the shared turn id plus `inProgress` then `completed` statuses, and keeps exchange rejection at no response, no notifications, and no fake events. This still does not open a socket, own app-server process lifecycle, call a model, persist state, or execute tools.

### TUI-LIVE-19 JSON-RPC Turn Notification Shell Events

Status: TUI-LIVE-19 adds `TuiPromptJsonRpcNotificationProjector` so accepted fake JSON-RPC prompt submissions route `turn/started` and `turn/completed` notifications into the typed shell event stream. The default exchange now leaves the assistant echo as its fake stream event while the transport derives working and ready thread statuses from the notification objects, preserving the generated demo's visible echo behavior and making the shell consume the app-protocol-shaped stream surface. The prompt-submit gate proves the deterministic event order, parser-backed notifications, rejection no-response/no-notification/no-event behavior, and generated live shell gates. This still does not open a socket, own app-server process lifecycle, call a model, persist state, or execute tools.

### TUI-LIVE-20 JSON-RPC Agent Message Delta Shell Event

Status: TUI-LIVE-20 adds a typed `ItemId`, `TuiPromptAgentMessageDeltaNotification`, and ordered `TuiPromptJsonRpcStreamNotification` enum so the fake prompt exchange emits `turn/started`, `item/agentMessage/delta`, and `turn/completed` as app-server-shaped stream notifications. `JsonRpcTuiPromptTransport` records that ordered stream and projects it into working status, assistant delta, and ready status shell events, while retaining the turn-only notification accessors for existing assertions. The prompt-submit gate parses the delta fixture through `AppProtocol.parseFixtureItem`, proves deterministic stream/event order, keeps rejection at no response/no notifications/no stream events/no shell events, and compiles through haxe.rust. This still does not open a socket, own app-server process lifecycle, call a model, persist state, or execute tools.

### TUI-LIVE-21 JSON-RPC Agent Message Completed Notification

Status: TUI-LIVE-21 adds `TuiPromptAgentMessageCompletedNotification` for the assistant message `item/completed` lifecycle event. The fake prompt exchange now emits `turn/started`, `item/agentMessage/delta`, `item/completed`, and `turn/completed` in deterministic stream order. The projector treats completed assistant items as a shell no-op, so the visible shell event order remains working status, assistant delta, and ready status without duplicating transcript rows. The prompt-submit gate parses the completed item fixture through `AppProtocol.parseFixtureItem`, proves rejection records no stream notifications, and compiles through haxe.rust. This still does not open a socket, own app-server process lifecycle, call a model, persist state, or execute tools.

### TUI-LIVE-22 JSON-RPC Agent Message Started Notification

Status: TUI-LIVE-22 adds `TuiPromptAgentMessageStartedNotification` for the assistant message `item/started` lifecycle event. The fake prompt exchange now emits `turn/started`, `item/started`, `item/agentMessage/delta`, `item/completed`, and `turn/completed` in deterministic stream order. The projector treats started and completed assistant items as shell no-ops, so visible shell event order remains working status, assistant delta, and ready status without duplicating transcript rows. The prompt-submit gate parses the started item fixture through `AppProtocol.parseFixtureItem`, proves the full lifecycle stream order, and compiles through haxe.rust. This still does not open a socket, own app-server process lifecycle, call a model, persist state, or execute tools.

### TUI-LIVE-23 JSON-RPC Submitted User Message Completion

Status: TUI-LIVE-23 adds `TuiPromptUserMessageCompletedNotification` for the submitted prompt's `userMessage` `item/completed` lifecycle event. The fake prompt exchange now emits `turn/started`, user `item/completed`, assistant `item/started`, `item/agentMessage/delta`, assistant `item/completed`, and `turn/completed` in deterministic stream order. The projector treats the submitted user item as a shell no-op, preserving the single local prompt row already rendered by composer submit. The prompt-submit gate parses the user item fixture through `AppProtocol.parseFixtureItem`, proves the no-duplicate transcript row behavior, and compiles through haxe.rust. This still does not open a socket, own app-server process lifecycle, call a model, persist state, or execute tools.

### TUI-LIVE-24 JSON-RPC Raw Response Item Completion

Status: TUI-LIVE-24 adds `TuiPromptRawResponseItemCompletedNotification` for the assistant raw response `rawResponseItem/completed` lifecycle event. The fake prompt exchange now emits `turn/started`, user `item/completed`, assistant `item/started`, `item/agentMessage/delta`, `rawResponseItem/completed`, assistant `item/completed`, and `turn/completed` in deterministic stream order. The projector treats the raw response item as a shell no-op, preserving visible shell event order while making the transport stream closer to upstream app-server output. The prompt-submit gate parses the raw response item fixture through `AppProtocol.parseFixtureItem` and compiles through haxe.rust. This still does not open a socket, own app-server process lifecycle, call a model, persist state, or execute tools.

### TUI-LIVE-25 JSON-RPC Thread Status Changed Stream Notifications

Status: TUI-LIVE-25 adds `TuiPromptThreadStatusChangedNotification` for the prompt stream's `thread/status/changed` active and idle notifications. The fake prompt exchange now emits active thread status before `turn/started` and idle thread status after `turn/completed`, while keeping the existing user, assistant, and raw-response item ordering intact. The projector treats thread-status stream notifications as shell no-ops because turn start/completion still own visible working/ready status in the minimal shell. The prompt-submit gate parses both status fixtures through `AppProtocol.parseFixtureItem` and compiles through haxe.rust. This still does not open a socket, own app-server process lifecycle, call a model, persist state, or execute tools.

### TUI-LIVE-26 Ordered JSON-RPC Prompt Frames

Status: TUI-LIVE-26 adds `TuiPromptJsonRpcFrame` and records a socket-shaped prompt transcript on `JsonRpcTuiPromptTransport`: outbound `turn/start` request, inbound `turn/start` response, then each inbound stream notification in deterministic delivery order. The prompt-submit gate proves the accepted frame count/order/message JSON and proves rejected exchanges keep only the outbound request frame while recording no response or stream frames. This keeps trace strings out of the production-facing transport seam and gives later real socket work a typed ordering contract. This still does not open a socket, own app-server process lifecycle, call a model, persist state, or execute tools.

### TUI-LIVE-27 JSON-RPC Prompt Wire Records

Status: TUI-LIVE-27 adds typed newline-oriented wire-record views for the prompt frame log. `TuiPromptJsonRpcFrameDirection`, `TuiPromptJsonRpcFrameKind`, `TuiPromptJsonRpcFrameRecord`, and `TuiPromptJsonRpcFrameCodec` derive outbound/inbound direction, request/response/notification kind, method text, message JSON, and JSON-lines payloads from the existing typed frame enum. `JsonRpcTuiPromptTransport` exposes ordered records and JSONL while preserving shell projection behavior; rejected exchanges expose only the outbound request record. The prompt-submit gate asserts directions, kinds, methods, lines, and JSONL payloads through interpreter and generated Rust. This still does not open a socket, own app-server process lifecycle, call a model, persist state, or execute tools.

### TUI-LIVE-28 JSON-RPC Prompt Response Correlation

Status: TUI-LIVE-28 adds `TuiPromptJsonRpcCorrelationStatus` and `TuiPromptJsonRpcFrameCorrelation` for typed request/response matching on the prompt frame log. `JsonRpcTuiPromptTransport` now rejects accepted exchange responses whose JSON-RPC response id does not match the outbound request id before storing an accepted response, recording stream notifications, projecting shell events, or queueing fake exchange events. The diagnostic frame and wire-record log still keeps the outbound request and mismatched inbound response. The prompt-submit gate proves matched, rejected, and mismatched paths through interpreter and generated Rust. This still does not open a socket, own app-server process lifecycle, call a model, persist state, or execute tools.

### TUI-LIVE-29 JSON-RPC Prompt Stream Scope

Status: TUI-LIVE-29 adds `TuiPromptJsonRpcStreamScopeStatus` and `TuiPromptJsonRpcStreamScopeReport` for typed validation of prompt stream notifications against the submitted thread id and accepted turn id. `JsonRpcTuiPromptTransport` now rejects accepted exchanges whose stream notification thread or turn scope does not match before storing accepted response state, recording stream notifications, projecting shell events, or queueing fake exchange events. Diagnostic frame and wire-record logs retain the request, response, and offending stream notification. The prompt-submit gate proves matched, rejected, wrong-thread, and wrong-turn paths through interpreter and generated Rust. This still does not open a socket, own app-server process lifecycle, call a model, persist state, or execute tools.

### TUI-LIVE-30 Prompt Pending Request Lifecycle

Status: TUI-LIVE-30 adds `TuiPromptPendingRequestStatus` and `TuiPromptPendingRequestLifecycle` so prompt submit uses the same facade pending-request map as session attach. `FakeTuiAppServerFacade` now stores a `PromptSubmit` pending request before calling the transport, resolves it on accepted transport completion, and rejects/removes it on transport refusal. The prompt-submit gate proves accepted, transport-rejected, stale-response, wrong-thread, and wrong-turn paths all leave no stranded pending prompt request. This still does not open a socket, own app-server process lifecycle, call a model, persist state, or execute tools.

### TUI-LIVE-31 Prompt Turn Lifecycle Completion

Status: TUI-LIVE-31 adds `TuiPromptTurnLifecycleStatus` and `TuiPromptTurnLifecycleReport` for typed validation that an accepted prompt stream contains both `turn/started` and `turn/completed` for the selected turn. `JsonRpcTuiPromptTransport` now rejects otherwise correlated and in-scope streams with typed lifecycle refusals such as `missing_started` or `missing_completed` before recording accepted response state, accepted stream notifications, or shell events. The prompt-submit gate proves complete, missing-started, missing-completed, stale-response, wrong-thread, and wrong-turn paths through interpreter and generated Rust. This still does not open a socket, own app-server process lifecycle, call a model, persist state, or execute tools.

### TUI-LIVE-32 App-Server JSON-RPC Transport Contract

Status: TUI-LIVE-32 adds `TuiAppServerJsonRpcTransport`, `TuiAppServerJsonRpcTransportOutcome`, `TuiAppServerJsonRpcTransportStatus`, and `FakeTuiAppServerJsonRpcTransport` so the prompt path depends on an app-server transport contract instead of a direct fake exchange. The fake exchange remains as the credential-free implementation behind that boundary, while `JsonRpcTuiPromptTransport` now handles accepted, rejected, disconnected, missing-response, stale-response, wrong-scope, and incomplete-lifecycle outcomes before shell event projection. The prompt-submit gate proves disconnect-before-response and missing-response transport failures queue no shell events, and generated Rust for the new boundary was inspected for warning-clean module shape and avoidable helper-surface churn. This still does not open a socket, own app-server process lifecycle, call a model, persist state, or execute tools.

### TUI-LIVE-33 App-Server Transport Frame Transcript

Status: TUI-LIVE-33 adds `TuiAppServerJsonRpcTransportTranscript` so the app-server JSON-RPC transport contract owns outbound request and inbound response/stream frame evidence. `FakeTuiAppServerJsonRpcTransport` now returns accepted, rejected, and disconnected outcomes with transcripts, and `JsonRpcTuiPromptTransport` consumes transcript frames for correlation, scope, lifecycle, diagnostic frame, and wire-record state instead of reconstructing them locally. The prompt-submit gate proves accepted transcript frame ownership, disconnect and missing-response outbound-only transcripts, stale response and stream rejection diagnostics, and generated Rust shape for direct `frameAt` lookup. This still does not open a socket, own app-server process lifecycle, call a model, persist state, or execute tools.

### TUI-LIVE-34 App-Server JSON-RPC Wire Session

Status: TUI-LIVE-34 adds `TuiAppServerJsonRpcWireSession`, `TuiAppServerJsonRpcWireOutcome`, and `FakeTuiAppServerJsonRpcWireSession` underneath the app-server prompt transport. `FakeTuiAppServerJsonRpcTransport` now writes an outbound sequence-0 `TuiPromptJsonRpcFrameRecord` into the wire session and receives typed inbound response/stream records before building its transport transcript. The fake wire session validates outbound direction, kind, request frame, and JSONL line text before delegating to the existing credential-free exchange; accepted inbound records are sequenced from 1 so a later process/socket transport can preserve the same boundary. The prompt-submit gate proves inbound record sequencing, mismatched outbound rejection, transport-to-wire handoff, and generated Rust output inspection. This still does not open a socket, spawn an app-server process, call a model, persist state, or execute tools.

### TUI-LIVE-35 App-Server JSON-RPC Line Transport

Status: TUI-LIVE-35 adds `TuiAppServerJsonRpcLineTransport`, `TuiAppServerJsonRpcLineOutcome`, and `FakeTuiAppServerJsonRpcLineTransport` below the wire session. The fake line transport validates the outbound JSONL request line, delegates to the credential-free exchange, and returns raw inbound JSONL lines for the response and stream notifications. `FakeTuiAppServerJsonRpcWireSession` now consumes those raw lines, rebuilds typed inbound records, and rejects mismatches between typed record lines and raw line evidence before handing records to the transport transcript. The prompt-submit gate proves raw inbound line emission, mismatched outbound line rejection, mismatched inbound line rejection, generated Rust output inspection, and the existing transport/session behavior. This still does not open a socket, spawn an app-server process, call a model, persist state, or execute tools.

### TUI-LIVE-36 App-Server Line Transport Lifecycle

Status: TUI-LIVE-36 adds `TuiAppServerJsonRpcLineTransportState` and `TuiAppServerJsonRpcLineCloseReport` to the line transport boundary. `FakeTuiAppServerJsonRpcLineTransport` now starts open, tracks outbound and inbound line counts, closes with a typed report, and rejects send attempts after close as a disconnected `line_transport_closed` outcome without mutating counters. The prompt-submit gate proves open state, accepted line accounting, close reporting, closed state, send-after-close refusal, generated Rust output inspection, and the existing line/wire/transport behavior. This still does not open a socket, spawn an app-server process, call a model, persist state, or execute tools.

### TUI-LIVE-37 App-Server Line Transport Transcript

Status: TUI-LIVE-37 adds `TuiAppServerJsonRpcLineTranscript` to the line transport outcome so raw JSONL evidence is owned at the line boundary. `FakeTuiAppServerJsonRpcLineTransport` now attaches outbound+inbound transcripts to accepted exchanges, outbound-only transcripts to post-write rejections, and empty transcripts to pre-write or closed refusals. The prompt-submit gate proves accepted transcript lines, post-write rejection evidence, pre-write mismatch emptiness, send-after-close emptiness, generated Rust output inspection, and the existing line/wire/transport behavior. This still does not open a socket, spawn an app-server process, call a model, persist state, or execute tools.

### TUI-LIVE-38 App-Server Process Line Endpoint

Status: TUI-LIVE-38 adds typed future endpoint plans for the app-server JSON-RPC line transport. `TuiAppServerJsonRpcLineEndpoint` models stdio process launch, TCP socket, and explicitly unsupported endpoint variants without a stringly kind field; `TuiAppServerJsonRpcProcessLaunchPlan` and `TuiAppServerJsonRpcProcessEnvVar` own command, argv, cwd, and env values; and `TuiAppServerJsonRpcLineEndpointReport` validates ready, invalid, and unsupported plans. The prompt-submit gate proves stdio command/env/cwd shape, argv/env copy semantics, socket host/port validation, invalid command/env/socket rejection, unsupported endpoint reporting, generated Rust output inspection, and the existing line/wire/transport behavior. This still does not open a socket, spawn an app-server process, call a model, persist state, or execute tools.

### TUI-LIVE-39 App-Server Line Open Intent

Status: TUI-LIVE-39 adds `TuiAppServerJsonRpcLineOpenIntent`, `TuiAppServerJsonRpcLineOpenIntentKind`, and `TuiAppServerJsonRpcLineOpenIntentReport` to project validated app-server line endpoints into typed native boundary actions. Stdio endpoints become `spawn_stdio` intents, TCP endpoints become `connect_tcp` intents, and invalid or unsupported endpoints become refusal reports carrying the endpoint validation code. The prompt-submit gate proves stdio spawn intent, TCP connect intent, invalid command refusal, unsupported endpoint refusal, generated Rust output inspection, and the existing line/wire/transport behavior. This still does not open a socket, spawn an app-server process, call a model, persist state, or execute tools.

### TUI-LIVE-40 App-Server Line Open Outcome

Status: TUI-LIVE-40 adds `TuiAppServerJsonRpcLineNativeOpener`, `DryRunTuiAppServerJsonRpcLineNativeOpener`, `TuiAppServerJsonRpcLineOpenOutcome`, and `TuiAppServerJsonRpcLineOpenOutcomeStatus` to model the native-open result for app-server JSON-RPC line connections. The dry-run opener maps actionable stdio/socket intents to opened outcomes with deterministic connection labels and connection indices, while invalid or unsupported intents remain typed refusals and do not increment the opened count. The prompt-submit gate proves stdio opened outcome, socket opened outcome, invalid refusal, unsupported refusal, opened count stability, generated Rust output inspection, and the existing line/wire/transport behavior. This still does not open a socket, spawn an app-server process, call a model, persist state, or execute tools.

### TUI-LIVE-41 App-Server Line Transport Attachment

Status: TUI-LIVE-41 adds `TuiAppServerJsonRpcLineTransportAttacher`, `DryRunTuiAppServerJsonRpcLineTransportAttacher`, `TuiAppServerJsonRpcLineTransportAttachment`, `TuiAppServerJsonRpcLineTransportAttachmentReport`, and `TuiAppServerJsonRpcLineAttachmentStatus` to bind native-open outcomes to the line-transport layer. Ready attachments carry opened connection metadata and can materialize a fake line transport through the dry-run attacher; refused attachments preserve invalid or unsupported open outcomes without producing a transport. The prompt-submit gate proves stdio attachment readiness, socket attachment readiness, invalid refusal, unsupported refusal, sending a prompt line through the materialized fake transport, generated Rust output inspection, and the existing line/wire/transport behavior. This still does not open a socket, spawn an app-server process, call a model, persist state, or execute tools.

### TUI-LIVE-42 App-Server Line Endpoint Connector

Status: TUI-LIVE-42 adds `TuiAppServerJsonRpcLineConnector`, `DryRunTuiAppServerJsonRpcLineConnector`, `TuiAppServerJsonRpcLineConnectReport`, and `TuiAppServerJsonRpcLineConnectStatus` to compose endpoint validation, open-intent projection, dry-run native open, attachment, and fake transport materialization into one endpoint-to-line-transport pipeline. Ready stdio/socket endpoints produce structured connection reports and can materialize a fake line transport through the connector; invalid or unsupported endpoints remain typed refusals with endpoint, intent, open, and attachment status preserved. The prompt-submit gate proves stdio connection, socket connection, invalid refusal, unsupported refusal, sending a prompt line through the connector-materialized fake transport, generated Rust output inspection, and the existing line/wire/transport behavior. This still does not open a socket, spawn an app-server process, call a model, persist state, or execute tools.

### TUI-LIVE-43 Connector-Backed Prompt Transport

Status: TUI-LIVE-43 adds `DryRunTuiAppServerJsonRpcLineConnectedTransport`, a `TuiAppServerJsonRpcTransport` implementation backed by the dry-run line connector. It connects the configured endpoint, materializes a fake line transport only after a ready connect report, sends the prompt JSONL line through that transport, converts accepted line outcomes back into transport outcomes, and preserves the connect report plus line outcome for diagnostics. The prompt-submit gate proves `JsonRpcTuiPromptTransport` over the connector-backed transport, accepted frame/wire/correlation/lifecycle state, invalid endpoint refusal before line send, generated Rust output inspection, and the existing line/wire/transport behavior. This still does not open a socket, spawn an app-server process, call a model, persist state, or execute tools.

### TUI-LIVE-44 Connector-Backed Transport Cleanup
...
### TUI-LIVE-46 Connector Transport Attempt Report

Status: TUI-LIVE-46 adds `TuiAppServerJsonRpcLineTransportAttemptReport`, a scalar typed diagnostic value for connector-backed app-server JSON-RPC prompt attempts. `DryRunTuiAppServerJsonRpcLineConnectedTransport` records the report for accepted sends, post-write line exchange rejection, missing line transport, and invalid endpoint refusal. The prompt-submit gate proves connect status/code, transport materialization, line outcome presence/status/code, close presence/state/code, close-time line counts, generated Rust output inspection, and the existing connector-backed prompt behavior. This still does not open a socket, spawn an app-server process, call a model, persist state, or execute tools.

### TUI-LIVE-47 Line Connector Materialization Contract

Status: TUI-LIVE-47 promotes `transportFor` onto the `TuiAppServerJsonRpcLineConnector` interface so connector-backed prompt transport materializes line transports through the typed connector contract rather than the dry-run concrete class. The contract returns `Null<TuiAppServerJsonRpcLineTransport>` because a failed materialization is explicit connector state; generated Rust now lowers this path as nullable `HxDynRef` instead of an invalid trait-object `Default::default()`. The prompt-submit gate proves accepted sends, post-write rejection cleanup, invalid endpoint refusal, attempt-report preservation, generated Rust output inspection, and the existing connector-backed prompt behavior. The slice exposed and landed the generic haxe.rust `interface_return_null` fix in `../haxe.rust` at `bf0a2d17`, with `nullable_interface_null` and `interface_basic` snapshots still green. This still does not open a socket, spawn an app-server process, call a model, persist state, or execute tools.

### TUI-LIVE-48 Live Shell Line Transport Selection

Status: TUI-LIVE-48 lets `TuiLiveShellRunRequest` select a concrete `JsonRpcTuiPromptTransport` or build a dry-run connector-backed line transport from a typed `TuiAppServerJsonRpcLineEndpoint` without manually replacing the whole `FakeTuiAppServerFacade`. The default live shell remains credential-free and fake-backed, while the runner gate now proves a headless prompt submit can travel through `DryRunTuiAppServerJsonRpcLineConnectedTransport`, render the assistant echo, and preserve typed line transport attempt and close diagnostics. Generated Rust output inspection keeps the request surface free of stored interface handles, `Default::default()` trait-object fallbacks, raw Rust, `Dynamic`, `cast`, and smoke imports. This still does not open a socket, spawn an app-server process, call a model, persist state, or execute tools.

### TUI-LIVE-49 Live Shell Demo Line Transport Mode

Status: TUI-LIVE-49 adds `TuiLiveShellDemoConfig` and `TuiLiveShellDemoTransportMode` so the user-runnable generated live shell demo can select the existing dry-run connector-backed JSONL line transport from typed CLI-derived configuration while preserving the fake default. The demo accepts a fake mode, a line-stdio mode, stdio command/cwd/arg/env values, and a deterministic line rejection code; the harness proves default parsing, line plan parsing, invalid argument refusal, no-TTY fake completion, and a headless demo prompt submitted through the selected line transport. This still does not open a socket, spawn an app-server process, call a model, persist state, or execute tools.

### TUI-LIVE-50 Live Shell Demo Scripted Prompt Mode

Status: TUI-LIVE-50 adds a typed `--scripted-prompt=...` option to the user-runnable generated live shell demo. Scripted mode converts the CLI prompt into `TerminalEvent.Key` values through `TuiLiveShellDemoMain.scriptedBackend`, selects a headless terminal setup, submits the prompt through the same live-shell runner, and exits after a bounded idle window. The harness proves config parsing, scripted dry-run line transport submission, generated binary output that reports `transport=line_stdio` and `prompts=1`, and focused generated Rust source inspection for the touched config/transport surfaces. The broader generated demo main exposed the generic haxe.rust fresh concrete interface-upcast null guard tracked in `codex-hxrust-yg1a`; haxe.rust `2577c5b7` resolves it with a product-neutral lowering fix, and generated live-shell demo inspection no longer shows the fresh-upcast `Null Access` guard. This still does not open a socket, spawn an app-server process, call a model, persist state, or execute tools.

### TUI-LIVE-51 App-server Line Connector Injection

Status: TUI-LIVE-51 lets `DryRunTuiAppServerJsonRpcLineConnectedTransport` receive an explicit `TuiAppServerJsonRpcLineConnector` while keeping the existing dry-run connector as the default factory. `TuiLiveShellRunRequest.withLineConnectedPromptTransportUsingConnector()` routes the same injection through the minimal shell. The prompt-submit harness proves injected connector acceptance, pre-connect refusal, missing transport materialization, close cleanup, and typed attempt reports; the live-shell runner harness proves scripted prompt submission through the injected connector path. Generated Rust inspection confirms the touched connector surfaces are free of `Null Access`, `Dynamic`, raw Rust escapes, and panic/todo stubs. This still does not spawn an app-server process, open a socket, call a model, persist state, or execute tools; it is the typed seam needed before those live connectors replace dry-run transport.

### TUI-LIVE-52 App-server Line Connector Boundary Injection

Status: TUI-LIVE-52 moves the connector internals onto injected typed boundaries. `DryRunTuiAppServerJsonRpcLineConnector` now takes non-null `TuiAppServerJsonRpcLineNativeOpener` and `TuiAppServerJsonRpcLineTransportAttacher` implementations, keeps `dryRun()` as the default factory, stores the most recent attachment for the synchronous prompt path, and delegates transport materialization through `TuiAppServerJsonRpcLineTransportAttacher.transportFor()`. The prompt-submit harness proves injected opener/attacher acceptance, injected native-open refusal, missing attached transport materialization, connector-backed prompt routing, and generated Rust output with no optional interface constructor `Null Access`; the live-shell runner gate stays green with the same connector path. This still does not spawn an app-server process, open a socket, call a model, persist state, or execute tools; it is the final dry-run seam before a real stdio/TCP opener and transport implementation can replace the boundary.

### TUI-LIVE-53 App-server Line Open Payload Preservation

Status: TUI-LIVE-53 preserves the typed app-server line open payload that future transport attachers need. `TuiAppServerJsonRpcLineOpenOutcome` now keeps the actionable stdio `TuiAppServerJsonRpcProcessLaunchPlan` or TCP host/port alongside its diagnostic report, and `TuiAppServerJsonRpcLineTransportAttachment` exposes that payload directly. The prompt-submit harness proves stdio command/args/cwd/env carry-through, TCP host/port carry-through, attachment-level payload access, and non-actionable refused outcomes; the live-shell runner gate stays green with the same connector path. The focused generated Rust output for the open outcome and attachment has no `Default::default()` enum fallback, `Null Access`, raw dynamic calls, or panic/todo stubs. This still does not spawn an app-server process, open a socket, call a model, persist state, or execute tools; it removes the last report-reconstruction gap before a real stdio/TCP transport can materialize from typed data.

### TUI-LIVE-54 App-server Stdio Line Process Runner
...
Status: TUI-LIVE-56 adds `ProcessBackedTuiAppServerJsonRpcLineTransportAttacher`, a typed `TuiAppServerJsonRpcLineTransportAttacher` implementation that materializes `ProcessBackedTuiAppServerJsonRpcLineTransport` from ready stdio launch-plan attachments while preserving the dry-run fake attacher as the default connector path. The prompt-submit harness proves connector-backed accepted prompt submission through a real child-process JSONL echo, typed connect/attempt/close diagnostics, wire-record correlation, and decoder rejection propagation for unknown process output.
...
### TUI-LIVE-59 Persistent Connector-backed Prompt Transport

Status: TUI-LIVE-59 adds `PersistentStdioTuiAppServerJsonRpcLineTransportAttacher` and `PersistentTuiAppServerJsonRpcLineConnectedTransport`, lifting the persistent stdio session into the connector-backed JSON-RPC prompt path. The persistent transport connects and materializes one line transport lazily on first send, reuses it across multiple `sendPrompt` calls, records per-send line outcomes and attempt reports without closing between sends, and exposes an explicit close report for final cleanup. The prompt-submit harness proves two `FakeTuiAppServerFacade.submitPrompt` calls through one shell-backed stdio session, typed connect/attempt diagnostics, aggregate close counts, after-close disconnection, interpreter execution, generated Rust Cargo checks/tests, generated binary execution, and focused generated-Rust inspection with no raw Rust escapes, dynamic calls, panic/todo stubs, `Null Access`, `unsafe`, or `unwrap()` matches in the two new modules.
...
### TUI-LIVE-75 Submitted Turn Drain Readiness-Aware No-Data Stop

Status: TUI-LIVE-75 adds typed no-data/readiness evidence to the
composer-triggered submitted-turn late JSONL drain. `TuiAppServerJsonRpcLateJsonlBatch`
can now report `not_ready`; `TuiPromptSubmittedTurnLateJsonlPumpResult` maps it
to `no_data`; and the bounded drain reports `TuiPromptSubmittedTurnLateJsonlDrainStatus.NoData`
instead of treating an empty wait as a disconnect or batch failure.

The prompt-submit gate proves a composer prompt can be admitted through the
same facade, prompt transport, persistent JSON-RPC transport, and event-pump
path, then stop the late drain immediately with structured no-data evidence.
The active submitted turn remains active, the transcript only contains the user
row, and completion count stays unchanged. Existing completion, max-bound,
prefix-applied rejection, line-disconnect, unsupported notification, and
stale-after-interrupt paths remain covered by the same harness.

This is still deterministic, synchronous, bounded, and credential-free. It
models a readiness/no-data boundary for the current drain loop; it does not own
...
### TUI-LIVE-78 Submitted Turn Readiness Trigger Coalesces Duplicates

Status: TUI-LIVE-78 adds a typed readiness interaction status around repeated
submitted-turn late JSONL readiness events. `TuiAppServerReadinessInteraction`
now reports `Drained` when a readiness event attempted the bounded late JSONL
drain and `NoPendingSubmittedTurn` when a duplicate readiness event arrives
after the submitted turn has already completed and cleared `activeTurn`.

The prompt-submit gate proves both sides. A repeated readiness event while the
transport still reports `not_ready` / `no_data` keeps the active turn stable,
does not record a completion, and does not append a second transcript row. A
duplicate readiness event after assistant delta plus completion have already
been pumped returns typed no-op evidence, performs no second drain, leaves
`activeTurn` cleared, and preserves the single assistant row and single
completion record. Existing readiness resume, direct no-data, direct resume,
normal completion, max-bound, prefix-applied rejection, line-disconnect,
unsupported notification, and stale-after-interrupt paths remain covered.

This is still deterministic, synchronous, bounded, and credential-free. It
models readiness coalescing around the current drain loop; it does not own real
async socket polling, Tokio readiness, provider streaming, model calls, tool
execution, process teardown, or persistence.
...
### TUI-LIVE-80 Readiness Backpressure Resumes Through Scheduler Event

Status: TUI-LIVE-80 adds `TuiAppServerPumpEvent.DrainQueuedEvents` as the typed
event-pump input for recovering after readiness backpressure. The event maps to
the existing app-server pump drain, but it gives the scheduler/readiness path a
named typed trigger instead of requiring tests to call `pump.drain()` directly.

The prompt-submit gate keeps the TUI-LIVE-79 setup: a
`SubmittedTurnLateJsonlReady` event drains late JSONL, a bounded pump applies
only the assistant delta, and completion/status events remain queued. A later
`DrainQueuedEvents` pump event resumes the preserved queue, clears
`activeTurn`, records exactly one completion, and does not duplicate the
assistant transcript row. Existing no-data, readiness resume, duplicate no-op,
readiness backpressure, normal completion, max-bound, prefix-applied rejection,
line-disconnect, unsupported notification, and stale-after-interrupt paths
remain covered.

This is still deterministic, synchronous, bounded, and credential-free. It
models scheduler/pump recovery around the current readiness/drain loop; it does
...
### TUI-LIVE-83 Readiness-Triggered Backpressure Recovery Routes Through Live Shell Runner

Status: TUI-LIVE-83 extends the live-shell runner readiness path with
runner-owned backpressure recovery evidence. `TuiLiveShellRunOutcome` now
separates readiness-triggered backpressure from explicit pump-event
backpressure, and records the active submitted turn observed immediately after
readiness handling. Queued `TuiAppServerPumpEvent` triggers wait until there is
queued app-server work, so a recovery trigger queued before prompt submission is
consumed only after readiness has created a backpressured queue.

The live-shell runner gate proves a `SubmittedTurnLateJsonlReady` readiness
event can drain late assistant/completion JSONL under a bounded
`TuiAppServerPumpPolicy`, apply only the assistant delta during the readiness
pass, preserve the active submitted turn while completion remains queued, then
resume through an explicit runner-owned `DrainQueuedEvents` pass that clears the
completion exactly once. Existing readiness routing, pump-event routing,
terminal setup/restore, text submit, line transport, agent navigation, resize,
tick, Ctrl-C, q, and live-backend no-TTY paths remain covered.

This is still deterministic, synchronous, bounded, and credential-free. It
models live-runner backpressure recovery around the current scheduler/drain
loop; it does not own real async socket polling, Tokio readiness, provider
streaming, model calls, tool execution, process teardown, or persistence.

### TUI-LIVE-84 No-Data Readiness Retry Routes Through Live Shell Runner

Status: TUI-LIVE-84 extends the live-shell runner readiness path with
runner-owned no-data retry evidence. `TuiLiveShellRunOutcome` now records
readiness no-data counts and the active submitted turn observed immediately
after a no-data readiness stop, so a later successful readiness drain does not
overwrite the first retry boundary evidence.

The live-shell runner gate proves an initial `SubmittedTurnLateJsonlReady`
event can stop with typed `no_data` / `late_jsonl_not_ready` evidence without
clearing the active submitted turn, then a later runner-owned readiness event
can drain assistant/completion JSONL through the same path, render the assistant
row, and clear the completion exactly once. Existing readiness backpressure
recovery, readiness routing, pump-event routing, terminal setup/restore, text
submit, line transport, agent navigation, resize, tick, Ctrl-C, q, and
live-backend no-TTY paths remain covered.

This is still deterministic, synchronous, bounded, and credential-free. It
models live-runner no-data readiness retry around the current scheduler/drain
loop; it does not own real async socket polling, Tokio readiness, provider
streaming, model calls, tool execution, process teardown, or persistence.

### TUI-LIVE-85 Duplicate Post-Completion Readiness No-Ops Through Live Shell Runner

Status: TUI-LIVE-85 extends the live-shell runner readiness path so duplicate
post-completion readiness events route through the typed no-pending outcome
instead of being left unobserved. `TuiLiveShellRunner` still holds readiness
events before a submitted turn is pending, but after a submitted-turn readiness
route has opened it can consume later readiness events even if the active turn
has just cleared.

The live-shell runner gate proves a no-data readiness retry can complete a
submitted turn, then a third `SubmittedTurnLateJsonlReady` event routes as
`TuiAppServerReadinessInteractionStatus.NoPendingSubmittedTurn` without reading
an extra late JSONL line, duplicating assistant transcript rows, or incrementing
completion counts. Existing readiness no-data retry, readiness backpressure
recovery, readiness routing, pump-event routing, terminal setup/restore, text
submit, line transport, agent navigation, resize, tick, Ctrl-C, q, and
live-backend no-TTY paths remain covered.

This is still deterministic, synchronous, bounded, and credential-free. It
models live-runner duplicate-readiness coalescing around the current
...
### TUI-LIVE-86 Max-Batch Readiness Stop Routes Through Live Shell Runner

Status: TUI-LIVE-86 extends the live-shell runner readiness path with
runner-owned evidence for bounded late JSONL drains that reach the
`max_batches` stop before completion. The existing
`SubmittedTurnLateJsonlReady` route now has harness coverage for a drain that
applies the first assistant delta, records
`TuiPromptSubmittedTurnLateJsonlDrainStatus.MaxBatchesReached` /
`max_batches_reached`, and keeps the submitted turn active.

The live-shell runner gate proves the completion line remains unread, the
assistant transcript row is preserved, `activeTurn` remains populated, and
completed-turn count stays at zero. Existing duplicate post-completion
readiness no-op, no-data readiness retry, readiness backpressure recovery,
readiness routing, pump-event routing, terminal setup/restore, text submit,
line transport, agent navigation, resize, tick, Ctrl-C, q, and live-backend
no-TTY paths remain covered.

This is still deterministic, synchronous, bounded, and credential-free. It
models live-runner max-batch readiness stopping around the current
scheduler/drain loop; it does not own real async socket polling, Tokio
readiness, provider streaming, model calls, tool execution, process teardown,
or persistence.

### TUI-LIVE-87 Prefix-Applied Readiness Rejection Routes Through Live Shell Runner

Status: TUI-LIVE-87 extends the live-shell runner readiness path with
runner-owned evidence for bounded late JSONL drains that apply an assistant
prefix before rejecting a wrong-turn completion. The existing
`SubmittedTurnLateJsonlReady` route now has harness coverage for a drain that
accepts the assistant delta batch, reads a stale completion batch, records
`TuiPromptSubmittedTurnLateJsonlDrainStatus.BatchRejected` / `wrong_turn`, and
keeps the submitted turn active.

The live-shell runner gate proves the assistant transcript row is preserved,
the rejected completion does not clear `activeTurn`, completed-turn count stays
at zero, and the persistent prompt transport reports the rejected completion
line as read. Existing max-batch readiness stop, duplicate post-completion
readiness no-op, no-data readiness retry, readiness backpressure recovery,
readiness routing, pump-event routing, terminal setup/restore, text submit,
line transport, agent navigation, resize, tick, Ctrl-C, q, and live-backend
no-TTY paths remain covered.
...
This is still deterministic, synchronous, bounded, and credential-free. It
models live-runner prefix-applied rejection around the current scheduler/drain
loop; it does not own real async socket polling, Tokio readiness, provider
streaming, model calls, tool execution, process teardown, or persistence.

### TUI-LIVE-88 Line-Read Readiness Rejection Routes Through Live Shell Runner

Status: TUI-LIVE-88 extends the live-shell runner readiness path with
runner-owned evidence for bounded late JSONL drains that fail while reading the
late JSONL line batch. `TuiLiveShellRunOutcome` now records the latest
readiness late-JSONL stop line status/code in addition to the drain status/code,
so a readiness-triggered line-read failure can prove both
`TuiPromptSubmittedTurnLateJsonlDrainStatus.LineReadRejected` and the
underlying disconnected line status/code.

The live-shell runner gate proves a deterministic prompt transport can accept
the submitted turn, reject the first late JSONL read, leave `activeTurn`
populated, keep completed-turn count at zero, and avoid assistant/completion
transcript mutation from unread late lines. Existing prefix-applied rejection,
max-batch readiness stop, duplicate post-completion readiness no-op, no-data
readiness retry, readiness backpressure recovery, readiness routing,
...
This is still deterministic, synchronous, bounded, and credential-free. It
models live-runner line-read rejection around the current scheduler/drain loop;
it does not own real async socket polling, Tokio readiness, provider streaming,
model calls, tool execution, process teardown, or persistence.

### TUI-LIVE-89 Unsupported-Notification Readiness Rejection Routes Through Live Shell Runner

Status: TUI-LIVE-89 extends the live-shell runner readiness path with
runner-owned evidence for bounded late JSONL drains that read a line containing
an unsupported submitted-turn notification. The existing
`SubmittedTurnLateJsonlReady` route now has harness coverage for a persistent
session batch that reads a `thread/statusChanged` notification, reports
accepted line status/code, records
`TuiPromptSubmittedTurnLateJsonlDrainStatus.BatchRejected` /
`unsupported_stream_notification`, and keeps the submitted turn active.

The live-shell runner gate proves the unsupported notification does not append
assistant/completion transcript rows, does not clear `activeTurn`, and leaves
completed-turn count at zero while still recording the read late JSONL line in
the prompt transport inbound count. Existing line-read rejection,
prefix-applied rejection, max-batch readiness stop, duplicate post-completion
readiness no-op, no-data readiness retry, readiness backpressure recovery,
readiness routing, pump-event routing, terminal setup/restore, text submit,
line transport, agent navigation, resize, tick, Ctrl-C, q, and live-backend
no-TTY paths remain covered.
...
This is still deterministic, synchronous, bounded, and credential-free. It
models live-runner unsupported-notification rejection around the current
scheduler/drain loop; it does not own real async socket polling, Tokio
readiness, provider streaming, model calls, tool execution, process teardown,
or persistence.

### TUI-LIVE-90 Stale-Interrupted Readiness Rejection Routes Through Live Shell Runner

Status: TUI-LIVE-90 extends the live-shell runner readiness path so late JSONL
can still be classified after an active submitted turn has been interrupted.
`FakeTuiAppServerFacade.canDrainSubmittedTurnLateJsonl()` now treats the last
interrupted turn as a valid late-JSONL classification target, while completed
turns still route duplicate readiness through the existing no-pending path.

The live-shell runner gate proves a first readiness event can stop at no-data
while the submitted turn is active, Ctrl-C can interrupt and clear the active
turn, and a later readiness event can read the interrupted turn's assistant
delta, record `TuiPromptSubmittedTurnLateJsonlDrainStatus.BatchRejected` /
`stale_interrupted_turn_delivery`, preserve `lastInterruptedTurn`, keep
completed-turn count at zero, and avoid assistant/completion transcript
mutation. Existing unsupported-notification rejection, line-read rejection,
prefix-applied rejection, max-batch readiness stop, duplicate post-completion
readiness no-op, no-data readiness retry, readiness backpressure recovery,
readiness routing, pump-event routing, terminal setup/restore, text submit,
line transport, agent navigation, resize, tick, Ctrl-C, q, and live-backend
...
This is still deterministic, synchronous, bounded, and credential-free. It
models live-runner stale-interrupted assistant-delta rejection around the
current scheduler/drain loop; it does not own real async socket polling, Tokio
readiness, provider streaming, model calls, tool execution, process teardown,
or persistence.

### TUI-LIVE-91 Stale-Interrupted Completion Readiness Rejection Routes Through Live Shell Runner

Status: TUI-LIVE-91 extends the live-shell runner readiness path with
runner-owned evidence for interrupted `turn/completed` notifications. The
live-shell runner gate proves a first readiness event can stop at no-data while
the submitted turn is active, Ctrl-C can interrupt and clear the active turn,
and a later readiness event can read the interrupted turn's `turn/completed`
JSONL, record `TuiPromptSubmittedTurnLateJsonlDrainStatus.BatchRejected` /
`stale_interrupted_turn_completion`, preserve interrupted-turn state, keep
completed-turn count at zero, and avoid transcript mutation.

Existing stale-interrupted assistant rejection, unsupported-notification
rejection, line-read rejection, prefix-applied rejection, max-batch readiness
stop, duplicate post-completion readiness no-op, no-data readiness retry,
readiness backpressure recovery, readiness routing, pump-event routing,
terminal setup/restore, text submit, line transport, agent navigation, resize,
tick, Ctrl-C, q, and live-backend no-TTY paths remain covered.

This is still deterministic, synchronous, bounded, and credential-free. It
models live-runner stale-interrupted completion rejection around the current
scheduler/drain loop; it does not own real async socket polling, Tokio
readiness, provider streaming, model calls, tool execution, process teardown,
or persistence.

### TUI-LIVE-92 Decode-Rejected Readiness Routes Through Live Shell Runner

Status: TUI-LIVE-92 extends the live-shell runner readiness path with
runner-owned evidence for typed decode rejection during a late JSONL drain. The
live-shell runner gate proves a first readiness event can stop at no-data while
the submitted turn is active, and a later readiness event can read an unknown
app-server notification method, record
`TuiPromptSubmittedTurnLateJsonlDrainStatus.BatchRejected` /
`unknown_inbound_method`, preserve the active submitted turn, keep
completed-turn count at zero, and avoid transcript mutation.

Existing stale-interrupted completion rejection, stale-interrupted assistant
rejection, unsupported-notification rejection, line-read rejection,
prefix-applied rejection, max-batch readiness stop, duplicate post-completion
readiness no-op, no-data readiness retry, readiness backpressure recovery,
readiness routing, pump-event routing, terminal setup/restore, text submit,
line transport, agent navigation, resize, tick, Ctrl-C, q, and live-backend
...
This is still deterministic, synchronous, bounded, and credential-free. It
models live-runner unknown-method decode rejection around the current
scheduler/drain loop; it does not own real async socket polling, Tokio
readiness, provider streaming, model calls, tool execution, process teardown,
or persistence.

### TUI-LIVE-93 Malformed-JSON Readiness Routes Through Live Shell Runner

Status: TUI-LIVE-93 extends the live-shell runner readiness path with
runner-owned evidence for malformed JSONL decode rejection during a late JSONL
drain. The live-shell runner gate proves a first readiness event can stop at
no-data while the submitted turn is active, and a later readiness event can
read malformed app-server JSONL, record
`TuiPromptSubmittedTurnLateJsonlDrainStatus.BatchRejected` /
`invalid_json_line`, preserve the active submitted turn, keep completed-turn
count at zero, and avoid transcript mutation.

Existing unknown-method decode rejection, stale-interrupted completion
rejection, stale-interrupted assistant rejection, unsupported-notification
rejection, line-read rejection, prefix-applied rejection, max-batch readiness
stop, duplicate post-completion readiness no-op, no-data readiness retry,
readiness backpressure recovery, readiness routing, pump-event routing,
terminal setup/restore, text submit, line transport, agent navigation, resize,
...
This is still deterministic, synchronous, bounded, and credential-free. It
models live-runner malformed-JSON decode rejection around the current
scheduler/drain loop; it does not own real async socket polling, Tokio
readiness, provider streaming, model calls, tool execution, process teardown,
or persistence.

### TUI-LIVE-94 Schema-Rejected Readiness Routes Through Live Shell Runner

Status: TUI-LIVE-94 extends the live-shell runner readiness path with
runner-owned evidence for syntactically valid but schema-invalid JSONL during a
late JSONL drain. The live-shell runner gate proves a first readiness event can
stop at no-data while the submitted turn is active, and a later readiness event
can read a known `turn/completed` method whose params are missing required
fields, record `TuiPromptSubmittedTurnLateJsonlDrainStatus.BatchRejected` /
`missing_field`, preserve the active submitted turn, keep completed-turn count
at zero, and avoid transcript mutation.

Existing malformed-JSON rejection, unknown-method decode rejection,
stale-interrupted completion rejection, stale-interrupted assistant rejection,
unsupported-notification rejection, line-read rejection, prefix-applied
rejection, max-batch readiness stop, duplicate post-completion readiness no-op,
no-data readiness retry, readiness backpressure recovery, readiness routing,
pump-event routing, terminal setup/restore, text submit, line transport, agent
navigation, resize, tick, Ctrl-C, q, and live-backend no-TTY paths remain
covered.

This is still deterministic, synchronous, bounded, and credential-free. It
models live-runner schema decode rejection around the current scheduler/drain
loop; it does not own real async socket polling, Tokio readiness, provider
streaming, model calls, tool execution, process teardown, or persistence.

### TUI-LIVE-95 Prefix-Then-Schema-Rejected Readiness Routes Through Live Shell Runner

Status: TUI-LIVE-95 extends the live-shell runner readiness path with
runner-owned evidence for partial late JSONL application followed by typed
schema rejection. The live-shell runner gate proves a first readiness event can
stop at no-data while the submitted turn is active, and a later readiness event
can apply a valid assistant delta before reading a schema-invalid
`turn/completed` notification, record
`TuiPromptSubmittedTurnLateJsonlDrainStatus.BatchRejected` / `missing_field`,
preserve the active submitted turn, keep completed-turn count at zero, and keep
the assistant transcript prefix visible.

Existing schema rejection, malformed-JSON rejection, unknown-method decode
rejection, stale-interrupted completion rejection, stale-interrupted assistant
rejection, unsupported-notification rejection, line-read rejection,
prefix-applied rejection, max-batch readiness stop, duplicate post-completion
readiness no-op, no-data readiness retry, readiness backpressure recovery,
readiness routing, pump-event routing, terminal setup/restore, text submit,
...
This is still deterministic, synchronous, bounded, and credential-free. It
models live-runner prefix-then-schema rejection around the current
scheduler/drain loop; it does not own real async socket polling, Tokio
readiness, provider streaming, model calls, tool execution, process teardown,
or persistence.

### TUI-LIVE-96 Prefix-Then-Decode-Rejected Readiness Routes Through Live Shell Runner

Status: TUI-LIVE-96 extends the live-shell runner readiness path with
runner-owned evidence for partial late JSONL application followed by typed
decode rejection. The live-shell runner gate proves a first readiness event can
stop at no-data while the submitted turn is active, and a later readiness event
can apply a valid assistant delta before reading a decode-invalid
`unknown/event` notification, record
`TuiPromptSubmittedTurnLateJsonlDrainStatus.BatchRejected` /
`unknown_inbound_method`, preserve the active submitted turn, keep
completed-turn count at zero, and keep the assistant transcript prefix visible.

Existing prefix-then-schema rejection, schema rejection, malformed-JSON
rejection, unknown-method decode rejection, stale-interrupted completion
rejection, stale-interrupted assistant rejection, unsupported-notification
rejection, line-read rejection, prefix-applied rejection, max-batch readiness
stop, duplicate post-completion readiness no-op, no-data readiness retry,
readiness backpressure recovery, readiness routing, pump-event routing,
...
This is still deterministic, synchronous, bounded, and credential-free. It
models live-runner prefix-then-decode rejection around the current
scheduler/drain loop; it does not own real async socket polling, Tokio
readiness, provider streaming, model calls, tool execution, process teardown,
or persistence.

### TUI-LIVE-97 Prefix-Then-Malformed-JSON Readiness Routes Through Live Shell Runner

Status: TUI-LIVE-97 extends the live-shell runner readiness path with
runner-owned evidence for partial late JSONL application followed by malformed
JSONL rejection. The live-shell runner gate proves a first readiness event can
stop at no-data while the submitted turn is active, and a later readiness event
can apply a valid assistant delta before reading malformed `turn/completed`
JSONL, record `TuiPromptSubmittedTurnLateJsonlDrainStatus.BatchRejected` /
`invalid_json_line`, preserve the active submitted turn, keep completed-turn
count at zero, and keep the assistant transcript prefix visible.

Existing prefix-then-decode rejection, prefix-then-schema rejection, schema
rejection, malformed-JSON rejection, unknown-method decode rejection,
stale-interrupted completion rejection, stale-interrupted assistant rejection,
unsupported-notification rejection, line-read rejection, prefix-applied
rejection, max-batch readiness stop, duplicate post-completion readiness no-op,
no-data readiness retry, readiness backpressure recovery, readiness routing,
...
This is still deterministic, synchronous, bounded, and credential-free. It
models live-runner prefix-then-malformed-JSON rejection around the current
scheduler/drain loop; it does not own real async socket polling, Tokio
readiness, provider streaming, model calls, tool execution, process teardown,
or persistence.

### TUI-LIVE-98 Prefix-Then-Unsupported-Notification Readiness Routes Through Live Shell Runner

Status: TUI-LIVE-98 extends the live-shell runner readiness path with
runner-owned evidence for partial late JSONL application followed by
unsupported-notification rejection. The live-shell runner gate proves a first
readiness event can stop at no-data while the submitted turn is active, and a
later readiness event can apply a valid assistant delta before reading a
thread-status notification that is not a submitted-turn stream update, record
`TuiPromptSubmittedTurnLateJsonlDrainStatus.BatchRejected` /
`unsupported_stream_notification`, preserve the active submitted turn, keep
completed-turn count at zero, and keep the assistant transcript prefix visible.

Existing prefix-then-malformed-JSON rejection, prefix-then-decode rejection,
...
This is still deterministic, synchronous, bounded, and credential-free. It
models live-runner prefix-then-unsupported-notification rejection around the
current scheduler/drain loop; it does not own real async socket polling, Tokio
readiness, provider streaming, model calls, tool execution, process teardown,
or persistence.

### TUI-LIVE-99 Prefix-Then-Line-Read Readiness Routes Through Live Shell Runner

Status: TUI-LIVE-99 extends the live-shell runner readiness path with
runner-owned evidence for partial late JSONL application followed by a later
line-read rejection. The live-shell runner gate proves a first readiness event
can stop at no-data while the submitted turn is active, and a later readiness
event can apply a valid assistant delta before the following late-JSONL read
fails, record `TuiPromptSubmittedTurnLateJsonlDrainStatus.LineReadRejected` /
`runner_late_jsonl_read_failed`, preserve the active submitted turn, keep
completed-turn count at zero, and keep the assistant transcript prefix visible.

Existing prefix-then-unsupported-notification rejection,
...
This is still deterministic, synchronous, bounded, and credential-free. It
models live-runner prefix-then-line-read rejection around the current
scheduler/drain loop; it does not own real async socket polling, Tokio
readiness, provider streaming, model calls, tool execution, process teardown,
or persistence.

### TUI-LIVE-100 Prefix-Then-Stale-Interrupted Readiness Routes Through Live Shell Runner

Status: TUI-LIVE-100 extends the live-shell runner readiness path with
runner-owned evidence for partial late JSONL application before a submitted
turn is interrupted, followed by stale interrupted-turn rejection. The
live-shell runner gate proves a first readiness event can stop at no-data while
the submitted turn is active, a later readiness event can apply exactly one
assistant delta before Ctrl-C interrupts that turn, and a final readiness event
can reject a later assistant delta for the interrupted turn, record
`TuiPromptSubmittedTurnLateJsonlDrainStatus.BatchRejected` /
`stale_interrupted_turn_delivery`, preserve interrupted-turn state, keep
completed-turn count at zero, and keep the applied assistant transcript prefix
visible.

Existing prefix-then-line-read rejection,
prefix-then-unsupported-notification rejection, prefix-then-malformed-JSON
rejection, prefix-then-decode rejection, prefix-then-schema rejection, schema
rejection, malformed-JSON rejection, unknown-method decode rejection,
stale-interrupted completion rejection, stale-interrupted assistant rejection,
unsupported-notification rejection, line-read rejection, prefix-applied
rejection, max-batch readiness stop, duplicate post-completion readiness no-op,
...
This is still deterministic, synchronous, bounded, and credential-free. It
models live-runner prefix-then-stale-interrupted rejection around the current
scheduler/drain loop; it does not own real async socket polling, Tokio
readiness, provider streaming, model calls, tool execution, process teardown,
or persistence.

### TUI-LIVE-101 Prefix-Then-Stale-Interrupted-Completion Readiness Routes Through Live Shell Runner

Status: TUI-LIVE-101 extends the live-shell runner readiness path with
runner-owned evidence for partial late JSONL application before a submitted
turn is interrupted, followed by stale interrupted-turn completion rejection.
The live-shell runner gate proves a first readiness event can stop at no-data
while the submitted turn is active, a later readiness event can apply exactly
one assistant delta before Ctrl-C interrupts that turn, and a final readiness
event can reject a later `turn/completed` notification for the interrupted turn,
record `TuiPromptSubmittedTurnLateJsonlDrainStatus.BatchRejected` /
`stale_interrupted_turn_completion`, preserve interrupted-turn state, keep
completed-turn count at zero, and keep the applied assistant transcript prefix
visible.

Existing prefix-then-stale-interrupted assistant rejection,
prefix-then-line-read rejection, prefix-then-unsupported-notification rejection,
prefix-then-malformed-JSON rejection, prefix-then-decode rejection,
prefix-then-schema rejection, schema rejection, malformed-JSON rejection,
unknown-method decode rejection, stale-interrupted completion rejection,
stale-interrupted assistant rejection, unsupported-notification rejection,
...
This is still deterministic, synchronous, bounded, and credential-free. It
models live-runner prefix-then-stale-interrupted-completion rejection around the
current scheduler/drain loop; it does not own real async socket polling, Tokio
readiness, provider streaming, model calls, tool execution, process teardown,
or persistence.
...
### TUI-LIVE-103 Prefix-Then-Line-Read Readiness Exposes Structured Applied-Prefix Evidence

Status: TUI-LIVE-103 extends the live-shell runner structured readiness
evidence to prefix-then-line-read rejection. The live-shell runner gate proves a
first readiness event can stop at no-data while the submitted turn is active,
then a later readiness drain can apply one assistant prefix before a subsequent
late-JSONL read fails, record
`TuiPromptSubmittedTurnLateJsonlDrainStatus.LineReadRejected` /
`runner_late_jsonl_read_failed`, and still expose applied notification count,
assistant delta count, zero completion count, latest applied thread/turn IDs,
and applied assistant delta text through `TuiLiveShellRunOutcome`.

Existing prefix-then-rejection structured evidence,
prefix-then-stale-interrupted-completion rejection,
prefix-then-stale-interrupted assistant rejection,
prefix-then-unsupported-notification rejection, prefix-then-malformed-JSON
rejection, prefix-then-decode rejection, prefix-then-schema rejection, schema
rejection, malformed-JSON rejection, unknown-method decode rejection,
stale-interrupted completion rejection, stale-interrupted assistant rejection,
...
This is still deterministic, synchronous, bounded, and credential-free. It
models live-runner structured prefix evidence for line-read rejection around
the current scheduler/drain loop; it does not own real async socket polling,
Tokio readiness, provider streaming, model calls, tool execution, process
teardown, or persistence.

### TUI-LIVE-104 Prefix-Then-Unsupported-Notification Readiness Exposes Structured Applied-Prefix Evidence

Status: TUI-LIVE-104 extends the live-shell runner structured readiness
evidence to prefix-then-unsupported-notification rejection. The live-shell
runner gate proves a first readiness event can stop at no-data while the
submitted turn is active, then a later readiness drain can apply one assistant
prefix before a subsequent unsupported stream notification rejects the batch,
record `TuiPromptSubmittedTurnLateJsonlDrainStatus.BatchRejected` /
`unsupported_stream_notification`, and still expose applied notification count,
assistant delta count, zero completion count, latest applied thread/turn IDs,
and applied assistant delta text through `TuiLiveShellRunOutcome`.

Existing prefix-then-line-read structured evidence,
prefix-then-rejection structured evidence,
prefix-then-stale-interrupted-completion rejection,
prefix-then-stale-interrupted assistant rejection, prefix-then-malformed-JSON
rejection, prefix-then-decode rejection, prefix-then-schema rejection, schema
rejection, malformed-JSON rejection, unknown-method decode rejection,
stale-interrupted completion rejection, stale-interrupted assistant rejection,
unsupported-notification rejection, line-read rejection, prefix-applied
...
This is still deterministic, synchronous, bounded, and credential-free. It
models live-runner structured prefix evidence for unsupported-notification
rejection around the current scheduler/drain loop; it does not own real async
socket polling, Tokio readiness, provider streaming, model calls, tool
execution, process teardown, or persistence.

### TUI-LIVE-105 Prefix-Then-Malformed-JSON Readiness Exposes Structured Applied-Prefix Evidence

Status: TUI-LIVE-105 extends the live-shell runner structured readiness
evidence to prefix-then-malformed-JSON rejection. The live-shell runner gate
proves a first readiness event can stop at no-data while the submitted turn is
active, then a later readiness drain can apply one assistant prefix before a
subsequent malformed JSONL line rejects the batch, record
`TuiPromptSubmittedTurnLateJsonlDrainStatus.BatchRejected` /
`invalid_json_line`, and still expose applied notification count, assistant
delta count, zero completion count, latest applied thread/turn IDs, and applied
assistant delta text through `TuiLiveShellRunOutcome`.

Existing prefix-then-unsupported-notification structured evidence,
prefix-then-line-read structured evidence, prefix-then-rejection structured
evidence, prefix-then-stale-interrupted-completion rejection,
prefix-then-stale-interrupted assistant rejection, prefix-then-decode rejection,
prefix-then-schema rejection, schema rejection, malformed-JSON rejection,
unknown-method decode rejection, stale-interrupted completion rejection,
stale-interrupted assistant rejection, unsupported-notification rejection,
line-read rejection, prefix-applied rejection, max-batch readiness stop,
...
This is still deterministic, synchronous, bounded, and credential-free. It
models live-runner structured prefix evidence for malformed-JSON rejection
...
### TUI-LIVE-106 Prefix-Then-Decode-Rejected Readiness Exposes Structured Applied-Prefix Evidence

Status: TUI-LIVE-106 extends the live-shell runner structured readiness
evidence to prefix-then-decode-rejected unknown-method rejection. The live-shell
runner gate proves a first readiness event can stop at no-data while the
submitted turn is active, then a later readiness drain can apply one assistant
prefix before a subsequent unknown-method notification rejects the batch, record
`TuiPromptSubmittedTurnLateJsonlDrainStatus.BatchRejected` /
`unknown_inbound_method`, and still expose applied notification count, assistant
delta count, zero completion count, latest applied thread/turn IDs, and applied
assistant delta text through `TuiLiveShellRunOutcome`.

Existing prefix-then-malformed-JSON structured evidence,
prefix-then-unsupported-notification structured evidence, prefix-then-line-read
structured evidence, prefix-then-rejection structured evidence,
prefix-then-stale-interrupted-completion rejection, prefix-then-stale-interrupted
assistant rejection, prefix-then-schema rejection, schema rejection,
malformed-JSON rejection, unknown-method decode rejection, stale-interrupted
completion rejection, stale-interrupted assistant rejection,
unsupported-notification rejection, line-read rejection, prefix-applied
rejection, max-batch readiness stop, duplicate post-completion readiness no-op,
no-data readiness retry, readiness backpressure recovery, readiness routing,
pump-event routing, terminal setup/restore, text submit, line transport, agent
...
This is still deterministic, synchronous, bounded, and credential-free. It
models live-runner structured prefix evidence for decode-rejected readiness
around the current scheduler/drain loop; it does not own real async socket
polling, Tokio readiness, provider streaming, model calls, tool execution,
process teardown, or persistence.

### TUI-LIVE-107 Prefix-Then-Schema-Rejected Readiness Exposes Structured Applied-Prefix Evidence

Status: TUI-LIVE-107 extends the live-shell runner structured readiness
evidence to prefix-then-schema-rejected missing-field rejection. The live-shell
runner gate proves a first readiness event can stop at no-data while the
submitted turn is active, then a later readiness drain can apply one assistant
prefix before a subsequent schema-invalid completion notification rejects the
batch, record `TuiPromptSubmittedTurnLateJsonlDrainStatus.BatchRejected` /
`missing_field`, and still expose applied notification count, assistant delta
count, zero completion count, latest applied thread/turn IDs, and applied
assistant delta text through `TuiLiveShellRunOutcome`.

Existing prefix-then-decode-rejected structured evidence,
prefix-then-malformed-JSON structured evidence,
prefix-then-unsupported-notification structured evidence, prefix-then-line-read
structured evidence, prefix-then-rejection structured evidence,
prefix-then-stale-interrupted-completion rejection, prefix-then-stale-interrupted
assistant rejection, schema rejection, malformed-JSON rejection, unknown-method
decode rejection, stale-interrupted completion rejection, stale-interrupted
assistant rejection, unsupported-notification rejection, line-read rejection,
prefix-applied rejection, max-batch readiness stop, duplicate post-completion
readiness no-op, no-data readiness retry, readiness backpressure recovery,
readiness routing, pump-event routing, terminal setup/restore, text submit, line
transport, agent navigation, resize, tick, Ctrl-C, q, and live-backend no-TTY
paths remain covered.

This is still deterministic, synchronous, bounded, and credential-free. It
models live-runner structured prefix evidence for schema-rejected readiness
around the current scheduler/drain loop; it does not own real async socket
polling, Tokio readiness, provider streaming, model calls, tool execution,
process teardown, or persistence.

### TUI-LIVE-108 Runner-Owned Readiness Source Feeds Submitted-Turn Readiness Events

Status: TUI-LIVE-108 adds `TuiLiveShellReadinessSource` as the runner-owned
typed source for app-server readiness events. `TuiLiveShellRunRequest` still
supports explicit queued `TuiAppServerReadinessEvent` values for existing gates,
but it can now carry a source that the live shell loop polls after a submitted
turn is active. The source currently has a deterministic in-memory
implementation so interpreter and generated-Rust gates can prove source-fed
readiness without sockets, Tokio tasks, provider streaming, model calls, tools,
or persistence.

The live-shell runner gate proves a source-fed `SubmittedTurnLateJsonlReady`
event can first hit no-data while preserving the active submitted turn, then a
later source-fed readiness event can drain assistant/completion JSONL, apply the
assistant transcript row, complete and clear the active turn exactly once, and
record structured readiness evidence through `TuiLiveShellRunOutcome`: event
count, drained/no-pending/no-data counts, latest drain status/code, line
status/code, applied notification count, assistant delta count, completion
count, latest applied thread/turn IDs, and latest applied assistant delta text.

Existing request-prequeued readiness routing, structured prefix evidence for
schema/decode/malformed-JSON/unsupported/line-read rejection, stale-interrupted
readiness rejection, duplicate post-completion readiness no-op, max-batch
readiness stop, readiness backpressure recovery, pump-event routing, terminal
setup/restore, text submit, line transport, agent navigation, resize, tick,
Ctrl-C, q, and live-backend no-TTY paths remain covered.

This is still deterministic, synchronous, bounded, and credential-free. It
models a readiness-source bridge toward live app-server event ownership around
the current scheduler/drain loop; it does not own real async socket polling,
Tokio readiness, provider streaming, model calls, tool execution, process
teardown, or persistence.

### TUI-LIVE-109 Readiness Source Backpressure Recovery Routes Through Live Shell Runner

Status: TUI-LIVE-109 extends the runner-owned `TuiLiveShellReadinessSource`
coverage to readiness-triggered backpressure recovery. The live-shell runner
gate proves a source-fed `SubmittedTurnLateJsonlReady` event can drain
assistant/completion JSONL under a bounded app-server policy, record readiness
backpressure while preserving the active submitted turn, and then recover
through the existing typed `TuiAppServerPumpEvent.DrainQueuedEvents` route.

The source-fed recovery gate preserves structured evidence through
`TuiLiveShellRunOutcome`: readiness event count, readiness backpressure count,
pump-event count, pump-event backpressure count, total backpressure count,
latest readiness status, latest late-JSONL drain status/code, line status/code,
applied notification count, assistant delta count, completion count, latest
applied thread/turn IDs, latest applied assistant delta text, transcript rows,
active-turn clearing, and completed-turn count.

Existing request-prequeued readiness routing, source-fed completion routing,
structured prefix evidence for schema/decode/malformed-JSON/unsupported/
line-read rejection, stale-interrupted readiness rejection, duplicate
post-completion readiness no-op, max-batch readiness stop, queued readiness
backpressure recovery, pump-event routing, terminal setup/restore, text submit,
line transport, agent navigation, resize, tick, Ctrl-C, q, and live-backend
no-TTY paths remain covered.

This is still deterministic, synchronous, bounded, and credential-free. It
models source-fed readiness backpressure recovery around the current
scheduler/drain loop; it does not own real async socket polling, Tokio
readiness, provider streaming, model calls, tool execution, process teardown, or
persistence.

### TUI-LIVE-110 Readiness Source No-Data Retry Routes Through Live Shell Runner

Status: TUI-LIVE-110 extends the runner-owned `TuiLiveShellReadinessSource`
coverage to repeated no-data readiness retry. The live-shell runner gate proves
a first source-fed `SubmittedTurnLateJsonlReady` event can record a no-data late
JSONL drain while preserving the active submitted turn, then a later source-fed
readiness event can drain assistant/completion JSONL, apply the assistant
transcript row, complete and clear the active turn exactly once, and preserve
structured readiness outcome evidence.

The source-fed no-data retry gate records readiness event count, drained/no-
pending/no-data counts, latest no-data active turn ID, latest late-JSONL drain
status/code, line status/code, applied notification count, assistant delta
count, completion count, latest applied thread/turn IDs, latest applied
assistant delta text, transcript rows, active-turn clearing, and completed-turn
count through `TuiLiveShellRunOutcome`.

Existing request-prequeued readiness routing, source-fed completion routing,
source-fed readiness backpressure recovery, structured prefix evidence for
schema/decode/malformed-JSON/unsupported/line-read rejection, stale-interrupted
readiness rejection, duplicate post-completion readiness no-op, max-batch
readiness stop, queued no-data readiness retry, queued readiness backpressure
recovery, pump-event routing, terminal setup/restore, text submit, line
transport, agent navigation, resize, tick, Ctrl-C, q, and live-backend no-TTY
paths remain covered.

This is still deterministic, synchronous, bounded, and credential-free. It
models source-fed no-data readiness retry around the current scheduler/drain
loop; it does not own real async socket polling, Tokio readiness, provider
streaming, model calls, tool execution, process teardown, or persistence.
...
### TUI-LIVE-112 Readiness Source Max-Batch Stop Routes Through Live Shell Runner

Status: TUI-LIVE-112 extends the runner-owned `TuiLiveShellReadinessSource`
coverage to max-batch readiness stops. The live-shell runner gate proves a
source-fed `SubmittedTurnLateJsonlReady` event can apply one assistant prefix,
stop with `MaxBatchesReached`, keep the submitted turn active, leave the
completion line unread, and avoid completed-turn mutation.

The source-fed max-batch gate records readiness event count, drained/no-pending
counts, latest `Drained` status, latest late-JSONL drain status/code, line
status/code, applied notification/assistant/completion counts, retained active
turn ID, applied thread/turn IDs and assistant delta text, prompt transport
inbound-line count, transcript count, and completed-turn count through
`TuiLiveShellRunOutcome`.

Existing request-prequeued readiness routing, source-fed duplicate
post-completion no-op, source-fed no-data retry, source-fed readiness
backpressure recovery, structured prefix evidence for schema/decode/malformed-
JSON/unsupported/line-read rejection, stale-interrupted readiness rejection,
queued max-batch readiness stop, queued duplicate post-completion readiness
no-op, queued no-data readiness retry, queued readiness backpressure recovery,
pump-event routing, terminal setup/restore, text submit, line transport, agent
navigation, resize, tick, Ctrl-C, q, and live-backend no-TTY paths remain
covered.

This is still deterministic, synchronous, bounded, and credential-free. It
models source-fed max-batch readiness stop behavior around the current
scheduler/drain loop; it does not own real async socket polling, Tokio
readiness, provider streaming, model calls, tool execution, process teardown, or
persistence.

### TUI-LIVE-113 Readiness Source Prefix-Applied Rejection Routes Through Live Shell Runner

Status: TUI-LIVE-113 extends the runner-owned `TuiLiveShellReadinessSource`
coverage to prefix-applied readiness rejection. The live-shell runner gate
proves a source-fed `SubmittedTurnLateJsonlReady` event can apply one assistant
prefix, reject a later wrong-turn completion with `BatchRejected`, keep the
submitted turn active, and avoid completed-turn mutation.

The source-fed prefix-applied rejection gate records readiness event count,
drained/no-pending counts, latest `Drained` status, latest late-JSONL drain
status/code, line status/code, applied notification/assistant/completion
counts, retained active turn ID, rejected turn ID, applied thread ID and
assistant delta text, prompt transport inbound-line count, transcript count,
and completed-turn count through `TuiLiveShellRunOutcome`.

Existing request-prequeued readiness routing, source-fed max-batch readiness
stop, source-fed duplicate post-completion no-op, source-fed no-data retry,
source-fed readiness backpressure recovery, structured prefix evidence for
schema/decode/malformed-JSON/unsupported/line-read rejection, stale-interrupted
readiness rejection, queued prefix-applied rejection, queued max-batch
...
This is still deterministic, synchronous, bounded, and credential-free. It
models source-fed prefix-applied readiness rejection behavior around the current
scheduler/drain loop; it does not own real async socket polling, Tokio
readiness, provider streaming, model calls, tool execution, process teardown, or
persistence.
...
### TUI-LIVE-115 Readiness Source Unsupported-Notification Rejection Routes Through Live Shell Runner

Status: TUI-LIVE-115 extends the runner-owned `TuiLiveShellReadinessSource`
coverage to unsupported-notification rejection. The live-shell runner gate
proves a source-fed `SubmittedTurnLateJsonlReady` event can surface
`BatchRejected` / `unsupported_stream_notification`, keep the submitted turn
active, avoid unsupported late-JSONL transcript mutation, and avoid
completed-turn mutation.

The source-fed unsupported-notification gate records readiness event count,
drained/no-pending counts, latest `Drained` status, latest late-JSONL drain
status/code, line status/code, zero applied notification/assistant/completion
counts, retained active turn ID, prompt transport inbound-line count, transcript
count, and completed-turn count through `TuiLiveShellRunOutcome`.

Existing request-prequeued readiness routing, source-fed line-read rejection,
source-fed prefix-applied rejection, source-fed max-batch readiness stop,
source-fed duplicate post-completion no-op, source-fed no-data retry,
source-fed readiness backpressure recovery, structured prefix evidence for
schema/decode/malformed-JSON rejection, stale-interrupted readiness rejection,
queued unsupported-notification rejection, queued line-read rejection, queued
prefix-applied rejection, queued max-batch readiness stop, queued duplicate
post-completion readiness no-op, queued no-data readiness retry, queued
readiness backpressure recovery, pump-event routing, terminal setup/restore,
text submit, line transport, agent navigation, resize, tick, Ctrl-C, q, and
live-backend no-TTY paths remain covered.

This is still deterministic, synchronous, bounded, and credential-free. It
models source-fed unsupported-notification readiness rejection behavior around
the current scheduler/drain loop; it does not own real async socket polling,
Tokio readiness, provider streaming, model calls, tool execution, process
teardown, or persistence.

### TUI-LIVE-116 Readiness Source Decode Rejection Routes Through Live Shell Runner

Status: TUI-LIVE-116 extends the runner-owned `TuiLiveShellReadinessSource`
coverage to unknown-method decode rejection. The live-shell runner gate proves
source-fed `SubmittedTurnLateJsonlReady` events can first preserve the active
submitted turn through no-data, then surface `BatchRejected` /
`unknown_inbound_method`, avoid rejected late-JSONL transcript mutation, and
avoid completed-turn mutation.

The source-fed decode-rejection gate records readiness event count,
drained/no-pending counts, no-data count and retained active turn, latest
`Drained` status, latest late-JSONL drain status/code, line status/code, zero
applied notification/assistant/completion counts, retained active turn ID,
prompt transport inbound-line count, transcript count, and completed-turn count
through `TuiLiveShellRunOutcome`.

Existing source-fed unsupported-notification rejection, source-fed line-read
rejection, source-fed prefix-applied rejection, source-fed max-batch readiness
stop, source-fed duplicate post-completion no-op, source-fed no-data retry,
source-fed readiness backpressure recovery, structured prefix evidence for
schema/decode/malformed-JSON rejection, stale-interrupted readiness rejection,
queued decode rejection, queued unsupported-notification rejection, queued
line-read rejection, queued prefix-applied rejection, queued max-batch readiness
stop, queued duplicate post-completion readiness no-op, queued no-data readiness
retry, queued readiness backpressure recovery, pump-event routing, terminal
setup/restore, text submit, line transport, agent navigation, resize, tick,
Ctrl-C, q, and live-backend no-TTY paths remain covered.

This is still deterministic, synchronous, bounded, and credential-free. It
models source-fed unknown-method decode rejection behavior around the current
scheduler/drain loop; it does not own real async socket polling, Tokio
readiness, provider streaming, model calls, tool execution, process teardown, or
persistence.

### TUI-LIVE-117 Readiness Source Malformed JSON Rejection Routes Through Live Shell Runner

Status: TUI-LIVE-117 extends the runner-owned `TuiLiveShellReadinessSource`
coverage to malformed JSONL rejection. The live-shell runner gate proves
source-fed `SubmittedTurnLateJsonlReady` events can first preserve the active
submitted turn through no-data, then surface `BatchRejected` /
`invalid_json_line`, avoid rejected malformed late-JSONL transcript mutation,
and avoid completed-turn mutation.

The source-fed malformed-JSON gate records readiness event count,
drained/no-pending counts, no-data count and retained active turn, latest
`Drained` status, latest late-JSONL drain status/code, line status/code, zero
applied notification/assistant/completion counts, retained active turn ID,
prompt transport inbound-line count, transcript count, and completed-turn count
...
Existing source-fed decode rejection, source-fed unsupported-notification
rejection, source-fed line-read rejection, source-fed prefix-applied rejection,
...
stale-interrupted readiness rejection, queued malformed-JSON rejection, queued
decode rejection, queued unsupported-notification rejection, queued line-read
...
This is still deterministic, synchronous, bounded, and credential-free. It
models source-fed malformed JSONL rejection behavior around the current
scheduler/drain loop; it does not own real async socket polling, Tokio
readiness, provider streaming, model calls, tool execution, process teardown, or
persistence.

### TUI-LIVE-118 Readiness Source Schema Rejection Routes Through Live Shell Runner

Status: TUI-LIVE-118 extends the runner-owned `TuiLiveShellReadinessSource`
coverage to schema rejection. The live-shell runner gate proves source-fed
`SubmittedTurnLateJsonlReady` events can first preserve the active submitted
turn through no-data, then surface `BatchRejected` / `missing_field`, avoid
schema-invalid late-JSONL transcript mutation, and avoid completed-turn
mutation.

The source-fed schema-rejection gate records readiness event count,
drained/no-pending counts, no-data count and retained active turn, latest
`Drained` status, latest late-JSONL drain status/code, line status/code, zero
applied notification/assistant/completion counts, retained active turn ID,
prompt transport inbound-line count, transcript count, and completed-turn count
...
Existing source-fed malformed-JSON rejection, source-fed decode rejection,
source-fed unsupported-notification rejection, source-fed line-read rejection,
source-fed prefix-applied rejection, source-fed max-batch readiness stop,
source-fed duplicate post-completion no-op, source-fed no-data retry,
source-fed readiness backpressure recovery, structured prefix evidence for
schema/decode/malformed-JSON rejection, stale-interrupted readiness rejection,
...
This is still deterministic, synchronous, bounded, and credential-free. It
models source-fed schema rejection behavior around the current scheduler/drain
loop; it does not own real async socket polling, Tokio readiness, provider
streaming, model calls, tool execution, process teardown, or persistence.
...
### TUI-LIVE-120 Readiness Source Prefix-Then-Decode Rejection Routes Through Live Shell Runner

Status: TUI-LIVE-120 extends the runner-owned `TuiLiveShellReadinessSource`
coverage to prefix-then-decode rejection. The live-shell runner gate proves
source-fed `SubmittedTurnLateJsonlReady` events can first preserve the active
submitted turn through no-data, then apply an assistant prefix before an
unknown-method late JSONL line surfaces `BatchRejected` /
`unknown_inbound_method`.

The source-fed prefix-then-decode gate records readiness event count,
drained/no-pending counts, no-data count and retained active turn, latest
`Drained` status, latest late-JSONL drain status/code, line status/code, applied
notification count, assistant delta count, completion count, latest
thread/turn/delta evidence, retained active turn ID, prompt transport
...
Existing source-fed prefix-then-schema rejection, source-fed schema rejection,
source-fed malformed-JSON rejection, source-fed decode rejection, source-fed
unsupported-notification rejection, source-fed line-read rejection,
source-fed prefix-applied rejection, source-fed max-batch readiness stop,
source-fed duplicate post-completion no-op, source-fed no-data retry,
source-fed readiness backpressure recovery, structured prefix evidence for
schema/decode/malformed-JSON rejection, stale-interrupted readiness rejection,
queued prefix-then-decode rejection, queued prefix-then-schema rejection, queued
schema rejection, queued malformed-JSON rejection, queued decode rejection,
queued unsupported-notification rejection, queued line-read rejection, queued
prefix-applied rejection, queued max-batch readiness stop, queued duplicate
post-completion readiness no-op, queued no-data readiness retry, queued
readiness backpressure recovery, pump-event routing, terminal setup/restore,
...
This is still deterministic, synchronous, bounded, and credential-free. It
models source-fed prefix-then-decode rejection behavior around the current
scheduler/drain loop; it does not own real async socket polling, Tokio
readiness, provider streaming, model calls, tool execution, process teardown, or
persistence.

### TUI-LIVE-121 Readiness Source Prefix-Then-Malformed JSON Rejection Routes Through Live Shell Runner

Status: TUI-LIVE-121 extends the runner-owned `TuiLiveShellReadinessSource`
coverage to prefix-then-malformed JSON rejection. The live-shell runner gate
proves source-fed `SubmittedTurnLateJsonlReady` events can first preserve the
active submitted turn through no-data, then apply an assistant prefix before a
malformed `turn/completed` late JSONL line surfaces `BatchRejected` /
`invalid_json_line`.

The source-fed prefix-then-malformed-JSON gate records readiness event count,
drained/no-pending counts, no-data count and retained active turn, latest
`Drained` status, latest late-JSONL drain status/code, line status/code, applied
notification count, assistant delta count, completion count, latest
thread/turn/delta evidence, retained active turn ID, prompt transport
...
Existing source-fed prefix-then-decode rejection, source-fed prefix-then-schema
...
no-op, source-fed no-data retry, source-fed readiness backpressure recovery,
structured prefix evidence for schema/decode/malformed-JSON rejection,
stale-interrupted readiness rejection, queued prefix-then-malformed JSON
rejection, queued prefix-then-decode rejection, queued prefix-then-schema
rejection, queued schema rejection, queued malformed-JSON rejection, queued
decode rejection, queued unsupported-notification rejection, queued line-read
rejection, queued prefix-applied rejection, queued max-batch readiness stop,
queued duplicate post-completion readiness no-op, queued no-data readiness
retry, queued readiness backpressure recovery, pump-event routing, terminal
...
This is still deterministic, synchronous, bounded, and credential-free. It
models source-fed prefix-then-malformed JSON rejection behavior around the
current scheduler/drain loop; it does not own real async socket polling, Tokio
readiness, provider streaming, model calls, tool execution, process teardown, or
persistence.

### TUI-LIVE-122 Readiness Source Prefix-Then-Unsupported Notification Rejection Routes Through Live Shell Runner

Status: TUI-LIVE-122 extends the runner-owned `TuiLiveShellReadinessSource`
coverage to prefix-then-unsupported notification rejection. The live-shell
...
prefix before a `thread/status_changed` late JSONL notification surfaces
`BatchRejected` / `unsupported_stream_notification`.

The source-fed prefix-then-unsupported-notification gate records readiness event
count, drained/no-pending counts, no-data count and retained active turn, latest
`Drained` status, latest late-JSONL drain status/code, line status/code, applied
notification count, assistant delta count, completion count, latest
thread/turn/delta evidence, retained active turn ID, prompt transport
inbound-line count, transcript count, final-frame assistant row, and
...
Existing source-fed prefix-then-malformed JSON rejection, source-fed
...
rejection, queued prefix-then-unsupported notification rejection, queued
...
This is still deterministic, synchronous, bounded, and credential-free. It
models source-fed prefix-then-unsupported notification rejection behavior around
the current scheduler/drain loop; it does not own real async socket polling,
Tokio readiness, provider streaming, model calls, tool execution, process
teardown, or persistence.

### TUI-LIVE-123 Readiness Source Prefix-Then-Line-Read Rejection Routes Through Live Shell Runner

Status: TUI-LIVE-123 extends the runner-owned `TuiLiveShellReadinessSource`
coverage to prefix-then-line-read rejection. The live-shell runner gate proves
source-fed `SubmittedTurnLateJsonlReady` events can first preserve the active
submitted turn through no-data, then apply an assistant prefix before a rejected
late JSONL line read surfaces `LineReadRejected` /
`runner_late_jsonl_read_failed`.

The source-fed prefix-then-line-read gate records readiness event count,
drained/no-pending counts, no-data count and retained active turn, latest
`Drained` status, latest late-JSONL drain status/code, line status/code, applied
notification count, assistant delta count, completion count, latest
thread/turn/delta evidence, retained active turn ID, prompt transport
...
Existing source-fed prefix-then-unsupported notification rejection, source-fed
prefix-then-malformed JSON rejection, source-fed prefix-then-decode rejection,
source-fed prefix-then-schema rejection, source-fed schema rejection, source-fed
malformed-JSON rejection, source-fed decode rejection, source-fed
unsupported-notification rejection, source-fed line-read rejection, source-fed
prefix-applied rejection, source-fed max-batch readiness stop, source-fed
duplicate post-completion no-op, source-fed no-data retry, source-fed readiness
backpressure recovery, structured prefix evidence for
schema/decode/malformed-JSON rejection, stale-interrupted readiness rejection,
queued prefix-then-line-read rejection, queued prefix-then-unsupported
notification rejection, queued prefix-then-malformed JSON rejection, queued
prefix-then-decode rejection, queued prefix-then-schema rejection, queued schema
rejection, queued malformed-JSON rejection, queued decode rejection, queued
unsupported-notification rejection, queued line-read rejection, queued
prefix-applied rejection, queued max-batch readiness stop, queued duplicate
post-completion readiness no-op, queued no-data readiness retry, queued
readiness backpressure recovery, pump-event routing, terminal setup/restore,
text submit, line transport, agent navigation, resize, tick, Ctrl-C, q, and
...
This is still deterministic, synchronous, bounded, and credential-free. It
models source-fed prefix-then-line-read rejection behavior around the current
scheduler/drain loop; it does not own real async socket polling, Tokio
readiness, provider streaming, model calls, tool execution, process teardown, or
persistence.

### TUI-LIVE-124 Readiness Source Prefix-Then-Stale-Interrupted Rejection Routes Through Live Shell Runner

Status: TUI-LIVE-124 extends the runner-owned `TuiLiveShellReadinessSource`
coverage to prefix-then-stale-interrupted stream rejection. The live-shell
runner gate proves source-fed `SubmittedTurnLateJsonlReady` events can first
preserve the active submitted turn through no-data, then apply an assistant
...
drained/no-pending counts, no-data count and retained active turn, latest
`Drained` status, latest late-JSONL drain status/code, line status/code,
rejected notification count evidence, latest thread/turn/delta evidence for the
stale line, retained cleared active turn ID, last interrupted turn ID, interrupt
code, prompt transport inbound/outbound-line counts, transcript count,
final-frame assistant prefix row, interrupted-turn count, and completed-turn
count through `TuiLiveShellRunOutcome`.

Existing source-fed prefix-then-line-read rejection, source-fed
prefix-then-unsupported notification rejection, source-fed prefix-then-malformed
JSON rejection, source-fed prefix-then-decode rejection, source-fed
prefix-then-schema rejection, source-fed schema rejection, source-fed
malformed-JSON rejection, source-fed decode rejection, source-fed
unsupported-notification rejection, source-fed line-read rejection, source-fed
prefix-applied rejection, source-fed max-batch readiness stop, source-fed
duplicate post-completion no-op, source-fed no-data retry, source-fed readiness
backpressure recovery, structured prefix evidence for
schema/decode/malformed-JSON rejection, stale-interrupted readiness rejection,
queued prefix-then-stale-interrupted rejection, queued prefix-then-line-read
rejection, queued prefix-then-unsupported notification rejection, queued
prefix-then-malformed JSON rejection, queued prefix-then-decode rejection,
queued prefix-then-schema rejection, queued schema rejection, queued
malformed-JSON rejection, queued decode rejection, queued unsupported-notification
...
This is still deterministic, synchronous, bounded, and credential-free. It
models source-fed prefix-then-stale-interrupted rejection behavior around the
current scheduler/drain loop; it does not own real async socket polling, Tokio
readiness, provider streaming, model calls, tool execution, process teardown, or
persistence.
...
### TUI-LIVE-126 Readiness Source Stale-Interrupted Rejection Routes Through Live Shell Runner

Status: TUI-LIVE-126 extends the runner-owned `TuiLiveShellReadinessSource`
coverage to non-prefix stale-interrupted stream rejection. The live-shell
runner gate proves source-fed `SubmittedTurnLateJsonlReady` events can first
preserve the active submitted turn through no-data, accept Ctrl-C interruption,
and reject a later stale assistant delta with `BatchRejected` /
`stale_interrupted_turn_delivery`.

The source-fed stale-interrupted gate records readiness event count,
drained/no-pending counts, no-data count and retained active turn, latest
`Drained` status, latest late-JSONL drain status/code, line status/code, zero
applied notification/completion count evidence, latest thread/turn/delta
evidence for the stale line, cleared active turn ID, last interrupted turn ID,
interrupt code, prompt transport inbound/outbound-line counts, transcript
count, interrupted-turn count, and completed-turn count through
`TuiLiveShellRunOutcome`.

Existing source-fed prefix-then-stale-interrupted completion rejection,
...
readiness stop, source-fed duplicate post-completion no-op, source-fed no-data
retry, source-fed readiness backpressure recovery, structured prefix evidence
for schema/decode/malformed-JSON rejection, stale-interrupted completion
readiness rejection, stale-interrupted readiness rejection, queued
prefix-then-stale-interrupted completion rejection, queued
prefix-then-stale-interrupted rejection, queued prefix-then-line-read
rejection, queued prefix-then-unsupported notification rejection, queued
prefix-then-malformed JSON rejection, queued prefix-then-decode rejection,
queued prefix-then-schema rejection, queued schema rejection, queued
malformed-JSON rejection, queued decode rejection, queued unsupported-notification
...
This is still deterministic, synchronous, bounded, and credential-free. It
models source-fed stale-interrupted stream rejection behavior around the current
scheduler/drain loop; it does not own real async socket polling, Tokio
readiness, provider streaming, model calls, tool execution, process teardown, or
persistence.

### TUI-LIVE-127 Readiness Source Stale-Interrupted Completion Rejection Routes Through Live Shell Runner

Status: TUI-LIVE-127 extends the runner-owned `TuiLiveShellReadinessSource`
coverage to non-prefix stale-interrupted completion rejection. The live-shell
runner gate proves source-fed `SubmittedTurnLateJsonlReady` events can first
preserve the active submitted turn through no-data, accept Ctrl-C interruption,
and reject a later stale `turn/completed` line with `BatchRejected` /
`stale_interrupted_turn_completion`.

The source-fed stale-interrupted-completion gate records readiness event count,
drained/no-pending counts, no-data count and retained active turn, latest
`Drained` status, latest late-JSONL drain status/code, line status/code, zero
applied notification and completion count evidence, latest thread/turn evidence
for the stale completion, empty rejected-delta evidence, cleared active turn ID,
last interrupted turn ID, interrupt code, prompt transport
inbound/outbound-line counts, transcript count, interrupted-turn count, and
completed-turn count through `TuiLiveShellRunOutcome`.

Existing source-fed stale-interrupted stream rejection, source-fed
prefix-then-stale-interrupted completion rejection, source-fed
prefix-then-stale-interrupted stream rejection, source-fed prefix-then-line-read
rejection, source-fed prefix-then-unsupported notification rejection,
source-fed prefix-then-malformed JSON rejection, source-fed prefix-then-decode
rejection, source-fed prefix-then-schema rejection, source-fed schema rejection,
source-fed malformed-JSON rejection, source-fed decode rejection, source-fed
unsupported-notification rejection, source-fed line-read rejection, source-fed
prefix-applied rejection, source-fed max-batch readiness stop, source-fed
duplicate post-completion no-op, source-fed no-data retry, source-fed readiness
backpressure recovery, structured prefix evidence for
schema/decode/malformed-JSON rejection, stale-interrupted completion readiness
rejection, stale-interrupted readiness rejection, queued
prefix-then-stale-interrupted completion rejection, queued
prefix-then-stale-interrupted rejection, queued prefix-then-line-read
rejection, queued prefix-then-unsupported notification rejection, queued
prefix-then-malformed JSON rejection, queued prefix-then-decode rejection,
...
This is still deterministic, synchronous, bounded, and credential-free. It
models source-fed stale-interrupted completion rejection behavior around the
current scheduler/drain loop; it does not own real async socket polling, Tokio
readiness, provider streaming, model calls, tool execution, process teardown, or
persistence.

### TUI-LIVE-128 App-Server Session Readiness Source Boundary Feeds Live Shell Runner

Status: TUI-LIVE-128 introduces `TuiAppServerSessionReadinessSource` as the
deterministic app-server session-facing side of live-shell readiness. The
live-shell runner still consumes readiness through `TuiLiveShellReadinessSource`,
but that source can now delegate to a typed app-server session boundary instead
of only holding a raw queued event list.

The app-server session readiness source gate records the same submitted-turn
readiness evidence as the queued/source-fed happy path: readiness event count,
drained/no-pending counts, no-data count and retained active turn, latest
late-JSONL drain status/code, line status/code, applied notification count,
assistant-delta count, completion count, latest thread/turn/delta evidence,
last started/completed turn IDs, cleared active turn ID, prompt transcript rows,
final frame row, and zero remaining readiness source count after the runner
drains the session-facing source.

Existing source-fed stale-interrupted completion rejection, source-fed
stale-interrupted stream rejection, source-fed prefix-then-stale-interrupted
completion rejection, source-fed prefix-then-stale-interrupted stream rejection,
source-fed prefix-then-line-read rejection, source-fed prefix-then-unsupported
...
max-batch readiness stop, source-fed duplicate post-completion no-op,
source-fed no-data retry, source-fed readiness backpressure recovery,
structured prefix evidence for schema/decode/malformed-JSON rejection, queued
stale-interrupted completion readiness rejection, queued stale-interrupted
readiness rejection, queued prefix-then-stale-interrupted completion rejection,
queued prefix-then-stale-interrupted rejection, queued prefix-then-line-read
rejection, queued prefix-then-unsupported notification rejection, queued
prefix-then-malformed JSON rejection, queued prefix-then-decode rejection,
queued prefix-then-schema rejection, queued schema rejection, queued
...
This is still deterministic, synchronous, bounded, and credential-free. It
models the typed boundary where a live app-server session will later publish
readiness; it does not own real async socket polling, Tokio readiness, provider
streaming, model calls, tool execution, process teardown, or persistence.

### TUI-LIVE-129 App-Server Session Readiness Source Backpressure Recovery Feeds Live Shell Runner

Status: TUI-LIVE-129 extends the deterministic
`TuiAppServerSessionReadinessSource` bridge to readiness-triggered backpressure
recovery. The live-shell runner consumes `SubmittedTurnLateJsonlReady` through
`TuiLiveShellReadinessSource.fromAppServerSession`, records the same
readiness-backpressure evidence as the queued/source-fed recovery path, then
recovers remaining queued app-server work through `TuiAppServerPumpEvent`.

The app-server session readiness backpressure gate records readiness event
count, readiness backpressure count, latest active turn preserved after the
readiness drain, pump-event count, pump-event backpressure count, aggregate
backpressure count, latest `Drained` status, completed late-JSONL drain
status/code, line status/code, applied notification count, assistant-delta
count, completion count, latest thread/turn/delta evidence, last
started/completed turn IDs, cleared active turn ID, prompt transcript rows,
final frame row, and zero remaining session/readiness source count after the
runner drains the session-facing source.

Existing app-server session readiness submitted-turn delivery, source-fed
stale-interrupted completion rejection, source-fed stale-interrupted stream
rejection, source-fed prefix-then-stale-interrupted completion rejection,
source-fed prefix-then-stale-interrupted stream rejection, source-fed
prefix-then-line-read rejection, source-fed prefix-then-unsupported notification
rejection, source-fed prefix-then-malformed JSON rejection, source-fed
prefix-then-decode rejection, source-fed prefix-then-schema rejection,
...
readiness stop, source-fed duplicate post-completion no-op, source-fed no-data
retry, source-fed readiness backpressure recovery, structured prefix evidence
for schema/decode/malformed-JSON rejection, queued stale-interrupted completion
readiness rejection, queued stale-interrupted readiness rejection, queued
prefix-then-stale-interrupted completion rejection, queued
prefix-then-stale-interrupted rejection, queued prefix-then-line-read
rejection, queued prefix-then-unsupported notification rejection, queued
prefix-then-malformed JSON rejection, queued prefix-then-decode rejection,
...
### TUI-LIVE-130 App-Server Session Readiness Source No-Data Retry Feeds Live Shell Runner

Status: TUI-LIVE-130 extends the deterministic
`TuiAppServerSessionReadinessSource` bridge to no-data retry behavior. The
live-shell runner consumes repeated `SubmittedTurnLateJsonlReady` events through
`TuiLiveShellReadinessSource.fromAppServerSession`, preserves the active
submitted turn through the first no-data readiness pass, then applies late
assistant/completion lines on the later readiness pass.

The app-server session readiness no-data gate records readiness event count,
drained/no-pending counts, no-data count and retained active turn, final
completed late-JSONL drain status/code, line status/code, applied notification
count, assistant-delta count, completion count, latest thread/turn/delta
evidence, last started/completed turn IDs, cleared active turn ID, prompt
transcript rows, final frame row, and zero remaining session/readiness source
count after the runner drains the session-facing source.

Existing app-server session readiness backpressure recovery, app-server session
readiness submitted-turn delivery, source-fed stale-interrupted completion
rejection, source-fed stale-interrupted stream rejection, source-fed
prefix-then-stale-interrupted completion rejection, source-fed
prefix-then-stale-interrupted stream rejection, source-fed prefix-then-line-read
rejection, source-fed prefix-then-unsupported notification rejection,
source-fed prefix-then-malformed JSON rejection, source-fed prefix-then-decode
...
duplicate post-completion no-op, source-fed no-data retry, source-fed readiness
backpressure recovery, structured prefix evidence for
schema/decode/malformed-JSON rejection, queued stale-interrupted completion
readiness rejection, queued stale-interrupted readiness rejection, queued
prefix-then-stale-interrupted completion rejection, queued
prefix-then-stale-interrupted rejection, queued prefix-then-line-read
rejection, queued prefix-then-unsupported notification rejection, queued
...
### TUI-LIVE-131 App-Server Session Readiness Source Duplicate Post-Completion No-Op Feeds Live Shell Runner

Status: TUI-LIVE-131 extends the deterministic
`TuiAppServerSessionReadinessSource` bridge to duplicate post-completion
readiness no-op behavior. The live-shell runner consumes repeated
`SubmittedTurnLateJsonlReady` events through
`TuiLiveShellReadinessSource.fromAppServerSession`, completes the submitted turn
exactly once, then records a later `NoPendingSubmittedTurn` readiness no-op
without reading the extra late JSONL line or mutating transcript/completion
state.

The app-server session duplicate readiness gate records readiness event count,
drained/no-pending counts, no-data count and retained active turn, latest
`NoPendingSubmittedTurn` status, retained completed late-JSONL drain
status/code, line status/code, applied notification count, assistant-delta
count, completion count, latest thread/turn/delta evidence, last
started/completed turn IDs, cleared active turn ID, exactly-once completion
count, prompt transcript rows, final frame row, line close counts proving the
duplicate did not read extra late JSONL, and zero remaining session/readiness
source count after the runner drains the session-facing source.

Existing app-server session readiness no-data retry, app-server session
readiness backpressure recovery, app-server session readiness submitted-turn
delivery, source-fed stale-interrupted completion rejection, source-fed
stale-interrupted stream rejection, source-fed prefix-then-stale-interrupted
completion rejection, source-fed prefix-then-stale-interrupted stream rejection,
source-fed prefix-then-line-read rejection, source-fed prefix-then-unsupported
notification rejection, source-fed prefix-then-malformed JSON rejection,
...
### TUI-LIVE-132 App-Server Session Readiness Source Max-Batch Stop Feeds Live Shell Runner

Status: TUI-LIVE-132 extends the deterministic
`TuiAppServerSessionReadinessSource` bridge to max-batch readiness stop
behavior. The live-shell runner consumes `SubmittedTurnLateJsonlReady` through
`TuiLiveShellReadinessSource.fromAppServerSession`, applies the assistant prefix,
stops at the configured max-batch limit with `MaxBatchesReached`, retains the
active submitted turn, and leaves the completion line unread.

The app-server session max-batch gate records readiness event count,
drained/no-pending counts, latest `Drained` status, max-batch late-JSONL drain
status/code, line status/code, applied notification count, assistant-delta
count, zero completion count, latest thread/turn/delta evidence, retained active
turn ID, last started turn ID, empty completed turn ID, zero completed turn
count, prompt transcript rows, final frame row, line close counts proving the
completion line remains unread, and zero remaining session/readiness source
count after the runner drains the session-facing source.

Existing app-server session duplicate post-completion no-op, app-server session
readiness no-data retry, app-server session readiness backpressure recovery,
app-server session readiness submitted-turn delivery, source-fed
stale-interrupted completion rejection, source-fed stale-interrupted stream
rejection, source-fed prefix-then-stale-interrupted completion rejection,
source-fed prefix-then-stale-interrupted stream rejection, source-fed
prefix-then-line-read rejection, source-fed prefix-then-unsupported notification
...
### TUI-LIVE-133 App-Server Session Readiness Source Prefix-Applied Rejection Feeds Live Shell Runner

Status: TUI-LIVE-133 extends the deterministic
`TuiAppServerSessionReadinessSource` bridge to prefix-applied readiness rejection
behavior. The live-shell runner consumes `SubmittedTurnLateJsonlReady` through
`TuiLiveShellReadinessSource.fromAppServerSession`, applies the assistant prefix,
rejects a later wrong-turn completion with `BatchRejected` / `wrong_turn`,
retains the active submitted turn, and preserves structured prefix evidence.

The app-server session prefix-applied rejection gate records readiness event
count, drained/no-pending counts, latest `Drained` status, rejected late-JSONL
drain status/code, line status/code, applied notification count, assistant-delta
count, zero completion count, latest applied thread/delta evidence, rejected
turn evidence, retained active turn ID, last started turn ID, empty completed
turn ID, zero completed turn count, prompt transcript rows, final frame row,
line close counts proving the rejected completion was read, and zero remaining
session/readiness source count after the runner drains the session-facing
source.

Existing app-server session max-batch readiness stop, app-server session
duplicate post-completion no-op, app-server session readiness no-data retry,
app-server session readiness backpressure recovery, app-server session
readiness submitted-turn delivery, source-fed stale-interrupted completion
rejection, source-fed stale-interrupted stream rejection, source-fed
prefix-then-stale-interrupted completion rejection, source-fed
prefix-then-stale-interrupted stream rejection, source-fed prefix-then-line-read
rejection, source-fed prefix-then-unsupported notification rejection, source-fed
prefix-then-malformed JSON rejection, source-fed prefix-then-decode rejection,
source-fed prefix-then-schema rejection, source-fed schema rejection, source-fed
malformed-JSON rejection, source-fed decode rejection, source-fed
unsupported-notification rejection, source-fed line-read rejection, source-fed
...
### TUI-LIVE-135 App-Server Session Readiness Source Unsupported-Notification Rejection Feeds Live Shell Runner

Status: TUI-LIVE-135 extends the deterministic
`TuiAppServerSessionReadinessSource` bridge to unsupported-notification
rejection behavior. The live-shell runner consumes `SubmittedTurnLateJsonlReady`
through `TuiLiveShellReadinessSource.fromAppServerSession`, reads an unsupported
late `thread/status/changed` stream notification, records `BatchRejected` /
`unsupported_stream_notification`, retains the active submitted turn, and drains
the session-facing readiness source without applying the notification or
mutating completion state.

The app-server session unsupported-notification gate records readiness event
count, drained/no-pending counts, latest `Drained` status, rejected late-JSONL
drain status/code, accepted line status/code, zero applied notification count,
zero assistant-delta count, zero completion count, retained active turn ID, last
started turn ID, empty completed turn ID, zero completed turn count, prompt
transcript row, line close/outbound/inbound counts proving the unsupported line
was read, and zero remaining session/readiness source count after the runner
drains the session-facing source.

Existing app-server session line-read rejection, app-server session
prefix-applied rejection, app-server session max-batch readiness stop,
app-server session duplicate post-completion no-op, app-server session
readiness no-data retry, app-server session readiness backpressure recovery,
app-server session readiness submitted-turn delivery, source-fed
stale-interrupted completion rejection, source-fed stale-interrupted stream
rejection, source-fed prefix-then-stale-interrupted completion rejection,
...
### TUI-LIVE-136 App-Server Session Readiness Source Decode Rejection Feeds Live Shell Runner

Status: TUI-LIVE-136 extends the deterministic
`TuiAppServerSessionReadinessSource` bridge to decode-rejected readiness
behavior. The live-shell runner consumes repeated `SubmittedTurnLateJsonlReady`
events through `TuiLiveShellReadinessSource.fromAppServerSession`, preserves the
active submitted turn through an initial no-data drain, then reads an unknown
late JSONL method and records `BatchRejected` / `unknown_inbound_method` without
applying notifications or mutating completion state.

The app-server session decode rejection gate records readiness event count,
drained/no-pending counts, no-data count and retained no-data active turn,
latest `Drained` status, rejected late-JSONL drain status/code, accepted line
status/code, zero applied notification count, zero assistant-delta count, zero
completion count, retained active turn ID, last started turn ID, empty completed
turn ID, zero completed turn count, prompt transcript row, line
close/outbound/inbound counts proving the unknown-method line was read, and zero
remaining session/readiness source count after the runner drains the
session-facing source.

Existing app-server session unsupported-notification rejection, app-server
session line-read rejection, app-server session prefix-applied rejection,
app-server session max-batch readiness stop, app-server session duplicate
post-completion no-op, app-server session readiness no-data retry, app-server
session readiness backpressure recovery, app-server session readiness
submitted-turn delivery, source-fed stale-interrupted completion rejection,
source-fed stale-interrupted stream rejection, source-fed
prefix-then-stale-interrupted completion rejection, source-fed
prefix-then-stale-interrupted stream rejection, source-fed prefix-then-line-read
rejection, source-fed prefix-then-unsupported notification rejection, source-fed
prefix-then-malformed JSON rejection, source-fed prefix-then-decode rejection,
...
### TUI-LIVE-137 App-Server Session Readiness Source Malformed JSON Rejection Feeds Live Shell Runner

Status: TUI-LIVE-137 extends the deterministic
`TuiAppServerSessionReadinessSource` bridge to malformed-JSON readiness
behavior. The live-shell runner consumes repeated `SubmittedTurnLateJsonlReady`
events through `TuiLiveShellReadinessSource.fromAppServerSession`, preserves the
active submitted turn through an initial no-data drain, then reads a malformed
late JSONL line and records `BatchRejected` / `invalid_json_line` without
applying notifications or mutating completion state.

The app-server session malformed-JSON gate records readiness event count,
drained/no-pending counts, no-data count and retained no-data active turn,
latest `Drained` status, rejected late-JSONL drain status/code, accepted line
status/code, zero applied notification count, zero assistant-delta count, zero
completion count, retained active turn ID, last started turn ID, empty completed
turn ID, zero completed turn count, prompt transcript row, line
close/outbound/inbound counts proving the malformed line was read, and zero
remaining session/readiness source count after the runner drains the
session-facing source.

Existing app-server session decode rejection, app-server session
unsupported-notification rejection, app-server session line-read rejection,
app-server session prefix-applied rejection, app-server session max-batch
readiness stop, app-server session duplicate post-completion no-op, app-server
session readiness no-data retry, app-server session readiness backpressure
recovery, app-server session readiness submitted-turn delivery, source-fed
stale-interrupted completion rejection, source-fed stale-interrupted stream
rejection, source-fed prefix-then-stale-interrupted completion rejection,
source-fed prefix-then-stale-interrupted stream rejection, source-fed
prefix-then-line-read rejection, source-fed prefix-then-unsupported
...
### TUI-LIVE-138 App-Server Session Readiness Source Schema Rejection Feeds Live Shell Runner

Status: TUI-LIVE-138 extends the deterministic
`TuiAppServerSessionReadinessSource` bridge to schema-rejected readiness
behavior. The live-shell runner consumes repeated `SubmittedTurnLateJsonlReady`
events through `TuiLiveShellReadinessSource.fromAppServerSession`, preserves the
active submitted turn through an initial no-data drain, then reads a
schema-invalid late JSONL line and records `BatchRejected` / `missing_field`
without applying notifications or mutating completion state.

The app-server session schema-rejection gate records readiness event count,
drained/no-pending counts, no-data count and retained no-data active turn,
latest `Drained` status, rejected late-JSONL drain status/code, accepted line
status/code, zero applied notification count, zero assistant-delta count, zero
completion count, retained active turn ID, last started turn ID, empty completed
turn ID, zero completed turn count, prompt transcript row, line
close/outbound/inbound counts proving the schema-invalid line was read, and
zero remaining session/readiness source count after the runner drains the
session-facing source.

Existing app-server session malformed-JSON rejection, app-server session decode
rejection, app-server session unsupported-notification rejection, app-server
session line-read rejection, app-server session prefix-applied rejection,
app-server session max-batch readiness stop, app-server session duplicate
post-completion no-op, app-server session readiness no-data retry, app-server
session readiness backpressure recovery, app-server session readiness
...
duplicate post-completion no-op, source-fed no-data retry, source-fed readiness
backpressure recovery, structured prefix evidence for schema/decode/malformed-
JSON rejection, queued stale-interrupted completion readiness rejection, queued
stale-interrupted readiness rejection, queued prefix-then-stale-interrupted
completion rejection, queued prefix-then-stale-interrupted rejection, queued
prefix-then-line-read rejection, queued prefix-then-unsupported notification
rejection, queued prefix-then malformed JSON rejection, queued
prefix-then-decode rejection, queued prefix-then-schema rejection, queued schema
rejection, queued malformed-JSON rejection, queued decode rejection, queued
unsupported-notification rejection, queued line-read rejection, queued
prefix-applied rejection, queued max-batch readiness stop, queued duplicate
...
### TUI-LIVE-139 App-Server Session Readiness Source Prefix-Then-Schema Rejection Feeds Live Shell Runner

Status: TUI-LIVE-139 extends the deterministic
`TuiAppServerSessionReadinessSource` bridge to prefix-then-schema readiness
behavior. The live-shell runner consumes repeated `SubmittedTurnLateJsonlReady`
events through `TuiLiveShellReadinessSource.fromAppServerSession`, preserves the
active submitted turn through an initial no-data drain, applies a valid
assistant-delta late JSONL line, then reads a schema-invalid completion line and
records `BatchRejected` / `missing_field` without completing the turn.

The app-server session prefix-then-schema gate records readiness event count,
drained/no-pending counts, no-data count and retained no-data active turn,
latest `Drained` status, rejected late-JSONL drain status/code, accepted line
status/code, one applied notification, one assistant-delta count, zero
completion count, applied thread/turn/delta evidence, retained active turn ID,
last started turn ID, empty completed turn ID, zero completed turn count, prompt
and assistant transcript rows, final-frame assistant row, line
close/outbound/inbound counts proving both late lines were read, and zero
remaining session/readiness source count after the runner drains the
session-facing source.

Existing app-server session schema rejection, app-server session malformed-JSON
rejection, app-server session decode rejection, app-server session
unsupported-notification rejection, app-server session line-read rejection,
app-server session prefix-applied rejection, app-server session max-batch
readiness stop, app-server session duplicate post-completion no-op, app-server
session readiness no-data retry, app-server session readiness backpressure
...
### TUI-LIVE-140 App-Server Session Readiness Source Prefix-Then-Decode Rejection Feeds Live Shell Runner

Status: TUI-LIVE-140 extends the deterministic
`TuiAppServerSessionReadinessSource` bridge to prefix-then-decode readiness
behavior. The live-shell runner consumes repeated `SubmittedTurnLateJsonlReady`
events through `TuiLiveShellReadinessSource.fromAppServerSession`, preserves the
active submitted turn through an initial no-data drain, applies a valid
assistant-delta late JSONL line, then reads an unknown-method late JSONL line and
records `BatchRejected` / `unknown_inbound_method` without completing the turn.

The app-server session prefix-then-decode gate records readiness event count,
drained/no-pending counts, no-data count and retained no-data active turn,
latest `Drained` status, rejected late-JSONL drain status/code, accepted line
status/code, one applied notification, one assistant-delta count, zero
completion count, applied thread/turn/delta evidence, retained active turn ID,
...
Existing app-server session prefix-then-schema rejection, app-server session
schema rejection, app-server session malformed-JSON rejection, app-server
session decode rejection, app-server session unsupported-notification rejection,
app-server session line-read rejection, app-server session prefix-applied
rejection, app-server session max-batch readiness stop, app-server session
duplicate post-completion no-op, app-server session readiness no-data retry,
app-server session readiness backpressure recovery, app-server session
readiness submitted-turn delivery, source-fed stale-interrupted completion
rejection, source-fed stale-interrupted stream rejection, source-fed
