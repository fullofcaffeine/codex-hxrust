...
- `app-protocol-roundtrip.v1.json` covers the selected upstream app-server protocol request, response, notification, transcript, completed plan item, plan update, streaming delta, item command execution output delta, terminal interaction, file-change output delta, patch update, MCP tool-call progress, MCP OAuth login completion, MCP server startup status, account update, account rate-limit update, app-list update, server request resolution, command/process output delta, process exit, and error payload subset.
