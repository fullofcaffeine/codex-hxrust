...
- `app-protocol-roundtrip.v1.json` covers the selected upstream app-server protocol request, response, notification, transcript, completed plan item, plan update, streaming delta, reasoning summary part creation and text delta, item command execution output delta, terminal interaction, file-change output delta, patch update, MCP tool-call progress, MCP OAuth login completion, MCP server startup status, account update, account rate-limit update, app-list update, remote-control status change, external agent config import completion, filesystem change, server request resolution, command/process output delta, process exit, and error payload subset.
